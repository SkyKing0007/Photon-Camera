...
if len(sys.argv)<4 or sys.argv[3]!='--out': raise SystemExit('usage: verify_26613_v1_1_shaders.py BASE CAND --out OUT [--compiler PATH]')
...
(O/'V1_1_26613_RUNTIME_EXPANDED_SHADERS.sha256').write_text('\n'.join(lines)+'\n')
results=[]
for p,stage,name in paths:
    status='STATIC_PASS'
    if compiler:
...
(O/'V1_1_26613_SHADER_VERIFICATION.json').write_text(json.dumps({'count':len(results),'compiler':compiler,'results':results},indent=2,sort_keys=True)+'\n')
