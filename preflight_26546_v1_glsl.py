...
def run(root,validator=None):
    ...
    # This unchanged shader becomes active in Sabre V1.4's optional RGBA16F export, so compile it too.
    ...
    contracts={
      ...
      'vgn_localMedian':['uniform float uChromaStrength','strength>=0.9999?sum','strength<=0.0001?centerChroma'],
      'vgn_directionalSmooth':['uniform float uChromaStrength','strength>=0.9999?fc','strength<=0.0001?originalChroma'],
      'vgn_iirRgb':['uniform float uChromaStrength','if(strength<0.9999)','strength<=0.0001?inCr'],
      'vgn_blendChroma':['uniform float uChromaStrength','if(strength<0.9999)f*=strength'],
    ...
    if validator:
        ...
        print(f'PASS: REAL GLSL COMPILE {len(sources)} 26546 V1 active/changed shaders via {validator}')
    print(f'PASS: 26546 V1 GLSL extraction/contracts shaders={len(sources)}')
def self_test():
    ...
    print('PASS: 26546 V1 GLSL preflight self-test')
