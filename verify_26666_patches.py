...
if len(sys.argv)!=4: raise SystemExit('usage: verify_26666_patches.py ROOT BASE CANDIDATE')
...
def make(first,second):
 with tempfile.TemporaryDirectory(prefix='iris26666_patch_') as td:
...
assert f==(root/'R1_26666_RUNTIME_DELTA_FROM_26665.patch').read_bytes();assert r==(root/'R1_26666_RUNTIME_ROLLBACK_TO_26665.patch').read_bytes()
for label,start,patch,target in [('forward',base,root/'R1_26666_RUNTIME_DELTA_FROM_26665.patch',cand),('rollback',cand,root/'R1_26666_RUNTIME_ROLLBACK_TO_26665.patch',base)]:
 with tempfile.TemporaryDirectory(prefix='iris26666_apply_') as td:
...
print('PASS 26666 deterministic full-index patches core.abbrev=7/12/40; forward/rollback exact application (git apply exact context / no 3-way)')
