...
HANDOFF_PARENT_COMMIT="4d9869dde11b9ca1ef63c5db8a8f183f73fd9397"
...
HANDOFF="$ROOT/V2_26582_HANDOFF_HASHES.sha256"
BASE_FULL="$ROOT/V2_26582_BASE_26581_FULL_APP.sha256"
CAND_FULL="$ROOT/V2_26582_EXPECTED_CANDIDATE_FULL_APP.sha256"
BASE_PROTECTED="$ROOT/V2_26582_PROTECTED_UNCHANGED_BASE.sha256"
CAND_PROTECTED="$ROOT/V2_26582_PROTECTED_UNCHANGED_CANDIDATE.sha256"
BASE_NATIVE="$ROOT/V2_26582_NATIVE_PROTECTED_BASE.sha256"
CAND_NATIVE="$ROOT/V2_26582_NATIVE_PROTECTED_CANDIDATE.sha256"
BASE_VENDOR="$ROOT/V2_26582_VENDOR_PROTECTED_BASE.sha256"
CAND_VENDOR="$ROOT/V2_26582_VENDOR_PROTECTED_CANDIDATE.sha256"
BASE_DNG="$ROOT/V2_26582_DNG_PROTECTED_BASE.sha256"
CAND_DNG="$ROOT/V2_26582_DNG_PROTECTED_CANDIDATE.sha256"
BASE_ARCH="$ROOT/V2_26582_PROTECTED_ARCHITECTURE_BASE.sha256"
CAND_ARCH="$ROOT/V2_26582_PROTECTED_ARCHITECTURE_CANDIDATE.sha256"
CHANGED="$ROOT/V2_26582_RUNTIME_CHANGED_PATHS.txt"
PREWRITE="$ROOT/V2_26582_PREWRITE_SOURCE_HASHES.sha256"
EXPECTED_CHANGED="$ROOT/V2_26582_EXPECTED_CHANGED_SOURCE_HASHES.sha256"
FORWARD="$ROOT/V2_26582_RUNTIME_DELTA_FROM_26581.patch"
ROLLBACK="$ROOT/V2_26582_RUNTIME_ROLLBACK_TO_26581.patch"
SHADER_PIN="$ROOT/V2_26582_RUNTIME_EXPANDED_SHADERS.sha256"
TRANSFORM="$ROOT/transform_26582_v2.py"
VALIDATE="$ROOT/validate_26582_v2.py"
AUTHORITY="$ROOT/verify_26582_v2_authority.py"
PATCHVERIFY="$ROOT/verify_26582_v2_patches.py"
SHADERVERIFY="$ROOT/verify_26582_v2_shaders.py"
GATEVERIFY="$ROOT/verify_26582_v2_regressions.py"
BUILD_SCRIPT="$ROOT/build_26582_v2_adaptive_global_tone_compile_fix.sh"
WORKFLOW="$ROOT/.github/workflows/build-26582-v2-adaptive-global-tone-compile-fix.yml"
OUT="$ROOT/build_26582_v2_adaptive_global_tone_compile_fix_outputs"
WORK="$ROOT/.build_26582_v2_adaptive_global_tone_compile_fix_work"
...
FINAL="$ROOT/IrisCamera-${VERSION_NAME}-${VERSION_BUILD}-v2-adaptive-global-tone-compile-fix-debug.apk"
...
cat > "$OUT/26582_V2_COMPILER_STATUS.txt" <<'STATUS'
...
STATUS
cat > "$OUT/26582_V2_STRICT_HANDOFF_REPORT.txt" <<'REPORT'
...
REPORT
set_report(){ local key="$1" val="$2" tmp="$OUT/.report.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26582_V2_STRICT_HANDOFF_REPORT.txt" > "$tmp" || fail "report key missing $key"; mv "$tmp" "$OUT/26582_V2_STRICT_HANDOFF_REPORT.txt"; }
set_compiler(){ local key="$1" val="$2" tmp="$OUT/.compiler.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26582_V2_COMPILER_STATUS.txt" > "$tmp" || fail "compiler key missing $key"; mv "$tmp" "$OUT/26582_V2_COMPILER_STATUS.txt"; }
...
verify_package(){
  ...
  [[ "$(sha "$CAND_FULL")" == "d40a9773f085e86c5ec268503ccd75f914030fd72d0ae9e02b910e343c3ea9db" ]] || fail "candidate full manifest SHA"
  ...
  [[ "$(sha "$EXPECTED_CHANGED")" == "37026661e5a66cebefff46fab2115939c517bc19ee94dad006f9588c01cfdeea" ]] || fail "expected changed-source SHA"
  [[ "$(sha "$FORWARD")" == "288c856158b69b323647427c5649c46feadcaf9c8f07987dda575056404fc81b" ]] || fail "forward patch SHA"
  [[ "$(sha "$ROLLBACK")" == "5c1456bdd2ee191b2544ac8420c5c4b3dbd39a55cc1ff3133486191f62788e2f" ]] || fail "rollback patch SHA"
...
PY
  bash -n "$BUILD_SCRIPT"
  grep -F 'no-HDR-tail scene must preserve exact 26581 base scene-white decision' "$ROOT/REGRESSION_V2_26582_ADAPTIVE_GLOBAL_TONE.txt" >/dev/null || fail "adaptive tone regression contract missing"
  grep -F 'motionv2_jpeg444_jni.cpp must remain byte-identical SHA-256 d73cd24452280958f089e9124545a81461939e8fa0c9c95272b2825942e4b43d' "$ROOT/REGRESSION_V2_26582_CARRIED_FAILURES.txt" >/dev/null || fail "GPU transport regression contract missing"
  grep -F 'run 33433465300 / job 99624165337' "$ROOT/REGRESSION_V2_26582_CARRIED_FAILURES.txt" >/dev/null || fail "carried scope regression missing"
  grep -F 'no verification gate is weakened' "$ROOT/V2_26582_INFRASTRUCTURE_DIFF_AUDIT.txt" >/dev/null || fail "infrastructure diff audit missing"
  grep -F 'MotionV2ViewfinderExposureMatcher.java:344 called presentedLuma(s, 1.0f)' "$ROOT/REGRESSION_V2_26582_CARRIED_FAILURES.txt" >/dev/null || fail "26582 V1 Java compiler regression missing"
...
verify_scope(){
  ...
  [[ "$(git rev-parse HEAD^)" == "$HANDOFF_PARENT_COMMIT" ]] || fail "26582 V2 must be one clean correction handoff commit directly on failed 26582 V1"
  [[ "$(git rev-parse "$HANDOFF_PARENT_COMMIT^")" == "$BASE_SUCCESS_COMMIT" ]] || fail "failed 26582 V1 lineage must be exactly one commit on successful 26581"
...
names.append('V2_26582_HANDOFF_HASHES.sha256')
print('\n'.join(sorted(names)))
PY
  ...
  ! grep -Eq '^app/' "$WORK/full_lineage_scope.txt" || fail "26581-to-V2 lineage contains live runtime app source"
...
PY
  while IFS= read -r rel; do [[ -n "$rel" ]] || continue; cmp "$ROOT/handoff_payload_26582_v2/$rel" "$AFTER/$rel" || fail "sealed payload differs frozen candidate $rel"; done < "$CHANGED"
...
verify_shaders(){
  ...
  if [[ -n "$LOCAL_ART" ]]; then
    ...
    cmp "$SHADER_OUT/V2_26582_RUNTIME_EXPANDED_SHADERS.sha256" "$SHADER_PIN" || fail "local runtime-expanded shader pin mismatch"
  ...
  cmp "$SHADER_OUT/V2_26582_RUNTIME_EXPANDED_SHADERS.sha256" "$SHADER_PIN" || fail "Actions runtime-expanded shader pin mismatch"
  cp "$SHADER_OUT/V2_26582_SHADER_VERIFICATION.json" "$OUT/26582_shader_verification.json"
  set_report "RUNTIME-EXPANDED GLSL RESERVED SCAN" "PASS (six inherited active VGN/SR variants=6; no GLSL runtime bytes changed)"
...
PY
  ...
  sha256sum "$FINAL" > "$OUT/26582_V2_APK.sha256"; set_report "EXACTLY ONE APK" "PASS"
  ...
  tar --sort=name --mtime='UTC 1970-01-01' --owner=0 --group=0 --numeric-owner -cf - -C "$POST" app | gzip -n > "$OUT/26582_V2_candidate_app_source.tar.gz"
  sha256sum "$OUT/26582_V2_candidate_app_source.tar.gz" > "$OUT/26582_V2_candidate_app_source.tar.gz.sha256"
  cp "$CAND_FULL" "$OUT/26582_V2_candidate_full_app.sha256"; cp "$CAND_NATIVE" "$OUT/26582_V2_native_protected_postbuild.sha256"; cp "$CAND_VENDOR" "$OUT/26582_V2_vendor_protected_postbuild.sha256"
...
if [[ -n "$LOCAL_ART" ]]; then
  ...
  cp "$OUT/26582_V2_STRICT_HANDOFF_REPORT.txt" "$OUT/26582_local_prebuild_report.txt"
