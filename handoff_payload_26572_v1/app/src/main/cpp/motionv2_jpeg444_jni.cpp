...
/* IRIS_26572_TRUE_DETAIL_LUMA_OWNER
 * Native Sabre Resolve + VGN remains the sole RGB/chroma/highlight and low-frequency identity owner.
 * Direct-CFA true2x contributes only bounded zero-mean 2x2 luminance microstructure. The CPU
...
extern "C" JNIEXPORT jboolean JNICALL Java_com_particlesdevs_photoncamera_processing_IrisTrue2xSrNative_prepareVgnGuidedRenderTile(
    ...
    if(!truePath||!guidePath||!phasePath||!outputBuffer||!detailStats||e->GetArrayLength(detailStats)<6||
    ...
    auto supportAt=[&](int x,int y)->int{x=iris26564::clampi(x,regionX,regionX+regionW-1);y=iris26564::clampi(y,regionY,regionY+regionH-1);return std::min(4,(int)support[(size_t)(y-regionY)*regionW+(x-regionX)]);};
    ...
    jlong stats[6]={0,0,0,0,0,0};bool ok=true;
    for(int ly=0;ok&&ly<regionH;ly++)for(int lx=0;lx<regionW;lx++){
        ...
        float agreement=std::fabs(std::log2((lowY+0.01f)/(guideBlockY+0.01f)));float agreementGate=1.f-iris26564::smooth01((agreement-0.08f)/0.27f);float safetyGate=std::min(signalGate,std::min(highlightGate,std::min(chromaGate,agreementGate)));float confidence=clampf(phaseGate*safetyGate,0.f,1.f);
        ...
        stats[0]++;if(confidence>0.02f)stats[1]++;if(confidence>=0.50f)stats[2]++;if(highlightGate<=0.001f)stats[3]++;if(chromaGate<=0.001f||agreementGate<=0.001f)stats[4]++;if(blockPhaseCount<2)stats[5]++;
    }
    close(tfd);close(gfd);close(pfd);if(ok){jlong previous[6];e->GetLongArrayRegion(detailStats,0,6,previous);if(e->ExceptionCheck())return JNI_FALSE;for(int k=0;k<6;k++)previous[k]+=stats[k];e->SetLongArrayRegion(detailStats,0,6,previous);if(e->ExceptionCheck())return JNI_FALSE;}return ok?JNI_TRUE:JNI_FALSE;
}
...
public:
    ...
    bool init(const Params&p,const SrgbLut&lut,int outW,int outH,int bandRows,std::string*reason){
        params=&p;outWidth=outW;outHeight=outH;maxBandRows=bandRows;
    ...
    bool renderReadback(int fd,float gainMax,bool generateGain,int top,int h,int slot,Iris26571ReadyBand*out,Iris26571GpuTiming*timing,std::string*reason){
        ...
        if(fence[slot]){glDeleteSync(fence[slot]);fence[slot]=0;}fence[slot]=glFenceSync(GL_SYNC_GPU_COMMANDS_COMPLETE,0);glFlush();auto ue=std::chrono::steady_clock::now();if(timing)timing->uploadDispatchNs+=iris26569Ns(us,ue);
        if(glGetError()!=GL_NO_ERROR||!fence[slot]){set(reason,"dispatch/readback");return false;}
    ...
    bool resolve(int slot,bool generateGain,Iris26571ReadyBand*out,Iris26571GpuTiming*timing,std::string*reason){
        if(!out||slot<0||slot>1||!fence[slot]||pendingH[slot]<=0)return false;auto ws=std::chrono::steady_clock::now();GLenum wait=GL_TIMEOUT_EXPIRED;while(wait==GL_TIMEOUT_EXPIRED)wait=glClientWaitSync(fence[slot],GL_SYNC_FLUSH_COMMANDS_BIT,1000000000ull);auto we=std::chrono::steady_clock::now();if(timing)timing->readbackWaitNs+=iris26569Ns(ws,we);if(wait==GL_WAIT_FAILED){set(reason,"glClientWaitSync");return false;}
        glBindBuffer(GL_PIXEL_PACK_BUFFER,pbo[slot]);size_t px=(size_t)outWidth*pendingH[slot],bytes=px*4;const uint8_t*mapped=(const uint8_t*)glMapBufferRange(GL_PIXEL_PACK_BUFFER,0,(GLsizeiptr)bytes,GL_MAP_READ_BIT);if(!mapped){set(reason,"glMapBufferRange");return false;}
        ...
        glUnmapBuffer(GL_PIXEL_PACK_BUFFER);glBindBuffer(GL_PIXEL_PACK_BUFFER,0);auto de=std::chrono::steady_clock::now();if(timing)timing->deinterleaveNs+=iris26569Ns(ds,de);glDeleteSync(fence[slot]);fence[slot]=0;pendingH[slot]=0;return glGetError()==GL_NO_ERROR;
    }
private:
    ...
    GLuint program=0,sourceTex=0,outputTex=0,fbo=0,lutSsbo=0,pbo[2]{0,0};GLsync fence[2]{0,0};int pendingTop[2]{0,0},pendingH[2]{0,0};int sourceAllocW=0,sourceAllocH=0,outWidth=0,outHeight=0,maxBandRows=0;const Params*params=nullptr;
...
inline bool iris26571EncodeGpuPublication(int sourceFd,float gainMax,const Params&p,const SrgbLut&lut,int outW,int outH,int bandRows,bool generateGain,const char*basePath,const char*gainPath,int quality,Iris26571GpuTiming*gpuTiming,uint64_t*baseJpegNs,uint64_t*gainJpegNs,std::string*reason){
    if(!basePath||outW<=0||outH<=0||bandRows<=0)return false;
    Iris26571BandQueue queue;auto producerStart=std::chrono::steady_clock::now();
    std::thread producer([&](){
        Iris26571GpuPublisher gpu;std::string why;if(!gpu.init(p,lut,outW,outH,bandRows,&why)){queue.finish(false,why.c_str());return;}
...
extern "C" JNIEXPORT jboolean JNICALL Java_com_particlesdevs_photoncamera_processing_ultrahdr_MotionV2Jpeg444Encoder_writeTrue2xNative(
     ...
     */
    bool gpuEligible=motionFast&&!water.enabled();bool gpuEncoded=false;Iris26571GpuTiming gpuTiming{};uint64_t gpuBaseJpegNs=0,gpuGainJpegNs=0;std::string gpuReason;
    ...
    if(gpuEligible)gpuEncoded=iris26571EncodeGpuPublication(sourceFd,(float)gainMaxRatio,p,lut,outW,outH,bandRows,generateGain,gpuBasePath.c_str(),generateGain?gpuGainPath.c_str():nullptr,(int)quality,&gpuTiming,&gpuBaseJpegNs,&gpuGainJpegNs,&gpuReason);
    auto gpuEnd=std::chrono::steady_clock::now();uint64_t gpuTotalNs=iris26569Ns(gpuStart,gpuEnd);
    if(gpuEncoded){
        unlink(op.c);if(rename(gpuBasePath.c_str(),op.c)!=0)gpuEncoded=false;
        if(gpuEncoded&&generateGain){unlink(gp.c);if(rename(gpuGainPath.c_str(),gp.c)!=0)gpuEncoded=false;}
    ...
    if(gpuEncoded){
        ...
        __android_log_print(ANDROID_LOG_INFO,"MotionTrace","PIPELINE_STATE stage=IRIS_26571_TRUE2X_GPU_PUBLICATION details=gpuEligible=1 gpuUsed=1 sourceReadMs=%.3f uploadDispatchMs=%.3f readbackWaitMs=%.3f deinterleaveMs=%.3f producerMs=%.3f baseJpegMs=%.3f gainJpegMs=%.3f totalMs=%.3f bandRows=%d pboSlots=2 gain1to1=%d",(double)gpuTiming.sourceReadNs/1000000.0,(double)gpuTiming.uploadDispatchNs/1000000.0,(double)gpuTiming.readbackWaitNs/1000000.0,(double)gpuTiming.deinterleaveNs/1000000.0,(double)gpuTiming.producerNs/1000000.0,(double)gpuBaseJpegNs/1000000.0,(double)gpuGainJpegNs/1000000.0,(double)gpuTotalNs/1000000.0,bandRows,generateGain?1:0);
        return JNI_TRUE;
    }
    unlink(gpuBasePath.c_str());if(generateGain)unlink(gpuGainPath.c_str());
    __android_log_print(ANDROID_LOG_INFO,TAG,"IRIS_26571_TRUE2X_GPU_FALLBACK eligible=%d reason=%s elapsedMs=%.3f",(int)gpuEligible,gpuReason.empty()?(gpuEligible?"gpu_failed":"jin_or_watermark"):gpuReason.c_str(),(double)gpuTotalNs/1000000.0);
    __android_log_print(ANDROID_LOG_INFO,"MotionTrace","PIPELINE_STATE stage=IRIS_26571_TRUE2X_GPU_FALLBACK details=eligible=%d reason=%s elapsedMs=%.3f",(int)gpuEligible,gpuReason.empty()?(gpuEligible?"gpu_failed":"jin_or_watermark"):gpuReason.c_str(),(double)gpuTotalNs/1000000.0);
