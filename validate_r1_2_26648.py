...
if len(sys.argv)!=3: raise SystemExit('usage: validate_r1_2_26648.py BASE CANDIDATE')
...
changed=set()
bh,ch=H(base),H(cand); assert len(bh)==len(ch)==1720 and bh==ch
...
print('PASS 26648 R1.2 semantic/ownership validation: zero runtime delta from successful R1.1; universal physical NORMAL+SHORT fusion, sample-valid aligned one-RGB authority, SHORT temporal/SR-detail isolation, pre-Resolve fused master, HEIC BT601/P3 + numerical HDR proof, contrast-safe white manual UI')
