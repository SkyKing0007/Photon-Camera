...
CHANGED=[x for x in (PKG/'V1_26611_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x]
...
def main():
 ...
 b,c,f,r=map(Path,sys.argv[1:]);ft=f.read_text();rt=r.read_text();tb=init(b,'p26611_base_');tc=init(c,'p26611_cand_')
 try:
  for ab in (7,12,40):
