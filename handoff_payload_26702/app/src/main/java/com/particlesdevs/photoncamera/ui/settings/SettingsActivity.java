...
public class SettingsActivity extends BaseActivity implements PreferenceFragmentCompat.OnPreferenceStartScreenCallback {
    ...
    public static class SettingsFragment extends PreferenceFragmentCompat implements SharedPreferences.OnSharedPreferenceChangeListener, PreferenceManager.OnPreferenceTreeClickListener {
        ...
        public void onCreate(@Nullable Bundle savedInstanceState) {
            ...
            configureFrameCountPreferenceForMode();
            setupIrisMotionPreferences();
            setupPhotoPrefixPreference();
        ...
        // IRIS_26642_PHOTO_PREFIX_SETTINGS_OWNER
        private void setupPhotoPrefixPreference() {
            EditTextPreference prefix = findPreference(PreferenceKeys.Key.KEY_PHOTO_PREFIX_NAME.mValue);
            if (prefix == null) return;
            prefix.setSummaryProvider(EditTextPreference.SimpleSummaryProvider.getInstance());
            prefix.setOnBindEditTextListener(editText -> {
                editText.setSingleLine(true);
                editText.setSelectAllOnFocus(true);
            });
            prefix.setOnPreferenceChangeListener((preference, newValue) -> {
                String candidate = newValue == null ? "" : newValue.toString();
                if (!ImagePath.isValidPhotoPrefix(candidate)) {
                    PhotonCamera.showToast("Prefix must be 1-64 filename-safe characters");
                    return false;
                }
                return true;
            });
        ...
        private void filterPreferencesByMode() {
            ...
            if (cameraMode == CameraMode.SPEKTRA) {
                // IRIS_26681_SPEKTRA_SETTINGS_FIREWALL
                // Spektra image formation is sealed. Keep only behavior-neutral app/UI categories;
                // no Photo/JPG/HDRX/RAW-video/video processing preference is visible or consumed.
            ...
            } else if (cameraMode == CameraMode.RAWVIDEO) {
                ...
                if (cameraMode == CameraMode.MOTION) {
                    ...
                    removePreferenceAnywhere(IrisMotionSettings.KEY_LOCAL_LAPLACIAN_TONE);
                }
            }
        }

        private void configureFrameCountPreferenceForMode() {
            UniversalSeekBarPreference framePreference =
                    findPreference(PreferenceKeys.Key.KEY_FRAME_COUNT.mValue);
            if (framePreference == null) return;
            CameraMode cameraMode = CameraMode.valueOf(
                    sCameraMode == -1 ? PreferenceKeys.getCameraModeOrdinal() : sCameraMode);
            framePreference.setRuntimeMaxValue(cameraMode == CameraMode.MOTION ? 30.0f : 50.0f);
            Log.i("SettingsFragment", "IRIS_26556_FRAME_SLIDER_VIEW mode=" + cameraMode
                    + " persisted=" + PreferenceKeys.getFrameCountValue()
                    + " displayMax=" + (cameraMode == CameraMode.MOTION ? 30 : 50));
        }
        ...
        private void removePreferenceAnywhere(String preferenceKey) {
