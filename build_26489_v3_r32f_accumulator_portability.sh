...
BACKUP_BRANCH="backup-26489-v2-glsl-failure-before-v3-r32f-accumulator"
BACKUP_EXPECTED="2a3370b45a21a9b1db773629c442a3a68f612b7f"
...
TRANSFORM="transform_26489_v3_r32f_accumulator_portability.py"
TRANSFORM_SHA="fc181e389ac775f65d46b6bef13d4c075bf05e0be436e5ce5fa0265be7766283"

NEW_VERSION="0.9726489"
NEW_BUILD="26489"
OUTDIR="build_26489_v3_outputs"
APK_NAME="IrisCamera-${NEW_VERSION}-${NEW_BUILD}-v3-r32f-accumulator-portability-debug.apk"
...
rm -rf "$OUTDIR"
mkdir -p "$OUTDIR"
AUDIT="$OUTDIR/26489_v3_source_audit.txt"
CANDLOG="$OUTDIR/26489_v3_temporary_candidate_build.log"
JAVACLOG="$OUTDIR/26489_v3_temporary_candidate_javac.log"
FINALLOG="$OUTDIR/26489_v3_final_build.log"
SHADERLOG="$OUTDIR/26489_v3_shader_validation.txt"
REPORT="$OUTDIR/26489_v3_build_report.txt"
PREPATCH="$OUTDIR/26489_v3_pre_edit_exact_26488_binary.patch"
DELTAOUT="$OUTDIR/26489_v3_delta_from_26488.patch"
SOURCEPATCH="$OUTDIR/26489_v3_source.patch"
AFTERHASH="$OUTDIR/26489_v3_after.sha256"
PREBUILDHASH="$OUTDIR/26489_v3_prebuild_canonical.sha256"

exec > >(tee "$AUDIT") 2>&1
echo "=== 26489 V3 BJZHOU HOST / GLES-LEGAL R32F PERSISTENT BAYER ACCUMULATOR / BANDED DIRECTIONAL RCD ==="
date -Iseconds || true
...
python3 -m py_compile "$TRANSFORM26485" "$TRANSFORM26486" "$TRANSFORM26487" "$TRANSFORM26488" "$TRANSFORM26489V2" "$TRANSFORM"
...
python3 "$REPO/$TRANSFORM26489V2" "$CAND"
python3 "$REPO/$TRANSFORM" "$CAND"
pass "26489 V3 temporary candidate transform PASS"
...
26489 V3 BJZHOU HOST / GLES-LEGAL R32F PERSISTENT BAYER / BANDED DIRECTIONAL RCD ARCHITECTURE
Version=$NEW_VERSION
Build=$NEW_BUILD
Branch=$BRANCH
...
Transform26489V3SHA=$TRANSFORM_SHA
APK=$APK_NAME
...
Accumulator=frame0 reference plus every admitted auxiliary execute one persistent Bayer accumulation contract; persistent numerator/denominator/support use raw-sized R32F scalar storage with exact 2x2 CFA phase mapping; additive FLOAT32 values survive entire burst; exactly one final normalization
...
BindingSafety=bidirectional type-aware invocation audit enforces sampler -> setTexture, image -> setTextureCompute, std430 buffer -> setBufferCompute, ordinary uniform -> setVar; additional GLSL ES memory-qualifier gate forbids non-r32 read/write images and catches the exact V2 rgba32f failure before glslang
