...
def main():
 ...
 allowed_changed={'shortRestoreMask26587'};added={'shortRestoreMaskProbe26590'}
 if set(ec)!=(set(eb)|added):fail('unexpected embedded Sabre shader set delta')
 for name,src in eb.items():
  if name not in allowed_changed and ec.get(name)!=src:fail('protected pre-existing Sabre shader changed '+name)
 if ec['shortRestoreMask26587']==eb['shortRestoreMask26587']:fail('near-clip SHORT mask did not change')
 ...
 app=(root/'app/src/main/assets/shaders/motionv2/adaptive_color_appearance_26563.glsl').read_text();prefix='#version 310 es\n\n#line 1\n'
 ...
 (out/'V1_26590_RUNTIME_EXPANDED_SHADERS.sha256').write_text(''.join(manifest));(out/'V1_26590_SHADER_VERIFICATION.json').write_text(json.dumps(rows,indent=2)+'\n')
 print('PASS all protected pre-existing Sabre shader strings byte-identical to successful 26589; only SHORT mask changed + read-only probe added')
 print('PASS exact runtime-expanded shader extraction + complete reserved scan variants=11')
