...
def main():
 ...
 specs=[
  ('vgn_universal_micro_object_26580',extract_kotlin(vgn,'universalAdaptiveColor26561'),'comp',['IRIS_26578_FAIL_CLOSED_REAL_COLOR_GATE','IRIS_26579_MICRO_OBJECT_COLOR_TOPOLOGY','IRIS_26580_MICRO_OBJECT_AREA_VS_RIBBON','IRIS_26580_FAIL_CLOSED_MULTICOLOR_OBJECT_VETO','microTwoAxisEvidence','phaseOverride','microObjectProtection','falseColorGate','maximumMove']),
  ...
  ('true2x_topology_chroma_26580',extract_kotlin(sabre,'true2xGuideRender26568'),'frag',['IRIS_26579_TRUE2X_TOPOLOGY_CHROMA_UPSAMPLE','irisTopologyGuide','sameMaterial','maxChromaMagnitude','directChromaOwner'])
 ]
 # directChromaOwner is in the contract comment outside this shader in Kotlin; use exact no-direct-chroma shader tokens instead.
 specs[-1]=(specs[-1][0],specs[-1][1],specs[-1][2],['IRIS_26579_TRUE2X_TOPOLOGY_CHROMA_UPSAMPLE','IRIS_26580_TRUE2X_SAME_MATERIAL_CHROMA_OWNERSHIP','IRIS_26580_NEUTRAL_GLYPH_OUTSIDE_EDGE_EXCLUSION','irisTopologyGuide','sameMaterial','neutralSideOwnership','maxChromaMagnitude','bilinearY','confidence'])
 rows=[];manifest=[]
 ...
 (out/'V1_26580_RUNTIME_EXPANDED_SHADERS.sha256').write_text(''.join(manifest))
 (out/'V1_26580_SHADER_VERIFICATION.json').write_text(json.dumps(rows,indent=2)+'\n')
 print('PASS exact runtime-expanded shader extraction + complete reserved scan variants=6')
