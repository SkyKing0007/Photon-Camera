...
public class CameraUIViewImpl implements CameraUIView {
    ...
    // Photo and removes the legacy Photo action entirely. Spektra is one additional value in the
    // same LiquidModePicker; no picker styling, typography, spacing, or drawing logic is changed.
    ...
    private long iris26699VideoStartElapsedMs = -1L;
    private Runnable iris26699VideoRecordingTicker;
    private View formatSelectorPill;
    private View formatExpandedPanel;
    private View formatJpgButton;
    private View formatRawButton;
    private View formatRawJpgButton;
    ...
    CameraUIViewImpl(CameraFragment cameraFragment) {
        ...
        this.iris26699VideoRecordingTicker = () -> {
            boolean active = displayedMode == CameraMode.VIDEO
                    && cameraFragment.captureController != null
                    && cameraFragment.captureController.mIsRecordingVideo;
            if (!active || mVideoRecordingInfo == null) {
                iris26699VideoStartElapsedMs = -1L;
                if (mVideoRecordingInfo != null && displayedMode != CameraMode.RAWVIDEO) {
            ...
            if (iris26699VideoStartElapsedMs < 0L) {
                iris26699VideoStartElapsedMs = SystemClock.elapsedRealtime();
            }
            long elapsedMs = Math.max(0L, SystemClock.elapsedRealtime() - iris26699VideoStartElapsedMs);
            long totalSeconds = elapsedMs / 1000L;
            mVideoRecordingInfo.setText(String.format(java.util.Locale.US, "%02d:%02d",
                    totalSeconds / 60L, totalSeconds % 60L));
            mVideoRecordingInfo.setAlpha(1.0f);
            mVideoRecordingInfo.setVisibility(View.VISIBLE);
            long delayMs = Math.max(100L, 1000L - (elapsedMs % 1000L));
            mVideoRecordingInfo.postDelayed(iris26699VideoRecordingTicker, delayMs);
        };
        this.initListeners();
    ...
    private void initModeState(CameraMode mode) {
        ...
        switch (mode) {
            ...
            case NIGHT:
                ...
                break;
            case SPEKTRA:
                currentState = new SpektraModeState();
                break;
            default:
        ...
        // IRIS_26683_SPEKTRA_ATOMIC_PRESENTATION: the Spektra owner reveals its SurfaceView
        // only after a real matched RAW+CaptureResult frame has rendered successfully.
        cameraFragment.setSpektraPreviewVisible(false);
    }
    ...
    private void initLiquidUi() {
        ...
        formatJpgButton = root.findViewById(R.id.format_jpg_button);
        formatRawButton = root.findViewById(R.id.format_raw_button);
        formatRawJpgButton = root.findViewById(R.id.format_raw_jpg_button);
    ...
    private void selectFormat(int value) {
        if (displayedMode == CameraMode.SPEKTRA) {
            refreshFormatStatus();
            collapseFormatPanel();
            return;
        }
    ...
    private void refreshFormatStatus() {
        ...
        final boolean spektra = displayedMode == CameraMode.SPEKTRA;
        final boolean heicAllowed = !spektra && iris26636HeicAllowed();
        if (formatJpgButton != null) formatJpgButton.setVisibility(View.VISIBLE);
        if (formatRawButton != null) formatRawButton.setVisibility(spektra ? View.GONE : View.VISIBLE);
        if (formatRawJpgButton != null) formatRawJpgButton.setVisibility(spektra ? View.GONE : View.VISIBLE);
        if (formatHeicButton != null) formatHeicButton.setVisibility(heicAllowed ? View.VISIBLE : View.GONE);
        if (spektra) {
            // Mode-local presentation only. Never mutate Photo/Night RAW/HEIC preferences.
            formatActiveLabel.setText("JPG");
            if (quadStatusContainer != null) quadStatusContainer.setVisibility(View.GONE);
            return;
        }
    ...
    private void applyVideoShutterStack(boolean videoStyle) {
        ...
        if (videoStyle) {
            ...
            // IRIS_26692_VIDEO_PHOTO_CONTROL_GEOMETRY: identical Photo shutter control scale.
    ...
    private void applyAdaptiveBottomSafeLayout(boolean videoStyle) {
        ...
        // Video and RAW Video retain recording behavior only; lower-control geometry is the exact
        // Photo geometry so lens/manual/shutter/gallery/switch positions cannot diverge by mode.
        final float preferredLensTranslationY = 0.0f;
    ...
    private void applyBottomGeometry(boolean videoStyle) {
        ...
        applyAdaptiveBottomSafeLayout(false);
    }
    private void switchToMode(CameraMode cameraMode) {
        ...
        // Never reveal Spektra merely because the picker changed. Keep the proven legacy
        // preview visible during handoff; SpektraCameraOwner flips presentation atomically after
        // its first successfully rendered RAW frame. Leaving Spektra always hides its surface.
        cameraFragment.setSpektraPreviewVisible(false);
    ...
    public void setVideoRecordingInfoVisible(boolean visible) {
        if (mVideoRecordingInfo != null) {
            mVideoRecordingInfo.post(() -> {
                if (displayedMode == CameraMode.VIDEO) {
                    mVideoRecordingInfo.removeCallbacks(iris26699VideoRecordingTicker);
                    if (visible
                            && cameraFragment.captureController != null
                            && cameraFragment.captureController.mIsRecordingVideo) {
                        iris26699VideoStartElapsedMs = SystemClock.elapsedRealtime();
                        iris26699VideoRecordingTicker.run();
                    } else {
                        iris26699VideoStartElapsedMs = -1L;
                        mVideoRecordingInfo.setText("");
                        mVideoRecordingInfo.setAlpha(0.0f);
    ...
    @Override
    public void destroy() {
        if (mVideoRecordingInfo != null && iris26699VideoRecordingTicker != null) {
            mVideoRecordingInfo.removeCallbacks(iris26699VideoRecordingTicker);
        }
        iris26699VideoStartElapsedMs = -1L;
        topbar = null;
    ...
    // IRIS_26681_SPEKTRA_MODE_UI_OWNER
    // Uses the existing still-camera visual language while withholding Iris image-forming controls.
    // Format/settings-specific firewalling is applied by the dedicated Spektra UI/settings gates.
    public class SpektraModeState implements CameraModeState {
        @Override
        public void reConfigureModeViews(CameraMode mode) {
            resetCaptureProgressBar();
            ...
            cameraFragment.cameraFragmentBinding.getUimodel().setDummyAspectRatio("3:4");
            cameraFragment.cameraFragmentBinding.layoutBottombar.layoutBottombar.setBackground(null);
            cameraFragment.cameraFragmentBinding.getRoot().setBackground(
                    Utilities.resolveDrawable(cameraFragment.requireActivity(), R.attr.cameraFragmentBackground));
            toggleConstraints(mode);
        }
    }
