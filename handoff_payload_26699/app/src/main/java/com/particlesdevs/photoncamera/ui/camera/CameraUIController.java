...
final class CameraUIController implements CameraUIEventsListener,
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.shutter_button:
                ...
                switch (cameraFragment.captureController.getAuthoritativeCameraMode()) {
                    ...
                    case NIGHT:
                        ...
                        break;
                    case SPEKTRA:
                        if (!cameraFragment.captureController.getSpektraModeController().isShutterReady()) {
                            Log.w(TAG, "IRIS_26687_SPEKTRA_SHUTTER_UI_REJECT status="
                                    + cameraFragment.captureController.getSpektraModeController().debugStatus());
                    ...
                    case VIDEO:
                        if (!cameraFragment.captureController.mIsRecordingVideo) {
                            ...
                            cameraFragment.setVideoRecordingInfoVisible(true);
                            view.setActivated(false);
                        } else {
                            cameraFragment.captureController.VideoEnd();
                            cameraFragment.setVideoRecordingInfoVisible(false);
                            view.setActivated(true);
                            restoreVideoIdleScaleAfterStop(view);
                        }
                        break;
            ...
            case R.id.approved_manual_handle:
                cameraFragment.cameraFragmentBinding
...
case R.id.flash_button:
    ...
    public void onCameraModeChanged(CameraMode cameraMode) {
         ...
         * The departing camera owner must be completely retired before the selected-mode
         * preference changes. This prevents callbacks/resources from the old mode from observing
         * the new mode and prevents the new owner from opening while the old owner is still alive.
         */
        if (previousMode != cameraMode) {
            // IRIS_26696_ASYNC_MODE_HANDOFF: the destination mode is not committed/opened until
            // the departing Spektra CameraDevice has actually delivered onClosed().
            cameraFragment.captureController.retireModeForTransition(previousMode, cameraMode,
                    () -> commitAndRestartMode(previousMode, cameraMode));
            return;
        }
        commitAndRestartMode(previousMode, cameraMode);
    }

    private void commitAndRestartMode(CameraMode previousMode, CameraMode cameraMode) {
        // Persist and synchronously update Settings.selectedMode before restartCamera() can
        // inspect the destination. The asynchronous Spektra release callback arrives on UI.
        cameraFragment.captureController.commitCameraModeForTransition(cameraMode);
        Log.i(TAG, "IRIS_26696_MODE_COMMIT previous=" + previousMode
    ...
    private void onTimerFinished() {
        ...
        /* IRIS_26550_SHUTTER_VIEWFINDER_SNAPSHOT
         * Motion keeps the exact 26516 shutter-time snapshot behavior. Night now requests the same
         * display-only PixelCopy so its post-capture presentation can restore the highlight-safe
            ...
            if (topBarSettingsData.getType() instanceof SettingType) {
                ...
                switch (type) {
                    ...
                    case SUPER_RES:
                        ...
                        // IRIS_26636_HEIC_SUPER_RES_EXCLUSION: refresh the pill immediately.
                        cameraFragment.updateSettingsBar();
                        break;
