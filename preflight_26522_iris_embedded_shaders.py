...
def main():
    ...
    text=p.read_text()
    for marker in ('IRIS_26521_V4_DIRECTIONAL_GREEN','IRIS_26521_V4_ROBUST_SPATIAL_KERNEL','IRIS_26521_V4_ROBUST_COLOR_DIFFERENCE','IRIS_26520_V5_CONTINUOUS_FINEST_LK_TRANSPORT','IRIS_26521_V5_CORRECTED_SPATIAL_INFRASTRUCTURE','IRIS_26522_DNG_EFFECTIVE_SUPPORT_ACCUMULATOR','IRIS_26522_DNG_EFFECTIVE_SUPPORT_Q8'):
    ...
    total=0
    with tempfile.TemporaryDirectory(prefix='iris26522_glsl_') as td:
        td=Path(td)
        for mat in TRIPLE.finditer(text):
            ...
            if cp.returncode!=0: print(cp.stdout); raise SystemExit(f'glslang failed {name}')
            total+=1
        ...
        for version in (300,310):
            ...
            if cp.returncode!=0: print(cp.stdout); raise SystemExit('fullscreen vertex failed')
            total+=1
    print(f'PASS: glslang compiled {total} Iris26522 embedded runtime shader programs')
