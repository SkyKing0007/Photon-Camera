...
if len(sys.argv)!=4:raise SystemExit('usage: verify_26702_patches.py ROOT BASE26701 CANDIDATE')
base=Path(sys.argv[2]);cand=Path(sys.argv[3]);pkg=Path(__file__).resolve().parent;f=pkg/'26702_RUNTIME_DELTA_FROM_26701.patch';r=pkg/'26702_RUNTIME_ROLLBACK_TO_26701.patch'
...
BH=H(base);CH=H(cand)
with tempfile.TemporaryDirectory(prefix='i26702_') as td:
...
print('PASS 26702 deterministic full-index forward/rollback patches at core.abbrev 7/12/40; fuzz=0 exact replay + rollback')
