...
FAILED_WORK="/workspaces/Photon-Camera/build_26175_iris_camera_brand_ui_resume_20260731_180633"
STAMP="$(date +%Y%m%d_%H%M%S)"

RECOVERY_WORK="/workspaces/Photon-Camera/recover_second_failed_iris_26175_${STAMP}"
BACKUP_BRANCH="backup-before-second-iris-26175-recovery-${STAMP}"

fail_recovery() {
    echo
    echo "============================================================"
    echo " IRIS 26175 SECOND RECOVERY STOPPED"
    echo "============================================================"
    echo "Reason: $1"
    exit 1
}
...
echo "============================================================"
echo " Iris Camera 26175 second failed-state recovery"
echo "============================================================"
echo

[ "$(git branch --show-current)" = "$EXPECTED_BRANCH" ] \
    || fail_recovery "Wrong branch"

[ "$(git rev-parse HEAD)" = "$EXPECTED_HEAD" ] \
    || fail_recovery "Unexpected base HEAD"

grep -q '^VERSION_BUILD=26175$' app/version.properties \
    || fail_recovery "Expected partial failed state with VERSION_BUILD=26175"

[ -d "$FAILED_WORK/before" ] \
    || fail_recovery "Missing pre-edit backup: $FAILED_WORK/before"

[ -f "$FAILED_WORK/working-tree-before-26175.patch" ] \
    || fail_recovery "Missing pre-edit patch"

mkdir -p "$RECOVERY_WORK"

echo "Protecting the current partial state..."
...
echo "Restoring the exact source set saved before the second failed attempt..."
...
echo
echo "Pre-failure 26174 state restored."
echo "Starting corrected Iris Camera 26175 continuation..."
echo
...
style_start_token = '<style name="LiquidGlassMenuText"'
style_start = text.find(style_start_token)

if style_start < 0:
    raise SystemExit("LiquidGlassMenuText style start was not found")

style_end = text.find("</style>", style_start)

if style_end < 0:
    raise SystemExit("LiquidGlassMenuText style end was not found")
...
for old, new, label in replacements:
    count = style_block.count(old)
    if count != 1:
        raise SystemExit(
            f"{label}: expected one match inside LiquidGlassMenuText, found {count}"
        )
    style_block = style_block.replace(old, new, 1)
