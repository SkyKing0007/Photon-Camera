...
def main():
 ...
 if sb==sc:fail('26592 SHORT shader owner did not change')
 ...
 if cpp==cpu:fail('26592 true2x native shader owner did not change')
 ...
 (out/'V1_26592_RUNTIME_EXPANDED_SHADERS.sha256').write_text(''.join(manifest));(out/'V1_26592_SHADER_VERIFICATION.json').write_text(json.dumps(rows,indent=2)+'\n')
 print('PASS exact runtime-expanded shader extraction + complete reserved scan variants=13 including modified SHORT mask, 1x render and true2x publication compute')
