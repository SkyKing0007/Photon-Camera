...
def init(td,src):
    ...
    run(['git','init','-q'],td); run(['git','config','user.name','Proof'],td); run(['git','config','user.email','proof@local'],td)
    run(['git','add','-A'],td); run(['git','commit','-q','-m','start'],td)
def regen(a,b,abbr):
    td=Path(tempfile.mkdtemp(prefix='p26598v11_'))
    try:
        init(td,a)
...
def apply(a,b,p):
    td=Path(tempfile.mkdtemp(prefix='a26598v11_'))
    try:
        init(td,a)
        run(['git','apply','--check','--whitespace=nowarn',str(p)],td)
        run(['git','apply','--whitespace=nowarn',str(p)],td)
