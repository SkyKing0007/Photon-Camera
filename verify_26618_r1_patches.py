...
if len(sys.argv)!=5: raise SystemExit('usage: verify_26618_r1_patches.py BASE CAND FWD ROLLBACK')
...
with tempfile.TemporaryDirectory() as td:
    ...
    subprocess.run(['git','init','-q'],cwd=repo,check=True); subprocess.run(['git','config','user.name','Photon26618'],cwd=repo,check=True); subprocess.run(['git','config','user.email','photon26618@example.invalid'],cwd=repo,check=True)
    ...
    for root,patch,target in [(fw,F,C),(rb,R,B)]:
        env=dict(**__import__('os').environ); env['GIT_CEILING_DIRECTORIES']=str(root.parent)
...
print('PASS 26618 deterministic full-index patches core.abbrev=7/12/40; fuzz=0 exact forward/rollback; diff --check clean')
