...
if len(sys.argv) not in (4,6): raise SystemExit('usage: verify_26658_shaders.py ROOT BASE CANDIDATE [--compiler PATH]')
...
def embedded(rootp):
 ...
 return out
bm=readm(root/'R1_26658_SHADER_UNIVERSE_BASE.sha256');cm=readm(root/'R1_26658_SHADER_UNIVERSE_CANDIDATE.sha256');assert len(bm)==len(cm)==257 and bm==cm
for r,h in bm.items(): assert sha(base/r)==h and sha(cand/r)==h,r
# Recompile the exact same 8 runtime-expanded variants that passed 26653.
def expand_asset(r,defines=None):
 ...
 for val in lines:
  ...
  if '#import' in val: raise SystemExit(f'FAIL 26658 unexpected import in inherited runtime shader {r}: {val}')
  if '#define' in val:
...
expected=readm(root/'R1_26658_INHERITED_RUNTIME_EXPANDED_SHADERS.sha256');assert len(expected)==len(variants)==8 and set(expected)==set(variants)
...
with tempfile.TemporaryDirectory(prefix='iris26658_shader_') as td:
 out=Path(td)
 for name,src in sorted(variants.items()):
  ...
  if compiler:
   ...
   if cp.returncode!=0: raise SystemExit(f'26658 GLSL FAIL {name}\n{cp.stdout}\n{cp.stderr}')
print(f'PASS 26658 inherited exact runtime GLSL variants=8 real_compiler={bool(compiler)} standaloneShaderUniverse=257 byte-identical to successful 26653')
