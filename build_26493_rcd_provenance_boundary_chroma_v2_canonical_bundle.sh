...
NEW_BUILD="26493"
APK_NAME="IrisCamera-${NEW_VERSION}-${NEW_BUILD}-rcd-provenance-boundary-chroma-v2-debug.apk"
REPO="$(pwd)"
OUTDIR="build_26493_v2_outputs"
WORK="${RUNNER_TEMP:-/tmp}/photon_26493_v2_work_$$"
PREPATCH="$OUTDIR/26493_pre_edit_exact_26492_v5b_complete_binary.patch"
FINALPATCH="$OUTDIR/26493_successful_complete_binary.patch"
...
echo "=== 26493 V2 RCD PROVENANCE-BOUNDARY CHROMA REFINEMENT — COMPLETE V5B BUNDLE ==="
date -Iseconds
...
pass "lineage + complete V5B bundle + transform identity"
...
sha256sum "$PREPATCH" > "$OUTDIR/26493_pre_edit_exact_26492_v5b_complete_binary.patch.sha256"
...
sha256sum "$BASELINE_BUNDLE" "$BASELINE_MANIFEST" "$TRANSFORM" "$PREPATCH" "$FINALPATCH" "$NEXTBUNDLE" "$AFTERHASH" "$REPORT" "$APK_NAME" > "$OUTDIR/26493_artifact_hashes.sha256"

echo "=== 26493 V2 BUILD SUCCESS ==="
cat "$REPORT"
