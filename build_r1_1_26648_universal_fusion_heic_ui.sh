...
HANDOFF="$ROOT/R1_1_26648_HANDOFF_HASHES.sha256"; UPLOADS="$ROOT/R1_1_26648_UPLOAD_PATHS.txt"
BASE_FULL="$ROOT/R1_1_26648_BASE_26646_R1_FULL_APP.sha256"; CAND_FULL="$ROOT/R1_1_26648_EXPECTED_CANDIDATE_FULL_APP.sha256"
BASE_PROTECTED="$ROOT/R1_1_26648_PROTECTED_UNCHANGED_BASE.sha256"; CAND_PROTECTED="$ROOT/R1_1_26648_PROTECTED_UNCHANGED_CANDIDATE.sha256"
BASE_NATIVE="$ROOT/R1_1_26648_NATIVE_PROTECTED_BASE.sha256"; CAND_NATIVE="$ROOT/R1_1_26648_NATIVE_PROTECTED_CANDIDATE.sha256"
BASE_VENDOR="$ROOT/R1_1_26648_VENDOR_PROTECTED_BASE.sha256"; CAND_VENDOR="$ROOT/R1_1_26648_VENDOR_PROTECTED_CANDIDATE.sha256"
BASE_DNG="$ROOT/R1_1_26648_DNG_BASE.sha256"; CAND_DNG="$ROOT/R1_1_26648_DNG_CANDIDATE.sha256"
SHADER_BASE="$ROOT/R1_1_26648_SHADER_UNIVERSE_BASE.sha256"; SHADER_CAND="$ROOT/R1_1_26648_SHADER_UNIVERSE_CANDIDATE.sha256"
CHANGED="$ROOT/R1_1_26648_RUNTIME_CHANGED_PATHS.txt"; PREWRITE="$ROOT/R1_1_26648_PREWRITE_SOURCE_HASHES.sha256"; ADDED="$ROOT/R1_1_26648_ADDED_PATHS_MUST_BE_ABSENT.txt"; EXPECTED_CHANGED="$ROOT/R1_1_26648_EXPECTED_CHANGED_SOURCE_HASHES.sha256"
FORWARD="$ROOT/R1_1_26648_RUNTIME_DELTA_FROM_26646_R1.patch"; ROLLBACK="$ROOT/R1_1_26648_RUNTIME_ROLLBACK_TO_26646_R1.patch"; SHADER_PIN="$ROOT/R1_1_26648_RUNTIME_EXPANDED_SHADERS.sha256"
TRANSFORM="$ROOT/transform_r1_1_26648.py"; VALIDATE="$ROOT/validate_r1_1_26648.py"; AUTHORITY="$ROOT/verify_r1_1_26648_authority.py"; INFRA="$ROOT/verify_r1_1_26648_infrastructure.py"; PATCHVERIFY="$ROOT/verify_r1_1_26648_patches.py"; SHADERVERIFY="$ROOT/verify_r1_1_26648_shaders.py"; GATEVERIFY="$ROOT/verify_r1_1_26648_regressions.py"
BUILD_SCRIPT="$ROOT/build_r1_1_26648_universal_fusion_heic_ui.sh"; WORKFLOW="$ROOT/.github/workflows/build-r1-1-26648-universal-fusion-heic-ui.yml"
OUT="$ROOT/build_r1_1_26648_universal_fusion_heic_ui_outputs"; WORK="$ROOT/.build_r1_1_26648_universal_fusion_heic_ui_work"
ARTZIP="$WORK/26646_r1_artifact.zip"; ARTDIR="$WORK/artifact"; BASE="$WORK/exact_26646_r1_compiled_candidate"; AFTER="$WORK/candidate_26648_r1_1"; AFTER2="$WORK/candidate_26648_r1_1_1_replay"; LIVE_CANON="$WORK/live_compiler_candidate_snapshot"; POST="$WORK/postbuild_source_snapshot"; GLSLANG_DIR="$WORK/glslang-${GLSLANG_VERSION}"
FINAL="$ROOT/IrisCamera-${VERSION_NAME}-${VERSION_BUILD}-r1-universal-fusion-heic-ui-debug.apk"
...
cat > "$OUT/26648_R1_1_COMPILER_STATUS.txt" <<'STATUS'
...
STATUS
cat > "$OUT/26648_R1_1_STRICT_HANDOFF_REPORT.txt" <<'REPORT'
...
REPORT
set_report(){ local key="$1" val="$2" tmp="$OUT/.report.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26648_R1_1_STRICT_HANDOFF_REPORT.txt" > "$tmp" || fail "report key missing $key"; mv "$tmp" "$OUT/26648_R1_1_STRICT_HANDOFF_REPORT.txt"; }
set_compiler(){ local key="$1" val="$2" tmp="$OUT/.compiler.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26648_R1_1_COMPILER_STATUS.txt" > "$tmp" || fail "compiler key missing $key"; mv "$tmp" "$OUT/26648_R1_1_COMPILER_STATUS.txt"; }
...
verify_package(){
  [[ -f "$HANDOFF" && -f "$UPLOADS" && -d "$ROOT/handoff_payload_r1_1_26648" ]] || fail "sealed 26648 package incomplete"
  [[ "$(find "$ROOT/handoff_payload_r1_1_26648" -type f | wc -l)" -eq 8 ]] || fail "sealed runtime payload must contain exactly 8 files"
  [[ "$(wc -l < "$CHANGED")" -eq 8 && "$(wc -l < "$ADDED")" -eq 0 ]] || fail "26648 changed/add counts"
  sha256sum -c "$HANDOFF" >/dev/null
  [[ "$(wc -l < "$BASE_FULL")" -eq 1720 && "$(wc -l < "$CAND_FULL")" -eq 1720 ]] || fail "full app count"
  [[ "$(wc -l < "$BASE_PROTECTED")" -eq 1712 && "$(wc -l < "$BASE_NATIVE")" -eq 800 && "$(wc -l < "$BASE_VENDOR")" -eq 778 && "$(wc -l < "$BASE_DNG")" -eq 7 ]] || fail "protected manifest counts"
...
verify_scope(){
  if [[ -n "$LOCAL_ART" ]]; then set_report "CHANGED RUNTIME SCOPE" "PASS (local sealed exact 8-path/0-added 26648 R1.1 candidate; no live app source packaged)"; return; fi
  ...
  [[ "$(git rev-parse HEAD^)" == "$failed26648" ]] || fail "26648 R1.1 handoff commit parent must be exact failed 26648 R1 handoff commit"
  [[ "$(git rev-parse ${failed26648}^)" == "$rejected26647" ]] || fail "failed 26648 R1 parent must remain exact rejected 26647 handoff commit"
  [[ "$(git rev-parse ${rejected26647}^)" == "$RUNTIME_AUTHORITY_COMMIT" ]] || fail "rejected 26647 parent must remain exact successful 26646 runtime authority"
  ! git diff --name-only "$RUNTIME_AUTHORITY_COMMIT".."$rejected26647" | grep -Eq '^app/' || fail "rejected 26647 handoff unexpectedly committed live app source"
  ! git diff --name-only "$rejected26647".."$failed26648" | grep -Eq '^app/' || fail "failed 26648 R1 handoff unexpectedly committed live app source"
  git diff --name-only "$failed26648"..HEAD | sort > "$WORK/actual_upload_scope.txt"
  sort "$UPLOADS" > "$WORK/expected_upload_scope.txt"
  diff -u "$WORK/expected_upload_scope.txt" "$WORK/actual_upload_scope.txt" || fail "26648 R1.1 upload scope mismatch"
  ! grep -Eq '^app/' "$WORK/actual_upload_scope.txt" || fail "26648 R1.1 handoff commit contains live app source"
  set_report "CHANGED RUNTIME SCOPE" "PASS (exact 8-path/0-added runtime candidate allowlist; 26647 + failed-26648 intervening commits proven handoff-only; runtime reconstructed from successful 26646 compiled candidate)"
}
obtain_authority(){
...
make_candidate(){
  ...
  while IFS= read -r rel; do [[ -n "$rel" ]] || continue; cmp "$ROOT/handoff_payload_r1_1_26648/$rel" "$AFTER/$rel" || fail "sealed payload differs frozen candidate: $rel"; done < "$CHANGED"
  ...
  set_report "26646 PRIOR PROTECTED RUNTIME FREEZE" "PASS (1712 protected files byte-identical; successful 26646 tone/CFA/denoise/DNG/capture/frame-policy/SR-detail and unrelated owners retained)"
  ...
  set_report "BASE/CANDIDATE MANIFEST COMPLETENESS" "PASS (1720 base / 1720 candidate / 1712 protected / 800 inherited native-protected / 778 vendor / 7 DNG / 257 shader universe)"
...
verify_successful_26646_mechanics(){
  ...
  set_report "INFRASTRUCTURE DELTA AUDIT" "PASS (exact successful-26646 compiler/native/patch/PRE-BUILD/assemble/postbuild ordering and workflow toolchain retained; only 26648 identity/scope/semantic assertions and required handoff-only intervening-commit scope proof changed)"
...
postbuild_proof(){
  ...
  tar --sort=name --mtime='UTC 1970-01-01' --owner=0 --group=0 --numeric-owner -cf - -C "$POST" app | gzip -n > "$OUT/26648_R1_1_candidate_app_source.tar.gz"
  sha256sum "$OUT/26648_R1_1_candidate_app_source.tar.gz" > "$OUT/26648_R1_1_candidate_app_source.tar.gz.sha256"
  cp "$CAND_FULL" "$OUT/26648_R1_1_candidate_full_app.sha256"; cp "$CAND_NATIVE" "$OUT/26648_R1_1_native_protected_postbuild.sha256"; cp "$CAND_VENDOR" "$OUT/26648_R1_1_vendor_protected_postbuild.sha256"; cp "$CAND_DNG" "$OUT/26648_R1_1_dng_postbuild.sha256"; cp "$SHADER_PIN" "$OUT/26648_R1_1_runtime_expanded_shaders.sha256"
...
if [[ -n "$LOCAL_ART" ]]; then
  ...
  cp "$OUT/26648_R1_1_STRICT_HANDOFF_REPORT.txt" "$OUT/26648_local_prebuild_report.txt"
  pass "26648 R1.1 LOCAL PREBUILD PREPARED: exact successful-26646 authority + exact 8-path universal-fusion/HEIC/UI candidate; applicable real compiler/build gates explicitly unproven locally"
  exit 0
fi
install_frozen_candidate_live
...
mv "${apks[0]}" "$FINAL"; set_report "EXACTLY ONE APK" "PASS"; sha256sum "$FINAL" > "$OUT/26648_R1_1_APK.sha256"
postbuild_proof
pass "26648 R1.1 ACTIONS BUILD COMPLETE"
