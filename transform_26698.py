...
if len(sys.argv)!=3: raise SystemExit('usage: transform_26698.py BASE26697 DEST')
base=Path(sys.argv[1]); dest=Path(sys.argv[2]); pkg=Path(__file__).resolve().parent; payload=pkg/'handoff_payload_26698'
changed=[x.strip() for x in (pkg/'26698_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x.strip()]
added=[x.strip() for x in (pkg/'26698_ADDED_PATHS_MUST_BE_ABSENT.txt').read_text().splitlines() if x.strip()]
...
def load(name):
 ...
 return out
pre=load('26698_PREWRITE_26697_SOURCE_HASHES.sha256'); exp=load('26698_EXPECTED_CHANGED_SOURCE_HASHES.sha256')
...
rm=shutil.rmtree
if dest.exists(): rm(dest)
shutil.copytree(base,dest)
for rel in changed:
 q=dest/rel; q.parent.mkdir(parents=True,exist_ok=True); shutil.copy2(payload/rel,q)
print(f'PASS transform 26698: overlaid {len(changed)} exact runtime files onto successful 26697 authority')
