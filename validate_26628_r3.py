...
if len(sys.argv)!=3: raise SystemExit('usage: validate_26628_r3.py BASE CAND')
B=Path(sys.argv[1]); C=Path(sys.argv[2])
CHANGED=[x.strip() for x in (Path(__file__).resolve().parent/'R3_26628_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x.strip()]
...
# Permanent regression from failed 26628 R1 NDK compile: pixel-coordinate parameter y may not be redeclared as float y.
if 'float y=luma(center)' in nat:
    raise SystemExit('FAIL native C++ regression: local luma variable shadows int y pixel coordinate')
if nat.count('float lum=luma(center)') != 2:
    raise SystemExit('FAIL native C++ regression: expected exactly two corrected lum declarations')

# General lens-button geometry and bounded 3dp upward preference. No device/model exceptions.
...
for tok in [
    ...
    'Math.max(-preferredLensLiftPx, minimumSelectedTop - selectedTop)',
    'final float preferredLensToControlsGapPx = 12.0f * density;',
]:
    if tok not in ui: raise SystemExit(f'FAIL bounded lens-row geometry token {tok}')
for forbidden in ['Build.MANUFACTURER','Build.MODEL','Xiaomi','OnePlus']:
    block=ui.split('private void applyAdaptiveBottomSafeLayout',1)[1].split('private void applyBottomGeometry',1)[0]
    if forbidden in block: raise SystemExit(f'FAIL device-specific lens layout regression {forbidden}')
for tok in [
    'TextView b = new TextView(getContext());','b.setGravity(Gravity.CENTER);',
    'b.setIncludeFontPadding(false);','b.setPadding(0, 0, 0, 0);',
    'b.setMinWidth(0);','b.setMinHeight(0);','b.setMinimumWidth(0);','b.setMinimumHeight(0);',
    'b.setSingleLine(true);'
]:
    if tok not in aux: raise SystemExit(f'FAIL general lens text geometry token {tok}')
for tok in ['if (!(child instanceof TextView)) continue;','TextView button = (TextView) child;']:
    if tok not in aux: raise SystemExit(f'FAIL R2 Java lens type repair token {tok}')
if re.search(r'\bButton\b', aux):
    raise SystemExit('FAIL R2 Java regression: standalone Button type/import/cast survived AuxButtonsLayout TextView migration')

print('PASS 26628 semantic/ownership/domain: exact 11-path delta; Motion/Night/SuperRes-only bjzhou-equivalent DNG color; full 3D HSV profile tables; 0.95 contraction; legacy Photon and reconstruction protected')
