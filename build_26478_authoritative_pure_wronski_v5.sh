...
EXPECTED_INFRA_PARENT="87de69aff3d927be9c031c2fce476dffe88e4d71"
...
BACKUP_TARGET="be24c16cb091e753852b900384dbde6f8d32744e"
...
DIAG_TRANSFORM="transform_26478_speaker_support_diagnostic_v1.py"
...
exec > >(tee "$AUDIT") 2>&1
echo "=== 26478 V5 PUSH-TRIGGER AUTHORITATIVE PURE WRONSKI + HIGHLIGHT-SAFE + SPEAKER DIAGNOSTIC ==="
date -Iseconds || true

BRANCH="${GITHUB_REF_NAME:-$(git branch --show-current)}"
[[ "$BRANCH" == "$EXPECTED_BRANCH" ]] || fail "wrong branch: $BRANCH"
[[ "$(git rev-parse HEAD^)" == "$EXPECTED_INFRA_PARENT" ]] || \
  fail "26478 V5 push workflow commit must be direct child of exact V4 infrastructure HEAD"
[[ "$(git rev-parse HEAD~2)" == "62fed09aa61ac963f951fd39b1a62be870d75654" ]] || \
  fail "26478 V5 chain missing exact V4 parent correction"
[[ "$(git rev-parse HEAD~3)" == "a5fa756f8b5cf1101e807b776728d08225f3e53e" ]] || \
  fail "26478 V5 chain missing exact first 26478 infrastructure commit"
[[ "$(git rev-parse HEAD~4)" == "be24c16cb091e753852b900384dbde6f8d32744e" ]] || \
  fail "26478 V5 chain must descend exactly from successful 26477 V3"
pass "branch/head exact four-commit infrastructure lineage gate"
...
cat > "$REPORT" <<EOF
26478 V5 PUSH-TRIGGER AUTHORITATIVE PURE WRONSKI + HIGHLIGHT-SAFE + SPEAKER DIAGNOSTIC
========================================================================
Rollback:
