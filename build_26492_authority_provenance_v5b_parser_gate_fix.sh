...
FAILED_V3_HEAD="0b4e8c6f716e86fb340e1a62636d184d7b4b32ce"
FAILED_V3_BACKUP_BRANCH="backup-26492-v3-failed-before-v4-proven-two-workspace-build"
BLACK_V4_HEAD="3476e961539469aeb4a7b9a6614c07a05b24e462"
BLACK_V4_BACKUP_BRANCH="backup-26492-v4-black-before-v5-rcd-parser-fix"
GATE_FAILED_V5_HEAD="fab1ae1b698cbb1e5462ec41fdac3a535eeed492"
...
TRANSFORM_V5="transform_26492_rcd_parser_compat_v5.py"
TRANSFORM_V5_SHA="b24ab77f4e4d6fb43c72c881b9dfb455ca7d43ffc2437bf1baa6ac74629dc866"
NEW_VERSION="0.9726492"
NEW_BUILD="26492"
OUTDIR="build_26492_v5b_outputs"
APK_NAME="IrisCamera-${NEW_VERSION}-${NEW_BUILD}-v5b-authority-provenance-r32f-rcd-parser-fix-debug.apk"
...
PREPATCH="$OUTDIR/26492_v4_black_pre_edit_binary.patch"
DELTAPATCH="$OUTDIR/26492_source.patch"
AFTERHASH="$OUTDIR/26492_after.sha256"
REPORT="$OUTDIR/26492_build_report.txt"
exec > >(tee "$AUDIT") 2>&1

echo "=== 26492 V5B AUTHORITY / PROVENANCE — RCD PARSER FIX + SEMANTIC RUNTIME GATE ==="
date -Iseconds || true
...
git cat-file -e "$FAILED_V3_HEAD^{commit}" || fail "failed 26492 V3 infrastructure commit unavailable"
git cat-file -e "$BLACK_V4_HEAD^{commit}" || fail "black 26492 V4 checkpoint unavailable"
git cat-file -e "$GATE_FAILED_V5_HEAD^{commit}" || fail "V5 gate-failed infrastructure commit unavailable"
...
git merge-base --is-ancestor "$FAILED_V2_HEAD" HEAD || fail "26492 V5 must descend from the recorded V2 infrastructure checkpoint"
git merge-base --is-ancestor "$FAILED_V3_HEAD" HEAD || fail "26492 V5 must descend from the recorded V3 infrastructure checkpoint"
git merge-base --is-ancestor "$FAILED_26492_V1_HEAD" HEAD || fail "26492 V5 must descend from failed V1 infrastructure checkpoint"
git merge-base --is-ancestor "$BLACK_V4_HEAD" HEAD || fail "26492 V5B must descend from exact black V4 checkpoint"
git merge-base --is-ancestor "$GATE_FAILED_V5_HEAD" HEAD || fail "26492 V5B must descend from exact V5 gate-failed checkpoint"
...
[[ -f "$TRANSFORM_V5" && "$(sha "$TRANSFORM_V5")" == "$TRANSFORM_V5_SHA" ]] || fail "26492 V5 parser-compat transform identity mismatch"
python3 -m py_compile "$TRANSFORM_26491" "$TRANSFORM_26492" "$TRANSFORM_V5" || fail "transform Python syntax"
python3 "$TRANSFORM_26491" --self-test || fail "26491 self-test"
python3 "$TRANSFORM_26492" --self-test || fail "26492 self-test"
python3 "$TRANSFORM_V5" --self-test || fail "26492 V5 parser-compat self-test"
...
v3_remote="$(git ls-remote origin "refs/heads/$FAILED_V3_BACKUP_BRANCH" | awk '{print $1}')"
[[ "$v3_remote" == "$FAILED_V3_HEAD" ]] || fail "V3 backup $FAILED_V3_BACKUP_BRANCH=$v3_remote expected=$FAILED_V3_HEAD"
pass "failed V3 backup exact checkpoint"
v4_remote="$(git ls-remote origin "refs/heads/$BLACK_V4_BACKUP_BRANCH" | awk '{print $1}')"
[[ "$v4_remote" == "$BLACK_V4_HEAD" ]] || fail "automatic V4 rollback branch $BLACK_V4_BACKUP_BRANCH=$v4_remote expected=$BLACK_V4_HEAD"
pass "automatic exact V4 black-build rollback branch verified"

TMP="$(mktemp -d)"
BASE="$TMP/exact-26491"
CAND="$TMP/candidate-26492"
FINAL="$TMP/final-26492"
cleanup(){
  set +e
  git worktree remove --force "$BASE" >/dev/null 2>&1 || true
  git worktree remove --force "$CAND" >/dev/null 2>&1 || true
  git worktree remove --force "$FINAL" >/dev/null 2>&1 || true
  rm -rf "$TMP"
}
trap cleanup EXIT
...
python3 "$REPO/$TRANSFORM_26492" "$CAND" || fail "26492 V4 architecture transform application"

# Required binary pre-edit patch: capture the exact BLACK V4 application source
# BEFORE the V5 runtime-parser correction touches one shader declaration region.
( cd "$CAND" && git diff --binary "$EXPECTED_APP_BASE" -- app/src/main app/version.properties ) > "$PREPATCH"
[[ -s "$PREPATCH" ]] || fail "pre-edit exact black-V4 source patch is empty"
sha256sum "$PREPATCH" | tee "$OUTDIR/26492_v4_black_pre_edit_binary.patch.sha256"
pass "binary pre-edit exact black-V4 source patch created before V5 modification"

python3 "$REPO/$TRANSFORM_V5" "$CAND" || fail "26492 V5 RCD parser-compat transform application"
pass "26492 V5 surgical RCD parser correction applied"
...
# GATE 4B: verify Photon runtime layout parsing semantically, not by brittle
# escaped-source-string equality. This catches the V4 black-frame SSBO binding bug
# while tolerating harmless Java formatting/escaping representation.
python3 - "$CAND" <<'PY_RUNTIME_LAYOUT'
from pathlib import Path
import re,sys
root=Path(sys.argv[1])
gl=root/'app/src/main/java/com/particlesdevs/photoncamera/processing/opengl/GLInterface.java'
shader=root/'app/src/main/assets/shaders/motionv2/rcd26489_populate.glsl'
host=root/'app/src/main/java/com/particlesdevs/photoncamera/processing/opengl/postpipeline/MotionV2RcdDemosaic.java'
g=gl.read_text()

# Isolate the actual getLayouts() method and verify the structural parser contract.
sig='public static Map<String, GLComputeLayout> getLayouts(String program)'
start=g.find(sig)
end=g.find('public static String readProgram', start)
if start < 0 or end < 0 or end <= start:
    raise SystemExit('GLInterface.getLayouts method not found or boundaries changed')
method=g[start:end]

structural_checks = {
    'layout-line filter': r'val\.contains\(\s*"layout"\s*\)',
    'brace removal + literal-space split': r'val\.replace\(\s*"\{"\s*,\s*""\s*\)\.split\(\s*" "\s*\)',
    'last-token resource naming': r'divided\s*\[\s*divided\.length\s*-\s*1\s*\]',
    'binding extraction': r'getParameter\(\s*parameters\s*,\s*"binding"\s*\)',
    'layout map write': r'layoutsMap\.put\(\s*last\s*,',
}
for label, pattern in structural_checks.items():
    if not re.search(pattern, method, re.S):
        raise SystemExit('GLInterface.getLayouts structural contract changed: '+label)

def java_space_split(text):
...
def parse(program):
    ...
    for val in program.splitlines():
        if 'layout' not in val:
            continue
        divided=java_space_split(val.replace('{',''))
        last=''
        if divided:
        ...
        if ri < 0:
            ri=len(val)-1
        params=val[li+1:ri].split(',')
        binding=0
        for x in params:
            pv=x.replace(' ','').split('=')
            if len(pv)==2 and pv[0]=='binding':
                binding=int(pv[1])
                break
        out[last]=binding
    return out

# Behavioral proof that our emulation detects the exact compact-layout failure
# and accepts the parser-safe multiline form.
compact='layout(std430, binding = 0) buffer CfaBuf { float cfa[]; };'
safe='layout(std430, binding = 0) buffer CfaBuf {\n    float cfa[];\n};'
compact_result=parse(compact)
safe_result=parse(safe)
if compact_result.get('}') != 0 or 'CfaBuf' in compact_result:
    raise SystemExit('runtime parser emulation no longer reproduces compact-layout failure')
if safe_result.get('CfaBuf') != 0:
    raise SystemExit('runtime parser emulation does not accept parser-safe multiline declaration')

# Apply the same emulation to the actual transformed 26492 RCD shader.
layouts=parse(shader.read_text())
expected={'CfaBuf':0,'RedBuf':1,'GreenBuf':2,'BlueBuf':3}
actual={k:layouts.get(k) for k in expected}
if actual != expected:
    raise SystemExit('PHOTON RUNTIME LAYOUT PARSER FAILURE expected='+repr(expected)+' actual='+repr(actual)+' keys='+repr(sorted(layouts)))
if '}' in layouts:
    raise SystemExit("PHOTON RUNTIME LAYOUT PARSER FAILURE bogus compact-layout key '}' survived")

# Host names must match the parser-visible resource names.
h=host.read_text()
for name in expected:
    if not re.search(r'setBufferCompute\(\s*"'+re.escape(name)+r'"', h):
        raise SystemExit('RCD host missing setBufferCompute for parser-visible '+name)

print('26492 PHOTON RUNTIME LAYOUT SEMANTIC PASS CfaBuf=0 RedBuf=1 GreenBuf=2 BlueBuf=3 compactFailureDetected=true safeMultilineAccepted=true')
PY_RUNTIME_LAYOUT
...
# Exact successful-26491 handling for known CMake configure-time downloads.
# Candidate owns its generated copies and KEEPS them. A separate fresh FINAL worktree
# must configure/download independently. Only after byte equality are FINAL copies
# removed before canonical source delta/manifests.
...
pass "candidate generated native dependencies captured and retained for final comparison"

# GATE 6: reconstruct a completely fresh exact-26491 FINAL worktree.
# This is the proven two-workspace sequence: candidate is validation-only; FINAL
# independently configures CMake and downloads its own native dependency headers.
reconstruct_26491 "$FINAL"
while IFS= read -r rel; do
  [[ -n "$rel" ]] || continue
  mkdir -p "$FINAL/$(dirname "$rel")"
  cp "$CAND/$rel" "$FINAL/$rel"
  cmp -s "$CAND/$rel" "$FINAL/$rel" || fail "candidate/final functional copy mismatch: $rel"
done < "$TMP/allowed.txt"
pass "fresh final worktree reconstructed from exact 26491 + proven candidate functional bytes"

# Before the final build, generated native dependencies must be absent from FINAL,
# proving its CMake configuration/download happens independently of CAND.
while IFS= read -r rel; do
  [[ -n "$rel" ]] || continue
  [[ ! -e "$FINAL/$rel" ]] || fail "fresh final tree unexpectedly contains generated native dep before build: $rel"
done < "$TMP/generated-native-deps.txt"
pass "fresh final tree has no inherited generated native dependencies"

# Exact 9-file source equality before version mutation.
python3 - "$BASE" "$FINAL" "$TMP/allowed.txt" <<'PY_PREFINAL'
from pathlib import Path
import hashlib,sys
A,B=Path(sys.argv[1]),Path(sys.argv[2]); allowed=set(Path(sys.argv[3]).read_text().splitlines())
def h(p): return hashlib.sha256(p.read_bytes()).digest()
changed=set()
rels={str(x.relative_to(A)) for x in (A/'app/src/main').rglob('*') if x.is_file()} | {str(x.relative_to(B)) for x in (B/'app/src/main').rglob('*') if x.is_file()}
for rel in rels:
    a,b=A/rel,B/rel
    if not a.exists() or not b.exists() or h(a)!=h(b): changed.add(rel)
if changed != allowed: raise SystemExit('fresh final pre-version scope mismatch '+repr(sorted(changed)))
print('26492 fresh final pre-version exact functional scope PASS files=9')
PY_PREFINAL

# VERSION IS LAST. Version increment and final APK build remain one guarded command block.
{
  sed -i 's/^VERSION_NAME=.*/VERSION_NAME='"$NEW_VERSION"'/; s/^VERSION_BUILD=.*/VERSION_BUILD='"$NEW_BUILD"'/' "$FINAL/app/version.properties"
  grep -q '^VERSION_NAME='"$NEW_VERSION"'$' "$FINAL/app/version.properties" || fail "final version name"
  grep -q '^VERSION_BUILD='"$NEW_BUILD"'$' "$FINAL/app/version.properties" || fail "final version build"
  ( cd "$FINAL" && ./gradlew clean assembleDebug --stacktrace ) > "$FINALLOG" 2>&1 || { tail -260 "$FINALLOG"; fail "final 26492 fresh-worktree build"; }
}
pass "version increment + final APK built in one guarded command block on fresh final worktree"

APK_SRC="$(find "$FINAL/app/build/outputs/apk/debug" -maxdepth 1 -type f -name '*.apk' -print | head -1)"
...
# Candidate/final independently generated native bytes must match exactly.
python3 - "$CAND" "$FINAL" "$TMP/generated-native-deps.txt" <<'PY_DEPS'
from pathlib import Path
import hashlib,sys
cand,final=Path(sys.argv[1]),Path(sys.argv[2])
for rel in [x for x in Path(sys.argv[3]).read_text().splitlines() if x.strip()]:
    a,b=cand/rel,final/rel
    if not a.is_file(): raise SystemExit('candidate generated native dependency missing: '+rel)
    if not b.is_file(): raise SystemExit('final generated native dependency missing: '+rel)
...
PY_DEPS
pass "candidate/final independently generated native dependency equality PASS"

# Only FINAL is canonical. Remove its generated downloads after successful build/equality.
# Candidate copies remain untouched as validation evidence until cleanup trap removes worktrees.
while IFS= read -r rel; do
  [[ -n "$rel" ]] || continue
  rm -f "$FINAL/$rel"
  [[ ! -e "$FINAL/$rel" ]] || fail "final generated native dependency cleanup failed: $rel"
done < "$TMP/generated-native-deps.txt"
pass "final generated native dependencies removed before canonical delta/manifests"

# Final exact delta must be 9 functional files + version only.
python3 - "$BASE" "$FINAL" "$TMP/allowed.txt" <<'PY_FINAL'
from pathlib import Path
import hashlib,sys
...
( cd "$FINAL" && git diff --binary "$EXPECTED_APP_BASE" -- app/src/main app/version.properties ) > "$OUTDIR/next_baseline_inputs/26492_successful_source.patch"
( cd "$FINAL" && find app/src/main -type f -print0 | sort -z | xargs -0 sha256sum; sha256sum app/version.properties ) > "$OUTDIR/next_baseline_inputs/26492_successful_after.sha256"
...
set +e
git diff --no-index --binary "$BASE/app/src/main" "$FINAL/app/src/main" > "$DELTAPATCH"; rc1=$?
git diff --no-index --binary "$BASE/app/version.properties" "$FINAL/app/version.properties" >> "$DELTAPATCH"; rc2=$?
set -e
...
26492 V5B Authority / Provenance — R32F + Semantic RCD Runtime Parser Gate
...
Failed-V3 backup branch: ${FAILED_V3_BACKUP_BRANCH}
Black-V4 rollback branch: ${BLACK_V4_BACKUP_BRANCH}
V5 gate-failed checkpoint: ${GATE_FAILED_V5_HEAD}
26492 architecture transform SHA256: ${TRANSFORM_26492_SHA}
V5 parser-compat transform SHA256: ${TRANSFORM_V5_SHA}
APK: ${APK_NAME}
APK SHA256: ${APK_SHA}
...
- sharpening excluded
- candidate and final builds use separate freshly reconstructed worktrees
- RCD SSBO declarations are Photon GLInterface.getLayouts parser-safe
- CMake native dependency downloads independently reproduced and byte-compared
- known CMake native deps candidate/final byte-equal then removed before canonical packaging
REPORT_EOF

sha256sum "$TRANSFORM_26492" "$TRANSFORM_V5" "$PREPATCH" "$DELTAPATCH" "$AFTERHASH" "$REPORT" "$OUTDIR/26492_candidate_generated_native_deps.sha256" "$REPO/$APK_NAME" > "$OUTDIR/26492_artifact_hashes.sha256"
cp "$TRANSFORM_26492" "$OUTDIR/next_baseline_inputs/"
cp "$TRANSFORM_V5" "$OUTDIR/next_baseline_inputs/"
cp "$REPO/$TRANSFORM_26491" "$OUTDIR/next_baseline_inputs/"

echo "=== 26492 V5B BUILD SUCCESS ==="
cat "$REPORT"
