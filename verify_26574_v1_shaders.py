...
def main():
    ...
    vgn=(root/'app/src/main/java/com/hinnka/mycamera/processor/GlesIris26529SpatialRgbChromaPostprocessor.kt').read_text()
    common=extract_kotlin(vgn,'common').rstrip()
    sabre=(root/'app/src/main/java/com/hinnka/mycamera/processor/GlesMgcRawSabreShaders.kt').read_text()
    specs=[
      ('vgn_universal_adaptive_color_26574',extract_kotlin(vgn,'universalAdaptiveColor26561'), 'comp', ['IRIS_26574_TOPOLOGY_PRESERVED_BRIGHT_SURFACE','topologyProtection']),
    ...
    (out/'V1_26574_RUNTIME_EXPANDED_SHADERS.sha256').write_text(''.join(manifest))
    (out/'V1_26574_SHADER_VERIFICATION.json').write_text(json.dumps(rows,indent=2)+'\n')
    print('PASS exact modified runtime-expanded shader extraction + complete reserved scan variants=5')
