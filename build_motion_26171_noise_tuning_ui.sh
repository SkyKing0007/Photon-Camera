...
OUT="/workspaces/Photon-Camera/build_${NEW_BUILD}_motion_noise_tuning_${STAMP}"
BACKUP_BRANCH="backup-before-motion-noise-tuning-${NEW_BUILD}-${STAMP}"
...
trap 'code=$?; if [ "$code" -ne 0 ]; then echo; echo "FAILED: build 26171 (exit $code)"; echo "Workspace: $OUT"; fi' EXIT

echo "============================================================"
echo " PhotonCamera ${NEW_VERSION}"
echo " Motion noise tuning UI and detail recovery"
echo "============================================================"
...
grep -Fq 'MOTION_26170_ESD3D2_STABLE_WEIGHTS' "$ESD_JAVA" \
    || fail "26170 ESD3D2 state missing"
...
echo
echo "=== APPLY 26171 MOTION TUNABLES ==="
