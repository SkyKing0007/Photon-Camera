...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='iris26536-java-') as td:
        td=Path(td);src=td/'ParseJava26536.java';src.write_text(PARSER)
        ...
        cp=subprocess.run(['java','-cp',str(td),'ParseJava26536',*paths],text=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT);req(cp.returncode==0,'changed Java parse\n'+cp.stdout)
    ks=kp.read_text();balance(ks)
    for t in ('IRIS_26536_ADAPTIVE_MGC_LUMA_RECOVERY','adaptiveLowLightFloor','textureClassifier=false'):
        req(t in ks,'Kotlin contract missing '+t)
    kotlinc=shutil.which('kotlinc')
    if kotlinc:
        with tempfile.TemporaryDirectory(prefix='iris26536-kotlin-') as td:
    ...
    print('PASS: 26536 javac parser accepted four changed Java files')
    print('PASS: 26536 Kotlin bridge lexical/grammar preflight accepted')
