...
if len(sys.argv)!=4: raise SystemExit('usage: verify_26690_patches.py ROOT BASE CANDIDATE')
root=Path(sys.argv[1]); base=Path(sys.argv[2]); cand=Path(sys.argv[3]); pkg=Path(__file__).resolve().parent
forward=pkg/'R1_26690_RUNTIME_DELTA_FROM_26689_R1_2.patch'; rollback=pkg/'R1_26690_RUNTIME_ROLLBACK_TO_26689_R1_2.patch'
...
with tempfile.TemporaryDirectory(prefix='iris26690_patch_') as td:
...
print('PASS 26690 full-index patch proof: core.abbrev 7/12/40 identical, fuzz=0, exact full-universe forward+rollback')
