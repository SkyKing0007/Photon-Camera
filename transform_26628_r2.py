...
if len(sys.argv)!=3: raise SystemExit('usage: transform_26628_r2.py BASE OUT')
...
def verify(root, manifest):
    ...
    return len(lines)
if verify(base,'R2_26628_PREWRITE_SOURCE_HASHES.sha256')!=11: raise SystemExit('prewrite count')
added=[x.strip() for x in (P/'R2_26628_ADDED_PATHS_MUST_BE_ABSENT.txt').read_text().splitlines() if x.strip()]
if added: raise SystemExit('26628 adds no runtime paths')
patch=P/'R2_26628_RUNTIME_DELTA_FROM_26627_R1.patch'
...
if verify(out,'R2_26628_EXPECTED_CHANGED_SOURCE_HASHES.sha256')!=11: raise SystemExit('candidate changed count')
