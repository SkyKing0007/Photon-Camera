...
if len(sys.argv)!=3: raise SystemExit('usage: transform_26645_r1.py BASE OUT')
base=Path(sys.argv[1]).resolve(); out=Path(sys.argv[2]).resolve(); root=Path(__file__).resolve().parent
payload=root/'handoff_payload_26645_r1'
changed=[x for x in (root/'R1_26645_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x]
def readm(p):
 d={}
...
pre=readm(root/'R1_26645_PREWRITE_SOURCE_HASHES.sha256'); expected=readm(root/'R1_26645_EXPECTED_CHANGED_SOURCE_HASHES.sha256')
if len(changed)!=5 or len(set(changed))!=5: raise SystemExit('runtime allowlist count mismatch')
...
print('PASS 26645 deterministic candidate transform: exact 5-path/0-added delta from successful 26644 compiled-candidate authority')
