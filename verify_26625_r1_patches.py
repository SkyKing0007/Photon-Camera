...
if len(sys.argv)!=5: raise SystemExit('usage: verify_26625_r1_patches.py BASE CAND FWD ROLLBACK')
...
with tempfile.TemporaryDirectory() as td:
    ...
    subprocess.run(['git','init','-q'],cwd=repo,check=True); subprocess.run(['git','config','user.name','Photon26625'],cwd=repo,check=True); subprocess.run(['git','config','user.email','photon26625@example.invalid'],cwd=repo,check=True)
    ...
    assert fs[0]==fs[1]==fs[2]==F.read_bytes(),'forward patch nondeterministic/mismatch'; assert rs[0]==rs[1]==rs[2]==R.read_bytes(),'rollback patch nondeterministic/mismatch'
    ...
    for root,patch,target in [(fw,F,C),(rb,R,B)]:
        ...
        probe=subprocess.run(['git','rev-parse','--show-toplevel'],cwd=root,env=env,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL); assert probe.returncode!=0
        subprocess.run(['git','apply','--check',str(patch)],cwd=root,env=env,check=True); subprocess.run(['git','apply',str(patch)],cwd=root,env=env,check=True); assert H(root)==H(target),patch.name
print('PASS 26625 deterministic full-index patches core.abbrev=7/12/40; fuzz=0 exact forward/rollback; diff --check clean')
