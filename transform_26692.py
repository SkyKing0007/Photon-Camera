...
if len(sys.argv)!=3: raise SystemExit('usage: transform_26692.py BASE DEST')
base=Path(sys.argv[1]); dest=Path(sys.argv[2]); root=Path(__file__).resolve().parent
changed=[x.strip() for x in (root/'R1_26692_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x.strip()]
...
def manifest(path):
 ...
 return out
pre=manifest(root/'R1_26692_PREWRITE_SOURCE_HASHES.sha256')
...
payload=root/'handoff_payload_26692'
for rel in changed:
 src=payload/rel; dst=dest/rel
 assert src.is_file(),f'payload missing: {rel}'
 dst.parent.mkdir(parents=True,exist_ok=True); shutil.copy2(src,dst)
expected=manifest(root/'R1_26692_EXPECTED_CHANGED_SOURCE_HASHES.sha256')
