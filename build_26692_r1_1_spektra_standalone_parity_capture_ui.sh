...
UPLOAD_BASE_COMMIT="c2a0a5464963c13a76b8a38d872c229042459a33"
...
HANDOFF="$ROOT/R1_1_26692_HANDOFF_HASHES.sha256"; UPLOADS="$ROOT/R1_1_26692_UPLOAD_PATHS.txt"
BASE_FULL="$ROOT/R1_1_26692_BASE_26691_R1_FULL_APP.sha256"; CAND_FULL="$ROOT/R1_1_26692_EXPECTED_CANDIDATE_FULL_APP.sha256"
BASE_PROTECTED="$ROOT/R1_1_26692_PROTECTED_UNCHANGED_BASE.sha256"; CAND_PROTECTED="$ROOT/R1_1_26692_PROTECTED_UNCHANGED_CANDIDATE.sha256"
BASE_NATIVE="$ROOT/R1_1_26692_NATIVE_PROTECTED_BASE.sha256"; CAND_NATIVE="$ROOT/R1_1_26692_NATIVE_PROTECTED_CANDIDATE.sha256"
BASE_NATIVE_FULL="$ROOT/R1_1_26692_NATIVE_FULL_BASE.sha256"; CAND_NATIVE_FULL="$ROOT/R1_1_26692_NATIVE_FULL_CANDIDATE.sha256"
BASE_VENDOR="$ROOT/R1_1_26692_VENDOR_PROTECTED_BASE.sha256"; CAND_VENDOR="$ROOT/R1_1_26692_VENDOR_PROTECTED_CANDIDATE.sha256"
BASE_DNG="$ROOT/R1_1_26692_DNG_BASE.sha256"; CAND_DNG="$ROOT/R1_1_26692_DNG_CANDIDATE.sha256"
SHADER_BASE="$ROOT/R1_1_26692_ASSET_SHADER_UNIVERSE_BASE.sha256"; SHADER_CAND="$ROOT/R1_1_26692_ASSET_SHADER_UNIVERSE_CANDIDATE.sha256"; INHERITED_RAW_SHADER="$ROOT/R1_1_26692_INHERITED_RAW_SHADER.sha256"
CHANGED="$ROOT/R1_1_26692_RUNTIME_CHANGED_PATHS.txt"; PREWRITE="$ROOT/R1_1_26692_PREWRITE_SOURCE_HASHES.sha256"; EXPECTED_CHANGED="$ROOT/R1_1_26692_EXPECTED_CHANGED_SOURCE_HASHES.sha256"
FORWARD="$ROOT/R1_1_26692_RUNTIME_DELTA_FROM_26691_R1.patch"; ROLLBACK="$ROOT/R1_1_26692_RUNTIME_ROLLBACK_TO_26691_R1.patch"
TRANSFORM="$ROOT/transform_26692_r1_1.py"; VALIDATE="$ROOT/validate_26692_r1_1.py"; AUTHORITY="$ROOT/repair_26692_authority.py"; INFRA="$ROOT/repair_26692_infrastructure.py"; PATCHVERIFY="$ROOT/repair_26692_patches.py"; GATEVERIFY="$ROOT/repair_26692_regressions.py"
BUILD_SCRIPT="$ROOT/build_26692_r1_1_spektra_standalone_parity_capture_ui.sh"; WORKFLOW="$ROOT/.github/workflows/build-26692-r1-1-spektra-standalone-parity-capture-ui.yml"
OUT="$ROOT/build_26692_r1_1_spektra_standalone_parity_capture_ui_outputs"; WORK="$ROOT/.build_26692_r1_1_spektra_standalone_parity_capture_ui_work"
...
FINAL="$ROOT/IrisCamera-${VERSION_NAME}-${VERSION_BUILD}-r1-1-spektra-standalone-parity-capture-ui-debug.apk"
...
cat > "$OUT/26692_R1_1_COMPILER_STATUS.txt" <<'STATUS'
...
STATUS
cat > "$OUT/26692_R1_1_STRICT_HANDOFF_REPORT.txt" <<'REPORT'
RUNTIME AUTHORITY: NOT RUN
VERIFICATION MECHANICS AUTHORITY: NOT RUN
BACKUP STATUS: NONE (compiler-only R1.1 repair over frozen 26692 design; exact successful 26691 authority + deterministic rollback patch)
CHANGED RUNTIME SCOPE: NOT RUN
INFRASTRUCTURE CHANGED FILES: distinct R1.1 workflow/build/transform/validators/handoff wrappers only; successful-26691 compiler/build ordering retained
INFRASTRUCTURE DELTA FROM LAST SUCCESS: successful 26691 mechanics/order retained exactly; R1.1 identity/scope/compiler-regression proof only
26690 DEVICE CRASH REGRESSION: NOT RUN
SPEKTRA STARTUP CONTAINMENT: NOT RUN
26691 NULL-LSC CAPTURE REGRESSION: NOT RUN
26692 R1.1 KOTLIN RAWFORMAT IMPORT REGRESSION: NOT RUN
...
REPORT
set_report(){ local key="$1" val="$2" tmp="$OUT/.report.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26692_R1_1_STRICT_HANDOFF_REPORT.txt" > "$tmp" || fail "report key missing $key"; mv "$tmp" "$OUT/26692_R1_1_STRICT_HANDOFF_REPORT.txt"; }
set_compiler(){ local key="$1" val="$2" tmp="$OUT/.compiler.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26692_R1_1_COMPILER_STATUS.txt" > "$tmp" || fail "compiler key missing $key"; mv "$tmp" "$OUT/26692_R1_1_COMPILER_STATUS.txt"; }
...
verify_package(){
 [[ -f "$HANDOFF" && -f "$UPLOADS" && -d "$ROOT/handoff_payload_26692_r1_1" ]] || fail "sealed 26692 package incomplete"
 [[ "$(find "$ROOT/handoff_payload_26692_r1_1" -type f | wc -l)" -eq 11 ]] || fail "runtime payload must contain exactly 11 files"
 [[ "$(wc -l < "$CHANGED")" -eq 11 ]] || fail "26692 changed-file count"
 [[ "$(wc -l < "$ROOT/R1_1_26692_ADDED_PATHS_MUST_BE_ABSENT.txt")" -eq 0 ]] || fail "26692 added-file count"
 sha256sum -c "$HANDOFF" >/dev/null
 (cd "$ROOT" && sha256sum -c R1_1_26692_PAYLOAD.sha256 >/dev/null) || fail "runtime payload hash manifest"
...
verify_scope(){
 ...
 git merge-base --is-ancestor "$UPLOAD_BASE_COMMIT" HEAD || fail "failed 26692 R1 activation commit must remain ancestor"
...
make_candidate(){
 ...
 set_report "26692 R1.1 KOTLIN RAWFORMAT IMPORT REGRESSION" "PASS (RawPreviewPlanSelector explicitly imports com.unspektrawesome.camera.RawFormat; exact failed R1 compiler condition blocked)"
...
postbuild_proof(){
 ...
 tar --sort=name --mtime='UTC 1970-01-01' --owner=0 --group=0 --numeric-owner -cf - -C "$POST" app | gzip -n > "$OUT/26692_R1_1_candidate_app_source.tar.gz"; sha256sum "$OUT/26692_R1_1_candidate_app_source.tar.gz" > "$OUT/26692_R1_1_candidate_app_source.tar.gz.sha256"; cp "$CAND_FULL" "$OUT/26692_R1_1_candidate_full_app.sha256"; cp "$CAND_NATIVE" "$OUT/26692_R1_1_native_protected_postbuild.sha256"; cp "$CAND_NATIVE_FULL" "$OUT/26692_R1_1_native_full_postbuild.sha256"; cp "$CAND_VENDOR" "$OUT/26692_R1_1_vendor_protected_postbuild.sha256"; cp "$CAND_DNG" "$OUT/26692_R1_1_dng_postbuild.sha256"; cp "$SHADER_CAND" "$OUT/26692_R1_1_asset_shader_universe_postbuild.sha256"; cp "$INHERITED_RAW_SHADER" "$OUT/26692_R1_1_spektra_raw_shader_source.sha256"; set_report "CLEAN ARTIFACT SOURCE EXPORT" "PASS (deterministic candidate tar + manifests)"
}
verify_package
verify_scope
obtain_authority
...
mv "${apks[0]}" "$FINAL"; [[ -f "$FINAL" ]] || fail "final APK missing after move"; set_report "EXACTLY ONE APK" "PASS"; sha256sum "$FINAL" > "$OUT/26692_R1_1_APK.sha256"
verify_apk_jni_contract
postbuild_proof
pass "26692 R1.1 ACTIONS BUILD COMPLETE"
