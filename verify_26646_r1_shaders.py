...
if len(sys.argv) not in (4,6): raise SystemExit('usage: verify_26646_r1_shaders.py ROOT BASE CANDIDATE [--compiler PATH]')
...
def variants(rootp):
 ...
 return [
  ('motionv2_local_laplacian_remap_26646.frag','#version 310 es\n\n#line 1\n'+(rootp/'app/src/main/assets/shaders/motionv2/local_laplacian_remap_26621.glsl').read_text(),'frag'),
  ('sabre_short_component_anchor_26646.frag',extract(k,'shortComponentAnchor26607'),'frag'),
  ('sabre_short_rescue_weight_26646.frag',extract(k,'shortRescueWeight26607'),'frag')]
bm=readm(root/'R1_26646_SHADER_UNIVERSE_BASE.sha256'); cm=readm(root/'R1_26646_SHADER_UNIVERSE_CANDIDATE.sha256')
...
expected=readm(root/'R1_26646_RUNTIME_EXPANDED_SHADERS.sha256'); assert len(expected)==3
...
def scan(name,src):
 ...
 if name=='sabre_short_component_anchor_26646.frag':
 ...
 if name=='sabre_short_rescue_weight_26646.frag':
  if 'mirrorUvs(referenceUv + flow.xy)' in src: raise SystemExit('FAIL rescue: mirrored border survives')
  for token in ['IRIS_26644_SHORT_WARP_DOMAIN_FAIL_CLOSED','IRIS_26645_PER_PIXEL_VISUAL_RADIANCE_PROOF','IRIS_26646_UNIVERSAL_HIGH_DYNAMIC_RANGE_SHORT_OWNER','visualRescueWeight = physicalWeight * visualRadianceConfidence']:
   if token not in src: raise SystemExit(f'FAIL {name}: missing {token}')
 if name=='motionv2_local_laplacian_remap_26646.frag':
  for token in ['IRIS_26645_STRUCTURE_AWARE_HIGHLIGHT_BASE','IRIS_26646_EDGE_ISOLATED_SOURCE_RADIANCE_SURVIVAL','IRIS_26646_EXTENDED_LINEAR_RADIANCE_SURVIVAL_OWNER','float boundedMissing=min(missing,0.18);','if(admitted<3||weightSum<2.55) continue;']:
   if token not in src: raise SystemExit(f'FAIL {name}: missing {token}')
mods=variants(cand)
with tempfile.TemporaryDirectory(prefix='iris26646_shader_') as td:
 out=Path(td)
 for name,src,stage in mods:
  ...
  if compiler:
   ...
   if cp.returncode!=0: raise SystemExit(f'26646 GLSL FAIL {name}\n{cp.stdout}\n{cp.stderr}')
print(f'PASS 26646 runtime-expanded GLSL variants=3 reserved/structure/exact-hash real_compiler={bool(compiler)} shaderUniverse=257')
