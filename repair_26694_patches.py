...
if len(sys.argv)!=4: raise SystemExit('usage: repair_26694_patches.py ROOT BASE CANDIDATE')
root=Path(sys.argv[1]); base=Path(sys.argv[2]); cand=Path(sys.argv[3]); pkg=Path(__file__).resolve().parent
f=pkg/'26694_RUNTIME_DELTA_FROM_26693.patch'; r=pkg/'26694_RUNTIME_ROLLBACK_TO_26693.patch'
...
with tempfile.TemporaryDirectory(prefix='iris26694_patch_') as td:
...
print('PASS 26694 full-index patch proof: core.abbrev 7/12/40 identical, fuzz=0, exact forward+rollback')
