...
def main():
    ...
    dng_pos=iris.find('IRIS_26520_V4_NORMAL_ONLY_TEMPORAL_CONTRIBUTION')
    rgb_pos=iris.find('submitOrRetainRgbFrame(',dng_pos)
    wait_pos=iris.find('online.passWindow.awaitResources(',dng_pos)
    upload_pos=iris.find('uploadRaw(images[index], texture, "frame $index")',wait_pos)
    assert dng_pos>=0 and rgb_pos>dng_pos
    assert wait_pos>=0 and upload_pos>wait_pos
