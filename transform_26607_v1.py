...
base=Path(sys.argv[1]);out=Path(sys.argv[2]);pre=read(PKG/'V1_26607_PREWRITE_SOURCE_HASHES.sha256');exp=read(PKG/'V1_26607_EXPECTED_CHANGED_SOURCE_HASHES.sha256')
if list(pre)!=CHANGED or list(exp)!=CHANGED: fail('manifest allowlist mismatch')
payload=PKG/'handoff_payload_26607_v1';paths=sorted(p.relative_to(payload).as_posix() for p in payload.rglob('*') if p.is_file())
...
print('PASS exact successful-26606 V1 compiled-candidate authority + exact 4-file 26607 V1 overlay')
