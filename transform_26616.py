...
if len(sys.argv)!=3: raise SystemExit('usage: transform_26616.py BASE OUT')
...
def verify(root,manifest):
    ...
    return len(lines)
if verify(base,'R1_26616_PREWRITE_SOURCE_HASHES.sha256')!=12: raise SystemExit('prewrite count')
patch=P/'R1_26616_RUNTIME_DELTA_FROM_26615_R1.patch'
# Exact successful R1 isolation regression: never let nested candidate discover parent .git.
...
if verify(out,'R1_26616_EXPECTED_CHANGED_SOURCE_HASHES.sha256')!=20: raise SystemExit('candidate changed count')
print('PASS 26616 deterministic candidate reconstruction from exact successful 26615 R1 + canonical full-index patch')
