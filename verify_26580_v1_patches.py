...
def regen(start,target,abbrev):
 td=Path(tempfile.mkdtemp(prefix='p26580_'))
 try:
  init(td,start)
...
def applyprove(start,target,patch):
 td=Path(tempfile.mkdtemp(prefix='a26580_'))
 try:
...
def main():
 ...
 print('PASS FORWARD PATCH FUZZ=0 exact 26580 candidate recreation')
 print('PASS ROLLBACK PATCH FUZZ=0 exact successful 26579 recreation')
