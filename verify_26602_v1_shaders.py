...
def trim_indent(s):
    ...
    non=[len(l)-len(l.lstrip()) for l in lines if l.strip()]
...
def main():
    ...
    sc=(root/'app/src/main/java/com/hinnka/mycamera/processor/GlesMgcRawSabreShaders.kt').read_text()
    ...
    specs=[
      ...
      ('sabre_sparse_flow_pure_geometry_26597',extract_kotlin(sc,'convertAlignmentSparse'),'frag'),
      ('short_boundary_geometry_seed_26600',extract_kotlin(sc,'shortBoundaryGeometrySeed26600'),'frag'),
      ('short_boundary_geometry_propagate_26600',extract_kotlin(sc,'shortBoundaryGeometryPropagate26600'),'frag'),
      ('short_boundary_geometry_probe_26600',extract_kotlin(sc,'shortBoundaryGeometryProbe26600'),'frag'),
      ('short_effective_sensor_loss_handoff_mask_26600',extract_kotlin(sc,'shortRestoreMask26596'),'frag'),
      ('short_restore_mask_probe_26590',extract_kotlin(sc,'shortRestoreMaskProbe26590'),'frag'),
      ('short_common_sabre_owner_26601_dormant',extract_kotlin(sc,'shortAccumulatorOwnership26601'),'frag'),
      ('short_protection_seed_26602',extract_kotlin(sc,'shortProtectionSeed26602'),'frag'),
      ('short_protection_propagate_26602',extract_kotlin(sc,'shortProtectionPropagate26602'),'frag'),
      ('short_bayer_quad_protected_mask_26602',extract_kotlin(sc,'shortProtectedMask26602'),'frag'),
      ('short_protected_accumulator_fuse_26602',extract_kotlin(sc,'shortProtectedAccumulatorFuse26602'),'frag'),
      ('short_restore_rgba16f_26592_dormant',extract_kotlin(sc,'shortRestoreRgba16f26587'),'frag')]
    ...
    specs.extend([
      ('adaptive_color_appearance_26598_universal',prefix+app,'frag'),
      ('adaptive_color_appearance_26598_calibrated',prefix+app.replace('#define CALIBRATED_PROFILE 0','#define CALIBRATED_PROFILE 1'),'frag')])
    de=(root/'app/src/main/assets/shaders/motionv2/display_exposure.glsl').read_text()
    if '#import' in de or '#version' in de or '#define' in de: fail('display exposure runtime expansion assumptions changed')
    specs.append(('motionv2_display_exposure_26598_restored',prefix+de,'frag'))
    ...
    specs.append(('motionv2_render_26602_uhdr_master_sdr_parity',prefix+rc,'frag'))
    sp=(root/'app/src/main/java/com/hinnka/mycamera/processor/GlesMgcRawSpatialShaders.kt').read_text(); specs.append(('short_full_mask_count_26596',extract_kotlin(sp,'bentoCountHighlightMask'),'comp'))
    cpp=(root/'app/src/main/cpp/motionv2_jpeg444_jni.cpp').read_text(); specs.append(('true2x_publication_compute_26602_uhdr_master_sdr_parity',extract_cpp_raw(cpp,'kIris26571PublicationCompute'),'comp'))
    if len(specs)!=24: fail('runtime-expanded shader variant count '+str(len(specs)))
    rows=[]; manifest=[]
    ...
    (out/'V1_26602_RUNTIME_EXPANDED_SHADERS.sha256').write_text(''.join(manifest))
    (out/'V1_26602_SHADER_VERIFICATION.json').write_text(json.dumps(rows,indent=2)+'\n')
    print(f'PASS {len(specs)} exact runtime-expanded shader variants + complete reserved identifier scan')
