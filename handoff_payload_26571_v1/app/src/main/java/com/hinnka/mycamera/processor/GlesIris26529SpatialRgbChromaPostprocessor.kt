...
internal object Iris26529SpatialRgbChromaShaders {
    ...
    /* IRIS_26571_COHERENT_CHROMA_PRESERVATION
     * Inherits the successful 26570 illumination-independent surface cleanup. Real chroma that
     * agrees in hue with same-surface support is protected from one-way magnitude loss, while
     * isolated unsupported chroma is still pulled toward consensus. A one-sided luma boundary
     * with chroma disagreement becomes a material edge, so sky/subject colors cannot average
     * across it. No target may exceed the center chroma magnitude, and highlight preservation is
    ...
    val universalAdaptiveColor26561 = """
        ...
        void main() {
            ...
            float chromaAgreement = dot(centerNormalizedChroma, consensusNormalizedChroma) /
                max(centerNormalizedMagnitude * consensusMagnitude, 1.0e-6);
            chromaAgreement = clamp(chromaAgreement, -1.0, 1.0);
            ...
            /* IRIS_26571_SAME_SIDE_MATERIAL_BOUNDARY
             * A moderate one-sided luma transition is a protected object/material boundary only
             * when chroma also disagrees. Smooth illumination on one material therefore keeps the
             * 26570 continuity behavior, while foliage/sky and other subject/background colors do
             * not become one shared chroma population.
             */
            ...
            /* Coherent real color may be stronger than a local average without being noise. This
             * only reduces cleanup when hue is strongly supported and the center is not an extreme
             * chroma outlier. It never adds chroma beyond the original center sample.
             */
            float coherentHue = smoothstep(0.72, 0.94, chromaAgreement) *
            ...
            float correction = 0.65 * unsupportedColor * surfaceSupport *
                (1.0 - edgeProtection) * (1.0 - 0.85 * coherentCenterProtection);
