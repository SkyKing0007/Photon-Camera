...
def main():
    if len(sys.argv)!=3: die('usage: validate_26568_v1.py BASE_ROOT CANDIDATE_ROOT')
    ...
    req('IRIS_26568_PHASE_STATS_NO_SECOND_READBACK' in cs,'no-second-readback contract missing')
    req('writeTrue2xGpuRgbTile(renderOut,render,left,top,tileWidth,tileHeight,fullOutputWidth,"IRIS26568 fused render",phaseHistogram)' in cs,
        'render readback does not collect phase histogram')
    req('IRIS26568 phase readback' not in cs and 'updateTrue2xGpuPhaseHistogram' not in cs,
        'retired second full-resolution phase readback survived')
    req('float(phaseCount)' in guide,'render alpha phase-count transport missing')
    print('PASS phase telemetry piggybacks existing RGBA16F render readback; second phase readback absent')
