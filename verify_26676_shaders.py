...
if len(sys.argv) not in (4,6): raise SystemExit('usage: verify_26676_shaders.py ROOT BASE CANDIDATE [--compiler PATH]')
 ...
 for raw in lines:
  ...
  if '#import' in v:
   if '//' not in v:
    ...
    imp=(rootdir/f'app/src/main/assets/shaders/utils/{token}.glsl').read_text()
    out.append('#line 1'); out.extend(imp.splitlines()); out.append(f'#line {linecnt+1}')
   continue
  if '#define' in v:
...
def variants(rootdir):
 mods=embedded(rootdir); req=['rejection','merge','universalNormalMasterShortFusion26651']
 for x in req:
 ...
 out={
  ...
  'asset_addwatermark_rotate_off.frag':expand(rootdir,'app/src/main/assets/shaders/addwatermark_rotate.glsl',{'WATERMARK':'0'}),
  'asset_addwatermark_rotate_on.frag':expand(rootdir,'app/src/main/assets/shaders/addwatermark_rotate.glsl',{'WATERMARK':'1'}),
 }
 for hs in (0,1):
 ...
 return out
bm=readm(root/'R1_26676_SHADER_UNIVERSE_BASE.sha256'); cm=readm(root/'R1_26676_SHADER_UNIVERSE_CANDIDATE.sha256'); assert len(bm)==len(cm)==257
for r,h in bm.items(): assert sha(base/r)==h,r
for r,h in cm.items(): assert sha(cand/r)==h,r
shader_delta=sorted(r for r in bm if bm[r]!=cm[r]); assert shader_delta==['app/src/main/assets/shaders/addwatermark_rotate.glsl','app/src/main/assets/shaders/preview/main_fs.glsl'],shader_delta
bv=variants(base); cv=variants(cand); assert len(bv)==len(cv)==14 and set(bv)==set(cv)
expected=readm(root/'R1_26676_RUNTIME_EXPANDED_SHADERS.sha256'); assert len(expected)==14 and set(expected)==set(cv)
changed_variants=sorted(k for k in bv if hashlib.sha256(bv[k].encode()).hexdigest()!=hashlib.sha256(cv[k].encode()).hexdigest())
expected_changed=[x for x in (root/'R1_26676_RUNTIME_EXPANDED_CHANGED_VARIANTS.txt').read_text().splitlines() if x]; assert changed_variants==expected_changed,(changed_variants,expected_changed)
...
def scan(name,s):
 clean=re.sub(r'/\*.*?\*/',' ',s,flags=re.S); clean=re.sub(r'//.*',' ',clean); ids=re.findall(r'\b'+typepat+r'\s+([A-Za-z_]\w*)\b',clean)+re.findall(r'\bstruct\s+([A-Za-z_]\w*)\b',clean)
 ...
 if '#import' in clean: raise SystemExit(f'FAIL {name} unresolved import')
with tempfile.TemporaryDirectory(prefix='iris26676_shader_') as td:
 o=Path(td)
 for n,s in sorted(cv.items()):
  ...
  if compiler:
   ...
   if cp.returncode: raise SystemExit(f'26676 GLSL FAIL {n}\n{cp.stdout}\n{cp.stderr}')
print(f'PASS 26676 exact runtime GLSL variants=14 changedVariants={len(changed_variants)} complete reserved/structure/exact-hash real_compiler={bool(compiler)} standaloneShaderUniverse=257')
