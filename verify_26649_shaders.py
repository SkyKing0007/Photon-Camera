...
if len(sys.argv) not in (4,6): raise SystemExit('usage: verify_26649_shaders.py ROOT BASE CANDIDATE [--compiler PATH]')
...
mods=readm(root/'R1_26649_RUNTIME_EXPANDED_SHADERS.sha256'); assert len(mods)==3
expected_mods={
...
def scan(name,src):
 ...
 if name.endswith('local_laplacian_global_log_26621.glsl'):
 ...
 if name.endswith('render.glsl'):
  for t in ['uniform float iris26649PhotonHighlightKnee;','return iris26649PhotonSoftShoulder(iris26623MapMotionSdrFinalGuide(sourceGuide));','float globalMapped=iris26649PhotonSoftShoulder(','return knee+span*g*g;']:
   if t not in src: raise SystemExit(f'FAIL {name}: missing {t}')
 if name.endswith('local_laplacian_remap_26621.glsl'):
...
with tempfile.TemporaryDirectory(prefix='iris26649_shader_') as td:
 td=Path(td)
 for rel,h in sorted(mods.items()):
  p=cand/rel; src=p.read_text(); scan(rel,src); got=hashlib.sha256(p.read_bytes()).hexdigest(); assert got==h,(rel,h,got)
  if compiler:
   out=td/(Path(rel).name+'.frag'); out.write_text(src)
   cp=subprocess.run([compiler,'-S','frag',str(out)],capture_output=True,text=True)
   if cp.returncode!=0: raise SystemExit(f'26649 GLSL FAIL {rel}\n{cp.stdout}\n{cp.stderr}')
print(f'PASS 26649 runtime-expanded GLSL variants=3 reserved/structure/exact-hash real_compiler={bool(compiler)} shaderUniverse=257')
