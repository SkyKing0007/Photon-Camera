...
HANDOFF_PARENT_COMMIT="6e1b655a145c8719fd64681fe5ff0e8eb11b8ce3"
...
HANDOFF="$ROOT/V1_26569_HANDOFF_HASHES.sha256"
BASE_PIN="$ROOT/V1_26569_BASE_26568_AUDITED_RUNTIME.sha256"
CAND_PIN="$ROOT/V1_26569_EXPECTED_CANDIDATE_AUDITED_RUNTIME.sha256"
BASE_FULL="$ROOT/V1_26569_BASE_26568_FULL_APP.sha256"
CAND_FULL="$ROOT/V1_26569_EXPECTED_CANDIDATE_FULL_APP.sha256"
BASE_VENDOR="$ROOT/V1_26569_BASE_26568_NATIVE_VENDOR.sha256"
CAND_VENDOR="$ROOT/V1_26569_EXPECTED_NATIVE_VENDOR.sha256"
BASE_DNG="$ROOT/V1_26569_DNG_PROTECTED_BASE.sha256"
CAND_DNG="$ROOT/V1_26569_DNG_PROTECTED_CANDIDATE.sha256"
BASE_PROTECTED="$ROOT/V1_26569_PROTECTED_UNCHANGED_BASE.sha256"
CAND_PROTECTED="$ROOT/V1_26569_PROTECTED_UNCHANGED_CANDIDATE.sha256"
SHADER_PIN="$ROOT/V1_26569_RUNTIME_EXPANDED_SHADERS.sha256"
CHANGED="$ROOT/V1_26569_RUNTIME_CHANGED_PATHS.txt"
PREWRITE="$ROOT/V1_26569_PREWRITE_SOURCE_HASHES.sha256"
FORWARD="$ROOT/V1_26569_RUNTIME_DELTA_FROM_26568.patch"
ROLLBACK="$ROOT/V1_26569_RUNTIME_ROLLBACK_TO_26568.patch"
TRANSFORM="$ROOT/transform_26569_v1.py"
VALIDATE="$ROOT/validate_26569_v1.py"
AUTHORITY="$ROOT/verify_26569_v1_authority.py"
PATCHVERIFY="$ROOT/verify_26569_v1_patches.py"
SHADERVERIFY="$ROOT/verify_26569_v1_shaders.py"
BUILD_SCRIPT="$ROOT/build_26569_v1_encoder_ui.sh"
WORKFLOW="$ROOT/.github/workflows/build-26569-v1-encoder-ui.yml"
SEALED_PY=("$TRANSFORM" "$VALIDATE" "$AUTHORITY" "$PATCHVERIFY" "$SHADERVERIFY")
SEALED_DEP_FILES=("${SEALED_PY[@]}" "$BUILD_SCRIPT" "$WORKFLOW")
OUT="$ROOT/build_26569_v1_encoder_ui_outputs"
WORK="$ROOT/.build_26569_v1_encoder_ui_work"
...
FINAL="$ROOT/IrisCamera-${VERSION_NAME}-${VERSION_BUILD}-v1-encoder-ui-debug.apk"
...
cat > "$OUT/26569_V1_COMPILER_STATUS.txt" <<'STATUS'
...
STATUS
cat > "$OUT/26569_V1_STRICT_HANDOFF_REPORT.txt" <<'REPORT'
...
REPORT
set_report(){ local key="$1" val="$2" tmp="$OUT/.report.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26569_V1_STRICT_HANDOFF_REPORT.txt" > "$tmp" || fail "report key missing $key"; mv "$tmp" "$OUT/26569_V1_STRICT_HANDOFF_REPORT.txt"; }
set_compiler(){ local key="$1" val="$2" tmp="$OUT/.compiler.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26569_V1_COMPILER_STATUS.txt" > "$tmp" || fail "compiler key missing $key"; mv "$tmp" "$OUT/26569_V1_COMPILER_STATUS.txt"; }
...
verify_package(){
  ...
  [[ "$(sha "$CAND_PIN")" == "10e0d2b9d46861e69d0373c4d51a42da4043a163558e263e5146e2e64954aa61" ]] || fail "candidate audited manifest SHA"
  [[ "$(sha "$BASE_FULL")" == "3bfae4fa481b21cf9528c8dd6ec894cb2b11f93d193bc05daf4b03056be151aa" ]] || fail "base full manifest SHA"
  [[ "$(sha "$CAND_FULL")" == "198f7750156f9decd7504acf282d395854c1908f6ac357abc35790c344b45788" ]] || fail "candidate full manifest SHA"
  ...
  [[ "$(sha "$FORWARD")" == "2a3176d7982bc215595c654d5ec082054a3eae8dc22064799830b47a86483058" ]] || fail "forward patch SHA"
  [[ "$(sha "$ROLLBACK")" == "e20384f17fd7f95257a0d97c3c7372c01840c7f2f8b5900937523c6975f4d5ca" ]] || fail "rollback patch SHA"
...
PY
  ...
  bash -n "$BUILD_SCRIPT"
  grep -F 'run 33339787968, job 99333216734' "$ROOT/REGRESSION_V1_26569_CARRIED_FAILURES.txt" >/dev/null || fail "Actions glslang bootstrap regression missing"
  grep -F 'run 33340190659, job 99334293921' "$ROOT/REGRESSION_V1_26569_CARRIED_FAILURES.txt" >/dev/null || fail "Actions Kotlin phase-stat regression missing"
  grep -F 'run 33340661287, job 99335599790' "$ROOT/REGRESSION_V1_26569_CARRIED_FAILURES.txt" >/dev/null || fail "Actions generated-scope regression missing"
  grep -F 'run 33350507622, job 99362814146' "$ROOT/REGRESSION_V1_26569_CARRIED_FAILURES.txt" >/dev/null || fail "Actions repository-scaffolding regression missing"
  grep -F 'run 33357873019, job 99383368441' "$ROOT/REGRESSION_V1_26569_CARRIED_FAILURES.txt" >/dev/null || fail "26568 V1 helper-closure regression missing"
  grep -F 'the 26569 runtime changed-file allowlist is exactly four paths' "$ROOT/REGRESSION_V1_26569_CARRIED_FAILURES.txt" >/dev/null || fail "26569 runtime allowlist regression missing"
  grep -F 'Already-safe layouts receive correction 0' "$ROOT/REGRESSION_V1_26569_ENCODER_UI_CONTRACT.txt" >/dev/null || fail "UI safe-layout regression missing"
  grep -F 'Motion-only fast path is gated by !jin.enabled()' "$ROOT/REGRESSION_V1_26569_ENCODER_UI_CONTRACT.txt" >/dev/null || fail "Motion encoder gate missing"
...
verify_scope(){
  ...
  [[ "$(git rev-parse HEAD^)" == "$HANDOFF_PARENT_COMMIT" ]] || fail "26569 handoff must be one clean commit directly on successful 26568 V1.1"
...
names.append('V1_26569_HANDOFF_HASHES.sha256')
print('\n'.join(sorted(names)))
PY
...
PY
  while IFS= read -r rel; do [[ -n "$rel" ]] || continue; cmp "$ROOT/handoff_payload_26569_v1/$rel" "$AFTER/$rel" || fail "sealed payload differs frozen candidate $rel"; done < "$CHANGED"
...
PYTREE
  ...
  sha256sum "$FINAL" > "$OUT/26569_V1_APK.sha256"
  ...
  tar --sort=name --mtime='UTC 1970-01-01' --owner=0 --group=0 --numeric-owner -czf \
    "$OUT/26569_V1_candidate_app_source.tar.gz" -C "$POST" app
  sha256sum "$OUT/26569_V1_candidate_app_source.tar.gz" > "$OUT/26569_V1_candidate_app_source.tar.gz.sha256"
  cp "$CAND_PIN" "$OUT/26569_V1_candidate_source.sha256"
  cp "$CAND_FULL" "$OUT/26569_V1_candidate_full_app.sha256"
...
if [[ -n "$LOCAL_ART" ]]; then
  ...
  cp "$OUT/26569_V1_STRICT_HANDOFF_REPORT.txt" "$OUT/26569_local_prebuild_report.txt"
