...
SUCCESS_26609_SCRIPT_BLOB='b66d0c8dcd2ca043ded8269eae85fe67df537ad5'
SUCCESS_26609_WORKFLOW_BLOB='85c2cc600c5972c95f0a27997c0bbfab598fdfbe'
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('build_script');ap.add_argument('workflow');ap.add_argument('--success-26609-script');ap.add_argument('--success-26609-workflow');ap.add_argument('--golden-26593-script');ap.add_argument('--golden-26593-workflow');ns=ap.parse_args()
 ...
 pins=[
  ...
  'BASE_RUN_ID="34138215341"','BASE_JOB_ID="101794013798"','BASE_ARTIFACT_ID="10024937067"',
 ...
 if 'BACKUP_BRANCH=' in s or 'BACKUP_COMMIT=' in s:fail('26610 no-backup policy violated')
 sig=check_order(s,'26610 V1');workflow_shape(w,'26610 V1')
 ...
 for t in ['Build 26610 V1 SHORT Protection + Source Rendition','build-26610-v1-short-protection-rendition-parity.yml','photon-26610-v1-short-protection-source-rendition','Build exact 26610 V1 candidate from successful compiled 26609 V1 authority']:
  if t not in w:fail('26610 workflow identity '+t)
 for stale in ["'26609_V1_README_UPLOAD.txt'","'V1_26609_*'","'build_26609_v1_short_protection_rendition_parity.sh'","'handoff_payload_26609_v1/**'"]:
  if stale in w:fail('stale/overlapping 26609 trigger survived '+stale)
 if ns.success_26609_script:
  if git_blob(ns.success_26609_script)!=SUCCESS_26609_SCRIPT_BLOB:fail('successful-26609 V1 build script blob')
  prior=Path(ns.success_26609_script).read_text()
  if check_order(prior,'26609 V1')!=sig:fail('core compiler/build mechanics differs from successful 26609 V1')
 ...
 if ns.success_26609_workflow:
  if git_blob(ns.success_26609_workflow)!=SUCCESS_26609_WORKFLOW_BLOB:fail('successful-26609 V1 workflow blob')
  workflow_shape(Path(ns.success_26609_workflow).read_text(),'26609 V1')
 ...
 print('PASS exact successful-26609 V1 compiler/build/workflow order, action versions, glslang pin, language/NDK/full-assemble ordering and candidate freeze mechanics inherited')
 print('PASS infrastructure delta limited to 26610 identity, successful-26609 authority, no-backup policy, 10-file scope, 36-shader coverage and clipped-core/source-domain SDR-UHDR/SR regressions')
