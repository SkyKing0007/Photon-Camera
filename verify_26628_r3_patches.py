...
if len(sys.argv)!=5: raise SystemExit('usage: verify_26628_r3_patches.py BASE CAND FORWARD ROLLBACK')
...
expected=sorted(x.strip() for x in (Path(__file__).resolve().parent/'R3_26628_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x.strip())
