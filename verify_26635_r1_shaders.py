...
if len(sys.argv)<5 or sys.argv[3]!='--out': raise SystemExit('usage: verify_26635_r1_shaders.py BASE CAND --out OUT [--compiler PATH]')
B=Path(sys.argv[1]); C=Path(sys.argv[2]); O=Path(sys.argv[4]); O.mkdir(parents=True,exist_ok=True); compiler=None
if len(sys.argv)>=7 and sys.argv[5]=='--compiler': compiler=sys.argv[6]
rel='app/src/main/assets/shaders/motionv2/local_laplacian_remap_26621.glsl'
if (B/rel).read_bytes()==(C/rel).read_bytes(): raise SystemExit('FAIL remap shader unexpectedly identical')
src='#version 300 es\n'+(C/rel).read_text(); mods=[('motionv2_local_laplacian_remap_26635',src,'frag')]
...
for name,src,stage in mods:
    clean=re.sub(r'/\*.*?\*/',' ',src,flags=re.S); clean=re.sub(r'//.*',' ',clean); ids=re.findall(r'\b'+typepat+r'\s+([A-Za-z_]\w*)\b',clean)+re.findall(r'\bstruct\s+([A-Za-z_]\w*)\b',clean); bad=sorted(set(ids)&reserved)
...
lines=[f'{hashlib.sha256(p.read_bytes()).hexdigest()}  {p.name}' for p,_ in paths]; (O/'R1_26635_RUNTIME_EXPANDED_SHADERS.sha256').write_text('\n'.join(lines)+'\n'); (O/'R1_26635_SHADER_VERIFICATION.json').write_text(json.dumps({'count':len(results),'compiler':compiler,'results':results},indent=2,sort_keys=True)+'\n')
print(f'PASS 26635 complete runtime-expanded GLSL reserved scan variants={len(results)} real_compiler={bool(compiler)}')
