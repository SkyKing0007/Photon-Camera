...
if len(sys.argv)!=5: raise SystemExit('usage: verify_26623_r1_patches.py BASE CAND FWD ROLLBACK')
...
with tempfile.TemporaryDirectory() as td:
    ...
    subprocess.run(['git','config','user.name','Photon26623'],cwd=repo,check=True)
    subprocess.run(['git','config','user.email','photon26623@example.invalid'],cwd=repo,check=True)
    subprocess.run(['git','add','app'],cwd=repo,check=True); subprocess.run(['git','commit','-q','-m','base'],cwd=repo,check=True)
    base=subprocess.check_output(['git','rev-parse','HEAD'],cwd=repo,text=True).strip()
    shutil.rmtree(repo/'app'); shutil.copytree(C/'app',repo/'app'); subprocess.run(['git','add','-A'],cwd=repo,check=True); subprocess.run(['git','commit','-q','-m','candidate'],cwd=repo,check=True)
    ...
    for root,patch,target in [(fw,F,C),(rb,R,B)]:
        ...
        subprocess.run(['git','apply','--check',str(patch)],cwd=root,env=env,check=True); subprocess.run(['git','apply',str(patch)],cwd=root,env=env,check=True)
        assert H(root)==H(target),patch.name
print('PASS 26623 deterministic full-index patches core.abbrev=7/12/40; fuzz=0 exact forward/rollback; diff --check clean')
