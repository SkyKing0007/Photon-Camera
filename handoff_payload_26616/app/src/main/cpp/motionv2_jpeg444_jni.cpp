...
inline Vec3 tone(Vec3 rgb,const Params&p){
    /* IRIS_26615_TRUE2X_MANUAL_POST_SPATIAL_PRESENTATION
     * Motion automatic brightness is already spatially allocated. Night never used the old Motion
     * virtualGain branch because motionHdrHandoff is false, so its behavior remains unchanged. */
...
inline bool samplePreRender(const SourceRegion&s,const Params&p,float x,float y,PresentedSample*out){
    ...
    int x0=(int)std::floor(x),y0=(int)std::floor(y),x1=std::min(x0+1,p.trueW-1),y1=std::min(y0+1,p.trueH-1);float fx=x-x0,fy=y-y0;PresentedSample a,b,c,d;
    if(!preRenderAt(s,p,x0,y0,&a)||!preRenderAt(s,p,x1,y0,&b)||!preRenderAt(s,p,x0,y1,&c)||!preRenderAt(s,p,x1,y1,&d))return false;
    out->rgb=mix3(mix3(a.rgb,b.rgb,fx),mix3(c.rgb,d.rgb,fx),fy);
    out->physicalGuide=(a.physicalGuide+(b.physicalGuide-a.physicalGuide)*fx)*(1.f-fy)+(c.physicalGuide+(d.physicalGuide-c.physicalGuide)*fx)*fy;
    return true;
}
...
inline Vec3 renderHeadroom(Vec3 rgb,const Params&p){
    ...
    float pk2=peak(rgb);if(pk2>1.f)rgb=mul(rgb,1.f/std::max(pk2,1.0e-6f));return clampNonnegative(rgb);
}
...
vec4 irisSamplePreRender(vec2 p){
    ...
    vec2 f=p-vec2(p0);
    vec4 a=irisPreRenderAt(p0),b=irisPreRenderAt(ivec2(p1.x,p0.y));
    vec4 c=irisPreRenderAt(ivec2(p0.x,p1.y)),d=irisPreRenderAt(p1);
...
void main(){
    ...
    if(uGenerateGain!=0){
        ...
        if(uMotionHdrHandoff!=0){
            ratio=clamp(max(hdrSample.a,1.0),1.0,contentMax);
        }else{
...
public:
    ...
    bool renderReadback(int fd,float gainMax,float gainContentMax,bool generateGain,int top,int h,int slot,Iris26571ReadyBand*out,Iris26571GpuTiming*timing,std::string*reason){
        ...
        auto us=std::chrono::steady_clock::now();glPixelStorei(GL_UNPACK_ALIGNMENT,1);glActiveTexture(GL_TEXTURE1);glBindTexture(GL_TEXTURE_2D,spatialBaseTex);glActiveTexture(GL_TEXTURE0);glBindTexture(GL_TEXTURE_2D,sourceTex);
...
private:
    ...
    GLuint program=0,sourceTex=0,spatialBaseTex=0,outputTex=0,fbo=0,lutSsbo=0,pbo[2]{0,0};GLsync fence[2]{0,0};int pendingTop[2]{0,0},pendingH[2]{0,0};int sourceAllocW=0,sourceAllocH=0,outWidth=0,outHeight=0,maxBandRows=0;bool explicitFenceSync=true;const Params*params=nullptr;
    ...
    void destroy(){
        ...
        for(int i=0;i<2;i++)if(fence[i]){glDeleteSync(fence[i]);fence[i]=0;}if(pbo[0]||pbo[1])glDeleteBuffers(2,pbo);if(lutSsbo)glDeleteBuffers(1,&lutSsbo);if(fbo)glDeleteFramebuffers(1,&fbo);if(outputTex)glDeleteTextures(1,&outputTex);if(spatialBaseTex)glDeleteTextures(1,&spatialBaseTex);if(sourceTex)glDeleteTextures(1,&sourceTex);if(program)glDeleteProgram(program);
        ...
        display=EGL_NO_DISPLAY;surface=EGL_NO_SURFACE;context=EGL_NO_CONTEXT;program=sourceTex=spatialBaseTex=outputTex=fbo=lutSsbo=0;pbo[0]=pbo[1]=0;
    }
};
...
extern "C" JNIEXPORT jboolean JNICALL Java_com_particlesdevs_photoncamera_processing_ultrahdr_MotionV2Jpeg444Encoder_writeTrue2xNative(
    ...
    unlink(op.c);if(generateGain)unlink(gp.c);const char*primaryReason=gpuReason.empty()?(gpuEligible?"missing_async_reason":"jin_or_watermark"):gpuReason.c_str();const char*compatReason=gpuCompatReason.empty()?(gpuEligible?"missing_compat_reason":"not_attempted"):gpuCompatReason.c_str();
