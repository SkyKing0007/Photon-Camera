...
if len(sys.argv) not in (4,6):raise SystemExit('usage: verify_26661_shaders.py ROOT BASE CANDIDATE [--compiler PATH]')
...
def embedded(rootp):
 ...
 return out
bm=readm(root/'R1_26661_SHADER_UNIVERSE_BASE.sha256');cm=readm(root/'R1_26661_SHADER_UNIVERSE_CANDIDATE.sha256');assert len(bm)==len(cm)==257
for r,h in bm.items():assert sha(base/r)==h,r
for r,h in cm.items():assert sha(cand/r)==h,r
assert {r for r in bm if bm[r]!=cm[r]}=={'app/src/main/assets/shaders/preview/main_fs.glsl'}
...
expected=readm(root/'R1_26661_RUNTIME_EXPANDED_SHADERS.sha256');assert len(expected)==len(variants)==9 and set(expected)==set(variants)
...
with tempfile.TemporaryDirectory(prefix='iris26661_shader_') as td:
 out=Path(td)
 for name,src in sorted(variants.items()):
  ...
  if compiler:
   ...
   if cp.returncode!=0:raise SystemExit(f'26661 GLSL FAIL {name}\n{cp.stdout}\n{cp.stderr}')
print(f'PASS 26661 exact runtime GLSL variants=9; preview main_fs changed; reserved/structure/exact-hash real_compiler={bool(compiler)} standaloneShaderUniverse=257')
