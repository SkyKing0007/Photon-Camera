...
if len(sys.argv)!=4:raise SystemExit('usage: verify_26685_patches.py ROOT BASE CAND')
root,base,cand=map(Path,sys.argv[1:]);fwd=(root/'R1_26685_RUNTIME_DELTA_FROM_26684_R1.patch').read_bytes();rb=(root/'R1_26685_RUNTIME_ROLLBACK_TO_26684_R1.patch').read_bytes()
...
with tempfile.TemporaryDirectory() as td:
 ...
 for rel in (root/'R1_26685_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines():shutil.copy2(cand/rel,t/rel)
...
print('PASS 26685 deterministic full-index forward/rollback patches at core.abbrev 7/12/40; exact forward and rollback')
