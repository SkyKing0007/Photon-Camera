...
UPLOAD_BASE_COMMIT="2502da697abfc243819f208714cba9ee6c646e06"
...
HANDOFF="$ROOT/26696_HANDOFF_HASHES.sha256"; UPLOADS="$ROOT/26696_UPLOAD_PATHS.txt"
BASE_FULL="$ROOT/26696_BASE_26695_FULL_APP.sha256"; CAND_FULL="$ROOT/26696_EXPECTED_CANDIDATE_FULL_APP.sha256"
BASE_PROTECTED="$ROOT/26696_PROTECTED_UNCHANGED_BASE.sha256"; CAND_PROTECTED="$ROOT/26696_PROTECTED_UNCHANGED_CANDIDATE.sha256"
BASE_NATIVE="$ROOT/26696_NATIVE_PROTECTED_BASE.sha256"; CAND_NATIVE="$ROOT/26696_NATIVE_PROTECTED_CANDIDATE.sha256"
BASE_NATIVE_FULL="$ROOT/26696_NATIVE_FULL_BASE.sha256"; CAND_NATIVE_FULL="$ROOT/26696_NATIVE_FULL_CANDIDATE.sha256"
BASE_VENDOR="$ROOT/26696_VENDOR_PROTECTED_BASE.sha256"; CAND_VENDOR="$ROOT/26696_VENDOR_PROTECTED_CANDIDATE.sha256"
BASE_DNG="$ROOT/26696_DNG_BASE.sha256"; CAND_DNG="$ROOT/26696_DNG_CANDIDATE.sha256"
SHADER_BASE="$ROOT/26696_ASSET_SHADER_UNIVERSE_BASE.sha256"; SHADER_CAND="$ROOT/26696_ASSET_SHADER_UNIVERSE_CANDIDATE.sha256"
CHANGED="$ROOT/26696_RUNTIME_CHANGED_PATHS.txt"; ADDED="$ROOT/26696_ADDED_PATHS_MUST_BE_ABSENT.txt"; RESTORED="$ROOT/26696_RESTORED_26694_PATHS.txt"; PREWRITE="$ROOT/26696_PREWRITE_26695_SOURCE_HASHES.sha256"; RESTORED_HASHES="$ROOT/26696_RESTORED_26694_SOURCE_HASHES.sha256"; EXPECTED_CHANGED="$ROOT/26696_EXPECTED_CHANGED_SOURCE_HASHES.sha256"; REF94_FULL="$ROOT/26696_REFERENCE_26694_FULL_APP.sha256"
FORWARD="$ROOT/26696_RUNTIME_DELTA_FROM_26695.patch"; ROLLBACK="$ROOT/26696_RUNTIME_ROLLBACK_TO_26695.patch"
TRANSFORM="$ROOT/transform_26696.py"; VALIDATE="$ROOT/validate_26696.py"; AUTHORITY="$ROOT/repair_26696_authority.py"; INFRA="$ROOT/repair_26696_infrastructure.py"; PATCHVERIFY="$ROOT/repair_26696_patches.py"; GATEVERIFY="$ROOT/repair_26696_regressions.py"
BUILD_SCRIPT="$ROOT/build_26696_spektra_runtime_parity.sh"; WORKFLOW="$ROOT/.github/workflows/build-26696-spektra-runtime-parity.yml"
OUT="$ROOT/build_26696_spektra_runtime_parity_outputs"; WORK="$ROOT/.build_26696_spektra_runtime_parity_work"
ARTZIP="$WORK/26695_artifact.zip"; ARTDIR="$WORK/artifact_26695"; BASE="$WORK/exact_successful_26695_compiled_candidate"; ROLLBACK_ARTZIP="$WORK/26694_artifact.zip"; ROLLBACK_ARTDIR="$WORK/artifact_26694"; ROLLBACK94="$WORK/exact_successful_26694_compiled_candidate"; AFTER="$WORK/candidate_26696"; AFTER2="$WORK/candidate_26696_replay"; LIVE_CANON="$WORK/live_compiler_candidate_snapshot"; POST="$WORK/postbuild_source_snapshot"; GLSLANG_DIR="$WORK/glslang-${GLSLANG_VERSION}"
...
cat > "$OUT/26696_COMPILER_STATUS.txt" <<'STATUS'
...
STATUS
cat > "$OUT/26696_STRICT_HANDOFF_REPORT.txt" <<'REPORT'
...
INFRASTRUCTURE CHANGED FILES: 26696 identity/scope/regression/build/workflow/handoff wrappers + explicit secondary 26694 rollback-reference authority; successful 26695 compiler-build mechanics preserved
INFRASTRUCTURE DELTA FROM LAST SUCCESS: explicit second authority input for exact 3-path 26694 restore plus 26696 identity/scope/regressions; compiler/build stage ordering unchanged
...
REPORT
set_report(){ local key="$1" val="$2" tmp="$OUT/.report.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26696_STRICT_HANDOFF_REPORT.txt" > "$tmp" || fail "report key missing $key"; mv "$tmp" "$OUT/26696_STRICT_HANDOFF_REPORT.txt"; }
set_compiler(){ local key="$1" val="$2" tmp="$OUT/.compiler.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26696_COMPILER_STATUS.txt" > "$tmp" || fail "compiler key missing $key"; mv "$tmp" "$OUT/26696_COMPILER_STATUS.txt"; }
...
verify_package(){
 [[ -f "$HANDOFF" && -f "$UPLOADS" && -d "$ROOT/handoff_payload_26696" ]] || fail "sealed 26696 package incomplete"
 [[ "$(find "$ROOT/handoff_payload_26696" -type f | wc -l)" -eq 9 ]] || fail "runtime payload must contain exactly 9 files"
...
make_candidate(){
 python3 -S "$TRANSFORM" "$BASE" "$ROLLBACK94" "$AFTER" | tee "$OUT/26696_transform.txt"; python3 -S "$TRANSFORM" "$BASE" "$ROLLBACK94" "$AFTER2" | tee "$OUT/26696_transform_replay.txt"; compare_app_trees "$AFTER" "$AFTER2"
 python3 -S "$VALIDATE" "$BASE" "$AFTER" | tee "$OUT/26696_semantic_validation.txt"; python3 -S "$GATEVERIFY" "$BASE" "$AFTER" | tee "$OUT/26696_regressions.txt"; python3 -S "$AUTHORITY" "$ROOT" "$BASE" "$ROLLBACK94" "$AFTER" | tee "$OUT/26696_authority_candidate.txt"
...
prepare_glslang(){
 ...
 rm -rf "$GLSLANG_DIR"; mkdir -p "$GLSLANG_DIR"; tar -xzf "$archive" -C "$GLSLANG_DIR"; compiler="$(resolve_glslang_compiler "$GLSLANG_DIR")" || fail "pinned glslang compiler missing"; "$compiler" --version | tee "$OUT/26696_glslang_version.txt"; export IRIS26681_SPEKTRA_GLSLANG="$compiler"; [[ -x "$IRIS26681_SPEKTRA_GLSLANG" ]] || fail "pinned glslang is not executable"
}
compile_spektra_raw_shader(){ local compiler="$IRIS26681_SPEKTRA_GLSLANG" outspv="$WORK/SpektraRawDevelop.comp.spv"; "$compiler" -V "$AFTER/app/src/main/cpp/spektra/SpektraRawDevelop.comp" -o "$outspv" 2>&1 | tee "$OUT/26696_glslang_raw_develop.log"; [[ -s "$outspv" ]] || fail "Spektra RAW shader SPIR-V missing"; sha256sum "$AFTER/app/src/main/cpp/spektra/SpektraRawDevelop.comp" "$outspv" > "$OUT/26696_spektra_raw_shader_compile.sha256"; set_report "REAL GLSL COMPILE" "PASS (pinned glslang 16.5.0; inherited shader; successful 26695 order retained)"; set_compiler "REAL GLSL COMPILE" "PASS (pinned glslang 16.5.0; inherited SpektraRawDevelop.comp)"; }
verify_candidate_patches(){ python3 -S "$PATCHVERIFY" "$ROOT" "$BASE" "$AFTER" | tee "$OUT/26696_patch_validation.txt"; set_report "FORWARD PATCH FUZZ=0" "PASS"; set_report "ROLLBACK PATCH FUZZ=0" "PASS"; }
...
verify_compiled_jni_callback(){ local cls; cls="$(find "$ROOT/app/build" -type f -path '*/com/unspektrawesome/diagnostics/InternalLogRecorder.class' -print -quit)"; [[ -n "$cls" && -f "$cls" ]] || fail "compiled InternalLogRecorder.class missing"; javap -s "$cls" | tee "$OUT/26696_internal_log_recorder_javap.txt"; grep -Fq 'public static void recordNative(int, java.lang.String, java.lang.String);' "$OUT/26696_internal_log_recorder_javap.txt" || fail "compiled recordNative method missing"; grep -Fq 'descriptor: (ILjava/lang/String;Ljava/lang/String;)V' "$OUT/26696_internal_log_recorder_javap.txt" || fail "compiled recordNative JNI descriptor mismatch"; set_report "JNI CALLBACK CLASS ABI" "PASS"; set_compiler "JNI CALLBACK CLASS ABI" "PASS"; }
verify_apk_jni_contract(){ python3 -S - "$FINAL" "$OUT/26696_apk_jni_contract.txt" <<'PY2'
...
postbuild_proof(){
 snapshot_candidate_from_authority "$BASE" "$ROOT" "$POST"; compare_app_trees "$AFTER" "$POST"; (cd "$POST" && sha256sum -c "$CAND_PROTECTED" >/dev/null && sha256sum -c "$CAND_NATIVE" >/dev/null && sha256sum -c "$CAND_NATIVE_FULL" >/dev/null && sha256sum -c "$CAND_VENDOR" >/dev/null && sha256sum -c "$CAND_DNG" >/dev/null && sha256sum -c "$SHADER_CAND" >/dev/null) || fail "post-build invariance"; python3 -S "$VALIDATE" "$BASE" "$POST" | tee "$OUT/26696_postbuild_semantic_validation.txt"; python3 -S "$GATEVERIFY" "$BASE" "$POST" | tee "$OUT/26696_postbuild_regressions.txt"; python3 -S "$AUTHORITY" "$ROOT" "$BASE" "$ROLLBACK94" "$POST" | tee "$OUT/26696_postbuild_authority.txt"; set_report "POST-BUILD INVARIANCE" "PASS"; set_compiler "POST-BUILD INVARIANCE" "PASS"
 tar --sort=name --mtime='UTC 1970-01-01' --owner=0 --group=0 --numeric-owner -cf - -C "$POST" app | gzip -n > "$OUT/26696_candidate_app_source.tar.gz"; sha256sum "$OUT/26696_candidate_app_source.tar.gz" > "$OUT/26696_candidate_app_source.tar.gz.sha256"; cp "$CAND_FULL" "$OUT/26696_candidate_full_app.sha256"; cp "$CAND_NATIVE" "$OUT/26696_native_protected_postbuild.sha256"; cp "$CAND_NATIVE_FULL" "$OUT/26696_native_full_postbuild.sha256"; cp "$CAND_VENDOR" "$OUT/26696_vendor_protected_postbuild.sha256"; cp "$CAND_DNG" "$OUT/26696_dng_postbuild.sha256"; cp "$SHADER_CAND" "$OUT/26696_asset_shader_universe_postbuild.sha256"; set_report "CLEAN ARTIFACT SOURCE EXPORT" "PASS (deterministic candidate tar + manifests)"
}
# IRIS_26696_AUTHORITATIVE_ACTIONS_STAGE_ORDER
verify_package
verify_scope
obtain_authority
make_candidate
...
if [[ -n "$LOCAL_ART" ]]; then
 ...
 cp "$OUT/26696_STRICT_HANDOFF_REPORT.txt" "$OUT/26696_local_prebuild_report.txt"
...
./gradlew clean :app:compileDebugKotlin :app:compileDebugJavaWithJavac --stacktrace 2>&1 | tee "$OUT/26696_gradle_language_compilers.log"
...
./gradlew ':app:buildCMakeDebug[arm64-v8a]' ':app:buildCMakeDebug[armeabi-v7a]' --stacktrace 2>&1 | tee "$OUT/26696_gradle_native_compiler.log"
...
verify_candidate_patches
set_report "PRE-BUILD SAFETY PROOF" "PASS"; pass "26696 PRE-BUILD SAFETY PROOF PASSED"
./gradlew :app:assembleDebug --stacktrace 2>&1 | tee "$OUT/26696_gradle_assemble.log"
...
mv "${apks[0]}" "$FINAL"; [[ -f "$FINAL" ]] || fail "final APK missing after move"; set_report "EXACTLY ONE APK" "PASS"; sha256sum "$FINAL" > "$OUT/26696_APK.sha256"
verify_apk_jni_contract
postbuild_proof
pass "26696 ACTIONS BUILD COMPLETE"
