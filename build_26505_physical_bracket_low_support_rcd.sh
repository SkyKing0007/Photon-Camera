...
ROOT="$(pwd)"
OUT="$ROOT/build_26505_physical_bracket_low_support_rcd_outputs"
WORK="$ROOT/.build_26505_physical_bracket_low_support_rcd_work"
PRE="$WORK/prechange26502"
AFTER="$WORK/candidate26505"
...
HANDOFF_26504_HEAD="299731b9447a4d0cd69f0c2e521216bf307724b3"
BACKUP_BRANCH="backup-26504-tested-before-26505-20260818"
...
exec > >(tee "$OUT/26505_build.log") 2>&1

BRANCH="$(git branch --show-current)"
START_HEAD="$(git rev-parse HEAD)"
[[ "$BRANCH" == "$EXPECTED_BRANCH" ]] || fail "branch=$BRANCH expected=$EXPECTED_BRANCH"
[[ "$BRANCH" != "dev" ]] || fail "refusing dev"
git merge-base --is-ancestor "$HANDOFF_26504_HEAD" HEAD || fail "26504 tested handoff is not an ancestor"
...
[[ "$REMOTE_BACKUP" == "$HANDOFF_26504_HEAD" ]] || fail "required backup missing/wrong: $BACKUP_BRANCH -> ${REMOTE_BACKUP:-MISSING}"

for f in "$SEED_26503" "$APPLY_26504" "$VALIDATOR_26504" "$APPLY_26505" "$VALIDATOR_26505" "$INTEGRITY"; do
  [[ -f "$f" ]] || fail "missing dependency $(basename "$f")"
done
[[ "$(sha "$SEED_26503")" == "$SEED_26503_SHA" ]] || fail "26503 seed patch hash mismatch"
pass "GATE 1 branch/lineage/runtime + exact tested-26504 backup verified"
...
git diff --binary "$EMPTY_TREE" "$CANONICAL_26502_HEAD" -- app/src/main app/version.properties > "$OUT/26505_PRECHANGE_FULL_26502_RUNTIME.patch"
[[ -s "$OUT/26505_PRECHANGE_FULL_26502_RUNTIME.patch" ]] || fail "full pre-change patch is empty"
sha256sum "$OUT/26505_PRECHANGE_FULL_26502_RUNTIME.patch" > "$OUT/26505_PRECHANGE_FULL_26502_RUNTIME.patch.sha256"
( cd "$PRE" && tar --sort=name --mtime='UTC 2026-08-18 00:00:00' --owner=0 --group=0 --numeric-owner -czf "$OUT/26505_PRECHANGE_FULL_26502_RUNTIME.tar.gz" app/src/main app/version.properties )
pass "GATE 2 complete pre-change patch + full source archive frozen before modification"

echo "=== GATE 3: RECONSTRUCT PROVEN 26504 THEN APPLY 26505 ==="
...
python3 "$APPLY_26505" "$AFTER" | tee "$OUT/26505_apply_transform.txt"
python3 "$VALIDATOR_26505" "$AFTER" --base-root "$PRE" | tee "$OUT/26505_validator.txt"

set +e
git diff --no-index --binary "$PRE/app" "$AFTER/app" > "$OUT/26505_EXACT_RUNTIME_DELTA_FROM_26502.patch"
rc=$?
set -e
[[ "$rc" -eq 1 && -s "$OUT/26505_EXACT_RUNTIME_DELTA_FROM_26502.patch" ]] || fail "26505 exact runtime delta generation failed"
sha256sum "$OUT/26505_EXACT_RUNTIME_DELTA_FROM_26502.patch" > "$OUT/26505_EXACT_RUNTIME_DELTA_FROM_26502.patch.sha256"
pass "GATE 3 exact 26503 + 26504 + 26505 candidate produced from tested 26502"
...
for i,(rel,stage) in enumerate(items):
    ...
    if cp.returncode:
        (out/'26505_glsl.log').write_text('\n'.join(logs))
        raise SystemExit('GLSL compile failed '+rel)
(out/'26505_glsl.log').write_text('\n'.join(logs))
print('PASS: eleven active/changed shaders compile with Khronos glslang 16.5.0')
PYGLSL
...
javac -proc:none -Xmaxerrs 10000 -d "$WORK/javac_parse" \
 ...
 > "$OUT/26505_javac_parse.log" 2>&1 || true
python3 - "$OUT/26505_javac_parse.log" <<'PYJAVA'
from pathlib import Path
import sys
...
echo "  exact tested-26504 backup + full prechange patch/archive PASS"
echo "  exact nine-file runtime allowlist PASS"
echo "  Wronski normal stack unchanged PASS"
echo "  intentional +2.5 EV Long-A isolated from normal MotionBatch PASS"
echo "  actual CaptureResult timestamp/exposure owns Long-A PASS"
echo "  local <=3.5-frame RCD/PPG fallback from immutable reference CFA PASS"
echo "  26503/26504 Short-A/highlight/render/exposure/EXIF/UHDR invariants PASS"

echo "=== GATE 5: VERSION 0.9726505 / 26505 + BUILD IN SAME GUARDED BLOCK ==="
python3 - "$AFTER/app/version.properties" <<'PYVER'
from pathlib import Path
import re,sys
p=Path(sys.argv[1]); s=p.read_text()
s=re.sub(r'^VERSION_NAME=.*$','VERSION_NAME=0.9726505',s,flags=re.M)
s=re.sub(r'^VERSION_BUILD=.*$','VERSION_BUILD=26505',s,flags=re.M)
p.write_text(s)
PYVER

find app -maxdepth 1 -type f ! -name version.properties -print0 | sort -z | xargs -0 sha256sum > "$OUT/app_shell_pre_26505_overlay.sha256"
rm -rf app/src/main
cp -a "$AFTER/app/src/main" app/src/main
cp "$AFTER/app/version.properties" app/version.properties
find app -maxdepth 1 -type f ! -name version.properties -print0 | sort -z | xargs -0 sha256sum > "$OUT/app_shell_post_26505_overlay.sha256"
diff -u "$OUT/app_shell_pre_26505_overlay.sha256" "$OUT/app_shell_post_26505_overlay.sha256" >/dev/null || fail "module shell changed"

python3 "$INTEGRITY" snapshot "$ROOT/app" "$OUT/26505_pre_gradle_manifest.json"
chmod +x ./gradlew
./gradlew clean :app:assembleDebug --stacktrace
python3 "$INTEGRITY" verify "$ROOT/app" "$OUT/26505_pre_gradle_manifest.json" | tee "$OUT/26505_post_gradle_integrity.txt"

mapfile -t APKS < <(find app/build -type f -name '*.apk' | sort)
[[ "${#APKS[@]}" -eq 1 ]] || fail "expected exactly one APK, found ${#APKS[@]}"
[[ "$(basename "${APKS[0]}")" == "IrisCamera-0.9726505-26505-debug.apk" ]] || fail "unexpected APK identity $(basename "${APKS[0]}")"

FINAL="$ROOT/IrisCamera-0.9726505-26505-physical-bracket-low-support-rcd-debug.apk"
rm -f "$ROOT"/*.apk
cp "${APKS[0]}" "$FINAL"
sha256sum "$FINAL" > "$OUT/26505_apk.sha256"
...
assets=['short_highlight_bayer_recover.glsl','display_exposure.glsl','render.glsl','color_transform.glsl','mfsr_spatial_rgb_normalize_26501.glsl','mfsr_spatial_rgb_contribute_26501.glsl','low_support_ppg_reference_26505.glsl']
...
with ZipFile(apk) as z:
    ...
    for m in [b'IRIS_26504_COMPOSITION_BOUNDED_DISPLAY_GAIN',b'IRIS_26504_SINGLE_EXPOSURE_LOCAL_SUPPORT',b'IRIS_26505_PHYSICAL_LONG_BRACKET',b'IRIS_26505_LOW_SUPPORT_PPG_FALLBACK']:
        assert m in dex,m
...
print('PASS: APK/source shader parity + 26505 runtime markers + source readback-disable proof')
PYAPK
pass "GATE 5 exactly one 26505 APK built"

echo "=== GATE 6: SUCCESSFUL SOURCE CHECKPOINT — NO PROMOTION ==="
( cd "$AFTER" && tar --sort=name --mtime='UTC 2026-08-18 00:00:00' --owner=0 --group=0 --numeric-owner -czf "$OUT/26505_successful_app_source.tar.gz" app/src/main app/version.properties )
( cd "$AFTER" && { find app/src/main -type f -print; echo app/version.properties; } | LC_ALL=C sort | while read -r f; do sha256sum "$f"; done ) > "$OUT/26505_successful_after.sha256"
[[ "$(wc -l < "$OUT/26505_successful_after.sha256")" -eq 870 ]] || fail "26505 source manifest count mismatch"

cat > "$OUT/26505_build_report.txt" <<EOF
26505 Physical Bracket / Low-Support RCD Foundation
Start infrastructure HEAD: $START_HEAD
Protected tested-26504 backup: $BACKUP_BRANCH -> $HANDOFF_26504_HEAD
Canonical runtime base: tested 26502 $CANONICAL_26502_HEAD
Version/build: 0.9726505 / 26505
APK: $(basename "$FINAL")
APK SHA256: $(sha "$FINAL")
HDR capture: equal-exposure Wronski normal stack + optional Short-A + intentional Long-A.
Low-support reconstruction: immutable-reference PPG/RCD-derived local fallback only.
Learned HDR: NOT implemented in 26505.
Promotion: NONE until on-device acceptance.
EOF

pass "clean 870-file 26505 successful-source checkpoint emitted"
echo "PASS: 26505 PHYSICAL BRACKET / LOW-SUPPORT RCD BUILD COMPLETE: $FINAL"
