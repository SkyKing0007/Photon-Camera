...
if len(sys.argv)!=3: raise SystemExit('usage: transform_26693.py BASE DEST')
base=Path(sys.argv[1]); dest=Path(sys.argv[2]); pkg=Path(__file__).resolve().parent; payload=pkg/'handoff_payload_26693'
...
def read_manifest(name):
 ...
 return out
pre=read_manifest('26693_PREWRITE_SOURCE_HASHES.sha256'); post=read_manifest('26693_EXPECTED_CHANGED_SOURCE_HASHES.sha256')
...
print('PASS 26693 deterministic transform: exact 7 modified paths, 0 additions/removals')
