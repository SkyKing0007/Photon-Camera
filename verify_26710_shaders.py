...
if len(sys.argv) not in (4,6): raise SystemExit('usage: verify_26710_shaders.py ROOT BASE CANDIDATE [--compiler PATH]')
...
bm=readm('26710_ASSET_SHADER_UNIVERSE_BASE.sha256');cm=readm('26710_ASSET_SHADER_UNIVERSE_CANDIDATE.sha256');assert len(bm)==len(cm)==271 and bm==cm
...
exp=readm('26710_PREVIEW_RUNTIME_EXPANDED_CANDIDATE.sha256');got=hashlib.sha256(src.encode()).hexdigest();assert got==exp['preview_main_fs_runtime_expanded.frag']
...
if compiler:
 with tempfile.TemporaryDirectory(prefix='iris26710_shader_') as td:
  ...
  if cp.returncode:raise SystemExit(f'26710 GLSL FAIL\n{cp.stdout}\n{cp.stderr}')
print(f'PASS 26710 shader invariance=271; inherited exact runtime-expanded preview reserved/structure/hash proof; real_compiler={bool(compiler)}')
