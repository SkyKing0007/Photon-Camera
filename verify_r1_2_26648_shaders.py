...
if len(sys.argv) not in (4,6): raise SystemExit('usage: verify_r1_2_26648_shaders.py ROOT BASE CANDIDATE [--compiler PATH]')
...
def variants(rootp):
 ...
 return [
  ('sabre_universal_normal_short_fusion_26648.frag',extract(k,'universalNormalShortFusion26648'),'frag'),
  ('sabre_universal_fusion_telemetry_26648.frag',extract(k,'universalFusionTelemetry26648'),'frag'),
  ('sabre_universal_short_validity_augment_26648.frag',extract(k,'universalShortValidityAugment26648'),'frag')]
bm=readm(root/'R1_2_26648_SHADER_UNIVERSE_BASE.sha256'); cm=readm(root/'R1_2_26648_SHADER_UNIVERSE_CANDIDATE.sha256')
assert len(bm)==len(cm)==257 and bm==cm
for r,h in bm.items(): assert sha(base/r)==h,r
for r,h in cm.items(): assert sha(cand/r)==h,r
expected=readm(root/'R1_2_26648_RUNTIME_EXPANDED_SHADERS.sha256'); assert len(expected)==3
...
def scan(name,src):
 ...
 # Permanent 26648 regression: every Iris-style uXxx shader reference must have an explicit uniform declaration.
 uniform_refs=set(re.findall(r'\bu[A-Z][A-Za-z0-9_]*\b',clean))
 uniform_decl=set(re.findall(r'\buniform\s+(?:highp\s+|mediump\s+|lowp\s+)?(?:[A-Za-z_]\w*(?:\s*\[[^;]*?\])?\s+)?(u[A-Z][A-Za-z0-9_]*)\b',clean))
 undeclared=sorted(uniform_refs-uniform_decl)
 if undeclared: raise SystemExit(f'FAIL {name}: undeclared uniform-style identifiers {undeclared}')
 if name.startswith('sabre_universal_normal_short'):
  for token in ['IRIS_26648_SAMPLE_LEVEL_SHORT_VALIDITY','texelFetch(uShortPhysicalReverseWeight','max(flow.w, 0.0)','accumulatedWeights = validAccumulatedWeights','vec3 fused = mix(normalMean.rgb, shortRgb, authority)','vec4(fused, normalMean.a)']:
   if token not in src: raise SystemExit(f'FAIL {name}: missing {token}')
  for forbidden in ['texture(uShortPhysicalReverseWeight','uGradient','uDilation','uSemanticClass']:
   if forbidden in src: raise SystemExit(f'FAIL {name}: forbidden {forbidden}')
 if name.startswith('sabre_universal_fusion_telemetry'):
  for token in ['uNormalMean','uFusedMean','uDecision','shortRadiance','oTelemetry']:
   if token not in src: raise SystemExit(f'FAIL {name}: missing {token}')
 if name.startswith('sabre_universal_short_validity'):
  for token in ['uDecision','totalWeight * authority','oValidityAdd']:
   if token not in src: raise SystemExit(f'FAIL {name}: missing {token}')
mods=variants(cand)
with tempfile.TemporaryDirectory(prefix='iris26648_shader_') as td:
 out=Path(td)
 for name,src,stage in mods:
  ...
  if compiler:
   ...
   if cp.returncode!=0: raise SystemExit(f'26648 GLSL FAIL {name}\n{cp.stdout}\n{cp.stderr}')
print(f'PASS 26648 R1.2 inherited runtime-expanded GLSL variants=3 reserved/structure/exact-hash real_compiler={bool(compiler)} shaderUniverse=257')
