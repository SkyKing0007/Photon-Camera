...
if len(sys.argv) not in (4,6): raise SystemExit('usage: verify_26644_r1_shaders.py ROOT BASE CANDIDATE [--compiler PATH]')
...
def variants(rootp):
 ...
 return [
  ('motionv2_local_laplacian_remap_26644.frag','#version 310 es\n\n#line 1\n'+(rootp/'app/src/main/assets/shaders/motionv2/local_laplacian_remap_26621.glsl').read_text(),'frag'),
  ('sabre_short_component_anchor_26644.frag',extract(k,'shortComponentAnchor26607'),'frag'),
  ('sabre_short_rescue_weight_26644.frag',extract(k,'shortRescueWeight26607'),'frag')]
bm=readm(root/'R1_26644_SHADER_UNIVERSE_BASE.sha256'); cm=readm(root/'R1_26644_SHADER_UNIVERSE_CANDIDATE.sha256')
...
expected=readm(root/'R1_26644_RUNTIME_EXPANDED_SHADERS.sha256'); assert len(expected)==3
...
def scan(name,src):
 ...
 if name=='sabre_short_component_anchor_26644.frag':
  for token in ['IRIS_26644_SHORT_SOURCE_SUPPORT_FAIL_CLOSED','return false;','phaseSamplePixelUnchecked']:
   if token not in src: raise SystemExit(f'FAIL {name}: missing {token}')
 if name=='sabre_short_rescue_weight_26644.frag':
  if 'mirrorUvs(referenceUv + flow.xy)' in src: raise SystemExit('FAIL rescue: mirrored border survives')
  for token in ['IRIS_26644_SHORT_WARP_DOMAIN_FAIL_CLOSED','IRIS_26644_SHORT_EXTRACTED_BILINEAR_FAIL_CLOSED','if (!shortSupportValid)']:
   if token not in src: raise SystemExit(f'FAIL {name}: missing {token}')
 if name=='motionv2_local_laplacian_remap_26644.frag':
  for token in ['IRIS_26644_VISUAL_HIGHLIGHT_SOURCE_STRUCTURE_OWNER','max(structureGate,sourceStructureGate)']:
   if token not in src: raise SystemExit(f'FAIL {name}: missing {token}')
mods=variants(cand)
with tempfile.TemporaryDirectory(prefix='iris26644_shader_') as td:
 out=Path(td)
 for name,src,stage in mods:
  ...
  if compiler:
   ...
   if cp.returncode!=0: raise SystemExit(f'26644 GLSL FAIL {name}\n{cp.stdout}\n{cp.stderr}')
print(f'PASS 26644 runtime-expanded GLSL variants=3 reserved/structure/exact-hash real_compiler={bool(compiler)} shaderUniverse=257')
