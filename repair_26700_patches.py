...
if len(sys.argv)!=4:raise SystemExit('usage: repair_26700_patches.py ROOT BASE26699 CANDIDATE')
base=Path(sys.argv[2]);cand=Path(sys.argv[3]);pkg=Path(__file__).resolve().parent;f=pkg/'26700_RUNTIME_DELTA_FROM_26699.patch';r=pkg/'26700_RUNTIME_ROLLBACK_TO_26699.patch'
...
BH=H(base);CH=H(cand)
with tempfile.TemporaryDirectory(prefix='i26700_') as td:
 ...
 generated=[];shutil.rmtree(repo/'app');shutil.copytree(cand/'app',repo/'app');subprocess.run(['git','add','-f','app'],cwd=repo,check=True)
 for ab in ('7','12','40'):
...
print('PASS 26700 deterministic full-index forward/rollback patches at core.abbrev 7/12/40; exact replay + rollback')
