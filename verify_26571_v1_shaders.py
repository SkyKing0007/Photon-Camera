...
KOTLIN_NAMES=['universalAdaptiveColor26561','localClamp','localMedian','directionalSmooth','iirRgb']
...
def trim_indent(s):
    ...
    return '\n'.join(l[n:] if len(l)>=n else '' for l in lines)+'\n'
...
def extract_native(text):
    a='static const char*kIris26571PublicationCompute=R"GLSL('
    i=text.find(a)
    if i<0: fail('publication compute raw-string start')
    i+=len(a); j=text.find(')GLSL";',i)
    if j<0: fail('publication compute raw-string end')
    return text[i:j].lstrip('\n')
...
def scan(label,src):
    ...
    if '#version 310 es' not in src: fail(label+' version missing')
...
def main():
    ...
    kt=(root/'app/src/main/java/com/hinnka/mycamera/processor/GlesIris26529SpatialRgbChromaPostprocessor.kt').read_text()
    common=extract_kotlin(kt,'common')
    shaders=[]
    for name in KOTLIN_NAMES:
        src=extract_kotlin(kt,name).replace('$common',common.rstrip('\n'))
        if '$common' in src or '${' in src: fail(name+' unresolved Kotlin interpolation')
        shaders.append((name,src))
    cpp=(root/'app/src/main/cpp/motionv2_jpeg444_jni.cpp').read_text()
    shaders.append(('true2x_publication',extract_native(cpp)))
    results=[]; manifest=[]
    for name,src in shaders:
        target=out/(name+'.comp'); target.write_text(src)
        n=scan(name,src); h=hashlib.sha256(target.read_bytes()).hexdigest(); status='NOT RUN'
        if ns.glslang:
            r=subprocess.run([ns.glslang,'-S','comp',str(target)],text=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
            if r.returncode: print(r.stdout); fail('glslang compile '+name)
            status='PASS'
        manifest.append((h,target.name)); results.append({'shader':name,'sha256':h,'declared_identifiers':n,'real_glslang':status})
    u=(out/'universalAdaptiveColor26561.comp').read_text(); lm=(out/'localMedian.comp').read_text(); ds=(out/'directionalSmooth.comp').read_text(); ii=(out/'iirRgb.comp').read_text(); pub=(out/'true2x_publication.comp').read_text(); lc=(out/'localClamp.comp').read_text()
    for token in ['IRIS_26571_SAME_SIDE_MATERIAL_BOUNDARY','coherentCenterProtection','highlightPreservePermission','targetMagnitude > centerMagnitude']:
        if token not in u: fail('universal contract '+token)
    if 'shadowNeed' in u: fail('absolute shadow gate survived')
    if 'imageStore(uOutput,p,imageLoad(uInput,p));' not in lc: fail('26570 luma pass-through lost')
    for token in ['IRIS_26571_CROSS_EDGE_CHROMA_OWNERSHIP','materialBoundary','center.r,']:
        if token not in lm: fail('localMedian contract '+token)
    for token in ['IRIS_26571_DIRECTIONAL_EDGE_CHROMA_FLOOR','0.80*originalMagnitude','highlightPreservePermission','legacyChroma']:
        if token not in ds: fail('directional contract '+token)
    for token in ['IRIS_26571_IIR_MATERIAL_EDGE_STATE_RESET','edgeRatio>0.55','edgeRatio>0.24&&chromaJump>0.45','highlightPreservePermission>0.5']:
        if token not in ii: fail('IIR contract '+token)
    for token in ['IRIS_26571_TRUE2X_PUBLICATION_COMPUTE','layout(rgba8ui,binding=0)','IrisGammaLut','irisAdaptive','irisHeadroom','gainCode','uBandH']:
        if token not in pub: fail('publication shader contract '+token)
    (out/'V1_26571_RUNTIME_EXPANDED_SHADERS.sha256').write_text(''.join(f'{h}  {n}\n' for h,n in manifest))
    (out/'V1_26571_SHADER_VERIFICATION.json').write_text(json.dumps(results,indent=2)+'\n')
    print(f'PASS exact modified runtime-expanded shader extraction + complete reserved scan variants={len(shaders)}')
