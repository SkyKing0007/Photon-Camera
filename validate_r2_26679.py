...
# Permanent 26679 R1 real-javac failure regression: Photon's Log.getStackTraceString accepts Exception, not Throwable.
assert 'catch (Throwable uiError)' not in cc
assert 'catch (Exception uiError)' in cc
assert 'Log.getStackTraceString(uiError)' in cc
...
for x in ['onCaptureStillPictureRejected(Object o)','IRIS_26679_SHUTTER_UI_RELEASE','activateShutterButton(true)','lockUIForBurst(false)']:
 assert x in cf,x
print('PASS 26679 R2 semantic validation: five-file lifecycle reliability correction; admitted Motion HDR transaction and all 26678 IQ/presentation/publication owners protected')
