...
/**
 * IRIS_26681_SPEKTRA_CAMERA_COLOR_OWNER
 *
 * Spektra-only camera color owner.  This is an independently written
 ...
 *
 * Iris/Photo/Night renderers, per-lens profiles, and every creative DCP/tone owner remain outside this class.  In particular an equal
 ...
 */
public final class SpektraColorSolver {
    private static final String TAG = "SpektraColorSolver";
    ...
    private SpektraColorSolver() {}

    public static final class Solution {
        /** Sensor-linear camera RGB -> scene-linear Rec.709/sRGB primaries. */
        ...
        private Solution(float[] sensorToLinearSrgb, float[] sceneWhiteXy, float[] cameraWhite,
                         float interpolationFactor, boolean usedForwardMatrix,
                         int camera2ColorCorrectionMode, float neutralChromaticityError) {
            this.sensorToLinearSrgb = sensorToLinearSrgb;
    ...
    public static Solution solve(CaptureResult result, float[] jpegNeutralOverride,
        ...
        return solveInternal(result, jpegNeutralOverride, color1Obj, color2Obj, calibration1Obj, calibration2Obj,
                forward1Obj, forward2Obj, illuminant1, illuminant2, true);
    }

    public static Solution solveQuiet(CaptureResult result, float[] jpegNeutralOverride,
                                 ColorSpaceTransform color1Obj, ColorSpaceTransform color2Obj,
                                 ColorSpaceTransform calibration1Obj, ColorSpaceTransform calibration2Obj,
                                 ColorSpaceTransform forward1Obj, ColorSpaceTransform forward2Obj,
                                 int illuminant1, int illuminant2) {
        return solveInternal(result, jpegNeutralOverride, color1Obj, color2Obj, calibration1Obj, calibration2Obj,
                forward1Obj, forward2Obj, illuminant1, illuminant2, false);
    }

    private static Solution solveInternal(CaptureResult result, float[] jpegNeutralOverride,
                                 ColorSpaceTransform color1Obj, ColorSpaceTransform color2Obj,
                                 ColorSpaceTransform calibration1Obj, ColorSpaceTransform calibration2Obj,
                                 ColorSpaceTransform forward1Obj, ColorSpaceTransform forward2Obj,
                                 int illuminant1, int illuminant2, boolean logSuccess) {
        ...
        try {
            ...
            final float[] sensorToLinearSrgb = multiply(XYZ_D50_TO_LINEAR_SRGB, cameraToD50);
            if (!finite9(sensorToLinearSrgb)) return fail("nonfinite_output_matrix");
            ...
            if (logSuccess) Log.i(TAG, "IRIS_26685_SPEKTRA_DNG_COLOR_SOLVED"
                    + " factor=" + secondAnchorWeight
                    + " sceneXy=" + Arrays.toString(sceneXy)
                    + " cameraWhite=" + Arrays.toString(cameraWhite)
                    + " forward=" + usedForward
                    + " creativeDcp=false perLensIq=false"
                    + " camera2Mode=" + mode
                    + " neutralError=" + neutralError
                    + " sensorToLinearSrgb=" + Arrays.toString(sensorToLinearSrgb));
            return new Solution(sensorToLinearSrgb, sceneXy, cameraWhite,
                    secondAnchorWeight, usedForward, mode, neutralError);
        } catch (Throwable t) {
            Log.e(TAG, "IRIS_26681_SPEKTRA_DNG_COLOR_SOLVER_EXCEPTION", t);
            return null;
        }
    }

    private static Solution fail(String reason) {
        Log.e(TAG, "IRIS_26681_SPEKTRA_DNG_COLOR_SOLVER_REJECT reason=" + reason);
        return null;
    }
