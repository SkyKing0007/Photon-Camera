...
if len(sys.argv) not in (4,6): raise SystemExit('usage: verify_26655_shaders.py ROOT BASE CANDIDATE [--compiler PATH]')
...
bm=readm(root/'R1_26655_SHADER_UNIVERSE_BASE.sha256');cm=readm(root/'R1_26655_SHADER_UNIVERSE_CANDIDATE.sha256');assert len(bm)==len(cm)==257
...
'asset_local_laplacian_downsample_26621.frag':expand('app/src/main/assets/shaders/motionv2/local_laplacian_downsample_26621.glsl'),
...
expected_hash=readm(root/'R1_26655_RUNTIME_EXPANDED_SHADERS.sha256');assert len(expected_hash)==4 and set(expected_hash)==set(variants)
...
with tempfile.TemporaryDirectory(prefix='iris26655_shader_') as td:
 out=Path(td)
 for name,src in sorted(variants.items()):
  ...
  if compiler:
   ...
   if cp.returncode!=0: raise SystemExit(f'26655 GLSL FAIL {name}\n{cp.stdout}\n{cp.stderr}')
print(f'PASS 26655 exact runtime GLSL variants=4 (render/local-downsample/local-remap/gainmap) reserved/structure/exact-hash real_compiler={bool(compiler)} standaloneShaderUniverse=257 exactly4changed')
