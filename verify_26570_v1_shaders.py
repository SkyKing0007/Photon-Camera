...
NAMES=['universalAdaptiveColor26561','localClamp','localMedian','iirRgb']
...
def extract_raw(src,name):
    # exact project form: [private ]val name = """ ... """.trimIndent()
...
def declared(src):
    ...
    # Catch struct fields/local comma declarations too.
...
def main():
    ...
    p=root/'app/src/main/java/com/hinnka/mycamera/processor/GlesIris26529SpatialRgbChromaPostprocessor.kt';text=p.read_text()
    common=extract_raw(text,'common')
    results=[];manifest=[]
    for name in NAMES:
        src=extract_raw(text,name).replace('$common',common.rstrip('\n'))
    ...
    # Exact new semantic shader contracts.
    u=(out/'universalAdaptiveColor26561.comp').read_text();lc=(out/'localClamp.comp').read_text();lm=(out/'localMedian.comp').read_text();ii=(out/'iirRgb.comp').read_text()
    for needle in ['centerNormalizedChroma','sameSurfaceWeight','surfaceSupport','maximumRelativeLumaDelta']:
        if needle not in u:fail('universal continuity contract '+needle)
    if 'shadowNeed' in u:fail('absolute shadow gate survived')
    if 'imageStore(uOutput,p,imageLoad(uInput,p));' not in lc:fail('one-sided luma authority not pass-through')
    if 'center.r,' not in lm or 'oneSidedProtection' not in lm:fail('median edge exclusion/luma envelope contract')
    if 'edgeRatio>0.55' not in ii:fail('IIR edge-state reset contract')
    (out/'V1_26570_RUNTIME_EXPANDED_SHADERS.sha256').write_text(''.join(f'{h}  {n}\n' for h,n in manifest))
    (out/'V1_26570_SHADER_VERIFICATION.json').write_text(json.dumps(results,indent=2)+'\n')
    print('PASS exact modified embedded shader expansion + reserved scan variants=4')
