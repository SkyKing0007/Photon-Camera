...
if len(sys.argv)!=4: raise SystemExit('usage: repair_26692_patches.py ROOT BASE CANDIDATE')
root=Path(sys.argv[1]); base=Path(sys.argv[2]); cand=Path(sys.argv[3]); pkg=Path(__file__).resolve().parent
f=pkg/'R1_1_26692_RUNTIME_DELTA_FROM_26691_R1.patch'; r=pkg/'R1_1_26692_RUNTIME_ROLLBACK_TO_26691_R1.patch'
...
with tempfile.TemporaryDirectory(prefix='iris26692_patch_') as td:
 ...
 shutil.rmtree(d/'app'); shutil.copytree(cand/'app',d/'app'); add(d)
 gs=[];rs=[]
 for a in (7,12,40):
...
print('PASS 26692 full-index patch proof: core.abbrev 7/12/40 identical, fuzz=0, exact forward+rollback')
