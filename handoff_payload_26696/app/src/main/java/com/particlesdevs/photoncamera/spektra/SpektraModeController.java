...
public final class SpektraModeController {
    ...
    private void bridgeProcessingStarted(@NonNull RawVulkanPreviewController source, long revisionId) {
        ...
        activity.runOnUiThread(() -> {
            ...
            // IRIS_26696_SPEKTRA_PROCESSING_UI_AUTHORITY: same owner used by Photo/Night.
    ...
    /**
     * Fail-contained, non-blocking handoff. Iris may open the destination camera only after the
     * exact Spektra CameraDevice reports onClosed().
     */
    public void exitForModeHandoff(@NonNull Runnable onReleased) {
        releaseCaptureUiBeforeHandoff();
        active = false;
        final int token = generation.incrementAndGet();
        selectedCamera = null;
        RawVulkanPreviewController current = controller;
        Runnable finish = () -> activity.runOnUiThread(() -> {
            if (generation.get() != token || shutdown) return;
            host.onSpektraPreviewStopped();
            Log.i("SpektraModeController", "IRIS_26696_MODE_HANDOFF_RELEASED generation=" + token);
            onReleased.run();
        });
        if (current != null) current.retireForModeHandoff(finish);
        else finish.run();
    }
