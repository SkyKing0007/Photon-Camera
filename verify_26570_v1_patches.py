...
CHANGED=['app/src/main/java/com/hinnka/mycamera/processor/GlesIris26529SpatialRgbChromaPostprocessor.kt','app/src/main/cpp/motionv2_jpeg444_jni.cpp','app/version.properties']
...
def regen(start,target,abbrev):
 td=Path(tempfile.mkdtemp(prefix='p26570_'))
 try:
  init(td,start)
...
def applyprove(start,target,patch):
 td=Path(tempfile.mkdtemp(prefix='a26570_'))
 try:
...
def main():
 ...
 print('PASS FORWARD PATCH FUZZ=0 exact candidate recreation')
 print('PASS ROLLBACK PATCH FUZZ=0 exact 26569 recreation')
