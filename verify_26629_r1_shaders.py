...
if len(sys.argv)<5 or sys.argv[3]!='--out': raise SystemExit('usage: verify_26629_r1_shaders.py BASE CAND --out OUT [--compiler PATH]')
...
mods=[frag('motionv2_render_26629',(C/render_rel).read_text()),frag('motionv2_gainmap_26629',(C/gain_rel).read_text())]
...
mods.append(('true2x_publication_compute_26629',compute,'comp'))
...
(O/'R1_26629_RUNTIME_EXPANDED_SHADERS.sha256').write_text('\n'.join(lines)+'\n')
(O/'R1_26629_SHADER_VERIFICATION.json').write_text(json.dumps({'count':len(results),'compiler':compiler,'results':results},indent=2,sort_keys=True)+'\n')
print(f'PASS 26629 complete runtime-expanded GLSL reserved scan variants={len(results)} real_compiler={bool(compiler)}')
