...
def main():
    ...
    specs=[
      ...
      ('short_common_sabre_owner_26601',extract_kotlin(sc,'shortAccumulatorOwnership26601'),'frag'),
    ...
    specs.append(('motionv2_render_26597_inherited',prefix+rc,'frag'))
    ...
    cpp=(root/'app/src/main/cpp/motionv2_jpeg444_jni.cpp').read_text(); specs.append(('true2x_publication_compute_26598_restored',extract_cpp_raw(cpp,'kIris26571PublicationCompute'),'comp'))
    if len(specs)!=20: fail('runtime-expanded shader variant count '+str(len(specs)))
    rows=[]; manifest=[]
    ...
    (out/'V1_26601_RUNTIME_EXPANDED_SHADERS.sha256').write_text(''.join(manifest))
    (out/'V1_26601_SHADER_VERIFICATION.json').write_text(json.dumps(rows,indent=2)+'\n')
