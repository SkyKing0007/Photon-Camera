...
import hashlib,sys
if len(sys.argv)!=4: raise SystemExit('usage: verify_r1_1_26648_authority.py ROOT BASE CANDIDATE')
...
def check(name,rootp):
 ...
 return m
bm=check('R1_1_26648_BASE_26646_R1_FULL_APP.sha256',base); cm=check('R1_1_26648_EXPECTED_CANDIDATE_FULL_APP.sha256',cand)
assert len(bm)==len(cm)==1720 and set(bm)==set(cm)
allow={r for r in (root/'R1_1_26648_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if r}; assert len(allow)==8
...
for prefix,count in [('PROTECTED_UNCHANGED',1712),('NATIVE_PROTECTED',800),('VENDOR_PROTECTED',778),('DNG',7)]:
 a=check(f'R1_1_26648_{prefix}_BASE.sha256',base); b=check(f'R1_1_26648_{prefix}_CANDIDATE.sha256',cand)
...
for prefix,rootp in [('SHADER_UNIVERSE_BASE',base),('SHADER_UNIVERSE_CANDIDATE',cand)]:
 m=check(f'R1_1_26648_{prefix}.sha256',rootp); assert len(m)==257
assert readm(root/'R1_1_26648_SHADER_UNIVERSE_BASE.sha256')==readm(root/'R1_1_26648_SHADER_UNIVERSE_CANDIDATE.sha256')
