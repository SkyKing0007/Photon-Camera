...
NEW_BUILD="26492"
OUTDIR="build_26492_v5_outputs"
APK_NAME="IrisCamera-${NEW_VERSION}-${NEW_BUILD}-v5-authority-provenance-r32f-rcd-parser-fix-debug.apk"
...
echo "=== 26492 V5 AUTHORITY / PROVENANCE — RCD RUNTIME PARSER FIX + PROVEN TWO-WORKSPACE BUILD ==="
date -Iseconds || true
...
git merge-base --is-ancestor "$BLACK_V4_HEAD" HEAD || fail "26492 V5 must descend from exact black V4 checkpoint"
...
# GATE 4B: emulate Photon GLInterface.getLayouts() exactly enough to catch the
# V4 runtime failure that glslang/host-binding audits cannot see.
...
g=gl.read_text()
for exact in ('val.replace("{","").split(" ")',
              'divided[divided.length-1].replace(";","").replace("\\\\n","")'):
    if exact not in g:
        raise SystemExit('GLInterface.getLayouts parser contract changed; V5 emulation must be updated: '+exact)

def java_space_split(s):
    parts=re.split(r' ',s)
...
print('26492 PHOTON RUNTIME LAYOUT PARSER PASS CfaBuf=0 RedBuf=1 GreenBuf=2 BlueBuf=3 compactKeyAbsent=true')
PY_RUNTIME_LAYOUT
...
26492 V5 Authority / Provenance — R32F + RCD Runtime Parser Compatibility
