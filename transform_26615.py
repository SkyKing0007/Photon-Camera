...
if len(sys.argv)!=3: raise SystemExit('usage: transform_26615.py BASE OUT')
...
def verify(root,manifest):
    ...
    return len(lines)
if verify(base,'R1_26615_PREWRITE_SOURCE_HASHES.sha256')!=11: raise SystemExit('prewrite count')
patch=P/'R1_26615_RUNTIME_DELTA_FROM_26614_R1.patch'
# Exact successful 26614 R1 isolation regression: never let nested candidate discover parent .git.
...
if verify(out,'R1_26615_EXPECTED_CHANGED_SOURCE_HASHES.sha256')!=11: raise SystemExit('candidate changed count')
print('PASS 26615 deterministic candidate reconstruction from exact successful 26614 R1 + canonical full-index patch')
