...
if len(sys.argv)!=3: raise SystemExit('usage: transform_26643_r1.py BASE OUT')
base=Path(sys.argv[1]).resolve(); out=Path(sys.argv[2]).resolve(); root=Path(__file__).resolve().parent
payload=root/'handoff_payload_26643_r1'
changed=[x for x in (root/'R1_26643_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x]
added={x for x in (root/'R1_26643_ADDED_PATHS_MUST_BE_ABSENT.txt').read_text().splitlines() if x}
def readm(p):
 d={}
...
pre=readm(root/'R1_26643_PREWRITE_SOURCE_HASHES.sha256'); expected=readm(root/'R1_26643_EXPECTED_CHANGED_SOURCE_HASHES.sha256')
...
print('PASS 26643 deterministic candidate transform: exact 10-path delta / 1 added from successful 26642 compiled-candidate authority')
