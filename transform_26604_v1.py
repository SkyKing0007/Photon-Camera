...
pre=read(pkg/'V1_26604_PREWRITE_SOURCE_HASHES.sha256'); exp=read(pkg/'V1_26604_EXPECTED_CHANGED_SOURCE_HASHES.sha256')
if list(pre)!=CHANGED or list(exp)!=CHANGED: fail('manifest allowlist mismatch')
payload=pkg/'handoff_payload_26604_v1'; paths=sorted(p.relative_to(payload).as_posix() for p in payload.rglob('*') if p.is_file())
...
print('PASS exact successful-26603 compiled-candidate authority + exact 12-file 26604 overlay')
