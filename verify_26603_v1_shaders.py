...
def main():
 ...
 specs.append(('motionv2_render_26603_master_faithful_sdr',prefix+rc,'frag'));sp=(root/'app/src/main/java/com/hinnka/mycamera/processor/GlesMgcRawSpatialShaders.kt').read_text();specs.append(('short_full_mask_count_26596',extract_kotlin(sp,'bentoCountHighlightMask'),'comp'));cpp=(root/'app/src/main/cpp/motionv2_jpeg444_jni.cpp').read_text();specs.append(('true2x_publication_compute_26603_master_faithful_sdr',extract_cpp_raw(cpp,'kIris26571PublicationCompute'),'comp'))
 if len(specs)!=26:fail('runtime-expanded shader variant count '+str(len(specs)))
 rows=[];manifest=[]
 ...
 (out/'V1_26603_RUNTIME_EXPANDED_SHADERS.sha256').write_text(''.join(manifest));(out/'V1_26603_SHADER_VERIFICATION.json').write_text(json.dumps(rows,indent=2)+'\n')
