...
if len(sys.argv)!=5: raise SystemExit('usage: verify_26626_r1_patches.py BASE CAND FORWARD ROLLBACK')
B=Path(sys.argv[1]); C=Path(sys.argv[2]); F=Path(sys.argv[3]); R=Path(sys.argv[4])
def H(root):
    return {str(p.relative_to(root)):hashlib.sha256(p.read_bytes()).hexdigest() for p in sorted((root/'app').rglob('*')) if p.is_file()}
BH,CH=H(B),H(C)
if len(BH)!=1713 or len(CH)!=1713: raise SystemExit('FAIL patch universe count')
# Full-index patches must carry full 40-hex blob ids, binary-safe, and diff --check clean.
for p in (F,R):
    text=p.read_text(errors='replace')
    for line in text.splitlines():
        if line.startswith('index '):
            ids=line.split()[1].split('..')
            if len(ids)!=2 or any(len(x.split()[0])!=40 for x in ids): raise SystemExit(f'FAIL non-full index line in {p.name}: {line}')
for abbrev in (7,12,40):
    with tempfile.TemporaryDirectory(prefix=f'iris26626_patch_{abbrev}_') as td:
        ...
        subprocess.run(['git','diff','--cached','--check'],cwd=root,check=True)
print('PASS 26626 deterministic full-index patches core.abbrev=7/12/40; --index exact preimage; diff --check clean; exact forward/rollback')
