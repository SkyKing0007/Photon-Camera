...
def all_embedded(src):
 out={}
 pat=re.compile(r'(?m)^\s*(?:private\s+)?val\s+([A-Za-z_]\w*)\s*=\s*"""')
...
def main():
 ...
 eb,ec=all_embedded(sb),all_embedded(sc)
 for name,src in eb.items():
  if name not in ec or ec[name]!=src:fail('pre-existing Sabre shader changed '+name)
 if set(ec)-set(eb)!={'shortRestoreMask26587','shortRestoreRgba16f26587'}:fail('unexpected new Sabre shader names '+repr(sorted(set(ec)-set(eb))))
 ...
 ('short_restore_mask_26587',extract_kotlin(sc,'shortRestoreMask26587'),'frag'),
 ...
 (out/'V1_26587_RUNTIME_EXPANDED_SHADERS.sha256').write_text(''.join(manifest));(out/'V1_26587_SHADER_VERIFICATION.json').write_text(json.dumps(rows,indent=2)+'\n')
 print('PASS all pre-existing embedded Sabre shader strings byte-identical to successful 26586')
 print('PASS exact runtime-expanded shader extraction + complete reserved scan variants=10')
