...
import hashlib, sys
if len(sys.argv)<3: raise SystemExit('usage: verify_26628_r3_infrastructure.py BUILD WORKFLOW [--successful-26627-build P --successful-26627-workflow P --successful-26627-transform P]')
bp=Path(sys.argv[1]); wp=Path(sys.argv[2]); b=bp.read_text(); w=wp.read_text(); root=bp.parent
def sha(p): return hashlib.sha256(Path(p).read_bytes()).hexdigest()
for t in [
'EXPECTED_BRANCH="experimental-clean-photon-rebuild"','RUNTIME_AUTHORITY_COMMIT="77ad63a80827ac103714be303678be1b47ad30d4"','HANDOFF_PARENT_COMMIT="dd9efefaa2070775cd46f51e143ab1c5783409e7"',
...
for required in ["'26628_R3_README_UPLOAD.txt'","'REGRESSION_R3_26628_*'","'R3_26628_*'","'build_26628_r3_bjzhou_color_motion_night_superres.sh'","'transform_26628_r3.py'","'validate_26628_r3.py'","'verify_26628_r3_*.py'","'handoff_payload_26628_r3/**'","'.github/workflows/build-26628-r3-bjzhou-color-motion-night-superres.yml'"]:
...
for tok in order:
    ...
    pos=n
t=(root/'transform_26628_r3.py').read_text()
        ...
        if sseq:
            ...
            for tok in sseq:
                if tok.startswith('python3 -S "$SHADERVERIFY"') or tok.startswith('rm -rf "$ROOT/app/src"') or tok.startswith('snapshot_candidate_from_authority') or tok.startswith('./gradlew') or tok in ['verify_candidate_patches','PRE-BUILD SAFETY PROOF PASSED','expected exactly one Gradle debug APK','POST-BUILD INVARIANCE','candidate_app_source.tar.gz']:
                    ...
                    pp=nn
print('PASS 26628 infrastructure: exact successful-26627 direct ordering/isolation/compiler/NDK/patch/PRE-BUILD/assemble/postbuild mechanics preserved; authority/identity/11-path color/UI/native+Java-regression validators only')
