...
HANDOFF="$ROOT/R1_26648_HANDOFF_HASHES.sha256"; UPLOADS="$ROOT/R1_26648_UPLOAD_PATHS.txt"
BASE_FULL="$ROOT/R1_26648_BASE_26646_R1_FULL_APP.sha256"; CAND_FULL="$ROOT/R1_26648_EXPECTED_CANDIDATE_FULL_APP.sha256"
BASE_PROTECTED="$ROOT/R1_26648_PROTECTED_UNCHANGED_BASE.sha256"; CAND_PROTECTED="$ROOT/R1_26648_PROTECTED_UNCHANGED_CANDIDATE.sha256"
BASE_NATIVE="$ROOT/R1_26648_NATIVE_PROTECTED_BASE.sha256"; CAND_NATIVE="$ROOT/R1_26648_NATIVE_PROTECTED_CANDIDATE.sha256"
BASE_VENDOR="$ROOT/R1_26648_VENDOR_PROTECTED_BASE.sha256"; CAND_VENDOR="$ROOT/R1_26648_VENDOR_PROTECTED_CANDIDATE.sha256"
BASE_DNG="$ROOT/R1_26648_DNG_BASE.sha256"; CAND_DNG="$ROOT/R1_26648_DNG_CANDIDATE.sha256"
SHADER_BASE="$ROOT/R1_26648_SHADER_UNIVERSE_BASE.sha256"; SHADER_CAND="$ROOT/R1_26648_SHADER_UNIVERSE_CANDIDATE.sha256"
CHANGED="$ROOT/R1_26648_RUNTIME_CHANGED_PATHS.txt"; PREWRITE="$ROOT/R1_26648_PREWRITE_SOURCE_HASHES.sha256"; ADDED="$ROOT/R1_26648_ADDED_PATHS_MUST_BE_ABSENT.txt"; EXPECTED_CHANGED="$ROOT/R1_26648_EXPECTED_CHANGED_SOURCE_HASHES.sha256"
FORWARD="$ROOT/R1_26648_RUNTIME_DELTA_FROM_26646_R1.patch"; ROLLBACK="$ROOT/R1_26648_RUNTIME_ROLLBACK_TO_26646_R1.patch"; SHADER_PIN="$ROOT/R1_26648_RUNTIME_EXPANDED_SHADERS.sha256"
TRANSFORM="$ROOT/transform_26648_r1.py"; VALIDATE="$ROOT/validate_26648_r1.py"; AUTHORITY="$ROOT/verify_26648_r1_authority.py"; INFRA="$ROOT/verify_26648_r1_infrastructure.py"; PATCHVERIFY="$ROOT/verify_26648_r1_patches.py"; SHADERVERIFY="$ROOT/verify_26648_r1_shaders.py"; GATEVERIFY="$ROOT/verify_26648_r1_regressions.py"
BUILD_SCRIPT="$ROOT/build_26648_r1_universal_fusion_heic_ui.sh"; WORKFLOW="$ROOT/.github/workflows/build-26648-r1-universal-fusion-heic-ui.yml"
OUT="$ROOT/build_26648_r1_universal_fusion_heic_ui_outputs"; WORK="$ROOT/.build_26648_r1_universal_fusion_heic_ui_work"
ARTZIP="$WORK/26646_r1_artifact.zip"; ARTDIR="$WORK/artifact"; BASE="$WORK/exact_26646_r1_compiled_candidate"; AFTER="$WORK/candidate_26648_r1"; AFTER2="$WORK/candidate_26648_r1_replay"; LIVE_CANON="$WORK/live_compiler_candidate_snapshot"; POST="$WORK/postbuild_source_snapshot"; GLSLANG_DIR="$WORK/glslang-${GLSLANG_VERSION}"
...
cat > "$OUT/26648_R1_COMPILER_STATUS.txt" <<'STATUS'
...
STATUS
cat > "$OUT/26648_R1_STRICT_HANDOFF_REPORT.txt" <<'REPORT'
...
REPORT
set_report(){ local key="$1" val="$2" tmp="$OUT/.report.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26648_R1_STRICT_HANDOFF_REPORT.txt" > "$tmp" || fail "report key missing $key"; mv "$tmp" "$OUT/26648_R1_STRICT_HANDOFF_REPORT.txt"; }
set_compiler(){ local key="$1" val="$2" tmp="$OUT/.compiler.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26648_R1_COMPILER_STATUS.txt" > "$tmp" || fail "compiler key missing $key"; mv "$tmp" "$OUT/26648_R1_COMPILER_STATUS.txt"; }
...
verify_package(){
  [[ -f "$HANDOFF" && -f "$UPLOADS" && -d "$ROOT/handoff_payload_26648_r1" ]] || fail "sealed 26648 package incomplete"
  [[ "$(find "$ROOT/handoff_payload_26648_r1" -type f | wc -l)" -eq 8 ]] || fail "sealed runtime payload must contain exactly 8 files"
...
verify_scope(){
  if [[ -n "$LOCAL_ART" ]]; then set_report "CHANGED RUNTIME SCOPE" "PASS (local sealed exact 8-path/0-added 26648 candidate; no live app source packaged)"; return; fi
  [[ "$(git branch --show-current)" == "$EXPECTED_BRANCH" ]] || fail "wrong branch"
  local intervening="3d9851e030cededc038e3a1a3287460c916c9e9d"
  [[ "$(git rev-parse HEAD^)" == "$intervening" ]] || fail "26648 handoff commit parent must be exact existing rejected-26647 handoff commit"
  [[ "$(git rev-parse ${intervening}^)" == "$RUNTIME_AUTHORITY_COMMIT" ]] || fail "26647 handoff parent must remain exact successful 26646 R1 runtime authority"
  ! git diff --name-only "$RUNTIME_AUTHORITY_COMMIT".."$intervening" | grep -Eq '^app/' || fail "intervening 26647 handoff unexpectedly committed live app source"
  git diff --name-only "$intervening"..HEAD | sort > "$WORK/actual_upload_scope.txt"
  sort "$UPLOADS" > "$WORK/expected_upload_scope.txt"
  diff -u "$WORK/expected_upload_scope.txt" "$WORK/actual_upload_scope.txt" || fail "26648 upload scope mismatch"
  ! grep -Eq '^app/' "$WORK/actual_upload_scope.txt" || fail "26648 handoff commit contains live app source"
  set_report "CHANGED RUNTIME SCOPE" "PASS (exact 8-path/0-added runtime candidate allowlist; handoff-only scope; 26647 intervening commit proven handoff-only; runtime reconstructed from successful 26646 compiled candidate)"
}
obtain_authority(){
...
make_candidate(){
  ...
  while IFS= read -r rel; do [[ -n "$rel" ]] || continue; cmp "$ROOT/handoff_payload_26648_r1/$rel" "$AFTER/$rel" || fail "sealed payload differs frozen candidate: $rel"; done < "$CHANGED"
...
postbuild_proof(){
  ...
  tar --sort=name --mtime='UTC 1970-01-01' --owner=0 --group=0 --numeric-owner -cf - -C "$POST" app | gzip -n > "$OUT/26648_R1_candidate_app_source.tar.gz"
  sha256sum "$OUT/26648_R1_candidate_app_source.tar.gz" > "$OUT/26648_R1_candidate_app_source.tar.gz.sha256"
  cp "$CAND_FULL" "$OUT/26648_R1_candidate_full_app.sha256"; cp "$CAND_NATIVE" "$OUT/26648_R1_native_protected_postbuild.sha256"; cp "$CAND_VENDOR" "$OUT/26648_R1_vendor_protected_postbuild.sha256"; cp "$CAND_DNG" "$OUT/26648_R1_dng_postbuild.sha256"; cp "$SHADER_PIN" "$OUT/26648_R1_runtime_expanded_shaders.sha256"
...
if [[ -n "$LOCAL_ART" ]]; then
  ...
  cp "$OUT/26648_R1_STRICT_HANDOFF_REPORT.txt" "$OUT/26648_local_prebuild_report.txt"
  pass "26648 R1 LOCAL PREBUILD PREPARED: exact successful-26646 authority + exact 8-path universal-fusion/HEIC/UI candidate; applicable real compiler/build gates explicitly unproven locally"
  exit 0
fi
install_frozen_candidate_live
...
mv "${apks[0]}" "$FINAL"; set_report "EXACTLY ONE APK" "PASS"; sha256sum "$FINAL" > "$OUT/26648_R1_APK.sha256"
postbuild_proof
pass "26648 R1 ACTIONS BUILD COMPLETE"
