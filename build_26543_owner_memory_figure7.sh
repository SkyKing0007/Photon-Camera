...
for f in "$APPLY" "$FINISH" "$VALIDATE" "$SYNTAX" "$CONTRACT" "$RUNTIME_META" "$V16_PROVENANCE_PREFLIGHT" "$SHADER_PREFLIGHT" "$NATIVE_JPEG_PREFLIGHT" "$JAVA_XML_PREFLIGHT" "$EMBEDDED_PREFLIGHT" "$INHERITED_SHADER_PREFLIGHT" "$KOTLIN_API_PREFLIGHT" "$NATIVE_DNG_PREFLIGHT" "$DNG_TEST" "$INHERITED_26540_JAVA" "$INHERITED_26541_JAVA" "$BJZHOU_MANIFEST" "$BJZHOU_COMMIT_FILE"; do
