...
HANDOFF_PARENT_COMMIT="e544a1a408d6dcda028c029dd4539ca4ded49f21"
...
HANDOFF="$ROOT/V1_1_26613_HANDOFF_HASHES.sha256"
BASE_FULL="$ROOT/V1_1_26613_BASE_26612_V1_FULL_APP.sha256"; CAND_FULL="$ROOT/V1_1_26613_EXPECTED_CANDIDATE_FULL_APP.sha256"
BASE_PROTECTED="$ROOT/V1_1_26613_PROTECTED_UNCHANGED_BASE.sha256"; CAND_PROTECTED="$ROOT/V1_1_26613_PROTECTED_UNCHANGED_CANDIDATE.sha256"
BASE_NATIVE="$ROOT/V1_1_26613_NATIVE_PROTECTED_BASE.sha256"; CAND_NATIVE="$ROOT/V1_1_26613_NATIVE_PROTECTED_CANDIDATE.sha256"
BASE_VENDOR="$ROOT/V1_1_26613_VENDOR_PROTECTED_BASE.sha256"; CAND_VENDOR="$ROOT/V1_1_26613_VENDOR_PROTECTED_CANDIDATE.sha256"
BASE_DNG="$ROOT/V1_1_26613_DNG_BASE.sha256"; CAND_DNG="$ROOT/V1_1_26613_DNG_CANDIDATE.sha256"
CHANGED="$ROOT/V1_1_26613_RUNTIME_CHANGED_PATHS.txt"; PREWRITE="$ROOT/V1_1_26613_PREWRITE_SOURCE_HASHES.sha256"; EXPECTED_CHANGED="$ROOT/V1_1_26613_EXPECTED_CHANGED_SOURCE_HASHES.sha256"
FORWARD="$ROOT/V1_1_26613_RUNTIME_DELTA_FROM_26612_V1.patch"; ROLLBACK="$ROOT/V1_1_26613_RUNTIME_ROLLBACK_TO_26612_V1.patch"; SHADER_PIN="$ROOT/V1_1_26613_RUNTIME_EXPANDED_SHADERS.sha256"
TRANSFORM="$ROOT/transform_26613_v1_1.py"; VALIDATE="$ROOT/validate_26613_v1_1.py"; AUTHORITY="$ROOT/verify_26613_v1_1_authority.py"; INFRA="$ROOT/verify_26613_v1_1_infrastructure.py"; PATCHVERIFY="$ROOT/verify_26613_v1_1_patches.py"; SHADERVERIFY="$ROOT/verify_26613_v1_1_shaders.py"; GATEVERIFY="$ROOT/verify_26613_v1_1_regressions.py"
BUILD_SCRIPT="$ROOT/build_26613_v1_1_fixed_domain_support_provenance.sh"; WORKFLOW="$ROOT/.github/workflows/build-26613-v1-1-fixed-domain-support-provenance.yml"
OUT="$ROOT/build_26613_v1_1_fixed_domain_support_provenance_outputs"; WORK="$ROOT/.build_26613_v1_1_fixed_domain_support_provenance_work"; ARTZIP="$WORK/26612_v1_artifact.zip"; ARTDIR="$WORK/artifact"; BASE="$WORK/exact_26612_v1_compiled_candidate"; AFTER="$WORK/candidate_26613_v1_1"; AFTER2="$WORK/candidate_26613_v1_1_replay"; SHADER_OUT="$WORK/runtime_expanded_shaders"; GLSLANG_DIR="$WORK/glslang-16.5.0"; LIVE_CANON="$WORK/live_compiler_candidate_snapshot"; POST="$WORK/postbuild_source_snapshot"; FINAL="$ROOT/IrisCamera-${VERSION_NAME}-${VERSION_BUILD}-v1-1-fixed-domain-support-provenance-debug.apk"
...
cat > "$OUT/26613_V1_1_COMPILER_STATUS.txt" <<'STATUS'
...
STATUS
cat > "$OUT/26613_V1_1_STRICT_HANDOFF_REPORT.txt" <<'REPORT'
...
REPORT
set_report(){ local key="$1" val="$2" tmp="$OUT/.report.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26613_V1_1_STRICT_HANDOFF_REPORT.txt" > "$tmp" || fail "report key missing $key"; mv "$tmp" "$OUT/26613_V1_1_STRICT_HANDOFF_REPORT.txt"; }
set_compiler(){ local key="$1" val="$2" tmp="$OUT/.compiler.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26613_V1_1_COMPILER_STATUS.txt" > "$tmp" || fail "compiler key missing $key"; mv "$tmp" "$OUT/26613_V1_1_COMPILER_STATUS.txt"; }
...
PY
  ...
  set_report "INFRASTRUCTURE DELTA AUDIT" "PASS (26613 V1.1 compiler-only identity/scope/regression; successful-26612 compiler/build mechanics order preserved; no backup)"
}
verify_scope(){
  ...
  [[ "$(git rev-parse HEAD^)" == "$HANDOFF_PARENT_COMMIT" ]] || fail "26613 V1.1 must be one repair handoff commit directly on failed 26613 V1"
  python3 -S - "$HANDOFF" > "$WORK/expected_scope.txt" <<'PY'
from pathlib import Path
import sys
names=[line.split('  ',1)[1] for line in Path(sys.argv[1]).read_text().splitlines() if line.strip()]; names.append('V1_1_26613_HANDOFF_HASHES.sha256'); print('\n'.join(sorted(names)))
PY
...
PY
  while IFS= read -r rel; do [[ -n "$rel" ]] || continue; cmp "$ROOT/handoff_payload_26613_v1_1/$rel" "$AFTER/$rel" || fail "sealed payload differs frozen candidate $rel"; done < "$CHANGED"
...
PY
  set_report "POST-BUILD INVARIANCE" "PASS (candidate/protected/DNG/native/vendor exact)"; set_compiler "POST-BUILD INVARIANCE" "PASS"; tar --sort=name --mtime='UTC 1970-01-01' --owner=0 --group=0 --numeric-owner -cf - -C "$POST" app | gzip -n > "$OUT/26613_V1_1_candidate_app_source.tar.gz"; sha256sum "$OUT/26613_V1_1_candidate_app_source.tar.gz" > "$OUT/26613_V1_1_candidate_app_source.tar.gz.sha256"; cp "$CAND_FULL" "$OUT/26613_V1_1_candidate_full_app.sha256"; cp "$CAND_NATIVE" "$OUT/26613_V1_1_native_protected_postbuild.sha256"; cp "$CAND_VENDOR" "$OUT/26613_V1_1_vendor_protected_postbuild.sha256"; set_report "CLEAN ARTIFACT SOURCE EXPORT" "PASS (deterministic candidate tar + manifests)"
}
verify_package
verify_scope
obtain_authority
