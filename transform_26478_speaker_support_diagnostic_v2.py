...
# IRIS_26478_SPEAKER_SUPPORT_DIAGNOSTIC_ANCHOR_V2
#
# 26477 already has IRIS_26468_PROCESSING_SEAM_DIAGNOSTIC between
# IRIS_26436_V2_SPATIAL_SUPPORT and IRIS_26413_V2_CFA_RECONSTRUCTION.
# V1 incorrectly required those two logs to be adjacent.
#
# Insert immediately before the unique IRIS_26413 log instead. The same
# in-scope SupportSummary `summary` and `supportGrid12x8` are passed through
# unchanged, so this remains correlation-only telemetry on the existing
# IRIS_26436 support map.
for required_existing in [
    "IRIS_26436_V2_SPATIAL_SUPPORT",
    "IRIS_26468_PROCESSING_SEAM_DIAGNOSTIC",
    "IRIS_26413_V2_CFA_RECONSTRUCTION",
]:
    if required_existing not in t:
        raise SystemExit(
            "26478 speaker diagnostic V2 prerequisite missing: "
            + required_existing)

call_anchor = '''            Log.d(TAG, "IRIS_26413_V2_CFA_RECONSTRUCTION"
'''
call_new = '''            if (directBayer && output != null) {
                /*
...
if t.count(call_anchor) != 1:
    raise SystemExit(
        "26478 speaker diagnostic V2 reconstruction-log anchor count="
...
print("26478 speaker diagnostic V2 candidate/source validation PASS")
print("26478 same-local-support-map correlation V2 PASS")
