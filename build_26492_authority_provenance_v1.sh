...
BACKUP_BRANCH="backup-26491-v4-tested-before-26492-provenance-authority-contract"
...
TRANSFORM_26492="transform_26492_authority_provenance_v1.py"
TRANSFORM_26492_SHA="43edbd89b8cc82a2982b58e1ca0a50165fbf74b43b1988f48c8efd0b1968cfa5"
NEW_VERSION="0.9726492"
NEW_BUILD="26492"
OUTDIR="build_26492_outputs"
APK_NAME="IrisCamera-${NEW_VERSION}-${NEW_BUILD}-authority-provenance-debug.apk"
...
echo "=== 26492 AUTHORITY / PROVENANCE CONTRACT BUILD ==="
date -Iseconds || true
...
# GATE 1: required pre-change backup must point exactly at tested 26491.
remote="$(git ls-remote origin "refs/heads/$BACKUP_BRANCH" | awk '{print $1}')"
[[ "$remote" == "$TESTED_26491_HEAD" ]] || fail "backup $BACKUP_BRANCH=$remote expected=$TESTED_26491_HEAD"
...
# Bridge continuity.
for text,tok in ((mer,'highlightProvenance'),(rec,'highlightProvenanceOutput'),(hdr,'motionV2HighlightProvenance'),(post,'motionV2HighlightProvenanceTexture'),(inp,'UNSIGNED_8, 1'),(rcdh,'HighlightProvenance')):
...
for tok in ('uniform highp usampler2D HighlightProvenance','IRIS_26492_CENSORED_PACKED_NEUTRAL_LOWER_BOUND','IRIS_26492_RCD_PROVENANCE_CONSUMER_ONLY','state != PROVENANCE_CENSORED'):
...
if 'layout(r8ui' not in prov or 'CENSORED_UNKNOWN_CHROMA' not in prov: raise SystemExit('R8UI provenance init contract missing')
...
26492 Authority / Provenance Contract
...
- one R8UI packed-CFA highlight provenance carrier
...
sha256sum "$TRANSFORM_26492" "$PREPATCH" "$DELTAPATCH" "$AFTERHASH" "$REPORT" "$REPO/$APK_NAME" > "$OUTDIR/26492_artifact_hashes.sha256"
cp "$TRANSFORM_26492" "$OUTDIR/next_baseline_inputs/"
cp "$REPO/$TRANSFORM_26491" "$OUTDIR/next_baseline_inputs/"

echo "=== 26492 BUILD SUCCESS ==="
cat "$REPORT"
