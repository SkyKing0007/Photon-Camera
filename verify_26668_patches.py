...
if len(sys.argv)!=4:
    raise SystemExit('usage: verify_26668_patches.py ROOT BASE CANDIDATE')
...
def make(first,second):
    with tempfile.TemporaryDirectory(prefix='iris26668_patch_') as td:
        t=Path(td)
        run(['git','init','-q'],t)
        run(['git','config','user.email','iris@example.invalid'],t)
        run(['git','config','user.name','Iris Handoff'],t)
        shutil.copytree(first/'app',t/'app')
        run(['git','add','app'],t)
        ...
        # IRIS_26668_ADDED_FILE_ROLLBACK_PATCH_REGRESSION:
        # expose only genuinely untracked additions. `git add -N app` against the whole tree can
        # stage deletion intent and hide added-file deletions from a reverse/rollback patch.
...
fpath=root/'R1_26668_RUNTIME_DELTA_FROM_26667.patch'
rpath=root/'R1_26668_RUNTIME_ROLLBACK_TO_26667.patch'
assert forward==fpath.read_bytes(),'forward patch not canonical'
assert rollback==rpath.read_bytes(),'rollback patch not canonical'
assert forward.count(b'new file mode 100644')==4,forward.count(b'new file mode 100644')
assert rollback.count(b'deleted file mode 100644')==4,rollback.count(b'deleted file mode 100644')
for label,start,patch,target in [('forward',base,fpath,cand),('rollback',cand,rpath,base)]:
    with tempfile.TemporaryDirectory(prefix='iris26668_apply_') as td:
...
print('PASS 26668 deterministic full-index patches core.abbrev=7/12/40; fuzz=0 exact forward/rollback; 4 added files explicitly deleted by rollback')
