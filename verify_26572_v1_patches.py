...
CHANGED=['app/src/main/cpp/motionv2_jpeg444_jni.cpp','app/src/main/java/com/hinnka/mycamera/processor/GlesMgcRawSabreShaders.kt','app/src/main/java/com/hinnka/mycamera/processor/GlesMgcRawSpatialStacker.kt','app/src/main/java/com/particlesdevs/photoncamera/processing/processor/PhotonMotionMgc1271Bridge.kt','app/version.properties']
...
def allhash(r):return {str(p.relative_to(r)):sha(p) for p in sorted((r/'app').rglob('*')) if p.is_file()}
def init(td,src):
...
def regen(start,target,abbrev):
 td=Path(tempfile.mkdtemp(prefix='p26572_'))
 try:
  init(td,start)
...
def applyprove(start,target,patch):
 td=Path(tempfile.mkdtemp(prefix='a26572_'))
 try:
...
def main():
 ...
 print('PASS FORWARD PATCH FUZZ=0 exact 26572 candidate recreation')
 print('PASS ROLLBACK PATCH FUZZ=0 exact 26571 recreation')
