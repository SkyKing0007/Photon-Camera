...
def main():
 ...
 specs=[
  ('true2xMerge26564','true2x_cross_frame_merge_26573',['IRIS_26573_CROSS_FRAME_LUMA_MOMENTS','oTemporalLumaStats','temporalWeight * temporalWeight']),
  ('true2xGuideRender26568','true2x_temporal_detail_render_26573',['IRIS_26573_CROSS_FRAME_TRUE_DETAIL_LUMA_OWNER','IRIS_26573_BLOCK_WIDE_TEMPORAL_PROOF','IRIS_26573_REQUIRED_SR_PROOF_DIAGNOSTIC','phaseGate * temporalGate * safetyGate']),
 ]
 rows=[];manifest=[]
 for kotlin_name,out_name,tokens in specs:
  src=extract_kotlin(kt,kotlin_name)
  if '$' in src:fail('unresolved Kotlin interpolation in '+kotlin_name)
  for token in tokens:
   if token not in src:fail(kotlin_name+' contract '+token)
  target=out/(out_name+'.frag');target.write_text(src)
  n=scan(out_name,src);h=hashlib.sha256(target.read_bytes()).hexdigest();status='NOT RUN'
  if ns.glslang:
   r=subprocess.run([ns.glslang,'-S','frag',str(target)],text=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
   if r.returncode:print(r.stdout);fail('glslang compile '+out_name)
   status='PASS'
  manifest.append(f'{h}  {target.name}\n')
  rows.append({'shader':out_name,'source_property':kotlin_name,'sha256':h,'declared_identifiers':n,'real_glslang':status})
 (out/'V1_26573_RUNTIME_EXPANDED_SHADERS.sha256').write_text(''.join(manifest))
 (out/'V1_26573_SHADER_VERIFICATION.json').write_text(json.dumps(rows,indent=2)+'\n')
 print('PASS exact modified runtime-expanded shader extraction + complete reserved scan variants=2')
