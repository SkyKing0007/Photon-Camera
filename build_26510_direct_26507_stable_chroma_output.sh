...
ROOT="$(pwd)"
OUT="$ROOT/build_26510_direct_26507_stable_chroma_output_outputs"
WORK="$ROOT/.build_26510_direct_26507_stable_chroma_output_work"
BASE="$WORK/successful26507"
AFTER="$WORK/candidate26510"
EXPECTED_BRANCH="experimental-clean-photon-rebuild"
REJECTED_26509_V4_HEAD="828f1ccbfa6894daf8846060abaaaca33681d252"
BACKUP_BRANCH="backup-26509-v4-rejected-before-26510-20260819"
...
DELTA_PATCH="$ROOT/26510_EXACT_DELTA_FROM_SUCCESSFUL_26507.patch"
DELTA_PATCH_SHA="b05c3092350c98569fff6e274a4f80c860409ea8b160c2a2a400d324b30806e3"
VALIDATOR="$ROOT/validate_26510_stable_chroma_output.py"
...
VERSION_NAME="0.9726510"
VERSION_BUILD="26510"
FINAL="$ROOT/IrisCamera-${VERSION_NAME}-${VERSION_BUILD}-stable-chroma-output-debug.apk"
rm -rf "$OUT" "$WORK"; mkdir -p "$OUT" "$BASE" "$AFTER"
exec > >(tee "$OUT/26510_build.log") 2>&1
...
git merge-base --is-ancestor "$REJECTED_26509_V4_HEAD" HEAD || fail "current handoff is not descended from rejected 26509 V4 checkpoint"
REMOTE_BACKUP="$(git ls-remote origin "refs/heads/$BACKUP_BRANCH" | awk '{print $1}')"
[[ "$REMOTE_BACKUP" == "$REJECTED_26509_V4_HEAD" ]] || fail "backup missing/wrong: $BACKUP_BRANCH -> ${REMOTE_BACKUP:-MISSING}"
PROTECTED_DRIFT="$OUT/protected_build_infrastructure_drift.txt"
git diff --name-only "$REJECTED_26509_V4_HEAD"..HEAD -- \
...
[[ "$(sha "$DELTA_PATCH")" == "$DELTA_PATCH_SHA" ]] || fail "26510 exact delta patch hash mismatch"
[[ "$(tr -d '\r\n' < "$BJZHOU_COMMIT_FILE")" == "$BJZHOU_HEAD" ]] || fail "26507 bjzhou dependency commit proof mismatch"
python3 -m py_compile "$VALIDATOR"
pass "GATE 1 backup exact + direct successful-26507 identities"
...
cp "$SOURCE_TAR" "$OUT/26510_PRECHANGE_EXACT_SUCCESSFUL_26507_SOURCE.tar.gz"
sha256sum "$OUT/26510_PRECHANGE_EXACT_SUCCESSFUL_26507_SOURCE.tar.gz" > "$OUT/26510_PRECHANGE_EXACT_SUCCESSFUL_26507_SOURCE.tar.gz.sha256"
pass "GATE 2 direct successful-26507 source verified; no historical runtime replay"

echo "=== GATE 3: apply exact seven-path 26510 delta once ==="
cp -a "$BASE/app" "$AFTER/app"
patch -s -d "$AFTER" -p1 < "$DELTA_PATCH"
python3 "$VALIDATOR" "$AFTER" --base-root "$BASE" | tee "$OUT/26510_validate.txt"
...
# Save a deterministic post-patch source manifest before version change.
( cd "$AFTER" && { find app/src/main -type f -print; echo app/version.properties; } | LC_ALL=C sort | while read -r f; do sha256sum "$f"; done ) > "$OUT/26510_prebuild_candidate_source.sha256"
...
java -cp "$WORK" ParseJava \
  ...
  "$JAVA_ROOT/processing/ultrahdr/MotionV2Jpeg444Encoder.java" | tee "$OUT/26510_java_parse.txt"

python3 - "$AFTER" <<'PYLEX'
from pathlib import Path
import re,sys
root=Path(sys.argv[1])/'app/src/main/assets/shaders/motionv2'
names=['mfsr_bjzhou_rejection_base.glsl','mfsr_spatial_rgb_contribute_26501.glsl','mfsr_spatial_rgb_vgn_chroma_26510.glsl']
...
print('PASS: GLSL reserved-identifier/delimiter preflight for three changed/new shaders')
PYLEX
pass "GATE 3 exact seven-path delta + Java/GLSL static preflight"

echo "=== GATE 4: pinned native dependencies + glslang 16.5.0 + host/shader contracts ==="
...
( cd "$THIRD" && sha256sum -c "$BJZHOU_MANIFEST" ) > "$OUT/26510_bjzhou_native_manifest_check.txt"
echo "$BJZHOU_HEAD" > "$OUT/26510_bjzhou_dependency_commit.txt"
...
items=[('mfsr_bjzhou_rejection_base.glsl','comp'),('mfsr_spatial_rgb_contribute_26501.glsl','frag'),('mfsr_spatial_rgb_vgn_chroma_26510.glsl','frag')]
...
print('PASS: all three changed/new shaders compile with glslang 16.5.0')
PYGLSL

python3 - "$AFTER" <<'PYBIND'
from pathlib import Path
import sys
r=Path(sys.argv[1]); h=(r/'app/src/main/java/com/particlesdevs/photoncamera/processing/processor/MotionV2CfaReconstruction.java').read_text()
v=(r/'app/src/main/assets/shaders/motionv2/mfsr_spatial_rgb_vgn_chroma_26510.glsl').read_text()
assert 'uniform highp sampler2D cameraRgb;' in v
assert 'uniform float wbR;' in v and 'uniform float wbB;' in v
assert 'setTexture("cameraRgb",iris26501RgbOutput)' in h
assert 'setVar("wbR",wronskiGlobalWbR)' in h and 'setVar("wbB",wronskiGlobalWbB)' in h
assert 'iris26480ReadbackOutput = iris26510VgnChromaOutput;' in h
rej=(r/'app/src/main/assets/shaders/motionv2/mfsr_bjzhou_rejection_base.glsl').read_text()
assert 'mirrorUv(' not in rej and 'IRIS_26510_PHYSICAL_MGC_WARP_BOUNDARY' in rej
contrib=(r/'app/src/main/assets/shaders/motionv2/mfsr_spatial_rgb_contribute_26501.glsl').read_text()
assert 'phaseClamp(' not in contrib and 'IRIS_26510_PHYSICAL_BORDER_EVIDENCE_OWNER' in contrib
print('PASS: 26510 host/shader binding + physical-border interface proof')
PYBIND
...
pass "GATE 4 pinned native + shader compiler + non-regression/interface proof"
...
PYVER
# Overlay only the audited direct 26507->26510 candidate. No 26508/26509 runtime is copied.
rm -rf app/src/main
cp -a "$AFTER/app/src/main" app/src/main
cp "$AFTER/app/version.properties" app/version.properties

# Exact build-generated dependency authority inherited from the successful 26507/V4 proof model.
...
audited_runtime_manifest > "$OUT/26510_pre_gradle_audited_runtime.sha256"
chmod +x ./gradlew
...
) > "$OUT/26510_post_gradle_generated_cpp_deps.sha256"
audited_runtime_manifest > "$OUT/26510_post_gradle_audited_runtime.sha256"
if ! cmp -s "$OUT/26510_pre_gradle_audited_runtime.sha256" "$OUT/26510_post_gradle_audited_runtime.sha256"; then
  diff -u "$OUT/26510_pre_gradle_audited_runtime.sha256" "$OUT/26510_post_gradle_audited_runtime.sha256" | tee "$OUT/26510_gradle_runtime_source_diff.txt" >&2 || true
  fail "Gradle mutated audited Photon runtime source; see 26510_gradle_runtime_source_diff.txt"
fi
...
with zipfile.ZipFile(apk) as z:
 ...
 runtime=[b'IRIS_26510_VGN_CHROMA_ONLY_APPLIED',b'IRIS_26510_CAPTURE_RELEASE_BEFORE_JPEGR_SAVE',b'IRIS_26510_ASYNC_JPEGR_OUTPUT_COMPLETE',b'IRIS_26510_JPEG444_TIMING',b'IRIS_26507_JPEG444']
 ...
 shaders=['mfsr_bjzhou_rejection_base.glsl','mfsr_spatial_rgb_contribute_26501.glsl','mfsr_spatial_rgb_vgn_chroma_26510.glsl','mfsr_flow_expand.glsl','mfsr_spatial_rgb_normalize_26501.glsl','mfsr_spatial_rgb_short_weight_26501.glsl','short_highlight_bayer_recover.glsl','shadow_aux_bayer_fuse.glsl']
 for n in shaders:
...
print('PASS: APK typed proof: 26510 DEX markers + exact packaged shaders + libmotionv2jpeg.so')
PYAPK
sha256sum "$FINAL" > "$OUT/26510_APK.sha256"

# Emit the actual successful 26510 source for the next incremental build.
rm -rf "$AFTER/app/src/main/cpp/third_party_26507"
( cd "$AFTER" && tar --sort=name --mtime='UTC 2026-08-19 00:00:00' --owner=0 --group=0 --numeric-owner -czf "$OUT/26510_successful_app_source.tar.gz" app/src/main app/version.properties )
( cd "$AFTER" && { find app/src/main -type f -print; echo app/version.properties; } | LC_ALL=C sort | while read -r f; do sha256sum "$f"; done ) > "$OUT/26510_successful_source.sha256"
pass "GATE 5 exactly one 26510 APK built from direct successful-26507 source"
...
pass "26510 stable chroma/output correction complete: 26507 AE/Wronski/HDR preserved + physical borders + VGN-derived chroma-only + async JPEG_R"
