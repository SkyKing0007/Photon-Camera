...
import hashlib, sys
if len(sys.argv)<3: raise SystemExit('usage: verify_26628_r1_infrastructure.py BUILD WORKFLOW [--successful-26627-build P --successful-26627-workflow P --successful-26627-transform P]')
bp=Path(sys.argv[1]); wp=Path(sys.argv[2]); b=bp.read_text(); w=wp.read_text(); root=bp.parent
def sha(p): return hashlib.sha256(Path(p).read_bytes()).hexdigest()
for t in [
'EXPECTED_BRANCH="experimental-clean-photon-rebuild"','RUNTIME_AUTHORITY_COMMIT="77ad63a80827ac103714be303678be1b47ad30d4"','HANDOFF_PARENT_COMMIT="77ad63a80827ac103714be303678be1b47ad30d4"',
...
'GLSLANG_ARCHIVE_SHA="b9b1f96acb898a62251b171f7695efcecfc206a530299054071919b06820f657"','runtime allowlist must be 9','1704','802','778','7 DNG','6 modified runtime-expanded variants',
...
for required in ["'26628_R1_README_UPLOAD.txt'","'REGRESSION_R1_26628_*'","'R1_26628_*'","'build_26628_r1_bjzhou_color_motion_night_superres.sh'","'transform_26628_r1.py'","'validate_26628_r1.py'","'verify_26628_r1_*.py'","'handoff_payload_26628_r1/**'","'.github/workflows/build-26628-r1-bjzhou-color-motion-night-superres.yml'"]:
...
for tok in order:
    ...
    pos=n
t=(root/'transform_26628_r1.py').read_text()
        ...
        if sseq:
            ...
            for tok in sseq:
                if tok.startswith('python3 -S "$SHADERVERIFY"') or tok.startswith('rm -rf "$ROOT/app/src"') or tok.startswith('snapshot_candidate_from_authority') or tok.startswith('./gradlew') or tok in ['verify_candidate_patches','PRE-BUILD SAFETY PROOF PASSED','expected exactly one Gradle debug APK','POST-BUILD INVARIANCE','candidate_app_source.tar.gz']:
                    ...
                    pp=nn
print('PASS 26628 infrastructure: exact successful-26627 direct ordering/isolation/compiler/NDK/patch/PRE-BUILD/assemble/postbuild mechanics preserved; authority/identity/9-path color validators only')
