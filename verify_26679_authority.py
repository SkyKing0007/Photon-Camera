...
assert b==m(root/'R1_26679_BASE_26678_FULL_APP.sha256');assert c==m(root/'R1_26679_EXPECTED_CANDIDATE_FULL_APP.sha256')
changed=[x for x in (root/'R1_26679_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x]
...
for f,n in [('PROTECTED_UNCHANGED',1722),('NATIVE_PROTECTED',805),('VENDOR_PROTECTED',778),('DNG',7)]:
 x=m(root/f'R1_26679_{f}_BASE.sha256');y=m(root/f'R1_26679_{f}_CANDIDATE.sha256');assert len(x)==len(y)==n and x==y
