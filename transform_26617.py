...
if len(sys.argv)!=3: raise SystemExit('usage: transform_26617.py BASE OUT')
...
def verify(root,manifest):
    ...
    return len(lines)
if verify(base,'R1_26617_PREWRITE_SOURCE_HASHES.sha256')!=7: raise SystemExit('prewrite count')
patch=P/'R1_26617_RUNTIME_DELTA_FROM_26616_R1.patch'
# Inherit exact successful 26616 nested-candidate isolation: never discover parent .git.
...
if verify(out,'R1_26617_EXPECTED_CHANGED_SOURCE_HASHES.sha256')!=7: raise SystemExit('candidate changed count')
print('PASS 26617 deterministic candidate reconstruction from exact successful 26616 R1.2 + canonical full-index patch')
