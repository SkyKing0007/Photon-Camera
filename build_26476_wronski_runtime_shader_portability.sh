...
PROTECTED_HEAD="f6ec8a90f392a9c2374a2496b7b6b0c206750450"
...
python3 - "$HASH_BEFORE" "$HASH_AFTER" <<'PY_HASH'
from pathlib import Path
import sys
...
# BEFORE is repository base, AFTER is full replayed 26475+26476 ephemeral source,
# so only enforce that no new 26476 change exists outside known lineage-generated
# source set by comparing against a reconstructed 26475 snapshot below.
print("hash files readable PASS")
PY_HASH

# Snapshot exact 26475 state by rerunning precursor in an isolated worktree-like copy
# is expensive; instead compare current source against candidate patch scope:
...
# Hard semantic source checks for the narrow 26476 delta.
python3 - "$SOURCEPATCH" <<'PY_SCOPE'
from pathlib import Path
import sys,re
p=Path(sys.argv[1]).read_text()
# New 26476 marker must occur exactly once.
if p.count('IRIS_26476_ADRENO_RGBA32F_GRADIENT_CARRIER_RG_ONLY') != 1:
    raise SystemExit("26476 marker scope mismatch")
print("26476 narrow source scope PASS")
PY_SCOPE
