...
import sys
if len(sys.argv)<3: raise SystemExit('usage: verify_26614_r1_infrastructure.py BUILD WORKFLOW [--success-26613-v1-1-script P --success-26613-v1-1-workflow P]')
...
assert 'IrisCamera-${VERSION_NAME}-${VERSION_BUILD}-r1-canonical-appearance-cfa-validity-debug.apk' in b
assert 'IrisCamera-0.9726614-26614-r1-canonical-appearance-cfa-validity-debug.apk' in w
...
for required in ["'R1_26614_*'","'verify_26614_r1_*.py'","'handoff_payload_26614_r1/**'","'build_26614_r1_canonical_appearance_cfa_validity.sh'"]:
    assert required in push_block

# R1 exact failure regression: transform must be isolated from the checked-out parent repository.
t=Path(Path(sys.argv[1]).parent/'transform_26614_r1.py').read_text()
for token in ["GIT_CEILING_DIRECTORIES","git','rev-parse','--show-toplevel","candidate transform unexpectedly discovered a parent Git worktree"]:
    assert token in t, f'missing nested-worktree transform isolation: {token}'
assert "subprocess.run(['git','apply','--check',str(patch)],cwd=out,check=True)" not in t
...
print('PASS 26614 R1 infrastructure: exact successful 26613 V1.1 compiler/NDK/patch/PRE-BUILD/assemble/postbuild order and pins preserved; nested-worktree transform isolation fixed; 26614-R1-only trigger paths; no backup')
