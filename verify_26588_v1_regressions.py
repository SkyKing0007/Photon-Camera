...
def main():
 ...
 # Exact failed-26587 Kotlin compiler condition is permanent: mergedFrameCount is Sabre-local and must never leak into generic processFrames.
 generic=stack[stack.index('fun processFrames('):stack.index('private fun processSabreFrames(')]
 req('frameCount = frames.size' in generic,'generic spatial kernel frame count must remain frames.size')
 req('frameCount = mergedFrameCount' not in stack,'exact failed 26587 compiler token survived')
 ...
 req('VERSION_NAME=0.9726588' in (c/'app/version.properties').read_text() and 'VERSION_BUILD=26588' in (c/'app/version.properties').read_text(),'version')
 print('PASS exact failed-26587 Kotlin mergedFrameCount scope regression + carried successful-26586 viewfinder/final-highlight authority and adaptive-color/render/Jin/Night owners')
