    ...
    public void Run() {
        ...
        try {
            ...
            if (iris26550Night) {
                ...
                /*
                 * IRIS_26661_GOOGLE_REFERENCE_PRESENTATION_COMPENSATION
                 *
                 * Keep the 26660 viewfinder-response solve, then undo only the deliberate
                 * acquisition underexposure stamped on the selected ZSL reference. LONG never
                 * supplies this scalar: its job remains extra shadow photons/SNR inside Sabre.
                 */
                matchStrengthPercent = MOTION_FIXED_MATCH_STRENGTH_PERCENT;
                float matchStrength = matchStrengthPercent / 100.0f;
                float referenceProtectionEv = clamp(
                        basePipeline.mParameters.motionV2ReferenceProtectionEv,
                        0.0f, 1.50f);
                /* The displayed preview already undoes the full capture protection. Because the
                 * raw viewfinder solver applies only matchStrength of the preview/candidate delta,
                 * it automatically restores matchStrength * protectionEv. Add only the residual
                 * so capture underexposure is compensated exactly once end-to-end. */
                float referenceResidualEv = referenceProtectionEv * (1.0f - matchStrength);
                solvedEv = clamp(
                        rawSolvedEv * matchStrength + referenceResidualEv,
                        MIN_EV, MAX_EV);
            }
            ...
            Log.i(Name, "IRIS_26519_VIEWFINDER_MATCH"
                    ...
                    + " referenceProtectionCompensatedInPresentation=" + (!iris26550Night)
