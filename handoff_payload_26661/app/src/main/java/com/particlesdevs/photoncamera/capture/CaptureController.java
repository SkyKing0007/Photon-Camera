    ...
    private final CameraCaptureSession.CaptureCallback mCaptureCallback = new CameraCaptureSession.CaptureCallback() {
        ...
        public void onCaptureCompleted(@NonNull CameraCaptureSession session,
            ...
            if (isZslMode()) {
                 ...
                 */
                updateMotion26661GoogleReferenceExposureAuthority(result);
            }
...
//    }
     ...
     * Sensor underexposure protects the RAW ZSL group, but the live SurfaceTexture is a display
     * product. Expose the positive magnitude to the preview renderer so it can undo only that
     * deliberate capture bias in display space. The RAW stream itself remains untouched.
     */
    public float getMotion26661ReferencePreviewProtectionEv() {
        return isZslMode()
                ? Math.max(0.0f, Math.min(MOTION_26661_MAX_PROTECTION_EV,
     ...
     * IRIS_26661_GOOGLE_HDR_REFERENCE_EXPOSURE_OWNER
     *
     * This is intentionally not the retired 26412/26368 positive-EV loop. It can only move the
     * Motion reference darker than the user's/HAL baseline and it never disables AE or writes
     * shutter/ISO directly. The long bracket remains the sole extra-photon shadow observation.
     */
    private void updateMotion26661GoogleReferenceExposureAuthority(
            @NonNull TotalCaptureResult result) {
        ...
        if (!mMotion26661ReferenceBaseReady) {
            ...
            if (currentBuilderSteps != expectedBuilderSteps) {
                /*
                 * A newly-created preview request or an external/user compensation owner wins.
                 * Rebase instead of dragging a stale Motion offset into the new request.
                 */
                mMotion26661ReferenceBaseSteps = Math.max(
                        aeRange.getLower(), Math.min(aeRange.getUpper(), currentBuilderSteps));
                mMotion26661ReferenceAppliedProtectionSteps = 0;
        ...
        boolean rawFresh = rawAgeNs <= 180_000_000L
                ...
                && Float.isFinite(mMotion26608StructuredPeakSecond);
        if (!rawFresh) return;
        ...
        float measuredGuide = Math.max(
                Math.max(0.0f, mMotion26608RawP995),
                Math.max(0.0f, mMotion26608StructuredPeakSecond));
        float unbiasedGuide = measuredGuide
        ...
        long highlightAgeNs = mMotion26496RecentHighlightEvidenceTimestampNs <= 0L
                ? Long.MAX_VALUE
                : Math.abs(sensorTs - mMotion26496RecentHighlightEvidenceTimestampNs);
        boolean coherentClip = mMotion26496RawCoherentHighlightCells > 0
                || highlightAgeNs <= MOTION_26496_HIGHLIGHT_EVIDENCE_HOLD_NS;
        boolean broadClip = mMotion26380RawHighlightFraction
                >= MOTION_26478_HIGHLIGHT_FRACTION_TRIGGER;
        boolean meaningfulClip = broadClip || (coherentClip && hdrConflict);

        float requestedProtectionEv = 0.0f;
        if (hdrConflict || meaningfulClip) {
            ...
            if (hdrConflict && unbiasedGuide >= 0.45f) {
        ...
        /*
         * Bound each visible preview change to at most two Camera2 AE steps. The target may be
         * stronger, but it is approached monotonically across subsequent stable result groups.
         */
        ...
        try {
            ...
            Log.i(TAG, "IRIS_26661_GOOGLE_REFERENCE_AE"
                    ...
                    + " oldPositiveAeLoopsDormant=true"
                    + " postShutterHighlightShort=false");
        } catch (IllegalArgumentException | IllegalStateException e) {
            Log.w(TAG, "IRIS_26661 reference AE protection skipped: "
