...
if len(sys.argv)!=3: raise SystemExit('usage: transform_26629_r1.py BASE OUT')
...
def verify(root, manifest, expected):
    ...
    return len(lines)
verify(base,'R1_26629_PREWRITE_SOURCE_HASHES.sha256',5)
added=[x.strip() for x in (P/'R1_26629_ADDED_PATHS_MUST_BE_ABSENT.txt').read_text().splitlines() if x.strip()]
if added: raise SystemExit('26629 adds no runtime paths')
patch=P/'R1_26629_RUNTIME_DELTA_FROM_26628_R3.patch'
...
verify(out,'R1_26629_EXPECTED_CHANGED_SOURCE_HASHES.sha256',5)
print('PASS 26629 deterministic candidate reconstruction from exact successful 26628 R3 + canonical full-index patch')
