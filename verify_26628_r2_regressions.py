...
if len(sys.argv)!=3: raise SystemExit('usage: verify_26628_r2_regressions.py BASE CAND')
...
if 'Button b = new Button(getContext());' in aux: raise SystemExit('FAIL legacy platform Button lens geometry survived')

print('PASS 26628 regressions: generated trees excluded; legacy/reconstruction/pink-edge owners protected; equal-FM Xiaomi valid; common-axis gamut/presentation floors; contraction-only color; full 3D DCP order; true2x profile parity gate; exact NDK y-shadow regression; bounded 3dp lens lift; device-independent centered lens text')
