...
if len(sys.argv)!=3: raise SystemExit('usage: transform_26623_r1.py BASE OUT')
...
def verify(root, manifest):
    ...
    return len(lines)
if verify(base,'R1_26623_PREWRITE_SOURCE_HASHES.sha256')!=7: raise SystemExit('prewrite count')
...
patch=P/'R1_26623_RUNTIME_DELTA_FROM_26622_R1.patch'
# Preserve successful 26622 nested-candidate isolation. The candidate must not discover carrier .git.
...
if verify(out,'R1_26623_EXPECTED_CHANGED_SOURCE_HASHES.sha256')!=7: raise SystemExit('candidate changed count')
print('PASS 26623 deterministic candidate reconstruction from exact successful 26622 R1 + canonical full-index patch')
