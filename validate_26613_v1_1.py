...
if len(sys.argv)!=3: raise SystemExit('usage: validate_26613_v1_1.py BASE CANDIDATE')
B=Path(sys.argv[1]); C=Path(sys.argv[2])
ALLOW=[x.strip() for x in (Path(__file__).parent/'V1_1_26613_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x.strip()]
...
# 26613 V1.1 compiler repair contract: true2x encoder is a different Java package.
rj=(C/'app/src/main/java/com/particlesdevs/photoncamera/processing/opengl/postpipeline/MotionV2Render.java').read_text()
ej=(C/'app/src/main/java/com/particlesdevs/photoncamera/processing/ultrahdr/MotionV2Jpeg444Encoder.java').read_text()
assert 'public static final float IRIS_26613_SDR_KNEE_FINAL = 0.80f;' in rj
assert 'MotionV2Render.IRIS_26613_SDR_KNEE_FINAL' in ej
