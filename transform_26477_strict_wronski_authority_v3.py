...
# The strict-authority candidate introduces two brand-new files. The candidate
# tree contains copied pre-existing paths only, so create these parents before
# write_text(). Infrastructure-only fix; no image math changes.
display_java.parent.mkdir(parents=True, exist_ok=True)
display_glsl.parent.mkdir(parents=True, exist_ok=True)
...
end_marker="""        final MotionV2IpolNoiseCurve.Curve ipolNoiseCurve ="""
if end_marker not in t[start:]:
    raise SystemExit(
        "26477 V2 exact-26476 IPOL noise-curve anchor missing after Photon noise block")
...
if not display_java.parent.is_dir() or not display_glsl.parent.is_dir():
    raise SystemExit("26477 V3 candidate new-file parent directory guard failed")
...
assert "final MotionV2IpolNoiseCurve.Curve ipolNoiseCurve =" in rt
assert "MotionV2IpolNoiseCurve.build(" in rt
assert "final float mfsrSnr = ipolNoiseCurve.snr;" in rt
