...
echo "=== 26472 V5 GUARDED WRONSKI REFERENCE COMPLETION / SDR-AUTHORITATIVE UHDR BUILD ==="
date -Iseconds || true
...
python3 - "$ACCUM" "$REFMERGE" "$FINALIZE" <<'PY_DUAL_EVIDENCE'
from pathlib import Path
import sys
acc=Path(sys.argv[1]).read_text()
ref=Path(sys.argv[2]).read_text()
fin=Path(sys.argv[3]).read_text()

# Exact post-26469/post-26471 auxiliary form; 26470/26471 do not rewrite this block.\nrequired_acc = [
...
for token in required_acc:
    if token not in acc:
        raise SystemExit("FAIL: auxiliary dual-evidence token missing: "+token)
for token in required_ref:
    if token not in ref:
        raise SystemExit("FAIL: reference dual-evidence token missing: "+token)
for token in required_fin:
    if token not in fin:
        raise SystemExit("FAIL: finalizer dual-evidence token missing: "+token)
print("26469 dual-evidence semantics preserved under 26472 reference-last architecture: PASS")
PY_DUAL_EVIDENCE
...
pass "26472 V5 SDR-AUTHORITATIVE UHDR BUILD SUCCESS"
