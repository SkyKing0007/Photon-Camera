...
if len(sys.argv)<5 or sys.argv[3]!='--out': raise SystemExit('usage: verify_26632_r1_shaders.py BASE CAND --out OUT [--compiler PATH]')
...
rels=[
 ('motionv2_gainmap_26632','app/src/main/assets/shaders/motionv2/gainmap.glsl','frag'),
 ('motionv2_render_26632','app/src/main/assets/shaders/motionv2/render.glsl','frag'),
 ('motionv2_local_global_26632','app/src/main/assets/shaders/motionv2/local_laplacian_global_log_26621.glsl','frag')]
mods=[]
for name,rel,stage in rels:
 if (B/rel).read_bytes()==(C/rel).read_bytes(): raise SystemExit(f'FAIL modified shader unexpectedly identical: {rel}')
 mods.append((name,'#version 300 es\n'+(C/rel).read_text(),stage))
...
mods.append(('true2x_publication_compute_26632',compute,'comp'))
...
(O/'R1_26632_RUNTIME_EXPANDED_SHADERS.sha256').write_text('\n'.join(lines)+'\n')
(O/'R1_26632_SHADER_VERIFICATION.json').write_text(json.dumps({'count':len(results),'compiler':compiler,'results':results},indent=2,sort_keys=True)+'\n')
print(f'PASS 26632 complete runtime-expanded GLSL reserved scan variants={len(results)} real_compiler={bool(compiler)}')
