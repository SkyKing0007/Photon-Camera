...
def main():
 ...
 for t in ['BASE_SUCCESS_COMMIT="0782fcff9a30e18bcf4858f6e66d507b3204d51a"','BASE_RUN_ID="34000574868"','BASE_JOB_ID="101398585029"','BASE_ARTIFACT_ID="9979399677"','BASE_ARTIFACT_SHA="0d1eaca37cafe424eb37c0aa31082ab866beff3766f1d48cd361f950be2d86b6"','BASE_TAR_SHA="1dd709dc3a73297fc45a91f658b1ff0514e2f215769b3fa747f662300d7d4410"','MECHANICS_GOLDEN_COMMIT="7c485416a8f41f9bf8a834bf4282e7c2318fa9fb"','GLSLANG_VERSION="16.5.0"','GLSLANG_ARCHIVE_SHA="b9b1f96acb898a62251b171f7695efcecfc206a530299054071919b06820f657"']:
  if t not in s:fail('authority/mechanics pin '+t)
 sig=check_order(s,'26605');blocks=re.findall(r"<<'PY'\n(.*?)\nPY",s,re.S)
 ...
 workflow_shape(w,'26605')
 for t in ['build-26605-v1-root-cause-hdr-flow.yml','photon-26605-v1-root-cause-hdr-flow','Build exact 26605 candidate from successful compiled 26604 authority']:
 ...
 print('PASS infrastructure delta limited to 26605 identity/base/9-file scope/no-backup status/root-cause regressions/33-shader coverage')
