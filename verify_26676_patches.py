...
if len(sys.argv)!=4: raise SystemExit('usage: verify_26676_patches.py ROOT BASE CANDIDATE')
...
def make(first,second):
 with tempfile.TemporaryDirectory(prefix='iris26676_patch_') as td:
...
forward=make(base,cand); rollback=make(cand,base); f=root/'R1_26676_RUNTIME_DELTA_FROM_26675.patch'; r=root/'R1_26676_RUNTIME_ROLLBACK_TO_26675.patch'
assert forward==f.read_bytes(),'forward patch not canonical'; assert rollback==r.read_bytes(),'rollback patch not canonical'
for p in (forward,rollback):
 assert p.count(b'diff --git ')==9 and p.count(b'new file mode ')==1 and p.count(b'deleted file mode ')==1
for label,start,patch,target in [('forward',base,f,cand),('rollback',cand,r,base)]:
 with tempfile.TemporaryDirectory(prefix='iris26676_apply_') as td:
...
print('PASS 26676 deterministic full-index patches core.abbrev=7/12/40; fuzz=0 exact forward/rollback; 7 modifications / 1 addition / 1 deletion')
