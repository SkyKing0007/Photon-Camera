...
vec3 guideReference(ivec2 q){
    ...
    for(int i=0;i<4;i++){
        int c=colorOf(i); float x=s[i]/(c==0?max(wbR,1e-6):(c==2?max(wbB,1e-6):max(wbG,1e-6)));
...
float normalPdf(float x){return 0.3989422804014327*exp(-0.5*x*x);}
float normalCdf(float x){
    // Abramowitz-Stegun 7.1.26, deterministic substitute for public MC expectation.
    float ax=abs(x); float t=1.0/(1.0+0.2316419*ax);
    float poly=t*(0.319381530+t*(-0.356563782+t*(1.781477937+t*(-1.821255978+t*1.330274429))));
    float c=1.0-normalPdf(ax)*poly; return x>=0.0?c:1.0-c;
}
float clippedVariance(float mu,float sig){
    sig=max(sig,1e-7); float a=(0.0-mu)/sig,b=(1.0-mu)/sig;
    float Pa=normalCdf(a),Pb=normalCdf(b),P=Pb-Pa;
    float pha=normalPdf(a),phb=normalPdf(b),phiDiff=pha-phb;
    float tail=1.0-Pb;
    float e1=mu*P+sig*phiDiff+tail;
    float e2=(mu*mu+sig*sig)*P+2.0*mu*sig*phiDiff+sig*sig*(a*pha-b*phb)+tail;
    return max(e2-e1*e1,1e-12);
}
 ...
 * Algorithms 6-9 geometry: 3x3 guide local stats, Dogson biquadratic
 * upsample/warp to physical RAW coordinates, flow-irregularity S, threshold,
 * followed by the separate 5x5 local-min pass. Public MC noise curves are
 * represented by deterministic clipped-Gaussian expected moments from the
 * same alpha/beta noise model (platform-equivalent, no stale Photon noiseS use).
 */
void main(){
    ...
    if(altLR.x<0.0||altLR.y<0.0||altLR.x>=float(rawHalf.x)||altLR.y>=float(rawHalf.y)){
    ...
    const float sampleStdFactor=0.9138749; // E[np.std 9 Gaussian samples, ddof=0] / sigma
    const float diffMeanFactor=0.3761264;  // E|mean9_a-mean9_b| / sigma
    for(int c=0;c<3;c++){
        float b=clamp(refMu[c],0.0,1.0);
        float sig=sqrt(max(noiseO+noiseS*b,1e-12));
        float varClip=clippedVariance(b,sig);
        float sigmaT=sampleStdFactor*sqrt(varClip);
        float dT=diffMeanFactor*sqrt(varClip);
        float dp2=dp[c]*dp[c];
        float shrink=dp2/max(dp2+dT*dT,1e-12);
    ...
    for(int y=-1;y<=1;y++)for(int x=-1;x<=1;x++){
        ivec2 q=clamp(p+ivec2(x,y)*ts,ivec2(0),rawSize-ivec2(1));
        vec2 f=denseRawFlowAt(q); mn=min(mn,f);mx=max(mx,f);
    }
    vec2 span=mx-mn;
    float S=dot(span,span)>0.8*0.8?2.0:12.0;
    float R=clamp(S*exp(-d2/max(sigma2,1e-12))-0.12,0.0,1.0);
