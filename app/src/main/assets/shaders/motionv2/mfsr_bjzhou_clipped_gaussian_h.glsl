...
/* IRIS_26487_BJZHOU_CLIPPED_GAUSSIAN_H_EXACT */
void main(){ivec2 p=ivec2(gl_GlobalInvocationID.xy);if(any(greaterThanEqual(p,size)))return;float f=0.0;for(int t=0;t<20;t++)f+=K[t]*at(p+ivec2(t-9,0));imageStore(outEvidence,p,vec4(min(at(p),f)));}
