...
import hashlib,sys
manifest=Path(sys.argv[1]); root=Path(sys.argv[2]); allowed=set(Path(sys.argv[3]).read_text().splitlines())
# version.properties is intentionally changed to 26481 immediately before the final
# Gradle build. At this post-build stage it is therefore a permitted delta, but
# every other canonical 26480 file must still match exactly.
allowed.add('app/version.properties')
for line in manifest.read_text().splitlines():
    if not line.strip():
        continue
    expected,path=line.split(None,1); path=path.strip()
...
cp "$0" "$OUTDIR/next_baseline_inputs/build_26481_direct_successful_26480_v1_3.sh"
if [[ -f .github/workflows/build-26481-direct-successful-26480-v1_3.yml ]]; then
  cp .github/workflows/build-26481-direct-successful-26480-v1_3.yml "$OUTDIR/next_baseline_inputs/"
fi
