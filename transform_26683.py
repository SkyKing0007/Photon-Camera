...
ROOT=Path(__file__).resolve().parent; PAYLOAD=ROOT/'handoff_payload_26683'
CHANGED=[x.strip() for x in (ROOT/'R1_26683_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x.strip()]
def h(p):return hashlib.sha256(Path(p).read_bytes()).hexdigest()
def snap(r):return {str(p.relative_to(r)):h(p) for p in sorted((Path(r)/'app').rglob('*')) if p.is_file()}
if len(sys.argv)!=3:raise SystemExit('usage: transform_26683.py BASE CANDIDATE')
base=Path(sys.argv[1]);out=Path(sys.argv[2]);ver=(base/'app/version.properties').read_text()
if 'VERSION_NAME=0.9726682' not in ver or 'VERSION_BUILD=26682' not in ver:raise SystemExit('base is not exact successful 26682 version')
...
print('PASS transform 26683: exact successful 26682 authority -> 1764-file frozen candidate; 6 modified / 0 added / 0 deleted')
