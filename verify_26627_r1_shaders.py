...
if len(sys.argv)<5 or sys.argv[3]!='--out': raise SystemExit('usage: verify_26627_r1_shaders.py BASE CAND --out OUT [--compiler PATH]')
...
if (B/rel).read_bytes()==(C/rel).read_bytes(): raise SystemExit('FAIL modified shader unexpectedly identical')
asset=(C/rel).read_text()
mods=[
('adaptive_color_appearance_26563_profileless_26627','#version 300 es\n'+asset,'frag'),
('adaptive_color_appearance_26563_calibrated_26627','#version 300 es\n#define CALIBRATED_PROFILE 1\n'+asset,'frag'),
]
...
(O/'R1_26627_RUNTIME_EXPANDED_SHADERS.sha256').write_text('\n'.join(lines)+'\n')
(O/'R1_26627_SHADER_VERIFICATION.json').write_text(json.dumps({'count':len(results),'compiler':compiler,'results':results},indent=2,sort_keys=True)+'\n')
print(f'PASS 26627 complete runtime-expanded GLSL reserved scan variants={len(results)} real_compiler={bool(compiler)}')
