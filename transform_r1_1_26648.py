...
if len(sys.argv)!=3: raise SystemExit('usage: transform_r1_1_26648.py BASE DEST')
base=Path(sys.argv[1]).resolve(); dest=Path(sys.argv[2]).resolve(); root=Path(__file__).resolve().parent
changed=[x for x in (root/'R1_1_26648_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x]
if len(changed)!=8 or len(set(changed))!=8: raise SystemExit(f'expected 8 unique changed paths, got {len(changed)}')
...
def readm(p):
 ...
 return d
pre=readm(root/'R1_1_26648_PREWRITE_SOURCE_HASHES.sha256'); post=readm(root/'R1_1_26648_EXPECTED_CHANGED_SOURCE_HASHES.sha256')
...
payload=root/'handoff_payload_r1_1_26648'
for rel in changed:
...
print('PASS 26648 deterministic payload transform: exact successful 26646 authority -> 1720-file candidate, 8 changed / 0 added')
