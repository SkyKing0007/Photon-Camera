...
if len(sys.argv)!=4: raise SystemExit('usage: verify_26672_patches.py ROOT BASE CANDIDATE')
...
def make(first,second):
 with tempfile.TemporaryDirectory(prefix='iris26672_patch_') as td:
...
forward=make(base,cand); rollback=make(cand,base); f=root/'R1_26672_RUNTIME_DELTA_FROM_26671.patch'; r=root/'R1_26672_RUNTIME_ROLLBACK_TO_26671.patch'
assert forward==f.read_bytes(),'forward patch not canonical'; assert rollback==r.read_bytes(),'rollback patch not canonical'
assert forward.count(b'diff --git ')==4 and forward.count(b'new file mode 100644')==1 and forward.count(b'deleted file mode 100644')==0
assert rollback.count(b'diff --git ')==4 and rollback.count(b'deleted file mode 100644')==1 and rollback.count(b'new file mode 100644')==0
for label,start,patch,target in [('forward',base,f,cand),('rollback',cand,r,base)]:
 with tempfile.TemporaryDirectory(prefix='iris26672_apply_') as td:
...
# Permanent added-file rollback regression: forward must create and rollback must remove the vector path exactly.
assert b'app/src/main/res/drawable-anydpi/iris_flip_arrows.xml' in forward and b'app/src/main/res/drawable-anydpi/iris_flip_arrows.xml' in rollback
print('PASS 26672 deterministic full-index patches core.abbrev=7/12/40; fuzz=0 exact forward/rollback; 3 modifications / 1 addition / 0 deletions; added-vector rollback removal PASS')
