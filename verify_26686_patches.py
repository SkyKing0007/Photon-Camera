...
if len(sys.argv)!=4: raise SystemExit('usage: verify_26686_patches.py ROOT BASE CAND')
root,base,cand=map(Path,sys.argv[1:]); fwd=(root/'R1_26686_RUNTIME_DELTA_FROM_26685_R1.patch').read_bytes(); rb=(root/'R1_26686_RUNTIME_ROLLBACK_TO_26685_R1.patch').read_bytes()
...
with tempfile.TemporaryDirectory() as td:
    ...
    for rel in (root/'R1_26686_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines():
        if not rel: continue
...
print('PASS 26686 deterministic full-index forward/rollback patches at core.abbrev 7/12/40; fuzz=0 exact forward and rollback')
