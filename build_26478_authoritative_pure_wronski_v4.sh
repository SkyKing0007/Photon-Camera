...
EXPECTED_INFRA_PARENT="62fed09aa61ac963f951fd39b1a62be870d75654"
...
exec > >(tee "$AUDIT") 2>&1
echo "=== 26478 V4 FINAL INFRA CORRECTION AUTHORITATIVE PURE WRONSKI + HIGHLIGHT-SAFE + SPEAKER DIAGNOSTIC ==="
date -Iseconds || true

BRANCH="${GITHUB_REF_NAME:-$(git branch --show-current)}"
[[ "$BRANCH" == "$EXPECTED_BRANCH" ]] || fail "wrong branch: $BRANCH"
[[ "$(git rev-parse HEAD^)" == "$EXPECTED_INFRA_PARENT" ]] || \
  fail "26478 V4 correction commit must be direct child of exact 62fed09 infrastructure HEAD"
[[ "$(git rev-parse HEAD~2)" == "a5fa756f8b5cf1101e807b776728d08225f3e53e" ]] || \
  fail "26478 V4 correction chain missing exact first 26478 infrastructure commit"
[[ "$(git rev-parse HEAD~3)" == "be24c16cb091e753852b900384dbde6f8d32744e" ]] || \
  fail "26478 V4 correction chain must descend exactly from 26477 V3"
pass "branch/head exact three-commit infrastructure lineage gate"
...
cat > "$REPORT" <<EOF
26478 V4 FINAL INFRA CORRECTION AUTHORITATIVE PURE WRONSKI + HIGHLIGHT-SAFE + SPEAKER DIAGNOSTIC
========================================================================
Rollback:
