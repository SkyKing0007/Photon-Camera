...
if len(sys.argv)!=3: raise SystemExit('usage: validate_26628_r2.py BASE CAND')
B=Path(sys.argv[1]); C=Path(sys.argv[2])
CHANGED=[x.strip() for x in (Path(__file__).resolve().parent/'R2_26628_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x.strip()]
...
if 'Button b = new Button(getContext());' in aux:
    raise SystemExit('FAIL platform Button geometry survived lens toggle fix')
