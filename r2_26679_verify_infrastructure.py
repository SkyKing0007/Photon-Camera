...
for x in ['GLSLANG_VERSION="16.5.0"','GLSLANG_ARCHIVE_SHA="b9b1f96acb898a62251b171f7695efcecfc206a530299054071919b06820f657"','expected exactly one Gradle debug APK']:
 assert x in b,x
for x in ['actions/checkout@v5','actions/setup-java@v5','actions/setup-python@v5','actions/upload-artifact@v4','ubuntu-24.04',"java-version: '17'",'build_26679_r2_java_compiler_repair.sh']:
 assert x in w,x
assert "'R2_26679_*'" in w and "'handoff_payload_26679_r2/**'" in w and "'r2_26679_verify_*.py'" in w
# Historical R1 workflow trigger patterns must not match any R2 upload path.
for forbidden in ["'R1_26679_*'","'handoff_payload_26679/**'","'verify_26679_*.py'",'build_26679_r1_shutter_session_recovery.sh']:
 assert forbidden not in w,forbidden
print('PASS 26679 R2 infrastructure: exact successful-26678 compiler/native/patch/PRE-BUILD/assemble/postbuild order retained; only repair identity, upload-parent, runtime scope and compiler regression differ')
