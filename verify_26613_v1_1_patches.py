...
if len(sys.argv)!=5: raise SystemExit('usage: verify_26613_v1_1_patches.py BASE CAND FWD ROLLBACK')
...
with tempfile.TemporaryDirectory() as td:
    ...
    subprocess.run(['git','init','-q'],cwd=repo,check=True); subprocess.run(['git','config','user.name','Photon26613'],cwd=repo,check=True); subprocess.run(['git','config','user.email','photon26613@example.invalid'],cwd=repo,check=True)
    ...
    check=subprocess.run(['git','diff','--check',base,cand],cwd=repo,capture_output=True,text=True)
    ...
    for root,patch,target in [(fw,F,C),(rb,R,B)]:
        ...
        assert H(root)==H(target), f'patch replay mismatch {patch.name}'
print('PASS 26613 deterministic full-index patches core.abbrev=7/12/40; git apply exact forward/rollback; diff --check clean')
