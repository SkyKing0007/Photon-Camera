...
if len(sys.argv)!=5: raise SystemExit('usage: verify_26629_r1_patches.py BASE CAND FORWARD ROLLBACK')
...
expected=sorted(x.strip() for x in (Path(__file__).resolve().parent/'R1_26629_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x.strip())
...
for abbrev in (7,12,40):
    with tempfile.TemporaryDirectory(prefix=f'iris26629_patch_{abbrev}_') as td:
...
print('PASS 26629 deterministic full-index patches core.abbrev=7/12/40; --index exact preimage; diff --check clean; exact forward/rollback')
