...
[[ -f "$V5_VERIFY" ]] || fail "missing V5 source-integrity verifier"
...
def once(old, new, label):
    ...
    n = src.count(old)
    if n != 1:
        raise SystemExit(f"ERROR: V6 transform anchor {label!r} count={n}, expected=1")
...
'OUT="$ROOT/build_26498_v1_3_recovery_v6_outputs"\n'
'rm -rf "$OUT" && mkdir -p "$OUT"\n'
'LOG="$OUT/26498_v1_3_recovery_v6_build.log"',
...
'echo "PASS: 26498 V1.3 RECOVERY V6 BUILD COMPLETE: $FINAL"',
"final marker"
)
...
for forbidden in [
    ...
    if forbidden in src:
        raise SystemExit(f"ERROR: forbidden V4 regression survived V6 transform: {forbidden}")
...
required = [
    ...
    'cd "$CAND"',
    ':app:assembleDebug',
    'RECOVERY V6 BUILD COMPLETE',
    'build_26498_v1_3_recovery_v6_outputs',
]
for token in required:
    if token not in src:
        raise SystemExit(f"ERROR: V6 required token missing after transform: {token}")
...
grep -q 'RECOVERY V6 BUILD COMPLETE' "$EFFECTIVE"
! grep -q 'rm -rf "$ROOT/app"' "$EFFECTIVE"
! grep -q 'Gradle mutated runtime source' "$EFFECTIVE"
! grep -q 'build_26498_v1_3_recovery_v4_outputs' "$EFFECTIVE"

chmod +x "$EFFECTIVE"
"$EFFECTIVE"

cp "$EFFECTIVE" "$ROOT/build_26498_v1_3_recovery_v6_outputs/26498_v1_3_recovery_v6_effective.sh"
sha256sum "$ROOT/build_26498_v1_3_recovery_v6_outputs/26498_v1_3_recovery_v6_effective.sh" \
  > "$ROOT/build_26498_v1_3_recovery_v6_outputs/26498_v1_3_recovery_v6_effective.sh.sha256"
