...
if len(sys.argv)!=5: raise SystemExit('usage: verify_26614_v1_patches.py BASE CAND FWD ROLLBACK')
...
with tempfile.TemporaryDirectory() as td:
    ...
    subprocess.run(['git','init','-q'],cwd=repo,check=True); subprocess.run(['git','config','user.name','Photon26614'],cwd=repo,check=True); subprocess.run(['git','config','user.email','photon26614@example.invalid'],cwd=repo,check=True)
...
print('PASS 26614 deterministic full-index patches core.abbrev=7/12/40; exact forward/rollback; diff --check clean')
