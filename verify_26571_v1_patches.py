...
CHANGED=['app/src/main/cpp/CMakeLists.txt','app/src/main/cpp/motionv2_jpeg444_jni.cpp','app/src/main/java/com/hinnka/mycamera/processor/GlesIris26529SpatialRgbChromaPostprocessor.kt','app/version.properties']
...
def regen(start,target,abbrev):
 td=Path(tempfile.mkdtemp(prefix='p26571_'))
 try:
  init(td,start)
...
def applyprove(start,target,patch):
 td=Path(tempfile.mkdtemp(prefix='a26571_'))
 try:
...
def main():
 ...
 print('PASS FORWARD PATCH FUZZ=0 exact 26571 candidate recreation')
 print('PASS ROLLBACK PATCH FUZZ=0 exact 26570 recreation')
