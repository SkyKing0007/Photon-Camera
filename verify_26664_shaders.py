...
if len(sys.argv) not in (4,6):raise SystemExit('usage: verify_26664_shaders.py ROOT BASE CANDIDATE [--compiler PATH]')
...
def embedded(rootp):
 ...
 return out
bm=readm(root/'R1_26664_SHADER_UNIVERSE_BASE.sha256');cm=readm(root/'R1_26664_SHADER_UNIVERSE_CANDIDATE.sha256');assert len(bm)==len(cm)==257
for r,h in bm.items():assert sha(base/r)==h,r
for r,h in cm.items():assert sha(cand/r)==h,r
assert {r for r in bm if bm[r]!=cm[r]}=={'app/src/main/assets/shaders/motionv2/local_laplacian_remap_26621.glsl','app/src/main/assets/shaders/motionv2/render.glsl'}
def expand_asset(r,defines=None):
 ...
 for val in lines:
  ...
  if '#import' in val:raise SystemExit(f'FAIL 26664 unexpected import {r}: {val}')
  if '#define' in val:
   for k,v in defines.items():
    if f' {k} ' in val:val=f'#define {k} {v}';break
  out.append(val)
 body='\n'.join(out)+'\n';return body if versioned else '#version 310 es\n#line 1\n'+body
mods=embedded(cand);variants={
 ...
 'asset_local_laplacian_remap_26621.frag':expand_asset('app/src/main/assets/shaders/motionv2/local_laplacian_remap_26621.glsl'),
 'asset_gainmap.frag':expand_asset('app/src/main/assets/shaders/motionv2/gainmap.glsl'),
 'asset_preview_main_fs.frag':expand_asset('app/src/main/assets/shaders/preview/main_fs.glsl')}
for hs in (0,1):
...
expected=readm(root/'R1_26664_RUNTIME_EXPANDED_SHADERS.sha256');assert len(expected)==len(variants)==10 and set(expected)==set(variants)
...
with tempfile.TemporaryDirectory(prefix='iris26664_shader_') as td:
 out=Path(td)
 for name,src in sorted(variants.items()):
  ...
  if compiler:
   ...
   if cp.returncode!=0:raise SystemExit(f'26664 GLSL FAIL {name}\n{cp.stdout}\n{cp.stderr}')
print(f'PASS 26664 exact runtime GLSL variants=10; render changed + local remap restored; reserved/structure/exact-hash real_compiler={bool(compiler)} standaloneShaderUniverse=257')
