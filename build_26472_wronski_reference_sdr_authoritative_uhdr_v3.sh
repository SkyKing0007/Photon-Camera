...
echo "=== 26472 V3 GUARDED WRONSKI REFERENCE COMPLETION / SDR-AUTHORITATIVE UHDR BUILD ==="
date -Iseconds || true
...
# 26472 intentionally replaces the 26469 reference initializer with a zero
# accumulator, so the old REFERENCE marker must not be required. Prove the
# dual-evidence semantics directly instead:
# - auxiliary clipped samples stay in radiometric num/den;
# - auxiliary unsaturated chroma support stays separate;
# - reference merge also keeps clipped brightness evidence + valid RGB support;
# - finalizer consumes support.gba as chroma authority.
grep -q 'addNum\[c\]+=w\*cfaSample' "$ACCUM" || fail "26472 aux clipped brightness evidence lost"
grep -q 'addDen\[c\]+=w' "$ACCUM" || fail "26472 aux radiometric denominator lost"
grep -q 'addValidDen\[c\]+=w\*sampleValidity' "$ACCUM" || fail "26472 aux unsaturated support separation lost"
grep -q 'refNum\[c\]+=s\*w' "$REFMERGE" || fail "26472 reference clipped brightness evidence lost"
grep -q 'refDen\[c\]+=w' "$REFMERGE" || fail "26472 reference radiometric denominator lost"
grep -q 'refValid\[c\]+=w\*sampleValidity' "$REFMERGE" || fail "26472 reference unsaturated support separation lost"
grep -q 'support.gba' "$FINALIZE" || fail "26472 final chroma-support authority lost"
pass "26469 dual-evidence semantics preserved under 26472 reference-last architecture PASS"
...
echo "  26469 dual-evidence semantics validated directly (retired initializer marker not required) PASS"
...
pass "26472 V3 SDR-AUTHORITATIVE UHDR BUILD SUCCESS"
