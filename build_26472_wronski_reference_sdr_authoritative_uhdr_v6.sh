...
echo "=== 26472 V6 GUARDED WRONSKI REFERENCE COMPLETION / SDR-AUTHORITATIVE UHDR BUILD ==="
date -Iseconds || true
...
# Proven 26469 dual-evidence lineage is already validated in the temporary
# candidate stage and by the surviving auxiliary marker. Do not add another
# post-apply parser/semantic validator here; 26470/26471-style marker gates only.
grep -q 'IRIS_26469_CENSORED_HIGHLIGHT_DUAL_EVIDENCE_AUX' "$ACCUM" \
  || fail "26469 auxiliary dual-evidence lineage lost"
grep -q 'support.gba' "$FINALIZE" \
  || fail "26469 separate unsaturated-support carrier lost"
pass "26469 dual-evidence lineage preservation PASS"
...
pass "26472 V6 SDR-AUTHORITATIVE UHDR BUILD SUCCESS"
