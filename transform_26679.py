...
def m(p):
 ...
 return d
ch=[x for x in (root/'R1_26679_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x]
adds=[x for x in (root/'R1_26679_ADDED_PATHS_MUST_BE_ABSENT.txt').read_text().splitlines() if x]
dels=[x for x in (root/'R1_26679_DELETED_PATHS_MUST_EXIST.txt').read_text().splitlines() if x]
pre=m(root/'R1_26679_PREWRITE_SOURCE_HASHES.sha256'); exp=m(root/'R1_26679_EXPECTED_CHANGED_SOURCE_HASHES.sha256')
...
shutil.copytree(base,out);payload=root/'handoff_payload_26679'
