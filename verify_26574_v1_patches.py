...
def regen(start,target,abbrev):
 td=Path(tempfile.mkdtemp(prefix='p26574_'))
 try:
  init(td,start)
...
def applyprove(start,target,patch):
 td=Path(tempfile.mkdtemp(prefix='a26574_'))
 try:
...
def main():
 ...
 print('PASS FORWARD PATCH FUZZ=0 exact 26574 candidate recreation')
 print('PASS ROLLBACK PATCH FUZZ=0 exact 26573 recreation')
