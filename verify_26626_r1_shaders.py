...
if len(sys.argv)<5 or sys.argv[3]!='--out': raise SystemExit('usage: verify_26626_r1_shaders.py BASE CAND --out OUT [--compiler PATH]')
...
old=extract_all((B/srel).read_text()); new=extract_all((C/srel).read_text())
added=sorted(set(new)-set(old))
want=['shortAccumulatorDelta26626','shortAccumulatorProxy26626','shortComponentDiagnostics26626']
if added!=want: raise SystemExit(f'FAIL diagnostic shader set {added}')
for n,s in old.items():
    if new.get(n)!=s: raise SystemExit(f'FAIL inherited embedded shader changed: {n}')
asset=(C/'app/src/main/assets/shaders/motionv2/local_laplacian_remap_26621.glsl').read_text()
mods=[('local_laplacian_remap_26621','#version 300 es\n'+asset,'frag')]
mods += [(n,new[n],'frag') for n in want]
...
for name,src,stage in mods:
    ...
    p=O/f'{name}_26626.{stage}'; p.write_text(src); paths.append((p,stage,name))
    status='STATIC_PASS'
    if compiler:
...
(O/'R1_26626_RUNTIME_EXPANDED_SHADERS.sha256').write_text('\n'.join(lines)+'\n')
(O/'R1_26626_SHADER_VERIFICATION.json').write_text(json.dumps({'count':len(results),'compiler':compiler,'results':results},indent=2,sort_keys=True)+'\n')
print(f'PASS 26626 complete runtime-expanded GLSL reserved scan variants={len(results)} real_compiler={bool(compiler)}')
