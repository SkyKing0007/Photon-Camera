...
PROTECTED_HEAD="8aa6d4e71402761672115571203b3dcb099a615b"
...
public final class MotionV2WronskiAlignment {
    ...
    public static MotionV2Alignment.Result align(
            Point rawHalf, int cfaPattern, float signalScale, float snr,
            GLProg glProg, GLTexture referenceCfa, GLTexture alterCfa) {
        ...
        GLTexture[] refTmp = new GLTexture[4];
        GLTexture[] altTmp = new GLTexture[4];
        ...
        try {
            ...
            for (int l=1;l<4;l++) {
                Point srcSize = levelSize[l-1];
                levelSize[l] = divCeil(srcSize, stepFactor[l]);

                refTmp[l] = new GLTexture(
                        srcSize,
                        new GLFormat(GLFormat.DataType.FLOAT_32,1),
                        null, GL_LINEAR, GL_CLAMP_TO_EDGE);
                altTmp[l] = new GLTexture(
                        srcSize,
                        new GLFormat(GLFormat.DataType.FLOAT_32,1),
                        null, GL_LINEAR, GL_CLAMP_TO_EDGE);
                ref[l] = new GLTexture(
                ...
                glProg.useAssetProgram(
                        "motionv2/mfsr_pyramid_gaussian", true);
                glProg.setVar("factor", stepFactor[l]);
                glProg.setVar("direction", 0);
                glProg.setTexture("InputGuide", ref[l-1]);
                glProg.setTextureCompute("OutputGuide", refTmp[l], true);
                glProg.computeAuto(srcSize,1);
                ...
                glProg.setTextureCompute("OutputGuide", altTmp[l], true);
                glProg.computeAuto(srcSize,1);
                ...
                glProg.setTexture("InputGuide", refTmp[l]);
                ...
                glProg.setVar("factor", stepFactor[l]);
                glProg.setVar("direction", 1);
                glProg.setTexture("InputGuide", altTmp[l]);
            ...
            for (int l=3;l>=0;l--) {
                ...
                GLTexture block = new GLTexture(
                        grid, new GLFormat(GLFormat.DataType.FLOAT_16,4),
                        null, GL_NEAREST, GL_CLAMP_TO_EDGE);
                ...
                glProg.setVar(
                        "hasPrevious", previousFlow != null ? 1 : 0);
                glProg.setVar(
                        "previousToCurrentScale",
            ...
            if (previousFlow != null) previousFlow.close();
            for (int i=0;i<4;i++) {
                if (refTmp[i] != null) refTmp[i].close();
                if (altTmp[i] != null) altTmp[i].close();
...
==============================
Protected successful 26472 V6 checkpoint: $PROTECTED_HEAD
