...
EXPECTED_INFRA_PARENT="b74d76bbda3b0464142ba84a46e4e9692ba69fbf"
EXPECTED_APP_BASE="8233415edf738bf35c0fe1c4907f5dfe51de31a4"

BACKUP_BRANCH="backup-26479-v9-before-v10-workflow-object-guard-fix"
BACKUP_TARGET="b74d76bbda3b0464142ba84a46e4e9692ba69fbf"
...
PORT_TRANSFORM="transform_26479_adreno_sample_keyword_portability_v1.py"

NEW_VERSION="0.9726479"
NEW_BUILD="26479"
OUTDIR="build_26479_outputs"
APK_NAME="IrisCamera-${NEW_VERSION}-${NEW_BUILD}-pure-wronski-highlight-safe-speaker-diag-adreno-portability-debug.apk"
...
rm -rf "$OUTDIR"
mkdir -p "$OUTDIR"
AUDIT="$OUTDIR/26479_source_audit.txt"
REPORT="$OUTDIR/26479_build_report.txt"
PREPATCH="$OUTDIR/26477_replay_pre_edit_binary.patch"
RECOVERY="$OUTDIR/26479_recovery_binary.patch"
SOURCEPATCH="$OUTDIR/26479_source.patch"
HASH_26477="$OUTDIR/26477_exact_replay.sha256"
HASH_AFTER="$OUTDIR/26479_after.sha256"

exec > >(tee "$AUDIT") 2>&1
echo "=== 26479 V10 WORKFLOW-OBJECT-GUARD CORRECTION / ADRENO PORTABILITY ==="
date -Iseconds || true

BRANCH="${GITHUB_REF_NAME:-$(git branch --show-current)}"
[[ "$BRANCH" == "$EXPECTED_BRANCH" ]] || fail "wrong branch: $BRANCH"
[[ "$(git rev-parse HEAD^)" == "$EXPECTED_INFRA_PARENT" ]] || \
  fail "26479 V10 commit must be direct child of exact V9 infrastructure HEAD"
[[ "$(git rev-parse HEAD~2)" == "fea487b2c9f6d41d630f78c8e4e1406ac3ccde7b" ]] || \
  fail "26479 V10 chain missing exact V8 infrastructure HEAD"
[[ "$(git rev-parse HEAD~3)" == "5755b1a05777423b5af9f803a57235ac73efdf68" ]] || \
  fail "26479 V10 chain missing exact successful V7 infrastructure HEAD"
[[ "$(git rev-parse HEAD~9)" == "be24c16cb091e753852b900384dbde6f8d32744e" ]] || \
  fail "26479 V10 chain must retain exact successful 26477 V3 ancestor"
pass "branch/head exact V10-on-V9 lineage gate"

REMOTE_BACKUP="$(git ls-remote origin "refs/heads/$BACKUP_BRANCH" | awk '{print $1}')"
[[ "$REMOTE_BACKUP" == "$BACKUP_TARGET" ]] || \
  fail "required pre-V9 backup missing/wrong"
pass "backup branch exact V8 infrastructure checkpoint"
...
[[ -f "$PORT_TRANSFORM" ]] || fail "missing 26479 portability transform"
[[ "$(git hash-object "$CORE_TRANSFORM")" == "3ccd4492fcf23bf5af3ef39f68a142c713faa18c" ]] || fail "26478 core transform blob mismatch"
[[ "$(git hash-object "$DIAG_TRANSFORM")" == "bc9fe5b72189b9b66025d5a12b8f1e6602b9d426" ]] || fail "26478 diagnostic transform blob mismatch"
python3 -m py_compile "$CORE_TRANSFORM"
python3 -m py_compile "$DIAG_TRANSFORM"
python3 -m py_compile "$PORT_TRANSFORM"
pass "26478 replay transforms + 26479 portability transform syntax PASS"
...
EXPECTED_26478_HASHES="$TMP/expected_26478_v7.sha256"
cat > "$EXPECTED_26478_HASHES" <<'EOF_26478_HASHES'
cc8e4735d4203ef1772b26ca84cd527b818e3ca89f3f1b400edf1faaa1002c23  app/src/main/assets/shaders/motionv2/color_transform.glsl
28671f27b8c587c63edce51ebec167f86ea52a136bffb5ff96729d6b0d93ac5f  app/src/main/assets/shaders/motionv2/direct_rgb_accumulate.glsl
90e87585c60bab23e6e12856fd4f2736914459424470142d23a0652c082692f4  app/src/main/assets/shaders/motionv2/direct_rgb_init.glsl
5e9c8b37a797aef7b20e19a37a38deb6c4383ce45b50853f3e2cc4ed90770b58  app/src/main/assets/shaders/motionv2/mfsr_finalize.glsl
e02229aa014354ff85c2a9ceb819668dba9e8959ef489f194ae675de94bee523  app/src/main/assets/shaders/motionv2/mfsr_low_support_reference.glsl
f5b649cd6f1f5d1f15bb2fdc13e21dd3bedc7de3a67cd4d4b7e6884f54f6e790  app/src/main/java/com/particlesdevs/photoncamera/capture/CaptureController.java
6daed015c4cdb6caa2737b900570396e5fac4aafb3fd3704db970b9934595026  app/src/main/java/com/particlesdevs/photoncamera/processing/processor/MotionV2CfaReconstruction.java
a3f1c79a30ac27e007078c19732612ebc2d08f922ec1aeafd8f9cf1e7bad4b4f  app/version.properties
EOF_26478_HASHES
(
  cd "$CAND"
  sha256sum -c "$EXPECTED_26478_HASHES"
) || fail "exact successful V7/26478 candidate hash replay failed"
pass "exact successful V7/26478 candidate hashes reproduced before 26479 fix"
...
sha256sum -c "$EXPECTED_26478_HASHES" || fail "real exact V7/26478 replay hash mismatch"
pass "real exact successful V7/26478 state reproduced"

HASH_26478="$OUTDIR/26479_exact_26478_prechange.sha256"
find app/src/main -type f -print0 | sort -z | xargs -0 sha256sum > "$HASH_26478"
sha256sum app/version.properties >> "$HASH_26478"

PREPATCH_26479="$OUTDIR/26479_pre_edit_binary.patch"
git diff --binary "$EXPECTED_APP_BASE" -- app/src/main app/version.properties > "$PREPATCH_26479"
[[ -s "$PREPATCH_26479" ]] || fail "26479 true pre-edit binary patch is empty"
git apply --check --reverse "$PREPATCH_26479" || \
  fail "26479 pre-edit patch cannot reverse exact V7/26478 state"
PREPATCH_26479_SHA="$(sha256sum "$PREPATCH_26479" | awk '{print $1}')"
pass "TRUE 26479 pre-edit binary patch created from exact successful V7/26478"

CAND79="$TMP/candidate26479"
mkdir -p "$CAND79/app/src/main/assets/shaders/motionv2" "$CAND79/app"
cp app/src/main/assets/shaders/motionv2/mfsr_low_support_reference.glsl \
   "$CAND79/app/src/main/assets/shaders/motionv2/mfsr_low_support_reference.glsl"
cp app/version.properties "$CAND79/app/version.properties"

python3 "$PORT_TRANSFORM" "$CAND79"

REF79="$CAND79/app/src/main/assets/shaders/motionv2/mfsr_low_support_reference.glsl"
grep -q 'IRIS_26479_ADRENO_GLSL_SAMPLE_KEYWORD_PORTABILITY' "$REF79" || \
  fail "26479 portability marker missing from temp candidate"
grep -q 'float cfaSample=cfaAt(p);' "$REF79" || \
  fail "26479 cfaSample declaration missing"
grep -q 'refNum\[c\]+=cfaSample\*w;' "$REF79" || \
  fail "26479 cfaSample accumulation missing"
grep -q '^VERSION_NAME=0\.9726479$' "$CAND79/app/version.properties" || \
  fail "26479 candidate version incorrect"
grep -q '^VERSION_BUILD=26479$' "$CAND79/app/version.properties" || \
  fail "26479 candidate build incorrect"

python3 - "$REF79" <<'PY_SAMPLE_GUARD'
from pathlib import Path
import re, sys
...
if re.search(r"\bsample\b",code):
    raise SystemExit("bare GLSL keyword sample remains in executable shader code")
print("26479 temp runtime GLSL reserved-keyword scan: PASS")
PY_SAMPLE_GUARD

pass "candidate/source validation PASS"
pass "Temporary-copy validation: PASS"

cp "$CAND79/app/src/main/assets/shaders/motionv2/mfsr_low_support_reference.glsl" \
   app/src/main/assets/shaders/motionv2/mfsr_low_support_reference.glsl
cp "$CAND79/app/version.properties" app/version.properties

find app/src/main -type f -print0 | sort -z | xargs -0 sha256sum > "$HASH_AFTER"
sha256sum app/version.properties >> "$HASH_AFTER"

python3 - "$HASH_26478" "$HASH_AFTER" <<'PY_26479_DELTA'
from pathlib import Path
import sys
def read(path):
    out={}
...
if changed != expected:
    raise SystemExit("26479 delta scope mismatch: "+repr(sorted(changed)))
print("26479 exact delta scope shader+version only: PASS")
PY_26479_DELTA
...
grep -q '^VERSION_NAME=0\.9726479$' app/version.properties || fail "wrong version"
grep -q '^VERSION_BUILD=26479$' app/version.properties || fail "wrong build"

# Validate whitespace only on the exact 26478 changed-file allowlist.
# Historical protected files outside this allowlist may contain pre-existing
# whitespace and must not block an otherwise source-identical 26478 build.
git diff --check "$EXPECTED_APP_BASE" -- \
  "$CAP" "$RECON" "$INIT" "$ACC" "$REFADD" "$FINAL" "$COLOR" \
  app/version.properties || \
  fail "26478 scoped git diff --check failed"
pass "26478 scoped changed-file whitespace check PASS"
python3 - app/src/main/assets/shaders/motionv2 <<'PY_GLOBAL_SAMPLE_GUARD'
from pathlib import Path
import re, sys
...
for p in sorted(root.glob("*.glsl")):
    ...
    if re.search(r"\bsample\b",code):
        bad.append(str(p))
if bad:
    raise SystemExit("reserved GLSL keyword sample used in executable shader code: "+repr(bad))
print("26479 Motion V2 executable GLSL reserved-keyword scan: PASS")
PY_GLOBAL_SAMPLE_GUARD
pass "26479 Adreno GLSL keyword portability scan PASS"
...
echo "  scoped changed-file git diff --check PASS"
echo "  exact successful V7/26478 replay PASS"
echo "  TRUE 26479 pre-edit patch before portability modification PASS"
echo "  26479 delta scope shader+version only PASS"
echo "  Adreno GLSL reserved-keyword scan PASS"
echo "  version/build increment in same guarded Gradle script PASS"

cat > "$REPORT" <<EOF
26479 V9 ADRENO SAMPLE-KEYWORD PORTABILITY / EXACT 26478 IMAGE-MATH
========================================================================
Rollback:
...
True 26479 pre-edit patch (exact successful V7/26478 state):
  $PREPATCH_26479
  SHA256=$PREPATCH_26479_SHA
