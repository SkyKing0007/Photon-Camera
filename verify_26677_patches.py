...
if len(sys.argv)!=4: raise SystemExit('usage: verify_26677_patches.py ROOT BASE CANDIDATE')
...
def make(first,second):
 with tempfile.TemporaryDirectory(prefix='iris26677_patch_') as td:
...
forward=make(base,cand); rollback=make(cand,base); f=root/'R1_26677_RUNTIME_DELTA_FROM_26676.patch'; r=root/'R1_26677_RUNTIME_ROLLBACK_TO_26676.patch'
...
for p in (forward,rollback):
 assert p.count(b'diff --git ')==4 and b'new file mode ' not in p and b'deleted file mode ' not in p
for label,start,patch,target in [('forward',base,f,cand),('rollback',cand,r,base)]:
 with tempfile.TemporaryDirectory(prefix='iris26677_apply_') as td:
...
print('PASS 26677 deterministic full-index patches core.abbrev=7/12/40; fuzz=0 exact forward/rollback; 4 modifications')
