...
if len(sys.argv)!=4: raise SystemExit('usage: verify_26638_r1_patches.py ROOT BASE CANDIDATE')
root,base,cand=map(Path,sys.argv[1:]); changed=[x for x in (root/'R1_26638_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x]; added=set(x for x in (root/'R1_26638_ADDED_PATHS_MUST_BE_ABSENT.txt').read_text().splitlines() if x)
fwd=(root/'R1_26638_RUNTIME_DELTA_FROM_26637_R1.patch').read_bytes(); rev=(root/'R1_26638_RUNTIME_ROLLBACK_TO_26637_R1.patch').read_bytes()
def run(*a,cwd): return subprocess.run(a,cwd=cwd,check=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE).stdout
def sha(p): return hashlib.sha256(p.read_bytes()).hexdigest()
with tempfile.TemporaryDirectory(prefix='iris26638_patch_') as td:
 ...
 p=t/'f.patch'; p.write_bytes(fwd); run('git','checkout','-q',b,cwd=t); run('git','apply','--check',str(p),cwd=t)
 run('git','apply',str(p),cwd=t)
 ...
 for rel in changed:
  if rel in added: assert not (t/rel).exists(),rel
  else: assert sha(t/rel)==sha(base/rel),rel
print('PASS 26638 full-index forward/rollback patch proof: core.abbrev 7/12/40, exact 10-path/1-added allowlist, fuzz=0 exact rollback')
