...
# Conservative union of GLSL + GLSL ES keyword/reserved sets, inherited from the
# successful 26559 scanner and applied only to declared identifiers.
RESERVED=set('''
...
struct common partition active asm class union enum typedef template this resource goto inline noinline public static extern external
...
def declared_identifiers(src):
    ...
    qualifiers={'const','uniform','in','out','inout','readonly','writeonly','highp','mediump','lowp','centroid','flat','smooth','noperspective','precise','coherent','volatile','restrict'}
    for i,(t,ln) in enumerate(toks[:-1]):
...
def scan(p):
    ...
    return decl,bad
def self_test():
    good='#version 310 es\nprecision highp float;\nfloat helper(float value){float result=value;return result;}\nvoid main(){float coherentSupport=1.0; float sampleValue=2.0;}'
    bad_coherent='#version 310 es\nprecision highp float;\nvoid main(){float coherent=2.0;}'
    bad_sample='#version 310 es\nprecision highp float;\nvoid main(){float sample=2.0;}'
    assert not [d for d in declared_identifiers(good) if d[0] in RESERVED]
    assert any(d[0]=='coherent' for d in declared_identifiers(bad_coherent) if d[0] in RESERVED)
    assert any(d[0]=='sample' for d in declared_identifiers(bad_sample) if d[0] in RESERVED)
    print('PASS reserved-identifier scanner self-test: coherent + sample regressions rejected')
def main():
