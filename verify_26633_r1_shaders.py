...
if len(sys.argv)<5 or sys.argv[3]!='--out': raise SystemExit('usage: verify_26633_r1_shaders.py BASE CAND --out OUT [--compiler PATH]')
...
if (B/rel).read_bytes()==(C/rel).read_bytes(): raise SystemExit('FAIL render shader unexpectedly identical')
mods.append(('motionv2_render_26633','#version 300 es\n'+(C/rel).read_text(),'frag'))
# Exact runtime Kotlin embedded SHORT rescue shader.
kt=(C/'app/src/main/java/com/hinnka/mycamera/processor/GlesMgcRawSabreShaders.kt').read_text()
m=re.search(r'val shortRescueWeight26607 = """(.*?)"""\.trimIndent\(\)',kt,re.S)
if not m: raise SystemExit('FAIL embedded SHORT rescue shader missing')
short=textwrap.dedent(m.group(1)).lstrip('\n')
if not short.startswith('#version 300 es\n'): raise SystemExit('FAIL SHORT rescue runtime #version')
mods.append(('short_rescue_weight_26633',short,'frag'))
# Exact embedded true2x publication compute shader modified for shadow-toe parity.
cpp=(C/'app/src/main/cpp/motionv2_jpeg444_jni.cpp').read_text(); m=re.search(r'static const char\*kIris26571PublicationCompute=R"GLSL\((.*?)\)GLSL";',cpp,re.S)
...
mods.append(('true2x_publication_compute_26633',compute,'comp'))
...
for name,src,stage in mods:
 ...
 p=O/f'{name}.{stage}'; p.write_text(src); paths.append((p,stage)); status='STATIC_PASS'
 if compiler:
...
lines=[f'{hashlib.sha256(p.read_bytes()).hexdigest()}  {p.name}' for p,_ in paths]
(O/'R1_26633_RUNTIME_EXPANDED_SHADERS.sha256').write_text('\n'.join(lines)+'\n')
(O/'R1_26633_SHADER_VERIFICATION.json').write_text(json.dumps({'count':len(results),'compiler':compiler,'results':results},indent=2,sort_keys=True)+'\n')
print(f'PASS 26633 complete runtime-expanded GLSL reserved scan variants={len(results)} real_compiler={bool(compiler)}')
