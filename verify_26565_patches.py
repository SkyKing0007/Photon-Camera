...
def main():
    ...
    fpin=(pkg/'V1_26565_RUNTIME_DELTA_FROM_26564_V1_4.patch').read_bytes(); rpin=(pkg/'V1_26565_RUNTIME_ROLLBACK_TO_26564_V1_4.patch').read_bytes()
    bt,ct=tree(base),tree(cand)
    with tempfile.TemporaryDirectory(prefix='iris26565_patch_') as td:
        ...
        p=subprocess.run(['patch','--batch','--fuzz=0','-p1','-i',str(pkg/'V1_26565_RUNTIME_DELTA_FROM_26564_V1_4.patch')],cwd=replay,text=True,capture_output=True)
        assert p.returncode==0,p.stdout+p.stderr; assert tree(replay)==ct,'forward fuzz=0 did not reproduce exact candidate'
        p=subprocess.run(['patch','--batch','--fuzz=0','-p1','-i',str(pkg/'V1_26565_RUNTIME_ROLLBACK_TO_26564_V1_4.patch')],cwd=replay,text=True,capture_output=True)
        assert p.returncode==0,p.stdout+p.stderr; assert tree(replay)==bt,'rollback fuzz=0 did not reproduce exact base'
    print('PASS deterministic full-index patches core.abbrev=7/12/40 + forward/rollback fuzz=0 exact trees')
