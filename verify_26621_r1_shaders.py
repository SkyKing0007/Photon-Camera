...
if len(sys.argv)<5 or sys.argv[3] != '--out':
    raise SystemExit('usage: verify_26621_r1_shaders.py BASE CAND --out OUT [--compiler PATH]')
...
# Exact GLInterface.readProgram expansion: GLProg.glVersion is '#version 310 es\n', then one
# extra blank line and '#line 1'. Adaptive color has one runtime-overridden define variant.
prefix='#version 310 es\n\n#line 1\n'
spec=[]
def asset(rel,name,defines=None):
    ...
    if defines:
        ...
        for line in s.splitlines(True):
            ...
            if not replaced: lines.append(line)
        s=''.join(lines)
    spec.append((name,prefix+s,'frag'))
asset('app/src/main/assets/shaders/motionv2/adaptive_color_appearance_26563.glsl','adaptive_color_appearance_26621_profile0')
asset('app/src/main/assets/shaders/motionv2/adaptive_color_appearance_26563.glsl','adaptive_color_appearance_26621_profile1',{'CALIBRATED_PROFILE':'1'})
for rel,name in [
('app/src/main/assets/shaders/motionv2/local_laplacian_global_log_26621.glsl','local_laplacian_26621_global_log'),
('app/src/main/assets/shaders/motionv2/local_laplacian_downsample_26621.glsl','local_laplacian_26621_downsample'),
('app/src/main/assets/shaders/motionv2/local_laplacian_remap_26621.glsl','local_laplacian_26621_remap'),
('app/src/main/assets/shaders/motionv2/local_laplacian_accumulate_26621.glsl','local_laplacian_26621_accumulate'),
('app/src/main/assets/shaders/motionv2/local_laplacian_reconstruct_26621.glsl','local_laplacian_26621_reconstruct'),
('app/src/main/assets/shaders/motionv2/render.glsl','motionv2_render_26621')]: asset(rel,name)
# True-2x GPU publication shader is compiled directly from this C++ string, so its runtime source
# is the literal string including its own #version.
...
cs=m.group(1).lstrip('\n'); assert cs.startswith('#version 310 es\n'); assert 'IRIS_26621_TRUE2X_LOCAL_LAPLACIAN_GPU_SHARED_FIELD' in cs
spec.append(('true2x_26621_shared_local_tone',cs,'comp'))
...
(O/'R1_26621_RUNTIME_EXPANDED_SHADERS.sha256').write_text('\n'.join(lines)+'\n')
results=[]
...
(O/'R1_26621_SHADER_VERIFICATION.json').write_text(json.dumps({'count':len(results),'compiler':compiler,'results':results},indent=2,sort_keys=True)+'\n')
print(f'PASS 26621 exact runtime-expanded shader extraction/reserved scan variants={len(results)} real_compiler={bool(compiler)}')
