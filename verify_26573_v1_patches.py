...
def regen(start,target,abbrev):
 td=Path(tempfile.mkdtemp(prefix='p26573_'))
 try:
  init(td,start)
...
def applyprove(start,target,patch):
 td=Path(tempfile.mkdtemp(prefix='a26573_'))
 try:
...
def main():
 ...
 print('PASS FORWARD PATCH FUZZ=0 exact 26573 candidate recreation')
 print('PASS ROLLBACK PATCH FUZZ=0 exact 26572 recreation')
