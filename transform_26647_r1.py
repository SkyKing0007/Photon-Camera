...
if len(sys.argv)!=3: raise SystemExit('usage: transform_26647_r1.py BASE DEST')
base=Path(sys.argv[1]).resolve(); dest=Path(sys.argv[2]).resolve(); root=Path(__file__).resolve().parent
changed=[x for x in (root/'R1_26647_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x]
if len(changed)!=5: raise SystemExit(f'expected 5 changed paths, got {len(changed)}')
...
def readm(p):
 ...
 return d
pre=readm(root/'R1_26647_PREWRITE_SOURCE_HASHES.sha256')
post=readm(root/'R1_26647_EXPECTED_CHANGED_SOURCE_HASHES.sha256')
...
payload=root/'handoff_payload_26647_r1'
for rel in changed:
...
print('PASS 26647 deterministic payload transform: exact successful 26646 authority -> 1720-file candidate, 5 changed / 0 added')
