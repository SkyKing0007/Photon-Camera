...
if len(sys.argv)!=4:raise SystemExit('usage: verify_26662_patches.py ROOT BASE CANDIDATE')
root=Path(sys.argv[1]).resolve();base=Path(sys.argv[2]).resolve();cand=Path(sys.argv[3]).resolve();fwd=(root/'R1_26662_RUNTIME_DELTA_FROM_26661.patch').read_bytes();rev=(root/'R1_26662_RUNTIME_ROLLBACK_TO_26661.patch').read_bytes();allow={x for x in (root/'R1_26662_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x};added={x for x in (root/'R1_26662_ADDED_PATHS_MUST_BE_ABSENT.txt').read_text().splitlines() if x};assert len(allow)==6 and not added
...
with tempfile.TemporaryDirectory(prefix='iris26662_patch_') as td:
...
print('PASS 26662 deterministic full-index forward/rollback proof: core.abbrev 7/12/40 identical, exact apply/rollback, 6-path allowlist / 0 additions')
