...
if len(sys.argv)!=3: raise SystemExit('usage: transform_26646_r1.py BASE OUT')
base=Path(sys.argv[1]).resolve(); out=Path(sys.argv[2]).resolve(); root=Path(__file__).resolve().parent
payload=root/'handoff_payload_26646_r1'
changed=[x for x in (root/'R1_26646_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x]
def readm(p):
 d={}
...
pre=readm(root/'R1_26646_PREWRITE_SOURCE_HASHES.sha256'); expected=readm(root/'R1_26646_EXPECTED_CHANGED_SOURCE_HASHES.sha256')
if len(changed)!=9 or len(set(changed))!=9: raise SystemExit('runtime allowlist count mismatch')
...
print('PASS 26646 deterministic candidate transform: exact 9-path/0-added delta from successful 26645 compiled-candidate authority')
