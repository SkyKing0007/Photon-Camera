...
UPLOAD_PARENT_COMMIT="f77d990f6e801a48c50c99a08e9c1ad2d45fce20"
...
HANDOFF="$ROOT/R2_26679_HANDOFF_HASHES.sha256"; UPLOADS="$ROOT/R2_26679_UPLOAD_PATHS.txt"
BASE_FULL="$ROOT/R2_26679_BASE_26678_FULL_APP.sha256"; CAND_FULL="$ROOT/R2_26679_EXPECTED_CANDIDATE_FULL_APP.sha256"
BASE_PROTECTED="$ROOT/R2_26679_PROTECTED_UNCHANGED_BASE.sha256"; CAND_PROTECTED="$ROOT/R2_26679_PROTECTED_UNCHANGED_CANDIDATE.sha256"
BASE_NATIVE="$ROOT/R2_26679_NATIVE_PROTECTED_BASE.sha256"; CAND_NATIVE="$ROOT/R2_26679_NATIVE_PROTECTED_CANDIDATE.sha256"
BASE_VENDOR="$ROOT/R2_26679_VENDOR_PROTECTED_BASE.sha256"; CAND_VENDOR="$ROOT/R2_26679_VENDOR_PROTECTED_CANDIDATE.sha256"
BASE_DNG="$ROOT/R2_26679_DNG_BASE.sha256"; CAND_DNG="$ROOT/R2_26679_DNG_CANDIDATE.sha256"
SHADER_BASE="$ROOT/R2_26679_SHADER_UNIVERSE_BASE.sha256"; SHADER_CAND="$ROOT/R2_26679_SHADER_UNIVERSE_CANDIDATE.sha256"
CHANGED="$ROOT/R2_26679_RUNTIME_CHANGED_PATHS.txt"; PREWRITE="$ROOT/R2_26679_PREWRITE_SOURCE_HASHES.sha256"; ADDED="$ROOT/R2_26679_ADDED_PATHS_MUST_BE_ABSENT.txt"; DELETED="$ROOT/R2_26679_DELETED_PATHS_MUST_EXIST.txt"; EXPECTED_CHANGED="$ROOT/R2_26679_EXPECTED_CHANGED_SOURCE_HASHES.sha256"
FORWARD="$ROOT/R2_26679_RUNTIME_DELTA_FROM_26678.patch"; ROLLBACK="$ROOT/R2_26679_RUNTIME_ROLLBACK_TO_26678.patch"; SHADER_PIN="$ROOT/R2_26679_RUNTIME_EXPANDED_SHADERS.sha256"
TRANSFORM="$ROOT/transform_r2_26679.py"; VALIDATE="$ROOT/validate_r2_26679.py"; AUTHORITY="$ROOT/r2_26679_verify_authority.py"; INFRA="$ROOT/r2_26679_verify_infrastructure.py"; PATCHVERIFY="$ROOT/r2_26679_verify_patches.py"; SHADERVERIFY="$ROOT/r2_26679_verify_shaders.py"; GATEVERIFY="$ROOT/r2_26679_verify_regressions.py"
BUILD_SCRIPT="$ROOT/build_26679_r2_java_compiler_repair.sh"; WORKFLOW="$ROOT/.github/workflows/build-26679-r2-java-compiler-repair.yml"
OUT="$ROOT/build_26679_r2_java_compiler_repair_outputs"; WORK="$ROOT/.build_26679_r2_java_compiler_repair_work"
...
FINAL="$ROOT/IrisCamera-${VERSION_NAME}-${VERSION_BUILD}-r2-java-compiler-repair-debug.apk"
...
cat > "$OUT/26679_R2_COMPILER_STATUS.txt" <<'STATUS'
...
STATUS
cat > "$OUT/26679_R2_STRICT_HANDOFF_REPORT.txt" <<'REPORT'
...
REPORT
set_report(){ local key="$1" val="$2" tmp="$OUT/.report.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26679_R2_STRICT_HANDOFF_REPORT.txt" > "$tmp" || fail "report key missing $key"; mv "$tmp" "$OUT/26679_R2_STRICT_HANDOFF_REPORT.txt"; }
set_compiler(){ local key="$1" val="$2" tmp="$OUT/.compiler.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26679_R2_COMPILER_STATUS.txt" > "$tmp" || fail "compiler key missing $key"; mv "$tmp" "$OUT/26679_R2_COMPILER_STATUS.txt"; }
...
verify_package(){
 [[ -f "$HANDOFF" && -f "$UPLOADS" && -f "$DELETED" && -d "$ROOT/handoff_payload_26679_r2" ]] || fail "sealed 26679 package incomplete"
 [[ -f "$ROOT/R2_26679_RUNTIME_EXPANDED_CHANGED_VARIANTS.txt" ]] || fail "sealed 26679 runtime-expanded variant list missing"
 [[ "$(find "$ROOT/handoff_payload_26679_r2" -type f | wc -l)" -eq 5 ]] || fail "sealed runtime payload must contain exactly 5 files"
...
verify_scope(){
 ...
 [[ "$(git rev-parse HEAD^)" == "$UPLOAD_PARENT_COMMIT" ]] || fail "26679 R2 parent must be exact failed-R1 handoff commit ${UPLOAD_PARENT_COMMIT}"
 ...
 git diff --name-only "$UPLOAD_PARENT_COMMIT"..HEAD | sort > "$WORK/actual_upload_scope.txt"; sort "$UPLOADS" > "$WORK/expected_upload_scope.txt"; diff -u "$WORK/expected_upload_scope.txt" "$WORK/actual_upload_scope.txt" || fail "26679 upload scope mismatch"
...
postbuild_proof(){
 ...
 tar --sort=name --mtime='UTC 1970-01-01' --owner=0 --group=0 --numeric-owner -cf - -C "$POST" app | gzip -n > "$OUT/26679_R2_candidate_app_source.tar.gz"; sha256sum "$OUT/26679_R2_candidate_app_source.tar.gz" > "$OUT/26679_R2_candidate_app_source.tar.gz.sha256"; cp "$CAND_FULL" "$OUT/26679_R2_candidate_full_app.sha256"; cp "$CAND_NATIVE" "$OUT/26679_R2_native_protected_postbuild.sha256"; cp "$CAND_VENDOR" "$OUT/26679_R2_vendor_protected_postbuild.sha256"; cp "$CAND_DNG" "$OUT/26679_R2_dng_postbuild.sha256"; cp "$SHADER_PIN" "$OUT/26679_R2_runtime_expanded_shaders.sha256"; set_report "CLEAN ARTIFACT SOURCE EXPORT" "PASS (deterministic candidate tar + manifests + runtime-expanded shader pins)"
}
verify_package
verify_scope
obtain_authority
...
mv "${apks[0]}" "$FINAL"; [[ -f "$FINAL" ]] || fail "final APK missing after move"; set_report "EXACTLY ONE APK" "PASS"; sha256sum "$FINAL" > "$OUT/26679_R2_APK.sha256"
postbuild_proof
pass "26679 R2 ACTIONS BUILD COMPLETE"
