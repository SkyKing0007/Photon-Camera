...
if len(sys.argv)!=3: raise SystemExit('usage: transform_26622_r1.py BASE OUT')
...
def verify(root, manifest):
    ...
    return len(lines)
if verify(base,'R1_26622_PREWRITE_SOURCE_HASHES.sha256')!=2: raise SystemExit('prewrite count')
...
patch=P/'R1_26622_RUNTIME_DELTA_FROM_26621_R1.patch'
# Preserve exact successful 26621 nested-candidate isolation: the isolated candidate must not
# discover the checked-out carrier repository while git validates/applies the canonical patch.
...
if verify(out,'R1_26622_EXPECTED_CHANGED_SOURCE_HASHES.sha256')!=2: raise SystemExit('candidate changed count')
print('PASS 26622 deterministic candidate reconstruction from exact successful 26621 R1 + canonical full-index patch')
