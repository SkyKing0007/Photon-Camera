...
if len(sys.argv)<5 or sys.argv[3]!='--out': raise SystemExit('usage: verify_26631_r1_shaders.py BASE CAND --out OUT [--compiler PATH]')
...
gain_rel='app/src/main/assets/shaders/motionv2/gainmap.glsl'
if (B/gain_rel).read_bytes()==(C/gain_rel).read_bytes(): raise SystemExit('FAIL gainmap shader unexpectedly identical')
mods=[('motionv2_gainmap_26631','#version 300 es\n'+(C/gain_rel).read_text(),'frag')]
...
mods.append(('true2x_publication_compute_26631',compute,'comp'))
...
(O/'R1_26631_RUNTIME_EXPANDED_SHADERS.sha256').write_text('\n'.join(lines)+'\n')
(O/'R1_26631_SHADER_VERIFICATION.json').write_text(json.dumps({'count':len(results),'compiler':compiler,'results':results},indent=2,sort_keys=True)+'\n')
print(f'PASS 26631 complete runtime-expanded GLSL reserved scan variants={len(results)} real_compiler={bool(compiler)}')
