...
if len(sys.argv) not in (4,6):raise SystemExit('usage: verify_26705_shaders.py ROOT BASE CANDIDATE [--compiler PATH]')
...
variants=[('sabre_universal_normal_master_short_fusion_26705.frag',extract_kotlin('universalNormalMasterShortFusion26651'),'frag')]
pins=readm(root/'26705_RUNTIME_EXPANDED_SHADERS.sha256');assert len(pins)==1
sb=readm(root/'26705_ASSET_SHADER_UNIVERSE_BASE.sha256');sc=readm(root/'26705_ASSET_SHADER_UNIVERSE_CANDIDATE.sha256');assert len(sb)==len(sc)==271 and sb==sc
for r,h in sb.items():assert sha(base/r)==h and sha(cand/r)==h,r
...
def scan(name,src):
 ...
 for t in ['IRIS_26704_FAIL_CLOSED_INFERRED_SHORT_RADIANCE','IRIS_26704_LOW_NORMAL_CONFIDENCE_NEVER_BOOSTS_SHORT','IRIS_26704_VALID_NORMAL_CHROMA_IMMUTABLE','IRIS_26705_SHORT_RADIANCE_HIGHLIGHT_DOMAIN_ONLY','float physicalHighlightContext = smoothstep(','float colorRecoveryNeed = allColorSupportLost;']:
...
with tempfile.TemporaryDirectory(prefix='iris26705_glsl_') as td:
 d=Path(td)
 for name,src,stage in variants:
  ...
  if compiler:
   ...
   if cp.returncode!=0:raise SystemExit(f'26705 GLSL FAIL {name}\n{cp.stdout}\n{cp.stderr}')
print(f'PASS 26705 runtime-expanded GLSL variants=1 complete reserved/structure/exact-hash real_compiler={bool(compiler)} assetShaderUniverse=271')
