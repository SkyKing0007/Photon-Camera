...
SUCCESS_26597_SCRIPT_BLOB='24d7ceba2c92dc07f7dac4fd2aef0146e632bf0d'
SUCCESS_26597_WORKFLOW_BLOB='a77ae542a4402def94f4144fa138c1771eece9cc'
...
def main():
    ...
    ap.add_argument('--success-26597-script'); ap.add_argument('--success-26597-workflow')
    ...
    for t in [
      ...
      'BASE_RUN_ID="33920477863"','BASE_JOB_ID="101177394212"','BASE_ARTIFACT_ID="9954920345"',
      ...
      'GLSLANG_VERSION="16.5.0"','BACKUP_BRANCH="backup-26597-before-26598-capture-ownership"','BACKUP_SHA="a46d2fb15a64a7676a92453bbfb2093626841ad1"','GLSLANG_ARCHIVE_SHA="b9b1f96acb898a62251b171f7695efcecfc206a530299054071919b06820f657"']:
        if t not in s: fail('authority/mechanics pin '+t)
    current_sig=check_order(s,'26598')
    ...
    if "names.append('V1_26598_HANDOFF_HASHES.sha256')\nprint('\\n'.join(sorted(names)))" not in s:
        fail('escaped expected-scope join missing')
    workflow_shape(w,'26598')
    for t in ['build-26598-v1-semantic-scene-white-authority.yml',
              'photon-26598-v1-1-scene-white-capture-ownership',
              'Build exact 26598 candidate from successful compiled 26597 authority']:
        if t not in w: fail('26598 workflow identity/order token '+t)
    # Explicit last-successful implementation diff audit when Actions can fetch exact 26597 sources.
    if ns.success_26597_script:
        if git_blob(ns.success_26597_script)!=SUCCESS_26597_SCRIPT_BLOB: fail('successful-26597 build script blob')
        prior=Path(ns.success_26597_script).read_text(); prior_sig=check_order(prior,'26597')
        if prior_sig!=current_sig: fail('core compiler/build mechanics signature differs from successful 26597')
        # Critical command/order/lifecycle tokens must remain exactly present in both implementations.
        ...
        for t in stable:
            if t not in prior or t not in s: fail('successful-26597 stable mechanics token '+t)
    if ns.success_26597_workflow:
        if git_blob(ns.success_26597_workflow)!=SUCCESS_26597_WORKFLOW_BLOB: fail('successful-26597 workflow blob')
        priorw=Path(ns.success_26597_workflow).read_text(); workflow_shape(priorw,'26597')
    ...
    print('PASS exact successful-26597 runtime/mechanics pins + successful-26593 compiler/build ordering inherited')
    if ns.success_26597_script and ns.success_26597_workflow:
        print('PASS exact successful-26597 build/workflow blob audit; mechanics delta limited to 26598 V1.1 identity/base/scope/backup/semantic-capture gates')
    else:
        print('PASS prior successful-26597 blob pins packaged; exact-source diff replay deferred to Actions')
