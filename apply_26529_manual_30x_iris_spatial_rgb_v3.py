...
''','stream catch cleanup')
    ...
    /**
     * IRIS_26529_V3_FRAMEBUFFER_DETACH_OWNER
     * Detach a diagnostic color target before its texture is released. V2 called this helper
     * twice but never defined it, which the real module Kotlin compile correctly rejected.
     */
    private fun detachFramebufferColorAttachment(framebuffer: Int) {
        ...
        GLES30.glBindFramebuffer(GLES30.GL_FRAMEBUFFER, 0)
        checkGlError("detach MGC Spatial RGB diagnostic color attachment")
    }
...
def self_test():
    ...
    probe='''private fun detachFramebufferColorAttachment(framebuffer: Int) {
        if (framebuffer == 0) return
    }'''
    assert 'private fun detachFramebufferColorAttachment(framebuffer: Int)' in probe
    print('PASS: 26529 V3 transform helper self-test')
