...
if len(sys.argv)!=3: raise SystemExit('usage: transform_26627_r1.py BASE OUT')
...
def verify(root, manifest):
    ...
    return len(lines)
if verify(base,'R1_26627_PREWRITE_SOURCE_HASHES.sha256')!=4: raise SystemExit('prewrite count')
added=[x.strip() for x in (P/'R1_26627_ADDED_PATHS_MUST_BE_ABSENT.txt').read_text().splitlines() if x.strip()]
if added: raise SystemExit('26627 adds no runtime paths')
patch=P/'R1_26627_RUNTIME_DELTA_FROM_26626_R1.patch'
...
if verify(out,'R1_26627_EXPECTED_CHANGED_SOURCE_HASHES.sha256')!=4: raise SystemExit('candidate changed count')
print('PASS 26627 deterministic candidate reconstruction from exact successful 26626 R1 + canonical full-index patch')
