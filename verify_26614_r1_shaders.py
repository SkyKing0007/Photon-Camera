...
if len(sys.argv)<5 or sys.argv[3] != '--out':
    raise SystemExit('usage: verify_26614_r1_shaders.py BASE CAND --out OUT [--compiler PATH]')
...
(O/'R1_26614_RUNTIME_EXPANDED_SHADERS.sha256').write_text('\n'.join(lines)+'\n')
results=[]
...
(O/'R1_26614_SHADER_VERIFICATION.json').write_text(json.dumps({'count':len(results),'compiler':compiler,'results':results},indent=2,sort_keys=True)+'\n')
