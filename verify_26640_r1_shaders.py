...
if len(sys.argv) not in (4,6): raise SystemExit('usage: verify_26640_r1_shaders.py ROOT BASE CANDIDATE [--compiler PATH]')
...
def variants(rootp):
 ...
 return [
  ('motionv2_gainmap_26640.frag',runtime_gain(rootp),'frag'),
  ('sabre_short_component_anchor_26640.frag',extract(k,'shortComponentAnchor26607'),'frag'),
  ('sabre_short_component_propagate_26640.frag',extract(k,'shortComponentPropagate26607'),'frag')]
# Full shader universe proves only the intended asset shader changed; embedded changes are checked separately.
bm=readm(root/'R1_26640_SHADER_UNIVERSE_BASE.sha256'); cm=readm(root/'R1_26640_SHADER_UNIVERSE_CANDIDATE.sha256')
...
expected=readm(root/'R1_26640_RUNTIME_EXPANDED_SHADERS.sha256'); assert len(expected)==3
...
def scan(name,src):
 ...
 # Permanent exact syntax regression for the four propagation neighbors.
 if name.endswith('propagate_26640.frag'):
  for n in range(4):
   token=f'min(trustAt(n{n}), compatibleFlow(p, n{n}))))'
   if src.count(token)!=1: raise SystemExit(f'FAIL {name}: neighbor n{n} count={src.count(token)}')
mods=variants(cand)
with tempfile.TemporaryDirectory(prefix='iris26640_shader_') as td:
 out=Path(td)
 for name,src,stage in mods:
  ...
  if compiler:
   ...
   if cp.returncode!=0: raise SystemExit(f'26640 GLSL FAIL {name}\n{cp.stdout}\n{cp.stderr}')
print(f'PASS 26640 runtime-expanded GLSL variants=3 reserved/structure/exact-hash real_compiler={bool(compiler)} shaderUniverse=257')
