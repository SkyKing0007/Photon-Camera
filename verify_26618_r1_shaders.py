...
if len(sys.argv)<5 or sys.argv[3] != '--out':
    raise SystemExit('usage: verify_26618_r1_shaders.py BASE CAND --out OUT [--compiler PATH]')
...
spec=[]
for rel,name in [
('app/src/main/assets/shaders/motionv2/guided_base_detail_ltm_coeff_26618.glsl','guided_ltm_26618_coeff'),
('app/src/main/assets/shaders/motionv2/guided_base_detail_ltm_gain_26618.glsl','guided_ltm_26618_gain'),
('app/src/main/assets/shaders/motionv2/guided_base_detail_ltm_apply_26618.glsl','guided_ltm_26618_apply')]:
...
assert 'IRIS_26618_TRUE2X_SHARED_GUIDED_LTM_GPU' in cs
spec.append(('true2x_26618_shared_guided_ltm',cs,'comp'))
# Complete declaration-name reserved scan inherited from successful 26614 rule.
...
(O/'R1_26618_RUNTIME_EXPANDED_SHADERS.sha256').write_text('\n'.join(lines)+'\n')
results=[]
...
(O/'R1_26618_SHADER_VERIFICATION.json').write_text(json.dumps({'count':len(results),'compiler':compiler,'results':results},indent=2,sort_keys=True)+'\n')
print(f'PASS 26618 runtime-expanded shader extraction/reserved scan variants={len(results)} real_compiler={bool(compiler)}')
