...
HANDOFF="$ROOT/V1_26614_HANDOFF_HASHES.sha256"
BASE_FULL="$ROOT/V1_26614_BASE_26613_V1_1_FULL_APP.sha256"; CAND_FULL="$ROOT/V1_26614_EXPECTED_CANDIDATE_FULL_APP.sha256"
BASE_PROTECTED="$ROOT/V1_26614_PROTECTED_UNCHANGED_BASE.sha256"; CAND_PROTECTED="$ROOT/V1_26614_PROTECTED_UNCHANGED_CANDIDATE.sha256"
BASE_NATIVE="$ROOT/V1_26614_NATIVE_PROTECTED_BASE.sha256"; CAND_NATIVE="$ROOT/V1_26614_NATIVE_PROTECTED_CANDIDATE.sha256"
BASE_VENDOR="$ROOT/V1_26614_VENDOR_PROTECTED_BASE.sha256"; CAND_VENDOR="$ROOT/V1_26614_VENDOR_PROTECTED_CANDIDATE.sha256"
BASE_DNG="$ROOT/V1_26614_DNG_BASE.sha256"; CAND_DNG="$ROOT/V1_26614_DNG_CANDIDATE.sha256"
CHANGED="$ROOT/V1_26614_RUNTIME_CHANGED_PATHS.txt"; PREWRITE="$ROOT/V1_26614_PREWRITE_SOURCE_HASHES.sha256"; EXPECTED_CHANGED="$ROOT/V1_26614_EXPECTED_CHANGED_SOURCE_HASHES.sha256"
FORWARD="$ROOT/V1_26614_RUNTIME_DELTA_FROM_26613_V1_1.patch"; ROLLBACK="$ROOT/V1_26614_RUNTIME_ROLLBACK_TO_26613_V1_1.patch"; SHADER_PIN="$ROOT/V1_26614_RUNTIME_EXPANDED_SHADERS.sha256"
TRANSFORM="$ROOT/transform_26614_v1.py"; VALIDATE="$ROOT/validate_26614_v1.py"; AUTHORITY="$ROOT/verify_26614_v1_authority.py"; INFRA="$ROOT/verify_26614_v1_infrastructure.py"; PATCHVERIFY="$ROOT/verify_26614_v1_patches.py"; SHADERVERIFY="$ROOT/verify_26614_v1_shaders.py"; GATEVERIFY="$ROOT/verify_26614_v1_regressions.py"
BUILD_SCRIPT="$ROOT/build_26614_v1_canonical_appearance_cfa_validity.sh"; WORKFLOW="$ROOT/.github/workflows/build-26614-v1-canonical-appearance-cfa-validity.yml"
OUT="$ROOT/build_26614_v1_canonical_appearance_cfa_validity_outputs"; WORK="$ROOT/.build_26614_v1_canonical_appearance_cfa_validity_work"; ARTZIP="$WORK/26613_v1_1_artifact.zip"; ARTDIR="$WORK/artifact"; BASE="$WORK/exact_26613_v1_1_compiled_candidate"; AFTER="$WORK/candidate_26614_v1"; AFTER2="$WORK/candidate_26614_v1_replay"; SHADER_OUT="$WORK/runtime_expanded_shaders"; GLSLANG_DIR="$WORK/glslang-16.5.0"; LIVE_CANON="$WORK/live_compiler_candidate_snapshot"; POST="$WORK/postbuild_source_snapshot"; FINAL="$ROOT/IrisCamera-${VERSION_NAME}-${VERSION_BUILD}-v1-canonical-appearance-cfa-validity-debug.apk"
...
cat > "$OUT/26614_V1_COMPILER_STATUS.txt" <<'STATUS'
...
STATUS
cat > "$OUT/26614_V1_STRICT_HANDOFF_REPORT.txt" <<'REPORT'
...
REPORT
set_report(){ local key="$1" val="$2" tmp="$OUT/.report.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26614_V1_STRICT_HANDOFF_REPORT.txt" > "$tmp" || fail "report key missing $key"; mv "$tmp" "$OUT/26614_V1_STRICT_HANDOFF_REPORT.txt"; }
set_compiler(){ local key="$1" val="$2" tmp="$OUT/.compiler.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26614_V1_COMPILER_STATUS.txt" > "$tmp" || fail "compiler key missing $key"; mv "$tmp" "$OUT/26614_V1_COMPILER_STATUS.txt"; }
...
PY
  ...
  set_report "INFRASTRUCTURE DELTA AUDIT" "PASS (26614 V1 identity/scope/regression only; successful-26613 V1.1 compiler/build mechanics order preserved; no backup)"
}
verify_scope(){
  ...
  [[ "$(git rev-parse HEAD^)" == "$HANDOFF_PARENT_COMMIT" ]] || fail "26614 V1 must be one 26614 handoff commit directly on successful 26613 V1.1"
  python3 -S - "$HANDOFF" > "$WORK/expected_scope.txt" <<'PY'
from pathlib import Path
import sys
names=[line.split('  ',1)[1] for line in Path(sys.argv[1]).read_text().splitlines() if line.strip()]; names.append('V1_26614_HANDOFF_HASHES.sha256'); print('\n'.join(sorted(names)))
PY
...
PY
  while IFS= read -r rel; do [[ -n "$rel" ]] || continue; cmp "$ROOT/handoff_payload_26614_v1/$rel" "$AFTER/$rel" || fail "sealed payload differs frozen candidate $rel"; done < "$CHANGED"
...
verify_shaders(){
  ...
  if [[ -n "$LOCAL_ART" ]]; then python3 -S "$SHADERVERIFY" "$BASE" "$AFTER" --out "$SHADER_OUT" | tee "$OUT/26614_shader_validation.txt"; cmp "$SHADER_OUT/V1_26614_RUNTIME_EXPANDED_SHADERS.sha256" "$SHADER_PIN" || fail "local shader pin mismatch"; cp "$SHADER_OUT/V1_26614_SHADER_VERIFICATION.json" "$OUT/26614_shader_verification.json"; set_report "RUNTIME-EXPANDED GLSL RESERVED SCAN" "PASS (7 exact modified variants)"; set_report "REAL GLSL COMPILE" "NOT RUN locally (Actions required)"; set_compiler "REAL GLSL COMPILE" "NOT RUN locally (Actions required)"; return; fi
  mkdir -p "$GLSLANG_DIR"; local archive="$WORK/glslang-${GLSLANG_VERSION}.tar.gz"; curl -L --fail --retry 3 "$GLSLANG_URL" -o "$archive"; [[ "$(sha "$archive")" == "$GLSLANG_ARCHIVE_SHA" ]] || fail "glslang archive SHA"; tar -xzf "$archive" -C "$GLSLANG_DIR"; local compiler; compiler="$(resolve_glslang_compiler "$GLSLANG_DIR")" || fail "pinned glslang compiler missing"; "$compiler" --version | tee "$OUT/26614_glslang_version.txt"; python3 -S "$SHADERVERIFY" "$BASE" "$AFTER" --out "$SHADER_OUT" --compiler "$compiler" | tee "$OUT/26614_shader_validation.txt"; cmp "$SHADER_OUT/V1_26614_RUNTIME_EXPANDED_SHADERS.sha256" "$SHADER_PIN" || fail "shader pin mismatch"; cp "$SHADER_OUT/V1_26614_SHADER_VERIFICATION.json" "$OUT/26614_shader_verification.json"; set_report "RUNTIME-EXPANDED GLSL RESERVED SCAN" "PASS (7 exact modified variants)"; set_report "REAL GLSL COMPILE" "PASS (pinned glslang 16.5.0)"; set_compiler "REAL GLSL COMPILE" "PASS (pinned glslang 16.5.0)"
}
install_and_build(){
...
PY
  set_report "POST-BUILD INVARIANCE" "PASS (candidate/protected/DNG/native/vendor exact)"; set_compiler "POST-BUILD INVARIANCE" "PASS"; tar --sort=name --mtime='UTC 1970-01-01' --owner=0 --group=0 --numeric-owner -cf - -C "$POST" app | gzip -n > "$OUT/26614_V1_candidate_app_source.tar.gz"; sha256sum "$OUT/26614_V1_candidate_app_source.tar.gz" > "$OUT/26614_V1_candidate_app_source.tar.gz.sha256"; cp "$CAND_FULL" "$OUT/26614_V1_candidate_full_app.sha256"; cp "$CAND_NATIVE" "$OUT/26614_V1_native_protected_postbuild.sha256"; cp "$CAND_VENDOR" "$OUT/26614_V1_vendor_protected_postbuild.sha256"; set_report "CLEAN ARTIFACT SOURCE EXPORT" "PASS (deterministic candidate tar + manifests)"
}
verify_package
verify_scope
obtain_authority
