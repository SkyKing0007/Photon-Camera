...
if len(sys.argv)!=3: raise SystemExit('usage: transform_26648_r1.py BASE DEST')
base=Path(sys.argv[1]).resolve(); dest=Path(sys.argv[2]).resolve(); root=Path(__file__).resolve().parent
changed=[x for x in (root/'R1_26648_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x]
...
def readm(p):
 ...
 return d
pre=readm(root/'R1_26648_PREWRITE_SOURCE_HASHES.sha256'); post=readm(root/'R1_26648_EXPECTED_CHANGED_SOURCE_HASHES.sha256')
...
payload=root/'handoff_payload_26648_r1'
for rel in changed:
