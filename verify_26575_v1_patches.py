...
def regen(start,target,abbrev):
 td=Path(tempfile.mkdtemp(prefix='p26575_'))
 try:
  init(td,start)
...
def applyprove(start,target,patch):
 td=Path(tempfile.mkdtemp(prefix='a26575_'))
 try:
...
def main():
 ...
 base,cand,fwd,rev=map(Path,sys.argv[1:])
 f=[regen(base,cand,a) for a in (7,12,40)];r=[regen(cand,base,a) for a in (7,12,40)]
 ...
 print('PASS FORWARD PATCH FUZZ=0 exact 26575 candidate recreation')
 print('PASS ROLLBACK PATCH FUZZ=0 exact 26574 recreation')
