...
if len(sys.argv)!=4: raise SystemExit('usage: verify_26674_patches.py ROOT BASE CANDIDATE')
...
def make(first,second):
 with tempfile.TemporaryDirectory(prefix='iris26674_patch_') as td:
...
forward=make(base,cand); rollback=make(cand,base); f=root/'R1_26674_RUNTIME_DELTA_FROM_26673.patch'; r=root/'R1_26674_RUNTIME_ROLLBACK_TO_26673.patch'
assert forward==f.read_bytes(),'forward patch not canonical'; assert rollback==r.read_bytes(),'rollback patch not canonical'
for p in (forward,rollback): assert p.count(b'diff --git ')==11 and b'new file mode ' not in p and b'deleted file mode ' not in p
for label,start,patch,target in [('forward',base,f,cand),('rollback',cand,r,base)]:
 with tempfile.TemporaryDirectory(prefix='iris26674_apply_') as td:
...
print('PASS 26674 deterministic full-index patches core.abbrev=7/12/40; fuzz=0 exact forward/rollback; 11 modifications / 0 additions / 0 deletions')
