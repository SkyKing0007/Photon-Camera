...
def main():
 ...
 v=(cand/'app/version.properties').read_text();need(v,'VERSION_NAME=0.9726580','version name');need(v,'VERSION_BUILD=26580','version build')

 vgn=(cand/CHANGED[0]).read_text();vgn0=(base/CHANGED[0]).read_text()
 for t in ['IRIS_26578_FAIL_CLOSED_REAL_COLOR_GATE','IRIS_26579_MICRO_OBJECT_COLOR_TOPOLOGY','IRIS_26580_MICRO_OBJECT_AREA_VS_RIBBON','IRIS_26580_FAIL_CLOSED_MULTICOLOR_OBJECT_VETO','microTwoAxisEvidence','microCompactAreaEvidence','phaseOverride = smoothstep(0.78, 0.96, phaseLikeEvidence)','microObjectProtection','realColorConfidence','falseColorGate','maximumMove = min(0.050, 0.40 * centerMagnitude)']:
  need(vgn,t,'26580 shared VGN refinement')
 ...
 for t in ['IRIS_26579_TRUE2X_TOPOLOGY_CHROMA_UPSAMPLE','IRIS_26580_TRUE2X_SAME_MATERIAL_CHROMA_OWNERSHIP','IRIS_26580_NEUTRAL_GLYPH_OUTSIDE_EDGE_EXCLUSION','selectorY = mix(sampleY[nearestIndex], directY, 0.60 * confidence)','0.002 + 0.998 * materialWeight','crossEdgeEvidence','neutralSideOwnership','localColorProtection','maxChromaMagnitude','vec3 guideRgb = irisTopologyGuide(globalP, bilinearGuideRgb, directY, confidence)']:
  need(sr,t,'26580 true2x chroma refinement')
 ...
 # Only guide-render chroma ownership may differ in this Kotlin owner.
 for name in ['true2xGuideAndCovariance26564','true2xAccumulate26564','true2xDehomogenize26564','true2xConfidence26564','true2xResolve26564','true2xFlowRefine26574']:
  if name in sr and name in sr0:
   if shader(sr0,name)!=shader(sr,name): fail('unexpected SR embedded shader change '+name)

 cpp0=base/GPU_CPP;cpp=cand/GPU_CPP
 if cpp0.read_bytes()!=cpp.read_bytes() or sha(cpp)!=GPU_CPP_SHA: fail('device-proven 26579 GPU publication C++ changed')
 ct=cpp.read_text()
 ...
 print('PASS exact successful-26579 1708-file authority -> three-file 26580 allowlist')
 print('PASS shared VGN: stronger 2-D multicolor object area-vs-ribbon veto; overwhelming Bayer phase still authoritative')
 print('PASS SR: native VGN chroma only; direct-CFA remains luma-only same-material selector')
 print('PASS SR neutral-side exclusion added without alignment/flow/detail change')
 print('PASS device-proven 26579 GPU publication C++ byte-identical and inherited')
