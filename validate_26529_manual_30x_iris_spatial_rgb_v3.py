...
def validate(base,cand,patch,rollback,postbuild=False):
    ...
    for tok in ('"uGlobalFrameWeight",','frame.calibration.globalFrameWeight','GlesIris26529SpatialRgbChromaPostprocessor','normalizationTargetTexture()','markBandWritten','colorNoiseIir=','renderRgb16ToFloat','rgbChromaPostprocessor?.release()','IRIS_26529_V3_FRAMEBUFFER_DETACH_OWNER'):
        req(tok in st, f'c317 host integration missing: {tok}')
    req(st.count('detachFramebufferColorAttachment(diagnosticFramebuffer)') == 2,
        'expected exactly two diagnostic framebuffer detach calls')
    req(st.count('private fun detachFramebufferColorAttachment(framebuffer: Int)') == 1,
        'diagnostic framebuffer detach helper must be defined exactly once')

    # Generic integration-symbol audit over newly added stacker lines. This specifically prevents
    # the V2 failure class: adding an unqualified helper call that the standalone new-owner compile
    # cannot see. Constructors and Kotlin built-ins are the only allowed non-local call names.
    added_stacker=[]
    in_stacker=False
    for line in Path(patch).read_text().splitlines():
        if line.startswith('--- '):
            in_stacker = line.endswith('/'+STACKER) or line[4:].split('\t',1)[0].endswith(STACKER)
        elif line.startswith('+++ '):
            in_stacker = line[4:].split('\t',1)[0].endswith(STACKER)
        elif in_stacker and line.startswith('+') and not line.startswith('+++'):
            added_stacker.append(line[1:])
    added='\n'.join(added_stacker)
    calls=set(re.findall(r'(?<![.\w])([A-Za-z_][A-Za-z0-9_]*)\s*\(', added))
    defs=set(re.findall(r'\bfun\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(', st))
    allowed_calls={
        'GlesIris26529SpatialRgbChromaPostprocessor','MgcSpatialRgbTile',
        'if','when','for','while','check','require','error','checkNotNull',
        'intArrayOf','longArrayOf','floatArrayOf','listOf','minOf','maxOf',
    }
    missing_calls=sorted(calls-defs-allowed_calls)
    req(not missing_calls, 'new stacker unqualified call(s) lack local owner: '+repr(missing_calls))
...
def main():
    ...
    print('PASS: 26529 V3 independent manual-30x + Iris-owned c317-semantic Spatial RGB validator')
