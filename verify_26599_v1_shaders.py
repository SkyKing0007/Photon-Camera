...
def main():
    ...
    specs=[
      ...
      ('short_region_seed_26599_phase_scoped',extract_kotlin(sc,'shortRegionSeed26595'),'frag'),
      ('short_region_propagate_26594',extract_kotlin(sc,'shortRegionPropagate26594'),'frag'),
      ('short_effective_sensor_loss_handoff_mask_26599',extract_kotlin(sc,'shortRestoreMask26596'),'frag'),
    ...
    if len(specs)!=18: fail('runtime-expanded shader variant count '+str(len(specs)))
    rows=[]; manifest=[]
    ...
    (out/'V1_26599_RUNTIME_EXPANDED_SHADERS.sha256').write_text(''.join(manifest))
    (out/'V1_26599_SHADER_VERIFICATION.json').write_text(json.dumps(rows,indent=2)+'\n')
