...
if len(sys.argv) not in (4,6):raise SystemExit('usage: verify_26663_shaders.py ROOT BASE CANDIDATE [--compiler PATH]')
...
def embedded(rootp):
 ...
 return out
bm=readm(root/'R1_26663_SHADER_UNIVERSE_BASE.sha256');cm=readm(root/'R1_26663_SHADER_UNIVERSE_CANDIDATE.sha256');assert len(bm)==len(cm)==257
...
def expand_asset(r,defines=None):
 ...
 for val in lines:
  ...
  if '#import' in val:raise SystemExit(f'FAIL 26663 unexpected import {r}: {val}')
  if '#define' in val:
...
expected=readm(root/'R1_26663_RUNTIME_EXPANDED_SHADERS.sha256');assert len(expected)==len(variants)==10 and set(expected)==set(variants)
...
with tempfile.TemporaryDirectory(prefix='iris26663_shader_') as td:
 out=Path(td)
 for name,src in sorted(variants.items()):
  ...
  if compiler:
   ...
   if cp.returncode!=0:raise SystemExit(f'26663 GLSL FAIL {name}\n{cp.stdout}\n{cp.stderr}')
print(f'PASS 26663 exact runtime GLSL variants=10; local_laplacian_remap_26621 changed; reserved/structure/exact-hash real_compiler={bool(compiler)} standaloneShaderUniverse=257')
