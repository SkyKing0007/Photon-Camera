...
if len(sys.argv) not in (4,6): raise SystemExit('usage: verify_26653_shaders.py ROOT BASE CANDIDATE [--compiler PATH]')
...
def embedded(rootp):
 ...
 return out
bm=readm(root/'R1_26653_SHADER_UNIVERSE_BASE.sha256');cm=readm(root/'R1_26653_SHADER_UNIVERSE_CANDIDATE.sha256');assert len(bm)==len(cm)==257
for r,h in bm.items(): assert sha(base/r)==h,r
for r,h in cm.items(): assert sha(cand/r)==h,r
standalone_changed={r for r in set(bm)|set(cm) if bm.get(r)!=cm.get(r)}
expected_assets={'app/src/main/assets/shaders/motionv2/color_transform.glsl','app/src/main/assets/shaders/motionv2/render.glsl','app/src/main/assets/shaders/motionv2/local_laplacian_global_log_26621.glsl','app/src/main/assets/shaders/motionv2/gainmap.glsl'}
assert standalone_changed==expected_assets,standalone_changed
# Match GLInterface.readProgram exactly for these assets. They intentionally contain no #import.
def expand_asset(r,defines=None):
 ...
 for val in lines:
  ...
  if '#import' in val: raise SystemExit(f'FAIL 26653 unexpected import in modified shader {r}: {val}')
  if '#define' in val:
...
mods=embedded(cand); fusion=mods['universalNormalMasterShortFusion26651']
variants={'embedded_universalNormalMasterShortFusion26651.frag':fusion,
 'asset_render.frag':expand_asset('app/src/main/assets/shaders/motionv2/render.glsl'),
 'asset_local_laplacian_global_log_26621.frag':expand_asset('app/src/main/assets/shaders/motionv2/local_laplacian_global_log_26621.glsl'),
 'asset_gainmap.frag':expand_asset('app/src/main/assets/shaders/motionv2/gainmap.glsl')}
for hs in (0,1):
...
expected=readm(root/'R1_26653_RUNTIME_EXPANDED_SHADERS.sha256');assert len(expected)==8 and set(expected)==set(variants)
...
with tempfile.TemporaryDirectory(prefix='iris26653_shader_') as td:
 out=Path(td)
 for name,src in sorted(variants.items()):
  ...
  if compiler:
   ...
   if cp.returncode!=0: raise SystemExit(f'26653 GLSL FAIL {name}\n{cp.stdout}\n{cp.stderr}')
print(f'PASS 26653 exact runtime GLSL variants=8 (1 embedded fusion + render/local/gainmap + 4 color-profile define variants) reserved/structure/exact-hash real_compiler={bool(compiler)} standaloneShaderUniverse=257 exactly4changed')
