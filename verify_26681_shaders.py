...
# Permanent exact regression for the real 26681 glslang failure: do not allow the
# local chroma shader to reintroduce the global uniform name `imageSize`.
# Do not generalize this into a blanket ban on GLSL built-in function names:
# pinned upstream SPEKTRA legally uses local identifiers such as `step`, and
# the real pinned compiler below remains the authority for language legality.
for name,s in variants.items():
 scan(name,s)
 clean=re.sub(r'/\*.*?\*/',' ',s,flags=re.S);clean=re.sub(r'//.*',' ',clean)
 if re.search(r'\buniform\b[^;{}]*\bimageSize\b',clean):
  raise SystemExit(f'FAIL {name} regression: global uniform imageSize reintroduced')
 assert sha_bytes(s.encode())==expected[name],name
