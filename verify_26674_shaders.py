...
if len(sys.argv) not in (4,6): raise SystemExit('usage: verify_26674_shaders.py ROOT BASE CANDIDATE [--compiler PATH]')
...
def variants(rootdir):
 ...
 return out
bm=readm(root/'R1_26674_SHADER_UNIVERSE_BASE.sha256'); cm=readm(root/'R1_26674_SHADER_UNIVERSE_CANDIDATE.sha256'); assert len(bm)==len(cm)==257
...
expected=readm(root/'R1_26674_RUNTIME_EXPANDED_SHADERS.sha256'); assert len(expected)==12 and set(expected)==set(cv)
changed_variants=sorted(k for k in bv if hashlib.sha256(bv[k].encode()).hexdigest()!=hashlib.sha256(cv[k].encode()).hexdigest())
expected_changed=[x for x in (root/'R1_26674_RUNTIME_EXPANDED_CHANGED_VARIANTS.txt').read_text().splitlines() if x]; assert changed_variants==expected_changed,(changed_variants,expected_changed)
...
with tempfile.TemporaryDirectory(prefix='iris26674_shader_') as td:
 o=Path(td)
 for n,s in sorted(cv.items()):
  ...
  if compiler:
   ...
   if cp.returncode: raise SystemExit(f'26674 GLSL FAIL {n}\n{cp.stdout}\n{cp.stderr}')
print(f'PASS 26674 exact runtime GLSL variants=12 changedVariants={len(changed_variants)} complete reserved/structure/exact-hash real_compiler={bool(compiler)} standaloneShaderUniverse=257')
