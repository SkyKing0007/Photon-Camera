...
import hashlib, sys
if len(sys.argv)!=4: raise SystemExit('usage: verify_26628_r1_authority.py ROOT BASE CAND')
...
def verify(root,name,count):
    ...
    return rows
base=verify(B,'R1_26628_BASE_26627_R1_FULL_APP.sha256',1713)
cand=verify(C,'R1_26628_EXPECTED_CANDIDATE_FULL_APP.sha256',1713)
...
for n,count,root in [
('R1_26628_PROTECTED_UNCHANGED_BASE.sha256',1704,B),('R1_26628_PROTECTED_UNCHANGED_CANDIDATE.sha256',1704,C),
('R1_26628_NATIVE_PROTECTED_BASE.sha256',802,B),('R1_26628_NATIVE_PROTECTED_CANDIDATE.sha256',802,C),
('R1_26628_VENDOR_PROTECTED_BASE.sha256',778,B),('R1_26628_VENDOR_PROTECTED_CANDIDATE.sha256',778,C),
('R1_26628_DNG_BASE.sha256',7,B),('R1_26628_DNG_CANDIDATE.sha256',7,C),
('R1_26628_PREWRITE_SOURCE_HASHES.sha256',9,B),('R1_26628_EXPECTED_CHANGED_SOURCE_HASHES.sha256',9,C),
]: verify(root,n,count)
for a,b in [
('R1_26628_PROTECTED_UNCHANGED_BASE.sha256','R1_26628_PROTECTED_UNCHANGED_CANDIDATE.sha256'),
...
changed=[x.strip() for x in (ROOT/'R1_26628_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x.strip()]
if len(changed)!=9: raise SystemExit('FAIL runtime allowlist count')
added=[x.strip() for x in (ROOT/'R1_26628_ADDED_PATHS_MUST_BE_ABSENT.txt').read_text().splitlines() if x.strip()]
...
native_paths={r for _,r in read_manifest('R1_26628_NATIVE_PROTECTED_BASE.sha256')}
...
print('PASS 26628 authority manifests 1713 base / 1713 candidate / 1704 protected / 802 native / 778 vendor / 7 DNG / 9 runtime delta')
