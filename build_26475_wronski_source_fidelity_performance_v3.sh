...
PROTECTED_HEAD="0937a8e5731b3fd58616e0fb77f34184aaa56c2c"
...
python3 - "$HASH_26474" "$HASH_AFTER" <<'PY_HASH'
from pathlib import Path
import sys
...
changed={n for n in all_names if before.get(n)!=after.get(n)}
unexpected=sorted(changed-allowed)
if unexpected:
    raise SystemExit("protected-file hash mismatch outside allowlist: "+", ".join(unexpected))
...
if missing:
    raise SystemExit("expected 26475 source change missing: "+", ".join(missing))

for new_file in [
    'app/src/main/assets/shaders/motionv2/mfsr_ica_reference_gradient.glsl',
    'app/src/main/assets/shaders/motionv2/mfsr_ica_reference_hessian.glsl',
]:
    if new_file not in after:
        raise SystemExit("expected new 26475 file missing: "+new_file)
