...
APK_NAME="IrisCamera-${NEW_VERSION}-${NEW_BUILD}-wronski-reference-uhdr-edgeaware-debug.apk"
...
echo "=== 26472 GUARDED WRONSKI REFERENCE COMPLETION / EDGE-AWARE UHDR BUILD ==="
date -Iseconds || true
...
/* IRIS_26472_EDGE_AWARE_3X3_LOG_GAINMAP
 * Keep 26471's low-frequency log-gain concept and no HDR-only microcontrast,
 * but use a compact 3x3 filter at gain-map resolution. SDR log-luminance is
 * the structural guide: smoothing remains strong within a surface and falls
 * rapidly across real SDR edges. This prevents broad HDR gain bleeding while
 * still suppressing isolated one-cell gain spikes.
 */
const float UHDR_OFFSET=0.015625;
const float GUIDE_SIGMA=0.22;
...
float spatial1(int x){return x==0?2.0:1.0;}

float sdrGuide(vec2 uv){
    vec3 sdr=srgbDecode(texture(SdrBuffer,clamp(uv,vec2(0.0),vec2(1.0))).rgb);
...
void main(){
    ...
    float centerGuide=sdrGuide(uv);
    float sum=0.0,wsum=0.0;

    for(int oy=-1;oy<=1;oy++) for(int ox=-1;ox<=1;ox++){
        vec2 q=uv+vec2(float(ox),float(oy))/gm;
        float dg=sdrGuide(q)-centerGuide;
        float edgeWeight=exp(-0.5*(dg*dg)/(GUIDE_SIGMA*GUIDE_SIGMA));
        float w=spatial1(ox)*spatial1(oy)*edgeWeight;
        sum+=w*localLogGain(q,maxLog);
        wsum+=w;
    }

    float encoded=clamp((sum/max(wsum,1.0e-6))/maxLog,0.0,1.0);
...
if "IRIS_26472_EDGE_AWARE_3X3_LOG_GAINMAP" not in gain:
    raise SystemExit("edge-aware UHDR missing")
out.write_text(
...
- 26471 blind 5x5 gain-map smoothing replaced by edge-aware 3x3
""")
...
'app/src/main/assets/shaders/motionv2/gainmap.glsl':
    ['IRIS_26472_EDGE_AWARE_3X3_LOG_GAINMAP','GUIDE_SIGMA=0.22',
     'for(int oy=-1;oy<=1;oy++)','edgeWeight'],
...
- 26472 gain map is compact SDR-guided 3x3 log-gain regularization
...
for marker in \
  ...
  IRIS_26472_WRONSKI_PUBLIC_ACCUMULATED_ROBUSTNESS_REFERENCE_MERGE \
  IRIS_26472_WRONSKI_MINIMAL_CENSORED_HIGHLIGHT_FINALIZER \
  IRIS_26472_EDGE_AWARE_3X3_LOG_GAINMAP; do
...
- New 3x3 SDR-guided log-gain regularizer limits smoothing across real SDR structure.
- HDR-only microcontrast remains removed.
- No sharpening is introduced.
