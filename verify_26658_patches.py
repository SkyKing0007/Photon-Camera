...
if len(sys.argv)!=4: raise SystemExit('usage: verify_26658_patches.py ROOT BASE CANDIDATE')
root=Path(sys.argv[1]).resolve();base=Path(sys.argv[2]).resolve();cand=Path(sys.argv[3]).resolve()
fwd=(root/'R1_26658_RUNTIME_DELTA_FROM_26653.patch').read_bytes();rev=(root/'R1_26658_RUNTIME_ROLLBACK_TO_26653.patch').read_bytes()
allow={x for x in (root/'R1_26658_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x};added={x for x in (root/'R1_26658_ADDED_PATHS_MUST_BE_ABSENT.txt').read_text().splitlines() if x}
assert len(allow)==6 and not added
def H(r): return {str(p.relative_to(r)):hashlib.sha256(p.read_bytes()).hexdigest() for p in sorted((r/'app').rglob('*')) if p.is_file()}
def run(a,cwd): return subprocess.run(a,cwd=cwd,check=True,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
bh,ch=H(base),H(cand);assert len(bh)==len(ch)==1721;assert {r for r in set(bh)|set(ch) if bh.get(r)!=ch.get(r)}==allow
for data in (fwd,rev):
 inds=re.findall(r'^index ([0-9a-f]{40})\.\.([0-9a-f]{40})',data.decode(),re.M);assert len(inds)==6,inds
with tempfile.TemporaryDirectory(prefix='iris26658_patch_') as td:
...
print('PASS 26658 deterministic full-index forward/rollback proof: core.abbrev 7/12/40 identical, exact apply/rollback, 6-path allowlist / 0 additions')
