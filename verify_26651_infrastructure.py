...
if len(sys.argv)!=3: raise SystemExit('usage: verify_26651_infrastructure.py BUILD_SCRIPT WORKFLOW')
build=Path(sys.argv[1]);workflow=Path(sys.argv[2]);bs=build.read_text();wf=workflow.read_text()
# Exact successful 26650 compiler/build order; implementation validators may change identity/scope only.
required=[
...
for t in required:
 if t not in bs: raise SystemExit('FAIL inherited successful-26650 mechanic: '+t)
...
for t in order:
 p=bs.find(t,pos+1)
 if p<0 or p<=pos: raise SystemExit('FAIL successful-26650 ordering: '+t)
 pos=p
...
for t in ['RUNTIME_AUTHORITY_COMMIT="eadba2036e758a3ae8633ce4c2743d7c4a624b39"','BASE_RUN_ID="35111917372"','BASE_ARTIFACT_ID="10453400320"','BASE_ARTIFACT_SHA="b23c9bbe4b7dd55a55f792ee3a07bb386355e2e6c9d1b579307a63d752b830be"','BASE_TAR_SHA="5dbc20d0c41bb9d2c7871cc24e4399b77584d1740434c74a862435c2f60efe0a"']:
 if t not in bs: raise SystemExit('FAIL exact successful-26650 authority contract: '+t)
...
# When running in the upload checkout, authenticate the exact successful 26650 mechanics blobs and compare key mechanics with git-show authority.
gitroot=build.parent
if (gitroot/'.git').exists():
 commit='eadba2036e758a3ae8633ce4c2743d7c4a624b39'
 refs=[('build_26650_r1_highlight_compression_toggle.sh','76a111d99c4da837eab88432a2f1301c46a84b26'),('.github/workflows/build-26650-r1-highlight-compression-toggle.yml','b6aefd0715db361213499eea220fc1259a3b8203')]
 for path,exp in refs:
  got=subprocess.check_output(['git','rev-parse',f'{commit}:{path}'],cwd=gitroot,text=True).strip()
  if got!=exp: raise SystemExit(f'FAIL 26650 mechanics blob {path}: {got} != {exp}')
 oldbs=subprocess.check_output(['git','show',f'{commit}:build_26650_r1_highlight_compression_toggle.sh'],cwd=gitroot,text=True)
 oldwf=subprocess.check_output(['git','show',f'{commit}:.github/workflows/build-26650-r1-highlight-compression-toggle.yml'],cwd=gitroot,text=True)
 for t in required:
  if t not in oldbs: raise SystemExit('FAIL claimed 26650 mechanic absent in authenticated script: '+t)
 for t in ['runs-on: ubuntu-24.04','uses: actions/checkout@v5','fetch-depth: 0','uses: actions/setup-java@v5',"java-version: '17'",'uses: actions/setup-python@v5',"python-version: '3.12'",'uses: actions/upload-artifact@v4','retention-days: 90','if-no-files-found: error']:
  if t not in oldwf: raise SystemExit('FAIL claimed 26650 workflow mechanic absent: '+t)
print('PASS 26651 infrastructure: successful 26650 compiler/native/patch/PRE-BUILD/assemble/postbuild order retained exactly; pinned environment retained; runtime authority advanced only to exact successful 26650 compiled artifact; no raw-shader shortcut')
