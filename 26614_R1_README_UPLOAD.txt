PHOTON / IRIS 26614 R1 — CANONICAL SDR/UHDR APPEARANCE + PHYSICAL CFA COLOR VALIDITY

UPLOAD TARGET
Branch: experimental-clean-photon-rebuild
Target version/build: 0.9726614 / 26614
Commit message: 26614 R1 repair nested candidate transform isolation
Backup: NONE (user explicitly requested no backup)


26614 V1 FAILURE / R1 REPAIR
Failed V1 handoff commit: 2a5db1fbacbc0835eddf691550a478070334e624
V1 runtime candidate math is NOT changed by R1. The failure occurred before compilers because plain git apply ran from a candidate directory nested under the checkout and discovered the parent .git, returning success without modifying the isolated candidate.
R1 keeps the exact same 12-file runtime candidate hashes and canonical forward/rollback patches, but forces patch execution into repository-less mode with GIT_CEILING_DIRECTORIES and proves git rev-parse cannot find a worktree before application.
This changed infrastructure is replayed against the exact successful 26613 V1.1 artifact inside a real nested Git worktree before packaging.
...
.github/workflows/build-26614-r1-canonical-appearance-cfa-validity.yml
