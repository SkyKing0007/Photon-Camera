...
if len(sys.argv)!=4: raise SystemExit('usage: verify_26652_patches.py ROOT BASE CANDIDATE')
root=Path(sys.argv[1]).resolve();base=Path(sys.argv[2]).resolve();cand=Path(sys.argv[3]).resolve()
fwd=(root/'R1_26652_RUNTIME_DELTA_FROM_26651.patch').read_bytes();rev=(root/'R1_26652_RUNTIME_ROLLBACK_TO_26651.patch').read_bytes()
allow={x for x in (root/'R1_26652_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x}; added={x for x in (root/'R1_26652_ADDED_PATHS_MUST_BE_ABSENT.txt').read_text().splitlines() if x}
...
for data in (fwd,rev):
 inds=re.findall(r'^index ([0-9a-f]{40})\.\.([0-9a-f]{40})',data.decode(),re.M); assert len(inds)==4
with tempfile.TemporaryDirectory(prefix='iris26652_patch_') as td:
...
print('PASS 26652 deterministic full-index forward/rollback proof: core.abbrev 7/12/40 identical, exact apply/rollback, 4-path allowlist / 0 additions')
