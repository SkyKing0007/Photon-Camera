...
if len(sys.argv) not in (4,6): raise SystemExit('usage: verify_26669_shaders.py ROOT BASE CANDIDATE [--compiler PATH]')
...
def variants(rootdir):
    ...
    out={
      'embedded_sabre_rejection26668.frag':mods['rejection'],
      'embedded_sabre_merge26668.frag':mods['merge'],
    ...
    for hs in (0,1):
        for lk in (0,1):
            out[f'asset_color_transform_huesat{hs}_look{lk}.frag']=expand(rootdir,'app/src/main/assets/shaders/motionv2/color_transform.glsl',{'USE_PROFILE_HUESAT':str(hs),'USE_PROFILE_LOOK':str(lk)})
    return out

bm=readm(root/'R1_26669_SHADER_UNIVERSE_BASE.sha256'); cm=readm(root/'R1_26669_SHADER_UNIVERSE_CANDIDATE.sha256')
...
assert shader_delta==['app/src/main/assets/shaders/preview/main_fs.glsl'],shader_delta

bv=variants(base); cv=variants(cand)
assert len(bv)==len(cv)==12 and set(bv)==set(cv)
changed_variants=sorted(k for k in bv if hashlib.sha256(bv[k].encode()).hexdigest()!=hashlib.sha256(cv[k].encode()).hexdigest())
assert changed_variants==['asset_preview_main_fs.frag'],changed_variants
expected=readm(root/'R1_26669_RUNTIME_EXPANDED_SHADERS.sha256')
assert len(expected)==12 and set(expected)==set(cv)
...
with tempfile.TemporaryDirectory(prefix='iris26669_shader_') as td:
    o=Path(td)
    for n,s in sorted(cv.items()):
        ...
        if compiler:
            ...
            if cp.returncode: raise SystemExit(f'26669 GLSL FAIL {n}\n{cp.stdout}\n{cp.stderr}')
print(f'PASS 26669 exact runtime GLSL variants=12; only preview runtime variant changed; inherited Sabre rejection/merge/hardened SHORT/render/gainmap/color variants byte-pinned; reserved/structure/exact-hash real_compiler={bool(compiler)} standaloneShaderUniverse=257')
