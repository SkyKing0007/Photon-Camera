...
NODE = r'''package com.particlesdevs.photoncamera.processing.opengl.postpipeline;
 ...
 * common extended linear-sRGB and before the solved display gain/tone/highlight/gamut render.
 *
...
public final class MotionV2AdaptiveColorAppearance extends Node {
    private static final float MAX_WEAK_CHROMA_GAIN = 1.22f;
    ...
    public void Run() {
        ...
        Log.i(Name, "IRIS_26563_UNIVERSAL_ADAPTIVE_COLOR_APPEARANCE"
 ...
 * IRIS_26563_UNIVERSAL_ADAPTIVE_COLOR_APPEARANCE
 *
 * Common extended linear-sRGB appearance stage. It must be able to increase weak legitimate
 * chroma, but may not invent color on neutrals, rotate hue, alter Rec.709 linear luminance,
...
void main() {
    ...
    /* Hard gamut/highlight safety. A pixel already at/above linear display gamut, or predicted to
     * cross it after the already-solved display exposure, gets exactly 1x colorfulness gain.
     * Otherwise cap the chroma-axis scalar before any RGB component could cross 0 or 1.
     */
    ...
    float adaptiveChromaGain = clamp(min(requestedGain, gamutGainLimit), 1.0, 1.22);
    if (adaptiveChromaGain <= 1.000001) {
        Output = centerRgb;
        return;
    }

    /* Scaling one chroma vector around its own Rec.709 luminance preserves that luminance and
...
def transform(root: Path):
    ...
    node = app / 'src/main/java/com/particlesdevs/photoncamera/processing/opengl/postpipeline/MotionV2AdaptiveColorAppearance.java'
    shader = app / 'src/main/assets/shaders/motionv2/adaptive_color_appearance_26563.glsl'
    version = app / 'version.properties'

    text = post.read_text()
    night_old = '''            add(new MotionV2ViewfinderExposureMatcher());\n            add(new StageTelemetry("IRIS_26550_NIGHT_PRESENTATION_SOLVE"));\n            add(new MotionV2DisplayExposure());'''
    night_new = '''            add(new MotionV2ViewfinderExposureMatcher());\n            add(new StageTelemetry("IRIS_26550_NIGHT_PRESENTATION_SOLVE"));\n            /* IRIS_26563_UNIVERSAL_ADAPTIVE_COLOR_APPEARANCE\n             * Device-specific profile color is already complete and the exposure solver has read\n             * the untouched calibrated image. Restore only reliable weak colorfulness in common\n             * extended linear-sRGB before display exposure/tone/highlight/gamut rendering.\n             */\n            add(new MotionV2AdaptiveColorAppearance());\n            add(new StageTelemetry("IRIS_26563_NIGHT_ADAPTIVE_COLOR_APPEARANCE"));\n            add(new MotionV2DisplayExposure());'''
    text = replace_once(text, night_old, night_new, 'Night insertion')

    motion_old = '''            add(new MotionV2ViewfinderExposureMatcher());\n            add(new StageTelemetry("V2_POST_VIEWFINDER_EXPOSURE_SOLVE"));\n            add(new MotionV2DisplayExposure());'''
    motion_new = '''            add(new MotionV2ViewfinderExposureMatcher());\n            add(new StageTelemetry("V2_POST_VIEWFINDER_EXPOSURE_SOLVE"));\n            /* IRIS_26563_UNIVERSAL_ADAPTIVE_COLOR_APPEARANCE\n             * Shared Motion/SR appearance stage in common extended linear-sRGB. It runs exactly\n             * once after device profile conversion/exposure solve and before presentation gain.\n             */\n            add(new MotionV2AdaptiveColorAppearance());\n            add(new StageTelemetry("IRIS_26563_MOTION_ADAPTIVE_COLOR_APPEARANCE"));\n            add(new MotionV2DisplayExposure());'''
    text = replace_once(text, motion_old, motion_new, 'Motion insertion')

    old_nodes = 'nodes=MotionV2CfaInput,DirectRGB-or-CFAFallback,OwnerSpecificPreColor,MotionV2ColorTransform,MotionV2ViewfinderExposureMatcher,MotionV2DisplayExposure,IrisManualOptional,MotionV2Render,RotateWatermark'
    new_nodes = 'nodes=MotionV2CfaInput,DirectRGB-or-CFAFallback,OwnerSpecificPreColor,MotionV2ColorTransform,MotionV2ViewfinderExposureMatcher,MotionV2AdaptiveColorAppearance,MotionV2DisplayExposure,IrisManualOptional,MotionV2Render,RotateWatermark'
    text = replace_once(text, old_nodes, new_nodes, 'Motion graph trace')
    post.write_text(text)

    if node.exists():
        raise SystemExit('new node unexpectedly already exists')
    node.write_text(NODE)
    if shader.exists():
        raise SystemExit('new shader unexpectedly already exists')
    shader.write_text(SHADER)

    v = version.read_text()
    v = replace_once(v, 'VERSION_NAME=0.9726562', 'VERSION_NAME=0.9726563', 'version name')
    v = replace_once(v, 'VERSION_BUILD=26562', 'VERSION_BUILD=26563', 'version build')
...
def main():
    ...
    p.add_argument('root')
    args=p.parse_args()
    transform(Path(args.root))
    print('26563 transform applied')
