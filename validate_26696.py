...
import hashlib,sys
if len(sys.argv)!=3:raise SystemExit('usage: validate_26696.py BASE26695 CANDIDATE')
...
B=H(b/'app');C=H(c/'app');changed=sorted('app/'+x for x in set(B)|set(C) if B.get(x)!=C.get(x));exp=sorted(x.strip() for x in (pkg/'26696_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x.strip());added=[x.strip() for x in (pkg/'26696_ADDED_PATHS_MUST_BE_ABSENT.txt').read_text().splitlines() if x.strip()]
...
print('PASS 26696 semantic/ownership validation: 1.1.2 generation+poll AE; actual-frame solver; async preview/export warmup; Iris processing ring; async CameraDevice release')
