...
if len(sys.argv)!=4: raise SystemExit('usage: verify_26678_patches.py ROOT BASE CANDIDATE')
...
def make(first,second):
 with tempfile.TemporaryDirectory(prefix='iris26678_patch_') as td:
  ...
  untracked=run(['git','ls-files','--others','--exclude-standard','--','app'],t,stdout=subprocess.PIPE,text=True).stdout.splitlines()
  for rel in untracked: run(['git','add','-N','--',rel],t)
  outs=[]
  for ab in ('7','12','40'):
...
forward=make(base,cand); rollback=make(cand,base); f=root/'R1_26678_RUNTIME_DELTA_FROM_26677.patch'; r=root/'R1_26678_RUNTIME_ROLLBACK_TO_26677.patch'
assert forward==f.read_bytes(),'forward patch not canonical'; assert rollback==r.read_bytes(),'rollback patch not canonical'
assert forward.count(b'diff --git ')==10 and rollback.count(b'diff --git ')==10
assert forward.count(b'new file mode ')==2 and forward.count(b'deleted file mode ')==1
assert rollback.count(b'new file mode ')==1 and rollback.count(b'deleted file mode ')==2
for label,start,patch,target in [('forward',base,f,cand),('rollback',cand,r,base)]:
 with tempfile.TemporaryDirectory(prefix='iris26678_apply_') as td:
...
print('PASS 26678 deterministic full-index patches core.abbrev=7/12/40; fuzz=0 exact forward/rollback; 7 modified / 2 added / 1 deleted')
