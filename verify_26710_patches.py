...
pkg=Path(__file__).resolve().parent;base=Path(sys.argv[2]);cand=Path(sys.argv[3]);f=pkg/'26710_RUNTIME_DELTA_FROM_26709.patch';r=pkg/'26710_RUNTIME_ROLLBACK_TO_26709.patch'
...
BH,CH=H(base),H(cand)
with tempfile.TemporaryDirectory(prefix='i26710_') as td:
...
print('PASS 26710 deterministic full-index forward/rollback patches core.abbrev 7/12/40 fuzz=0 exact replay/rollback')
