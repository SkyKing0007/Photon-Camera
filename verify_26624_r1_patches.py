...
if len(sys.argv)!=5: raise SystemExit('usage: verify_26624_r1_patches.py BASE CAND FWD ROLLBACK')
...
with tempfile.TemporaryDirectory() as td:
    ...
    subprocess.run(['git','init','-q'],cwd=repo,check=True); subprocess.run(['git','config','user.name','Photon26624'],cwd=repo,check=True); subprocess.run(['git','config','user.email','photon26624@example.invalid'],cwd=repo,check=True)
...
print('PASS 26624 deterministic full-index patches core.abbrev=7/12/40; fuzz=0 exact forward/rollback; diff --check clean')
