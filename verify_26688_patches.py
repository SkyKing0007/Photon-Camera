...
if len(sys.argv)!=4: raise SystemExit('usage: verify_26688_patches.py ROOT BASE CAND')
root,base,cand=map(Path,sys.argv[1:]); fwd=(root/'R1_26688_RUNTIME_DELTA_FROM_26687_R1_1.patch').read_bytes(); rb=(root/'R1_26688_RUNTIME_ROLLBACK_TO_26687_R1_1.patch').read_bytes()
...
with tempfile.TemporaryDirectory() as td:
    ...
    for rel in (root/'R1_26688_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines():
        if not rel: continue
        dst=t/rel
        if (cand/rel).is_file(): dst.parent.mkdir(parents=True,exist_ok=True); shutil.copy2(cand/rel,dst)
...
print('PASS 26688 deterministic full-index forward/rollback patches at core.abbrev 7/12/40; fuzz=0 exact forward and rollback')
