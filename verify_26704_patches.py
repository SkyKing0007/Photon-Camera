...
if len(sys.argv)!=4:raise SystemExit('usage: verify_26704_patches.py ROOT BASE26703 CANDIDATE')
base=Path(sys.argv[2]);cand=Path(sys.argv[3]);pkg=Path(__file__).resolve().parent;f=pkg/'26704_RUNTIME_DELTA_FROM_26703.patch';r=pkg/'26704_RUNTIME_ROLLBACK_TO_26703.patch'
...
BH=H(base);CH=H(cand)
with tempfile.TemporaryDirectory(prefix='i26704_') as td:
...
print('PASS 26704 deterministic full-index forward/rollback patches at core.abbrev 7/12/40; fuzz=0 exact replay + rollback')
