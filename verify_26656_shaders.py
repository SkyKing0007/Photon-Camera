...
if len(sys.argv) not in (4,6): raise SystemExit('usage: verify_26656_shaders.py ROOT BASE CANDIDATE [--compiler PATH]')
...
bm=readm(root/'R1_26656_SHADER_UNIVERSE_BASE.sha256');cm=readm(root/'R1_26656_SHADER_UNIVERSE_CANDIDATE.sha256');assert len(bm)==257 and len(cm)==262
...
expected_hash=readm(root/'R1_26656_RUNTIME_EXPANDED_SHADERS.sha256');assert len(expected_hash)==9 and set(expected_hash)==set(variants)
...
with tempfile.TemporaryDirectory(prefix='iris26656_shader_') as td:
 out=Path(td)
 for name,src in sorted(variants.items()):
  ...
  if compiler:
   ...
   if cp.returncode!=0: raise SystemExit(f'26656 GLSL FAIL {name}\n{cp.stdout}\n{cp.stderr}')
print(f'PASS 26656 exact runtime GLSL variants=9 reserved/structure/exact-hash real_compiler={bool(compiler)} standaloneShaderUniverse=257->262 exactly6changed/added')
