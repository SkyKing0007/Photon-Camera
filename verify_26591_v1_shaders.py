...
def all_embedded(src):
 out={};pat=re.compile(r'(?m)^\s*(?:private\s+)?val\s+([A-Za-z_]\w*)\s*=\s*"""')
 for m in pat.finditer(src):
  a=m.end();z=src.find('""".trimIndent()',a)
  if z<0: fail('unterminated '+m.group(1))
  out[m.group(1)]=trim_indent(src[a:z])
 return out
...
def main():
 ...
 # 26591 must not change any embedded Sabre shader. The Kotlin stacker delta is telemetry only.
 sb=(b/'app/src/main/java/com/hinnka/mycamera/processor/GlesMgcRawSabreShaders.kt').read_text();sc=(root/'app/src/main/java/com/hinnka/mycamera/processor/GlesMgcRawSabreShaders.kt').read_text();eb,ec=all_embedded(sb),all_embedded(sc)
 if eb!=ec:fail('embedded Sabre shader universe/content changed from successful 26590')
 ...
 specs=[
  ...
  ('short_restore_mask_26590',extract_kotlin(sc,'shortRestoreMask26587'),'frag'),
  ('short_restore_mask_probe_26590',extract_kotlin(sc,'shortRestoreMaskProbe26590'),'frag'),
  ('short_restore_rgba16f_26587',extract_kotlin(sc,'shortRestoreRgba16f26587'),'frag')]
 ...
 # Exact GLInterface.readProgram runtime expansion for the modified render asset: no imports/defines,
 # so prepend glVersion + blank line + #line 1 exactly.
 rb=(b/'app/src/main/assets/shaders/motionv2/render.glsl').read_text();rc=(root/'app/src/main/assets/shaders/motionv2/render.glsl').read_text()
 if rb==rc:fail('26591 render shader did not change')
 if '#import' in rc or '#version' in rc or '#define' in rc:fail('render runtime expansion assumptions changed')
 specs.append(('motionv2_render_26591',prefix+rc,'frag'))
 rows=[];manifest=[]
 ...
 (out/'V1_26591_RUNTIME_EXPANDED_SHADERS.sha256').write_text(''.join(manifest));(out/'V1_26591_SHADER_VERIFICATION.json').write_text(json.dumps(rows,indent=2)+'\n')
 print('PASS all embedded Sabre/VGN/SHORT shaders byte-identical to successful 26590; stacker delta is telemetry only')
 print('PASS exact runtime-expanded shader extraction + complete reserved scan variants=12 including modified motionv2/render.glsl')
