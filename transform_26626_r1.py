...
if len(sys.argv)!=3: raise SystemExit('usage: transform_26626_r1.py BASE OUT')
...
def verify(root, manifest):
    ...
    return len(lines)
if verify(base,'R1_26626_PREWRITE_SOURCE_HASHES.sha256')!=5: raise SystemExit('prewrite count')
added=[x.strip() for x in (P/'R1_26626_ADDED_PATHS_MUST_BE_ABSENT.txt').read_text().splitlines() if x.strip()]
if added: raise SystemExit('26626 adds no runtime paths')
patch=P/'R1_26626_RUNTIME_DELTA_FROM_26625_R3.patch'
# Preserve successful 26624 nested-candidate isolation: candidate cannot discover carrier .git.
...
if verify(out,'R1_26626_EXPECTED_CHANGED_SOURCE_HASHES.sha256')!=5: raise SystemExit('candidate changed count')
print('PASS 26626 deterministic candidate reconstruction from exact successful 26625 R3 + canonical full-index patch')
