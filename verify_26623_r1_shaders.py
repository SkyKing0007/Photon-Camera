...
if len(sys.argv)<5 or sys.argv[3] != '--out': raise SystemExit('usage: verify_26623_r1_shaders.py BASE CAND --out OUT [--compiler PATH]')
...
asset('app/src/main/assets/shaders/motionv2/adaptive_color_appearance_26563.glsl','adaptive_color_appearance_26623_profile0')
asset('app/src/main/assets/shaders/motionv2/adaptive_color_appearance_26563.glsl','adaptive_color_appearance_26623_profile1',{'CALIBRATED_PROFILE':'1'})
asset('app/src/main/assets/shaders/motionv2/local_laplacian_global_log_26621.glsl','local_laplacian_26623_global_log')
asset('app/src/main/assets/shaders/motionv2/render.glsl','motionv2_render_26623')
...
(O/'R1_26623_RUNTIME_EXPANDED_SHADERS.sha256').write_text('\n'.join(lines)+'\n')
results=[]
...
(O/'R1_26623_SHADER_VERIFICATION.json').write_text(json.dumps({'count':len(results),'compiler':compiler,'results':results},indent=2,sort_keys=True)+'\n')
print(f'PASS 26623 exact runtime-expanded shader extraction/reserved scan variants={len(results)} real_compiler={bool(compiler)}')
