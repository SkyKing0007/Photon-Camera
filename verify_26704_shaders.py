...
if len(sys.argv) not in (4,6):raise SystemExit('usage: verify_26704_shaders.py ROOT BASE CANDIDATE [--compiler PATH]')
...
def expand_render():
 # Exact GLInterface.readProgram result for motionv2/render: no defines/imports, GLProg.glVersion 310 es.
 s=(cand/'app/src/main/assets/shaders/motionv2/render.glsl').read_text();lines=s.splitlines();versioned=False;out=[]
 for val in lines:
  if '#version' in val:versioned=True
  if '#import' in val:raise SystemExit('render gained #import; verifier must model exact imported source before allowing build')
  out.append(val+'\n')
 return ('' if versioned else '#version 310 es\n\n#line 1\n')+''.join(out)
def extract_cpp():
 s=(cand/'app/src/main/cpp/motionv2_jpeg444_jni.cpp').read_text();a='static const char*kIris26571PublicationCompute=R"GLSL('
 i=s.find(a)
 if i<0:raise SystemExit('missing true2x publication shader')
 i+=len(a);j=s.find(')GLSL";',i)
 if j<0:raise SystemExit('unterminated true2x publication shader')
 return s[i:j]
variants=[
 ('sabre_universal_normal_master_short_fusion_26704.frag',extract_kotlin('universalNormalMasterShortFusion26651'),'frag'),
 ('motionv2_render_26704.frag',expand_render(),'frag'),
 ('motionv2_true2x_publication_26704.comp',extract_cpp(),'comp'),
]
pins=readm(root/'26704_RUNTIME_EXPANDED_SHADERS.sha256');assert len(pins)==3
sb=readm(root/'26704_ASSET_SHADER_UNIVERSE_BASE.sha256');sc=readm(root/'26704_ASSET_SHADER_UNIVERSE_CANDIDATE.sha256');assert len(sb)==len(sc)==271
assert {r for r in sb if sb[r]!=sc[r]}=={'app/src/main/assets/shaders/motionv2/render.glsl'}
...
def scan(name,src):
 ...
 if name.startswith('sabre_'):
  for token in ['IRIS_26704_FAIL_CLOSED_INFERRED_SHORT_RADIANCE','IRIS_26704_LOW_NORMAL_CONFIDENCE_NEVER_BOOSTS_SHORT','IRIS_26704_VALID_NORMAL_CHROMA_IMMUTABLE','float colorRecoveryNeed = allColorSupportLost;']:
   if token not in src:raise SystemExit(f'FAIL {name}: missing {token}')
  if 'visualColorDeficit' in src:raise SystemExit(f'FAIL {name}: stale visualColorDeficit')
 if name.startswith('motionv2_render_'):
  for token in ['uniform float iris26704BodyToneStrength;','float iris26704BodyToneGuide(float mappedGuide)','mappedGuide=iris26704BodyToneGuide(mappedGuide);']:
   if token not in src:raise SystemExit(f'FAIL {name}: missing {token}')
 if name.startswith('motionv2_true2x_'):
  for token in ['uniform float uBodyToneStrength;','IRIS_26704_RANGE_SEPARATED_BODY_TONE_TRUE2X_GPU','globalMapped=iris26704BodyToneGuide(globalMapped);']:
   if token not in src:raise SystemExit(f'FAIL {name}: missing {token}')
with tempfile.TemporaryDirectory(prefix='iris26704_glsl_') as td:
 d=Path(td)
 for name,src,stage in variants:
  ...
  if compiler:
   ...
   if cp.returncode!=0:raise SystemExit(f'26704 GLSL FAIL {name}\n{cp.stdout}\n{cp.stderr}')
print(f'PASS 26704 runtime-expanded GLSL variants=3 complete reserved/structure/exact-hash real_compiler={bool(compiler)} assetShaderUniverse=271')
