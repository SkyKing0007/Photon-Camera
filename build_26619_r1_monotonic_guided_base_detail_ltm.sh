...
HANDOFF_PARENT_COMMIT="34dce306c8f1b333511db8e2fbdb58555e9e9bd2"
...
HANDOFF="$ROOT/R1_26619_HANDOFF_HASHES.sha256"
BASE_FULL="$ROOT/R1_26619_BASE_26618_R1_FULL_APP.sha256"; CAND_FULL="$ROOT/R1_26619_EXPECTED_CANDIDATE_FULL_APP.sha256"
BASE_PROTECTED="$ROOT/R1_26619_PROTECTED_UNCHANGED_BASE.sha256"; CAND_PROTECTED="$ROOT/R1_26619_PROTECTED_UNCHANGED_CANDIDATE.sha256"
BASE_NATIVE="$ROOT/R1_26619_NATIVE_PROTECTED_BASE.sha256"; CAND_NATIVE="$ROOT/R1_26619_NATIVE_PROTECTED_CANDIDATE.sha256"
BASE_VENDOR="$ROOT/R1_26619_VENDOR_PROTECTED_BASE.sha256"; CAND_VENDOR="$ROOT/R1_26619_VENDOR_PROTECTED_CANDIDATE.sha256"
BASE_DNG="$ROOT/R1_26619_DNG_BASE.sha256"; CAND_DNG="$ROOT/R1_26619_DNG_CANDIDATE.sha256"
CHANGED="$ROOT/R1_26619_RUNTIME_CHANGED_PATHS.txt"; PREWRITE="$ROOT/R1_26619_PREWRITE_SOURCE_HASHES.sha256"; ADDED="$ROOT/R1_26619_ADDED_PATHS_MUST_BE_ABSENT.txt"; DELETED="$ROOT/R1_26619_DELETED_PATHS_MUST_BE_ABSENT.txt"; EXPECTED_CHANGED="$ROOT/R1_26619_EXPECTED_CHANGED_SOURCE_HASHES.sha256"
FORWARD="$ROOT/R1_26619_RUNTIME_DELTA_FROM_26618_R1.patch"; ROLLBACK="$ROOT/R1_26619_RUNTIME_ROLLBACK_TO_26618_R1.patch"; SHADER_PIN="$ROOT/R1_26619_RUNTIME_EXPANDED_SHADERS.sha256"
TRANSFORM="$ROOT/transform_26619_r1.py"; VALIDATE="$ROOT/validate_26619_r1.py"; AUTHORITY="$ROOT/verify_26619_r1_authority.py"; INFRA="$ROOT/verify_26619_r1_infrastructure.py"; PATCHVERIFY="$ROOT/verify_26619_r1_patches.py"; SHADERVERIFY="$ROOT/verify_26619_r1_shaders.py"; GATEVERIFY="$ROOT/verify_26619_r1_regressions.py"
BUILD_SCRIPT="$ROOT/build_26619_r1_monotonic_guided_base_detail_ltm.sh"; WORKFLOW="$ROOT/.github/workflows/build-26619-r1-monotonic-guided-base-detail.yml"
OUT="$ROOT/build_26619_r1_monotonic_guided_base_detail_ltm_outputs"; WORK="$ROOT/.build_26619_r1_monotonic_guided_base_detail_ltm_work"; ARTZIP="$WORK/26618_r1_artifact.zip"; ARTDIR="$WORK/artifact"; BASE="$WORK/exact_26618_r1_compiled_candidate"; AFTER="$WORK/candidate_26619_r1"; AFTER2="$WORK/candidate_26619_r1_replay"; SHADER_OUT="$WORK/runtime_expanded_shaders"; GLSLANG_DIR="$WORK/glslang-16.5.0"; LIVE_CANON="$WORK/live_compiler_candidate_snapshot"; POST="$WORK/postbuild_source_snapshot"; FINAL="$ROOT/IrisCamera-${VERSION_NAME}-${VERSION_BUILD}-r1-monotonic-guided-base-detail-debug.apk"
...
cat > "$OUT/26619_R1_COMPILER_STATUS.txt" <<'STATUS'
...
STATUS
cat > "$OUT/26619_R1_STRICT_HANDOFF_REPORT.txt" <<'REPORT'
...
REPORT
set_report(){ local key="$1" val="$2" tmp="$OUT/.report.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26619_R1_STRICT_HANDOFF_REPORT.txt" > "$tmp" || fail "report key missing $key"; mv "$tmp" "$OUT/26619_R1_STRICT_HANDOFF_REPORT.txt"; }
set_compiler(){ local key="$1" val="$2" tmp="$OUT/.compiler.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26619_R1_COMPILER_STATUS.txt" > "$tmp" || fail "compiler key missing $key"; mv "$tmp" "$OUT/26619_R1_COMPILER_STATUS.txt"; }
...
from pathlib import Path
import sys
names=[line.split('  ',1)[1] for line in Path(sys.argv[1]).read_text().splitlines() if line.strip()]; names.append('R1_26619_HANDOFF_HASHES.sha256'); print('\n'.join(sorted(names)))
PY
...
PY
  while read -r _sha rel; do [[ -n "${rel:-}" ]] || continue; cmp "$ROOT/handoff_payload_26619/$rel" "$AFTER/$rel" || fail "sealed payload differs frozen candidate $rel"; done < "$EXPECTED_CHANGED"
...
PY
  set_report "POST-BUILD INVARIANCE" "PASS (candidate/protected/DNG/native/vendor exact)"; set_compiler "POST-BUILD INVARIANCE" "PASS"; tar --sort=name --mtime='UTC 1970-01-01' --owner=0 --group=0 --numeric-owner -cf - -C "$POST" app | gzip -n > "$OUT/26619_R1_candidate_app_source.tar.gz"; sha256sum "$OUT/26619_R1_candidate_app_source.tar.gz" > "$OUT/26619_R1_candidate_app_source.tar.gz.sha256"; cp "$CAND_FULL" "$OUT/26619_R1_candidate_full_app.sha256"; cp "$CAND_NATIVE" "$OUT/26619_R1_native_protected_postbuild.sha256"; cp "$CAND_VENDOR" "$OUT/26619_R1_vendor_protected_postbuild.sha256"; set_report "CLEAN ARTIFACT SOURCE EXPORT" "PASS (deterministic candidate tar + manifests)"
}
verify_package
verify_scope
obtain_authority
