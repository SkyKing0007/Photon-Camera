...
def validate(base:Path,cand:Path):
 ...
 need(saver,'Log.w(TAG, "IRIS_26539_NIGHT_BASE_EXIF_NONFATAL", exifFailure);','compile-safe Throwable base EXIF logging')
 need(saver,'Log.w(TAG, "IRIS_26539_NIGHT_FINAL_EXIF_NONFATAL", exifFailure);','compile-safe Throwable final EXIF logging')
 forbid(saver,'Log.getStackTraceString(exifFailure)','Throwable must not call Exception-only Log helper')
