...
HANDOFF="$ROOT/R1_26649_HANDOFF_HASHES.sha256"; UPLOADS="$ROOT/R1_26649_UPLOAD_PATHS.txt"
...
FORWARD="$ROOT/R1_26649_RUNTIME_DELTA_FROM_26648_R1_2.patch"; ROLLBACK="$ROOT/R1_26649_RUNTIME_ROLLBACK_TO_26648_R1_2.patch"; SHADER_PIN="$ROOT/R1_26649_RUNTIME_EXPANDED_SHADERS.sha256"
TRANSFORM="$ROOT/transform_26649.py"; VALIDATE="$ROOT/validate_26649.py"; AUTHORITY="$ROOT/verify_26649_authority.py"; INFRA="$ROOT/verify_26649_infrastructure.py"; PATCHVERIFY="$ROOT/verify_26649_patches.py"; SHADERVERIFY="$ROOT/verify_26649_shaders.py"; GATEVERIFY="$ROOT/verify_26649_regressions.py"
BUILD_SCRIPT="$ROOT/build_26649_r1_photon_highlight_compression.sh"; WORKFLOW="$ROOT/.github/workflows/build-26649-r1-photon-highlight-compression.yml"
OUT="$ROOT/build_26649_r1_photon_highlight_compression_outputs"; WORK="$ROOT/.build_26649_r1_photon_highlight_compression_work"
...
FINAL="$ROOT/IrisCamera-${VERSION_NAME}-${VERSION_BUILD}-r1-photon-highlight-compression-debug.apk"
...
cat > "$OUT/26649_R1_COMPILER_STATUS.txt" <<'STATUS'
...
STATUS
cat > "$OUT/26649_R1_STRICT_HANDOFF_REPORT.txt" <<'REPORT'
...
REPORT
set_report(){ local key="$1" val="$2" tmp="$OUT/.report.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26649_R1_STRICT_HANDOFF_REPORT.txt" > "$tmp" || fail "report key missing $key"; mv "$tmp" "$OUT/26649_R1_STRICT_HANDOFF_REPORT.txt"; }
set_compiler(){ local key="$1" val="$2" tmp="$OUT/.compiler.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26649_R1_COMPILER_STATUS.txt" > "$tmp" || fail "compiler key missing $key"; mv "$tmp" "$OUT/26649_R1_COMPILER_STATUS.txt"; }
...
verify_scope(){
  ...
  [[ "$(git rev-parse HEAD^)" == "$RUNTIME_AUTHORITY_COMMIT" ]] || fail "26649 handoff parent must be exact successful 26648 R1.2 commit"
...
verify_successful_26648_mechanics(){
  ...
  set_report "INFRASTRUCTURE DELTA AUDIT" "PASS (exact successful 26648 R1.2 compiler/native/patch/PRE-BUILD/assemble/postbuild order retained; only 26649 identity/scope/semantic assertions and artifact names differ)"
  set_report "VERIFICATION MECHANICS AUTHORITY" "PASS (successful 26648 R1.2 build-script ${AUTH_R1_2_BUILD_SCRIPT_BLOB} + workflow ${AUTH_R1_2_WORKFLOW_BLOB}; exact sequence inherited)"
}
...
postbuild_proof(){
  ...
  tar --sort=name --mtime='UTC 1970-01-01' --owner=0 --group=0 --numeric-owner -cf - -C "$POST" app | gzip -n > "$OUT/26649_R1_candidate_app_source.tar.gz"
  sha256sum "$OUT/26649_R1_candidate_app_source.tar.gz" > "$OUT/26649_R1_candidate_app_source.tar.gz.sha256"
  cp "$CAND_FULL" "$OUT/26649_R1_candidate_full_app.sha256"; cp "$CAND_NATIVE" "$OUT/26649_R1_native_protected_postbuild.sha256"; cp "$CAND_VENDOR" "$OUT/26649_R1_vendor_protected_postbuild.sha256"; cp "$CAND_DNG" "$OUT/26649_R1_dng_postbuild.sha256"; cp "$SHADER_PIN" "$OUT/26649_R1_runtime_expanded_shaders.sha256"
...
if [[ -n "$LOCAL_ART" ]]; then
  ...
  cp "$OUT/26649_R1_STRICT_HANDOFF_REPORT.txt" "$OUT/26649_R1_local_prebuild_report.txt"
  pass "26649 R1 LOCAL PREBUILD PREPARED: exact successful 26648 R1.2 compiled candidate + exact 5-path Photon Highlight Compression candidate; applicable real compiler/build gates explicitly unproven locally"
  exit 0
fi
install_frozen_candidate_live
...
mv "${apks[0]}" "$FINAL"; [[ -f "$FINAL" ]] || fail "final APK missing after move"; set_report "EXACTLY ONE APK" "PASS"; sha256sum "$FINAL" > "$OUT/26649_R1_APK.sha256"
postbuild_proof
pass "26649 R1 ACTIONS BUILD COMPLETE"
