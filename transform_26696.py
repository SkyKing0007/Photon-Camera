...
if len(sys.argv)!=4: raise SystemExit('usage: transform_26696.py BASE26695 ROLLBACK26694 DEST')
base=Path(sys.argv[1]); rollback=Path(sys.argv[2]); dest=Path(sys.argv[3]); pkg=Path(__file__).resolve().parent; payload=pkg/'handoff_payload_26696'
changed=[x.strip() for x in (pkg/'26696_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x.strip()]
added=[x.strip() for x in (pkg/'26696_ADDED_PATHS_MUST_BE_ABSENT.txt').read_text().splitlines() if x.strip()]
restored=[x.strip() for x in (pkg/'26696_RESTORED_26694_PATHS.txt').read_text().splitlines() if x.strip()]
...
def load(n):
 ...
 return out
pre=load('26696_PREWRITE_26695_SOURCE_HASHES.sha256'); r94=load('26696_RESTORED_26694_SOURCE_HASHES.sha256'); post=load('26696_EXPECTED_CHANGED_SOURCE_HASHES.sha256')
...
print('PASS 26696 deterministic transform: successful 26695 universe; exact 3-path 26694 runtime restore; exact 9-path final allowlist (8 modified + 1 added)')
