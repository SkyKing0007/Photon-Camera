...
APPLY="$ROOT/apply_26529_manual_30x_iris_spatial_rgb_v3.py"
VALIDATE="$ROOT/validate_26529_manual_30x_iris_spatial_rgb_v3.py"
SHADER_PREFLIGHT="$ROOT/preflight_26529_iris_embedded_shaders_v3.py"
AUDIT_REFERENCE="$ROOT/audit_26529_bjzhou_semantic_reference_v3.py"
IRIS_TEMPLATE="$ROOT/26529_V3_IRIS_SPATIAL_RGB_CHROMA_REWRITE.kt"
EXPECTED_PATCH="$ROOT/26529_V3_RUNTIME_DELTA_FROM_SUCCESSFUL_26528.patch"
EXPECTED_PATCH_SHA="$ROOT/26529_V3_RUNTIME_DELTA_FROM_SUCCESSFUL_26528.patch.sha256"
EXPECTED_ROLLBACK="$ROOT/26529_V3_RUNTIME_ROLLBACK_TO_SUCCESSFUL_26528.patch"
EXPECTED_ROLLBACK_SHA="$ROOT/26529_V3_RUNTIME_ROLLBACK_TO_SUCCESSFUL_26528.patch.sha256"
IRIS_TEMPLATE_SHA="5e112314f0795e4294e3af9e8b127d7d86cdfa494cd8df042fd5c6ba9d7949a4"
HANDOFF="$ROOT/26529_V3_HANDOFF_HASHES.sha256"
BASE_FILE="$ROOT/26529_BASE_26528_HEAD.txt"
PROVENANCE="$ROOT/26529_V3_SCOPE_PROVENANCE.txt"
PROJECT_RULES="$ROOT/26529_V3_PROJECT_RULES.txt"
...
OUT="$ROOT/build_26529_manual_30x_iris_spatial_rgb_v3_outputs"
WORK="$ROOT/.build_26529_manual_30x_iris_spatial_rgb_v3_work"
...
FINAL="$ROOT/IrisCamera-${VERSION_NAME}-${VERSION_BUILD}-manual-30x-iris-spatial-rgb-v3-debug.apk"
...
cmp -s "$GEN_PATCH" "$EXPECTED_PATCH" || fail "generated forward patch differs from certified 26529 V3 patch"
cmp -s "$GEN_ROLLBACK" "$EXPECTED_ROLLBACK" || fail "generated rollback patch differs from certified 26529 V3 rollback"
...
PY
pass "26529 V3 forward + rollback patches regenerated byte-identical to certified handoff patches before candidate writes"
...
STACKER_CAND="$AFTER/app/src/main/java/com/hinnka/mycamera/processor/GlesIris26521SpatialRgbStacker.kt"
[[ "$(grep -Fc 'detachFramebufferColorAttachment(diagnosticFramebuffer)' "$STACKER_CAND")" -eq 2 ]] || fail "V3 expected exactly two framebuffer-detach call sites"
[[ "$(grep -Fc 'private fun detachFramebufferColorAttachment(framebuffer: Int)' "$STACKER_CAND")" -eq 1 ]] || fail "V3 framebuffer-detach helper missing/duplicated"
grep -F 'IRIS_26529_V3_FRAMEBUFFER_DETACH_OWNER' "$STACKER_CAND" >/dev/null || fail "V3 framebuffer-detach ownership marker missing"
echo "KOTLIN_INTEGRATION_CORRECTION=V3_FRAMEBUFFER_DETACH_OWNER_PRESENT"
...
26529 V3 Manual Physical-Lens 30x + Iris-owned Spatial RGB c317-semantic rewrite
Branch: $BRANCH
...
pass "26529 V3 exact successful-26528 artifact + certified patch-first safety proof"
pass "26529 guarded Gradle build + immutable audited runtime + exactly one APK"
pass "26529 V3 postbuild zoom/DNG/Iris-c317-semantic proof + deterministic next candidate source"
