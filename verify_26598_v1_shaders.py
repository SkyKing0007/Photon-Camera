...
def main():
    ...
    specs=[
      ...
      ('short_region_seed_26596',extract_kotlin(sc,'shortRegionSeed26595'),'frag'),
      ('short_region_propagate_26594',extract_kotlin(sc,'shortRegionPropagate26594'),'frag'),
      ('short_phase_scoped_sensor_loss_handoff_mask_26597',extract_kotlin(sc,'shortRestoreMask26596'),'frag'),
    ...
    cpp=(root/'app/src/main/cpp/motionv2_jpeg444_jni.cpp').read_text(); specs.append(('true2x_publication_compute_26597_inherited',extract_cpp_raw(cpp,'kIris26571PublicationCompute'),'comp'))
    if len(specs)!=17: fail('runtime-expanded shader variant count '+str(len(specs)))
    rows=[]; manifest=[]
    ...
    (out/'V1_26598_RUNTIME_EXPANDED_SHADERS.sha256').write_text(''.join(manifest))
    (out/'V1_26598_SHADER_VERIFICATION.json').write_text(json.dumps(rows,indent=2)+'\n')
