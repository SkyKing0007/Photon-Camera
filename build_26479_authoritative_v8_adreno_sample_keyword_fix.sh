...
EXPECTED_INFRA_PARENT="5755b1a05777423b5af9f803a57235ac73efdf68"
EXPECTED_APP_BASE="8233415edf738bf35c0fe1c4907f5dfe51de31a4"

BACKUP_BRANCH="backup-26478-v7-before-26479-adreno-sample-keyword-fix"
BACKUP_TARGET="5755b1a05777423b5af9f803a57235ac73efdf68"
...
exec > >(tee "$AUDIT") 2>&1
echo "=== 26479 V8 ADRENO SAMPLE-KEYWORD PORTABILITY / 26478 IMAGE-MATH PRESERVED ==="
date -Iseconds || true

BRANCH="${GITHUB_REF_NAME:-$(git branch --show-current)}"
[[ "$BRANCH" == "$EXPECTED_BRANCH" ]] || fail "wrong branch: $BRANCH"
[[ "$(git rev-parse HEAD^)" == "$EXPECTED_INFRA_PARENT" ]] || \
  fail "26479 V8 commit must be direct child of exact successful V7 infrastructure HEAD"
[[ "$(git rev-parse HEAD~2)" == "c8beb8882370d66b85c5e3dbe13c69096aef5202" ]] || \
  fail "26479 V8 chain missing exact V6 infrastructure HEAD"
[[ "$(git rev-parse HEAD~7)" == "be24c16cb091e753852b900384dbde6f8d32744e" ]] || \
  fail "26479 V8 chain must retain exact successful 26477 V3 ancestor"
pass "branch/head exact V8-on-V7 lineage gate"
...
cat > "$REPORT" <<EOF
26479 V8 ADRENO SAMPLE-KEYWORD PORTABILITY / EXACT 26478 IMAGE-MATH
========================================================================
Rollback:
