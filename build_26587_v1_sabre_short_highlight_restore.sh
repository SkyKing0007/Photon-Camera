...
HANDOFF_PARENT_COMMIT="40f0449cd0281c8b8d25afe2be0d68682afd7077"
...
VERSION_NAME="0.9726587"
VERSION_BUILD="26587"
GLSLANG_VERSION="16.5.0"
GLSLANG_ARCHIVE_SHA="b9b1f96acb898a62251b171f7695efcecfc206a530299054071919b06820f657"
GLSLANG_URL="https://github.com/KhronosGroup/glslang/releases/download/16.5.0/glslang-16.5.0-linux-x86_64-release.tar.gz"
SEALED_PAYLOAD_COUNT=38
HANDOFF="$ROOT/V1_26587_HANDOFF_HASHES.sha256"
BASE_FULL="$ROOT/V1_26587_BASE_26586_FULL_APP.sha256"
CAND_FULL="$ROOT/V1_26587_EXPECTED_CANDIDATE_FULL_APP.sha256"
BASE_PROTECTED="$ROOT/V1_26587_PROTECTED_UNCHANGED_BASE.sha256"
CAND_PROTECTED="$ROOT/V1_26587_PROTECTED_UNCHANGED_CANDIDATE.sha256"
BASE_NATIVE="$ROOT/V1_26587_NATIVE_PROTECTED_BASE.sha256"
CAND_NATIVE="$ROOT/V1_26587_NATIVE_PROTECTED_CANDIDATE.sha256"
BASE_VENDOR="$ROOT/V1_26587_VENDOR_PROTECTED_BASE.sha256"
CAND_VENDOR="$ROOT/V1_26587_VENDOR_PROTECTED_CANDIDATE.sha256"
BASE_DNG="$ROOT/V1_26587_DNG_PROTECTED_BASE.sha256"
CAND_DNG="$ROOT/V1_26587_DNG_PROTECTED_CANDIDATE.sha256"
BASE_ARCH="$ROOT/V1_26587_PROTECTED_ARCHITECTURE_BASE.sha256"
CAND_ARCH="$ROOT/V1_26587_PROTECTED_ARCHITECTURE_CANDIDATE.sha256"
CHANGED="$ROOT/V1_26587_RUNTIME_CHANGED_PATHS.txt"
PREWRITE="$ROOT/V1_26587_PREWRITE_SOURCE_HASHES.sha256"
EXPECTED_CHANGED="$ROOT/V1_26587_EXPECTED_CHANGED_SOURCE_HASHES.sha256"
FORWARD="$ROOT/V1_26587_RUNTIME_DELTA_FROM_26586.patch"
ROLLBACK="$ROOT/V1_26587_RUNTIME_ROLLBACK_TO_26586.patch"
SHADER_PIN="$ROOT/V1_26587_RUNTIME_EXPANDED_SHADERS.sha256"
TRANSFORM="$ROOT/transform_26587_v1.py"
VALIDATE="$ROOT/validate_26587_v1.py"
AUTHORITY="$ROOT/verify_26587_v1_authority.py"
PATCHVERIFY="$ROOT/verify_26587_v1_patches.py"
SHADERVERIFY="$ROOT/verify_26587_v1_shaders.py"
GATEVERIFY="$ROOT/verify_26587_v1_regressions.py"
BUILD_SCRIPT="$ROOT/build_26587_v1_sabre_short_highlight_restore.sh"
WORKFLOW="$ROOT/.github/workflows/build-26587-v1-sabre-short-highlight-restore.yml"
OUT="$ROOT/build_26587_v1_sabre_short_highlight_restore_outputs"
WORK="$ROOT/.build_26587_v1_sabre_short_highlight_restore_work"
ARTZIP="$WORK/26586_v1_artifact.zip"
ARTDIR="$WORK/artifact"
BASE="$WORK/exact_26586_v1_compiled_candidate"
AFTER="$WORK/candidate_26587"
AFTER2="$WORK/candidate_26587_replay"
...
FINAL="$ROOT/IrisCamera-${VERSION_NAME}-${VERSION_BUILD}-v1-sabre-short-highlight-restore-debug.apk"
...
cat > "$OUT/26587_V1_COMPILER_STATUS.txt" <<'STATUS'
...
STATUS
cat > "$OUT/26587_V1_STRICT_HANDOFF_REPORT.txt" <<'REPORT'
...
TARGET VERSION/BUILD: 0.9726587 / 26587
REPORT
set_report(){ local key="$1" val="$2" tmp="$OUT/.report.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26587_V1_STRICT_HANDOFF_REPORT.txt" > "$tmp" || fail "report key missing $key"; mv "$tmp" "$OUT/26587_V1_STRICT_HANDOFF_REPORT.txt"; }
set_compiler(){ local key="$1" val="$2" tmp="$OUT/.compiler.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26587_V1_COMPILER_STATUS.txt" > "$tmp" || fail "compiler key missing $key"; mv "$tmp" "$OUT/26587_V1_COMPILER_STATUS.txt"; }
...
verify_package(){
  ...
  [[ "$(sha "$CAND_FULL")" == "73f98415baec56bea1036e692854bebefe453f81fb534f52ae57b46e4c59b976" ]] || fail "candidate full manifest SHA"
  ...
  [[ "$(sha "$EXPECTED_CHANGED")" == "5ffce0bcb1e25cf92a4558d1ffb7f3b5c4d85e490041e6b016b74d76b542bec7" ]] || fail "expected changed-source SHA"
  [[ "$(sha "$FORWARD")" == "cf699ba4df0e6685bd018099713849370f25010874713a2dee7db0a948521f6c" ]] || fail "forward patch SHA"
  [[ "$(sha "$ROLLBACK")" == "5f978409c3493cc9284f99e99d477e6389fa15c72b2cc518dd2949ce69044920" ]] || fail "rollback patch SHA"
...
PY
  bash -n "$BUILD_SCRIPT"
  grep -F 'Broad pink, pink/green dotted edges, cyan/green speckling, zipper and ghost artifacts are release-blocking regressions.' "$ROOT/REGRESSION_V1_26587_SHORT_RESTORE_SAFETY.txt" >/dev/null || fail "SHORT safety regression missing"
  grep -F 'viewfinder/body meter authority remains the successful 26584 structured P90 / 0.965 branch' "$ROOT/REGRESSION_V1_26587_CARRIED_26586.txt" >/dev/null || fail "carried 26586 regression missing"
  grep -F 'app/build/** and app/.cxx/** never count as runtime-source changes.' "$ROOT/REGRESSION_V1_26587_WORKFLOW_PACKAGING.txt" >/dev/null || fail "packaging regression missing"
  grep -F 'no verification gate is weakened' "$ROOT/V1_26587_INFRASTRUCTURE_DIFF_AUDIT.txt" >/dev/null || fail "infrastructure audit missing"
  [[ -s "$ROOT/V1_26587_BUILD_SCRIPT_DIFF_AUDIT_FROM_SUCCESSFUL_26586.txt" ]] || fail "build script audit missing"
  [[ -s "$ROOT/V1_26587_WORKFLOW_DIFF_AUDIT_FROM_SUCCESSFUL_26586.txt" ]] || fail "workflow audit missing"
  ...
  set_report "INFRASTRUCTURE DELTA AUDIT" "PASS (successful 26586 order/mechanics preserved; only authority pins/names/version, exact six-file scope, two new shader variants and 26587 regressions differ; no gate weakened)"
...
verify_scope(){
  ...
  [[ "$(git rev-parse HEAD^)" == "$BASE_SUCCESS_COMMIT" ]] || fail "26587 V1 must be one clean handoff commit directly on successful 26586"
...
names.append('V1_26587_HANDOFF_HASHES.sha256')
print('\n'.join(sorted(names)))
PY
  ...
  ! grep -Eq '^app/' "$WORK/full_lineage_scope.txt" || fail "26586-to-26587 lineage contains live runtime app source"
...
make_candidate(){
  rm -rf "$AFTER" "$AFTER2"
  python3 -S "$TRANSFORM" "$BASE" "$AFTER" | tee "$OUT/26587_transform.txt"
  python3 -S "$TRANSFORM" "$BASE" "$AFTER2" | tee "$OUT/26587_transform_replay.txt"
...
PY
  while IFS= read -r rel; do [[ -n "$rel" ]] || continue; cmp "$ROOT/handoff_payload_26587_v1/$rel" "$AFTER/$rel" || fail "sealed payload differs frozen candidate $rel"; done < "$CHANGED"
  python3 -S "$VALIDATE" "$BASE" "$AFTER" | tee "$OUT/26587_semantic_validation.txt"
  python3 -S "$GATEVERIFY" "$AFTER" | tee "$OUT/26587_refinement_regressions.txt"
  python3 -S "$AUTHORITY" "$ROOT" "$BASE" "$AFTER" | tee "$OUT/26587_authority_candidate.txt"
...
verify_candidate_patches(){
  python3 -S "$PATCHVERIFY" "$BASE" "$AFTER" "$FORWARD" "$ROLLBACK" | tee "$OUT/26587_patch_validation.txt"
  set_report "FORWARD PATCH FUZZ=0" "PASS"; set_report "ROLLBACK PATCH FUZZ=0" "PASS"
}
verify_shaders(){
  rm -rf "$SHADER_OUT"; mkdir -p "$SHADER_OUT"
  if [[ -n "$LOCAL_ART" ]]; then
    python3 -S "$SHADERVERIFY" "$BASE" "$AFTER" --out "$SHADER_OUT" | tee "$OUT/26587_shader_validation.txt"
    cmp "$SHADER_OUT/V1_26587_RUNTIME_EXPANDED_SHADERS.sha256" "$SHADER_PIN" || fail "local runtime-expanded shader pin mismatch"
    cp "$SHADER_OUT/V1_26587_SHADER_VERIFICATION.json" "$OUT/26587_shader_verification.json"
  ...
  tar -tzf "$archive" > "$OUT/26587_glslang_archive_contents.txt"; tar -xzf "$archive" -C "$GLSLANG_DIR"
  ...
  "$compiler" --version | tee "$OUT/26587_glslang_version.txt"
  python3 -S "$SHADERVERIFY" "$BASE" "$AFTER" --out "$SHADER_OUT" --glslang "$compiler" | tee "$OUT/26587_shader_validation.txt"
  cmp "$SHADER_OUT/V1_26587_RUNTIME_EXPANDED_SHADERS.sha256" "$SHADER_PIN" || fail "Actions runtime-expanded shader pin mismatch"
  cp "$SHADER_OUT/V1_26587_SHADER_VERIFICATION.json" "$OUT/26587_shader_verification.json"
...
install_and_build(){
  ...
  python3 -S "$VALIDATE" "$BASE" "$LIVE_CANON" | tee "$OUT/26587_live_semantic_validation.txt"
  python3 -S "$GATEVERIFY" "$LIVE_CANON" | tee "$OUT/26587_live_refinement_regressions.txt"
  python3 -S "$AUTHORITY" "$ROOT" "$BASE" "$LIVE_CANON" | tee "$OUT/26587_live_authority.txt"
...
PY
  ./gradlew clean :app:compileDebugKotlin :app:compileDebugJavaWithJavac --stacktrace 2>&1 | tee "$OUT/26587_gradle_language_compilers.log"
  ...
  ./gradlew ':app:buildCMakeDebug[arm64-v8a]' ':app:buildCMakeDebug[armeabi-v7a]' --stacktrace 2>&1 | tee "$OUT/26587_gradle_native_compiler.log"
  set_report "REAL NATIVE/NDK COMPILE" "PASS (both ABIs)"; set_compiler "NATIVE/NDK COMPILE" "PASS (both ABIs)"
  verify_candidate_patches
  set_report "PRE-BUILD SAFETY PROOF" "PASS"; pass "26587 PRE-BUILD SAFETY PROOF PASSED"
  ./gradlew :app:assembleDebug --stacktrace 2>&1 | tee "$OUT/26587_gradle_assemble.log"
  ...
  sha256sum "$FINAL" > "$OUT/26587_V1_APK.sha256"; set_report "EXACTLY ONE APK" "PASS"
  snapshot_candidate_from_authority "$BASE" "$ROOT" "$POST"
  python3 -S "$VALIDATE" "$BASE" "$POST" | tee "$OUT/26587_postbuild_semantic_validation.txt"
  python3 -S "$GATEVERIFY" "$POST" | tee "$OUT/26587_postbuild_refinement_regressions.txt"
  python3 -S "$AUTHORITY" "$ROOT" "$BASE" "$POST" | tee "$OUT/26587_postbuild_authority.txt"
  ...
  tar --sort=name --mtime='UTC 1970-01-01' --owner=0 --group=0 --numeric-owner -cf - -C "$POST" app | gzip -n > "$OUT/26587_V1_candidate_app_source.tar.gz"
  sha256sum "$OUT/26587_V1_candidate_app_source.tar.gz" > "$OUT/26587_V1_candidate_app_source.tar.gz.sha256"
  cp "$CAND_FULL" "$OUT/26587_V1_candidate_full_app.sha256"; cp "$CAND_NATIVE" "$OUT/26587_V1_native_protected_postbuild.sha256"; cp "$CAND_VENDOR" "$OUT/26587_V1_vendor_protected_postbuild.sha256"
...
if [[ -n "$LOCAL_ART" ]]; then
  ...
  cp "$OUT/26587_V1_STRICT_HANDOFF_REPORT.txt" "$OUT/26587_local_prebuild_report.txt"
  pass "26587 LOCAL PREBUILD PREPARED: real GLSL/Kotlin/Java/NDK/full Android gates explicitly unproven locally"
  exit 0
fi
install_and_build
pass "26587 REAL GLSL + KOTLIN/JAVA + NDK + FULL ASSEMBLE PASSED"
pass "26587 POST-BUILD INVARIANCE PASSED"
