...
if len(sys.argv)<5 or sys.argv[3]!='--out': raise SystemExit('usage: verify_26630_r1_shaders.py BASE CAND --out OUT [--compiler PATH]')
...
render_rel='app/src/main/assets/shaders/motionv2/render.glsl'; gain_rel='app/src/main/assets/shaders/motionv2/gainmap.glsl'
...
def frag(name,body): return (name,'#version 300 es\n'+body,'frag')
mods=[frag('motionv2_render_26630',(C/render_rel).read_text()),frag('motionv2_gainmap_26630',(C/gain_rel).read_text())]
...
mods.append(('true2x_publication_compute_26630',compute,'comp'))
...
(O/'R1_26630_RUNTIME_EXPANDED_SHADERS.sha256').write_text('\n'.join(lines)+'\n')
(O/'R1_26630_SHADER_VERIFICATION.json').write_text(json.dumps({'count':len(results),'compiler':compiler,'results':results},indent=2,sort_keys=True)+'\n')
print(f'PASS 26630 complete runtime-expanded GLSL reserved scan variants={len(results)} real_compiler={bool(compiler)}')
