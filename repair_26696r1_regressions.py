...
import hashlib,sys
if len(sys.argv)!=3:raise SystemExit('usage: repair_26696r1_regressions.py BASE26695 CANDIDATE')
...
# Permanent Actions failure 35957174533: dormant source still compiles against the AE API.
...
assert 'owner.exposure.update(' in raw and 'pending.actualIso' in raw and 'pending.actualExposureNs' in raw
...
for bad in ['app/build/','app/.cxx/']:
 assert not any(x.startswith(bad) for x in [p.strip() for p in (Path(__file__).resolve().parent/'26696R1_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines()])
print('PASS 26696 R1 permanent regressions: RawFormat/native/fixed UI; 26694 AE one-shot; 26695 render-only AE; blocking warmup; blocking release latch; Photo processing UI; DCIM/Camera')
