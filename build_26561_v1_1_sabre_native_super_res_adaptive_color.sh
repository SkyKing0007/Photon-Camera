...
HANDOFF_PARENT_COMMIT="1dd9907234e82235dc9f418418e36fbad2b97221"
...
BACKUP_BRANCH="backup-26560-v1-before-26561-sabre-sr-universal-color"
BACKUP_SHA_EXPECTED="40714fc8077b66b8a9f04adaff6cff212c02f644"
VERSION_NAME="0.9726561"
VERSION_BUILD="26561"
TOKEN="${GH_TOKEN:-${GITHUB_TOKEN:-}}"

BASE_PIN="$ROOT/V1_1_26561_BASE_26560_V1_AUDITED_RUNTIME.sha256"
BASE_TAR_PIN="$ROOT/V1_1_26561_BASE_26560_V1_CANDIDATE_TAR.sha256"
CAND_PIN="$ROOT/V1_1_26561_EXPECTED_CANDIDATE_AUDITED_RUNTIME.sha256"
VENDOR_PIN="$ROOT/V1_1_26561_NATIVE_VENDOR_DEPENDENCIES.sha256"
RUNTIME_LIST="$ROOT/V1_1_26561_RUNTIME_FILES.txt"
PREWRITE="$ROOT/V1_1_26561_PREWRITE_SOURCE_HASHES.sha256"
CAND_CHANGED="$ROOT/V1_1_26561_CANDIDATE_CHANGED_HASHES.sha256"
PROTECTED_CORE="$ROOT/V1_1_26561_PROTECTED_UNCHANGED_CORE.sha256"
RUNTIME_GLSL_PIN="$ROOT/V1_1_26561_RUNTIME_EXPANDED_GLSL.sha256"
FORWARD="$ROOT/V1_1_26561_RUNTIME_DELTA_FROM_26560.patch"
ROLLBACK="$ROOT/V1_1_26561_RUNTIME_ROLLBACK_TO_26560.patch"
TRANSFORM="$ROOT/transform_26561_v1_1_sabre_native_super_res_adaptive_color.py"
VALIDATE="$ROOT/validate_26561_v1_1_sabre_native_super_res_adaptive_color.py"
EXTRACT="$ROOT/extract_26561_v1_1_runtime_glsl.py"
RESERVED="$ROOT/scan_glsl_reserved_identifiers_26561_v1_1.py"
HANDOFF_HASHES="$ROOT/V1_1_26561_HANDOFF_HASHES.sha256"
OUT="$ROOT/build_26561_v1_1_sabre_native_super_res_adaptive_color_outputs"
WORK="$ROOT/.build_26561_v1_1_sabre_native_super_res_adaptive_color_work"
...
FINAL="$ROOT/IrisCamera-${VERSION_NAME}-${VERSION_BUILD}-v1-1-sabre-native-super-res-adaptive-color-debug.apk"
...
cat > "$OUT/26561_V1_1_COMPILER_STATUS.txt" <<'EOF'
...
EOF
cat > "$OUT/26561_V1_1_STRICT_HANDOFF_REPORT.txt" <<'EOF'
...
TARGET VERSION/BUILD: 0.9726561 / 26561 V1.1
EOF
set_report(){ python3 - "$OUT/26561_V1_1_STRICT_HANDOFF_REPORT.txt" "$1" "$2" <<'PY'
from pathlib import Path
import sys
...
run_real_glsl(){
    ...
    python3 - "$OUT/26561_V1_1_COMPILER_STATUS.txt" <<'PY'
from pathlib import Path
import sys
...
install_frozen_candidate_into_live_root(){
    local live_root="$1" label="$2"
    [[ -d "$live_root/app" ]] || fail "live-root fixture missing app/: $live_root"
    manifest_audited "$live_root" "$WORK/${label}_preinstall_runtime.sha256"
    # Diagnostic only: repository checkout/native state is NOT runtime authority before candidate install.
    vendor_manifest "$live_root" "$WORK/${label}_preinstall_vendor.sha256"
    rm -rf "$live_root/app/src/main" "$live_root/app/version.properties" "$live_root/app/build.gradle"
    mkdir -p "$live_root/app/src"
    cp -a "$AFTER/app/src/main" "$live_root/app/src/"
    cp -a "$AFTER/app/version.properties" "$live_root/app/version.properties"
    cp -a "$AFTER/app/build.gradle" "$live_root/app/build.gradle"
    manifest_audited "$live_root" "$WORK/${label}_installed_runtime.sha256"
    cmp "$WORK/${label}_installed_runtime.sha256" "$CAND_PIN" >/dev/null || fail "installed live source differs from frozen candidate"
    vendor_manifest "$live_root" "$WORK/${label}_installed_vendor.sha256"
    cmp "$WORK/${label}_installed_vendor.sha256" "$VENDOR_PIN" >/dev/null || fail "installed candidate native/vendor differs from successful 26560 authority"
    python3 "$VALIDATE" --base "$BASE" --candidate "$live_root" > "$OUT/${label}_installed_contract.txt"
}

run_preinstall_vendor_authority_regression(){
    local fixture="$WORK/preinstall_vendor_authority_regression_checkout"
    rm -rf "$fixture"
    mkdir -p "$fixture/app/src/main/cpp/third_party_26507" "$fixture/app/src/main/cpp/deps"
    printf 'synthetic stale repository vendor fixture\n' > "$fixture/app/src/main/cpp/third_party_26507/stale.txt"
    printf 'VERSION_NAME=stale\nVERSION_BUILD=0\n' > "$fixture/app/version.properties"
    printf '// synthetic stale checkout build file\n' > "$fixture/app/build.gradle"
    vendor_manifest "$fixture" "$WORK/regression_stale_vendor.sha256"
    if cmp -s "$WORK/regression_stale_vendor.sha256" "$VENDOR_PIN"; then
        fail "preinstall-vendor regression fixture failed to create stale checkout"
    fi
    install_frozen_candidate_into_live_root "$fixture" "regression"
    set_report "PREINSTALL VENDOR AUTHORITY ORDERING" "PASS (stale checkout may differ; equality enforced only after frozen candidate install)"
    pass "permanent regression: stale repository vendor mismatch does not preempt candidate installation"
}

if [[ -n "$LOCAL_REPLAY_ARTIFACT" ]]; then
    echo "=== 26561 V1.1 LOCAL PREBUILD REPLAY: package/pins ==="
    verify_package_and_pins
    echo "=== 26561 V1.1 LOCAL PREBUILD REPLAY: exact 26560 artifact ==="
    verify_base_artifact_and_reconstruct "$LOCAL_REPLAY_ARTIFACT"
    echo "=== 26561 V1.1 LOCAL PREBUILD REPLAY: transform/ownership/GLSL/patches ==="
    build_candidate_and_precompile_proof
    echo "=== 26561 V1.1 LOCAL PREBUILD REPLAY: pre-install vendor-authority regression ==="
    run_preinstall_vendor_authority_regression
    if command -v glslangValidator >/dev/null 2>&1 && command -v dpkg-query >/dev/null 2>&1 && [[ "$(dpkg-query -W -f='${Version}' glslang-tools 2>/dev/null || true)" == "$GLSLANG_PKG_VERSION" ]]; then
        ...
        echo "LOCAL REAL GLSL COMPILE: NOT RUN (pinned package unavailable locally)" | tee "$OUT/26561_V1_1_LOCAL_COMPILER_LIMIT.txt"
    fi
    echo "LOCAL REAL KOTLIN/JAVA/ASSEMBLE: NOT RUN (Actions-only Android SDK/toolchain in this environment)" | tee -a "$OUT/26561_V1_1_LOCAL_COMPILER_LIMIT.txt"
    pass "26561 V1.1 local prebuild replay complete; authoritative Android compiler/build gates remain Actions-only"
    exit 0
fi

echo "=== 26561 V1.1 GATE 0: sealed handoff / failed-V1 parent / lineage / architectural backup ==="
[[ "$(git branch --show-current)" == "$EXPECTED_BRANCH" ]] || fail "wrong branch"
[[ "$(git rev-parse HEAD^)" == "$HANDOFF_PARENT_COMMIT" ]] || fail "V1.1 handoff commit must be direct child of failed 26561 V1 handoff"
[[ "$(git rev-parse "${HANDOFF_PARENT_COMMIT}^")" == "$BASE_SUCCESS_COMMIT" ]] || fail "failed V1 parent is not direct child of successful 26560"
git diff --quiet "$BASE_SUCCESS_COMMIT..$HANDOFF_PARENT_COMMIT" -- app || fail "failed V1 handoff modified repository app source"
...
set_report "BACKUP STATUS" "PASS (${BACKUP_BRANCH} @ ${BACKUP_SHA_EXPECTED}; existing architectural backup retained; no new backup for V1.1 infrastructure fix)"
python3 - "$HANDOFF_PARENT_COMMIT" <<'PY'
import subprocess,sys
base=sys.argv[1]
allowed={
'.github/workflows/build-26561-v1-1-sabre-native-super-res-adaptive-color.yml',
'V1_1_26561_BASE_26560_V1_AUDITED_RUNTIME.sha256','V1_1_26561_BASE_26560_V1_CANDIDATE_TAR.sha256',
'V1_1_26561_BASE_PROVENANCE.txt','V1_1_26561_CANDIDATE_CHANGED_HASHES.sha256',
'V1_1_26561_EXPECTED_CANDIDATE_AUDITED_RUNTIME.sha256','V1_1_26561_HANDOFF_HASHES.sha256',
'V1_1_26561_LOCAL_VALIDATION.txt','V1_1_26561_NATIVE_VENDOR_DEPENDENCIES.sha256',
'V1_1_26561_PREWRITE_SOURCE_HASHES.sha256','V1_1_26561_PROTECTED_UNCHANGED_CORE.sha256',
'V1_1_26561_RUNTIME_DELTA_FROM_26560.patch','V1_1_26561_RUNTIME_EXPANDED_GLSL.sha256',
'V1_1_26561_RUNTIME_FILES.txt','V1_1_26561_RUNTIME_ROLLBACK_TO_26560.patch',
'V1_1_26561_UPLOAD_INSTRUCTIONS.md','build_26561_v1_1_sabre_native_super_res_adaptive_color.sh',
'extract_26561_v1_1_runtime_glsl.py','scan_glsl_reserved_identifiers_26561_v1_1.py',
'transform_26561_v1_1_sabre_native_super_res_adaptive_color.py','validate_26561_v1_1_sabre_native_super_res_adaptive_color.py'}
...
print('PASS exact 21-file V1.1 handoff scope; repository app source untouched')
PY
pass "sealed 26561 V1.1 package / lineage / backup"

echo "=== 26561 V1.1 GATE 1: exact successful compiled 26560 runtime authority ==="
...
echo "=== 26561 V1.1 GATE 2: candidate-first transform / ownership / exact runtime GLSL / patches ==="
build_candidate_and_precompile_proof

echo "=== 26561 V1.1 GATE 3: pinned real glslangValidator on exact runtime-expanded GLSL ==="
run_real_glsl

echo "=== 26561 V1.1 GATE 4: controlled live install / real Kotlin+Java / full assemble ==="
run_preinstall_vendor_authority_regression
...
sha256sum "$FINAL" > "$OUT/26561_V1_1_APK.sha256"
pass "real Kotlin/Java/NDK compilers + full assemble + exactly one intended APK"

echo "=== 26561 V1.1 GATE 5: post-build frozen candidate / protected / native / vendor invariance ==="
...
echo "=== 26561 V1.1 GATE 6: clean candidate source export ==="
tar --sort=name --mtime='UTC 2020-01-01' --owner=0 --group=0 --numeric-owner -czf "$OUT/26561_V1_1_candidate_app_source.tar.gz" -C "$AFTER" app
sha256sum "$OUT/26561_V1_1_candidate_app_source.tar.gz" > "$OUT/26561_V1_1_candidate_app_source.tar.gz.sha256"
cp "$CAND_PIN" "$OUT/26561_V1_1_candidate_source.sha256"
set_report "CLEAN ARTIFACT SOURCE EXPORT" "PASS"
pass "26561 V1.1 BUILD-PROVEN Actions output"
