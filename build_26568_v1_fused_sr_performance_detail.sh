...
HANDOFF="$ROOT/V1_26568_HANDOFF_HASHES.sha256"
BASE_PIN="$ROOT/V1_26568_BASE_26567_AUDITED_RUNTIME.sha256"
CAND_PIN="$ROOT/V1_26568_EXPECTED_CANDIDATE_AUDITED_RUNTIME.sha256"
BASE_FULL="$ROOT/V1_26568_BASE_26567_FULL_APP.sha256"
CAND_FULL="$ROOT/V1_26568_EXPECTED_CANDIDATE_FULL_APP.sha256"
BASE_VENDOR="$ROOT/V1_26568_BASE_26567_NATIVE_VENDOR.sha256"
CAND_VENDOR="$ROOT/V1_26568_EXPECTED_NATIVE_VENDOR.sha256"
BASE_DNG="$ROOT/V1_26568_DNG_PROTECTED_BASE.sha256"
CAND_DNG="$ROOT/V1_26568_DNG_PROTECTED_CANDIDATE.sha256"
BASE_PROTECTED="$ROOT/V1_26568_PROTECTED_UNCHANGED_BASE.sha256"
CAND_PROTECTED="$ROOT/V1_26568_PROTECTED_UNCHANGED_CANDIDATE.sha256"
SHADER_PIN="$ROOT/V1_26568_RUNTIME_EXPANDED_SHADERS.sha256"
CHANGED="$ROOT/V1_26568_RUNTIME_CHANGED_PATHS.txt"
PREWRITE="$ROOT/V1_26568_PREWRITE_SOURCE_HASHES.sha256"
FORWARD="$ROOT/V1_26568_RUNTIME_DELTA_FROM_26567.patch"
ROLLBACK="$ROOT/V1_26568_RUNTIME_ROLLBACK_TO_26567.patch"
TRANSFORM="$ROOT/transform_26568_v1.py"
VALIDATE="$ROOT/validate_26568_v1.py"
AUTHORITY="$ROOT/verify_26568_authority.py"
PATCHVERIFY="$ROOT/verify_26568_patches.py"
SHADERVERIFY="$ROOT/verify_26568_shaders.py"
BUILD_SCRIPT="$ROOT/build_26568_v1_fused_sr_performance_detail.sh"
WORKFLOW="$ROOT/.github/workflows/build-26568-v1-fused-sr-performance-detail.yml"
SEALED_PY=("$TRANSFORM" "$VALIDATE" "$AUTHORITY" "$PATCHVERIFY" "$SHADERVERIFY")
SEALED_DEP_FILES=("${SEALED_PY[@]}" "$BUILD_SCRIPT" "$WORKFLOW")
OUT="$ROOT/build_26568_v1_fused_sr_performance_detail_outputs"
WORK="$ROOT/.build_26568_v1_fused_sr_performance_detail_work"
...
FINAL="$ROOT/IrisCamera-${VERSION_NAME}-${VERSION_BUILD}-v1-fused-sr-performance-detail-debug.apk"
...
cat > "$OUT/26568_V1_COMPILER_STATUS.txt" <<'EOF'
...
EOF
cat > "$OUT/26568_V1_STRICT_HANDOFF_REPORT.txt" <<'EOF'
...
EOF
set_report(){ local key="$1" val="$2" tmp="$OUT/.report.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26568_V1_STRICT_HANDOFF_REPORT.txt" > "$tmp" || fail "report key missing $key"; mv "$tmp" "$OUT/26568_V1_STRICT_HANDOFF_REPORT.txt"; }
set_compiler(){ local key="$1" val="$2" tmp="$OUT/.compiler.tmp"; awk -v key="$key:" -v val="$val" 'BEGIN{f=0} index($0,key)==1{print key" "val;f=1;next}{print} END{if(!f)exit 42}' "$OUT/26568_V1_COMPILER_STATUS.txt" > "$tmp" || fail "compiler key missing $key"; mv "$tmp" "$OUT/26568_V1_COMPILER_STATUS.txt"; }
...
verify_package(){
  ...
  [[ "$(sha "$CAND_PIN")" == "e350aa5e7ee0286f4dac87673fbb225b17caf74bdbea8ac8aa10f0afbad1771c" ]] || fail "candidate audited manifest SHA"
  [[ "$(sha "$BASE_FULL")" == "a13a7ef25b83551063e003a6c8a021795f31f005910026970ea386c2fe734b9e" ]] || fail "base full manifest SHA"
  [[ "$(sha "$CAND_FULL")" == "ca854f08ed1e08ca0620438a09636b8ebcaac29a1d6b889851f2d4a559537d14" ]] || fail "candidate full manifest SHA"
  ...
  [[ "$(sha "$FORWARD")" == "d095980bf683130f5aa02d8ea5374a0d4451399f5a6b8efbbe832ace1d96cb37" ]] || fail "forward patch SHA"
  [[ "$(sha "$ROLLBACK")" == "8fa0114a3267c3c4d22691e164a0c6a778d681ed0dddfca714f62e6433a18ce3" ]] || fail "rollback patch SHA"
...
PY
  ...
  bash -n "$BUILD_SCRIPT"
  grep -F '26567 reserved GLSL identifier regression:' "$ROOT/REGRESSION_26568_CARRIED_FAILURES.txt" >/dev/null || fail "reserved regression missing"
  grep -F '26567 local candidate transform JNI declaration drift:' "$ROOT/REGRESSION_26568_CARRIED_FAILURES.txt" >/dev/null || fail "JNI regression missing"
  grep -F 'run 33339787968, job 99333216734' "$ROOT/REGRESSION_26568_CARRIED_FAILURES.txt" >/dev/null || fail "Actions glslang bootstrap regression missing"
  grep -F 'run 33340190659, job 99334293921' "$ROOT/REGRESSION_26568_CARRIED_FAILURES.txt" >/dev/null || fail "Actions Kotlin phase-stat regression missing"
  grep -F 'run 33340661287, job 99335599790' "$ROOT/REGRESSION_26568_CARRIED_FAILURES.txt" >/dev/null || fail "Actions post-build generated-scope regression missing"
  grep -F 'run 33350507622, job 99362814146' "$ROOT/REGRESSION_26568_CARRIED_FAILURES.txt" >/dev/null || fail "Actions repository-scaffolding scope regression missing"
  grep -F 'phase telemetry must piggyback' "$ROOT/REGRESSION_26568_CARRIED_FAILURES.txt" >/dev/null || fail "26568 phase-readback regression missing"
  grep -F 'Direct-CFA true2x evidence contributes only one bounded scalar' "$ROOT/REGRESSION_26568_FUSED_SR_CONTRACT.txt" >/dev/null || fail "26568 scalar color contract missing"
  grep -F 'no full-resolution RGB8 or raw gain scratch files are allowed' "$ROOT/REGRESSION_26568_FUSED_SR_CONTRACT.txt" >/dev/null || fail "26568 streaming publication contract missing"
  ...
  grep -F 'DNG HARD INVARIANT' "$ROOT/REGRESSION_26568_FUSED_SR_CONTRACT.txt" >/dev/null || fail "DNG contract missing"
  grep -F 'true 2x dimensions' "$ROOT/REGRESSION_26568_FUSED_SR_CONTRACT.txt" >/dev/null || fail "2x publication regression missing"
...
verify_scope(){
  ...
  git merge-base --is-ancestor "$BASE_SUCCESS_COMMIT" HEAD || fail "successful 26567 commit is not ancestor"
...
names.append('V1_26568_HANDOFF_HASHES.sha256')
print('\n'.join(sorted(names)))
PY
...
PYTREE
  ...
  sha256sum "$FINAL" > "$OUT/26568_V1_APK.sha256"
  ...
  tar --sort=name --mtime='UTC 1970-01-01' --owner=0 --group=0 --numeric-owner -czf \
    "$OUT/26568_V1_candidate_app_source.tar.gz" -C "$POST" app
  sha256sum "$OUT/26568_V1_candidate_app_source.tar.gz" > "$OUT/26568_V1_candidate_app_source.tar.gz.sha256"
  cp "$CAND_PIN" "$OUT/26568_V1_candidate_source.sha256"
  cp "$CAND_FULL" "$OUT/26568_V1_candidate_full_app.sha256"
...
if [[ -n "$LOCAL_ART" ]]; then
  ...
  cp "$OUT/26568_V1_STRICT_HANDOFF_REPORT.txt" "$OUT/26568_local_prebuild_report.txt"
