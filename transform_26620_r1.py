...
if len(sys.argv)!=3: raise SystemExit('usage: transform_26620_r1.py BASE OUT')
...
def verify(root, manifest):
    ...
    return len(lines)
if verify(base,'R1_26620_PREWRITE_SOURCE_HASHES.sha256')!=6: raise SystemExit('prewrite count')
added=[x.strip() for x in (P/'R1_26620_ADDED_PATHS_MUST_BE_ABSENT.txt').read_text().splitlines() if x.strip()]
if len(added)!=3: raise SystemExit('added path count')
for rel in added:
    if (base/rel).exists(): raise SystemExit(f'26620 added path unexpectedly exists in 26614 authority: {rel}')
patch=P/'R1_26620_RUNTIME_DELTA_FROM_26614_R1.patch'
# Exact successful 26614 R1 nested-worktree isolation: the isolated candidate must not discover
...
if verify(out,'R1_26620_EXPECTED_CHANGED_SOURCE_HASHES.sha256')!=9: raise SystemExit('candidate changed count')
print('PASS 26620 deterministic candidate reconstruction from exact successful 26614 R1 + canonical full-index patch')
