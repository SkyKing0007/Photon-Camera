...
if len(sys.argv)!=5: raise SystemExit('usage: verify_26616_patches.py BASE CAND FWD ROLLBACK')
...
with tempfile.TemporaryDirectory() as td:
    ...
    subprocess.run(['git','init','-q'],cwd=repo,check=True); subprocess.run(['git','config','user.name','Photon26616'],cwd=repo,check=True); subprocess.run(['git','config','user.email','photon26616@example.invalid'],cwd=repo,check=True)
...
print('PASS 26616 deterministic full-index patches core.abbrev=7/12/40; exact forward/rollback; diff --check clean')
