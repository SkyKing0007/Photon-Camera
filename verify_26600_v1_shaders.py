...
def main():
    ...
    specs.append(('motionv2_display_exposure_26599_shared_tone',prefix+de,'frag'))
    ...
    cpp=(root/'app/src/main/cpp/motionv2_jpeg444_jni.cpp').read_text(); specs.append(('true2x_publication_compute_26599_shared_tone',extract_cpp_raw(cpp,'kIris26571PublicationCompute'),'comp'))
    if len(specs)!=19: fail('runtime-expanded shader variant count '+str(len(specs)))
    rows=[]; manifest=[]
    ...
    (out/'V1_26600_RUNTIME_EXPANDED_SHADERS.sha256').write_text(''.join(manifest))
    (out/'V1_26600_SHADER_VERIFICATION.json').write_text(json.dumps(rows,indent=2)+'\n')
