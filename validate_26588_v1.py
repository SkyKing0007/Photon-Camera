...
def main():
 ...
 v=(c/'app/version.properties').read_text();need(v,'VERSION_NAME=0.9726588','version');need(v,'VERSION_BUILD=26588','version')
 ...
 # Exact 26587 compiler failure regression: generic processFrames must retain its own scheduled frame count.
 generic=stack[stack.index('fun processFrames('):stack.index('private fun processSabreFrames(')]
 need(generic,'frameCount = frames.size','generic spatial kernel frame count')
 if 'frameCount = mergedFrameCount' in stack:fail('26587 compiler regression token survived')
 ...
 print('PASS exact successful-26586 authority -> six-file 26588 V1 allowlist')
