...
CAND_MANIFEST_SHA="6e528287c46615bb31c461c02ff5b3e9e78342231eae1eb6d8197620cd89aa6f"
...
OUT="$ROOT/build_26552_v1_dynamic_night_vgn_shutter_ring_outputs"
WORK="$ROOT/.build_26552_v1_dynamic_night_vgn_shutter_ring_work"
...
FINAL="$ROOT/IrisCamera-${VERSION_NAME}-${VERSION_BUILD}-v1-dynamic-night-vgn-shutter-ring-debug.apk"
...
cat > "$OUT/26552_V1_COMPILER_STATUS.txt" <<'EOF'
...
EOF
cat > "$OUT/26552_V1_STRICT_HANDOFF_REPORT.txt" <<'EOF'
...
TARGET VERSION/BUILD: 0.9726552 / 26552 V1
EOF
set_report(){ python3 - "$OUT/26552_V1_STRICT_HANDOFF_REPORT.txt" "$1" "$2" <<'PY'
from pathlib import Path
import sys
...
echo "=== 26552 GATE 0: sealed handoff / branch / exact package ==="
...
'V1_26552_UPLOAD_INSTRUCTIONS.md','build_26552_v1_dynamic_night_vgn_shutter_ring.sh',
...
print('PASS: exactly 17-file 26552 handoff; repository app source untouched')
PY
...
echo "=== 26552 GATE 1: recover exact successful compiled 26551 authority + backup ==="
...
echo "=== 26552 GATE 2: candidate-first exact nine-file transform / ownership ==="
cp -a "$BASE/." "$AFTER/"
...
echo "=== 26552 GATE 3: deterministic full-index patches BEFORE live source write ==="
...
echo "=== 26552 GATE 4: install exact candidate into ephemeral checkout ==="
...
echo "=== 26552 GATE 5: REAL exact-runtime GLSL compilers + permanent preprocess regression ==="
...
sed -i 's/REAL GLSL COMPILE: NOT RUN YET/REAL GLSL COMPILE: PASS (exact runtime-expanded modified directionalSmooth + iirRgb; inherited gainmap regression)/' "$OUT/26552_V1_COMPILER_STATUS.txt"
...
sed -i 's/REAL KOTLIN COMPILE: NOT RUN YET/REAL KOTLIN COMPILE: PASS/' "$OUT/26552_V1_COMPILER_STATUS.txt"
sed -i 's/REAL JAVA COMPILE: NOT RUN YET/REAL JAVA COMPILE: PASS/' "$OUT/26552_V1_COMPILER_STATUS.txt"
...
sed -i 's/FULL ANDROID ASSEMBLE: NOT RUN YET/FULL ANDROID ASSEMBLE: PASS/' "$OUT/26552_V1_COMPILER_STATUS.txt"; set_report "FULL ANDROID ASSEMBLE" "PASS"
pass "full assemble"
...
sed -i 's/POST-BUILD INVARIANCE: NOT RUN YET/POST-BUILD INVARIANCE: PASS/' "$OUT/26552_V1_COMPILER_STATUS.txt"; set_report "POST-BUILD INVARIANCE" "PASS"
...
EOF
cat "$OUT/26552_V1_COMPILER_STATUS.txt"; cat "$OUT/26552_V1_STRICT_HANDOFF_REPORT.txt"
echo "PRE-BUILD SAFETY PROOF PASSED"
echo "26552 V1 BUILD SUCCESS"
