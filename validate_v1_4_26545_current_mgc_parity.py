...
def validate(base: Path, cand: Path, base_pin: Path|None=None):
    ...
    req(sabre,'private val sabreMergeGradientThreshold: Float? = null','Sabre tuning owner')
    req(sabre,'mergeGradientThreshold = sabreMergeGradientThreshold','Sabre tuning call')
    req(sabre,'mergeGradientThreshold=${sabreMergeGradientThreshold ?: "adaptive"}','Sabre tuning log')
    forbid(sabre,'coreImagingTuning','V1.3 unresolved tuning-owner regression')
    req(sabre,'forceReferenceColorRgb=${sabreKernelParameters.forceReferenceColorRgb}','Sabre reference-color preservation')

    # Permanent regression gates from failed V1.3 real Kotlin compiler.
    req(stacker,'bentoFlowTexture = flow.texture','V1.3 ConvertedAlignment->Int regression')
    forbid(stacker,'bentoFlowTexture = flow\n','V1.3 ConvertedAlignment->Int regression')
    forbid(sabre,'coreImagingTuning','V1.3 unresolved tuning-owner regression')

    print('PASS: V1.4 exact six-file scope')
...
def self_test():
    ...
    print('PASS: 26545 V1.4 validator self-test')
