...
def scan(label,src):
    ...
    # Permanent 26597 V1 Actions failure: ESSL 3.20 const scalar initializers must be constant expressions.
    # Constructors are valid constant expressions; this gate targets the exact built-in-call failure class.
...
def main():
    ...
    specs.append(('motionv2_render_26597',prefix+rc,'frag'))
    ...
    cpp=(root/'app/src/main/cpp/motionv2_jpeg444_jni.cpp').read_text(); specs.append(('true2x_publication_compute_26597',extract_cpp_raw(cpp,'kIris26571PublicationCompute'),'comp'))
    rows=[]; manifest=[]
    ...
    (out/'V1_26597_RUNTIME_EXPANDED_SHADERS.sha256').write_text(''.join(manifest))
    (out/'V1_26597_SHADER_VERIFICATION.json').write_text(json.dumps(rows,indent=2)+'\n')
