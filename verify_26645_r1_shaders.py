...
if len(sys.argv) not in (4,6): raise SystemExit('usage: verify_26645_r1_shaders.py ROOT BASE CANDIDATE [--compiler PATH]')
...
def variants(rootp):
 ...
 return [
  ('motionv2_local_laplacian_remap_26645.frag','#version 310 es\n\n#line 1\n'+(rootp/'app/src/main/assets/shaders/motionv2/local_laplacian_remap_26621.glsl').read_text(),'frag'),
  ('sabre_short_component_anchor_26645.frag',extract(k,'shortComponentAnchor26607'),'frag'),
  ('sabre_short_rescue_weight_26645.frag',extract(k,'shortRescueWeight26607'),'frag')]
bm=readm(root/'R1_26645_SHADER_UNIVERSE_BASE.sha256'); cm=readm(root/'R1_26645_SHADER_UNIVERSE_CANDIDATE.sha256')
...
assert {r for r in bm if bm[r]!=cm[r]}=={'app/src/main/assets/shaders/motionv2/local_laplacian_remap_26621.glsl'}
expected=readm(root/'R1_26645_RUNTIME_EXPANDED_SHADERS.sha256'); assert len(expected)==3
...
def scan(name,src):
 ...
 if name=='sabre_short_component_anchor_26645.frag':
  for token in ['IRIS_26644_SHORT_SOURCE_SUPPORT_FAIL_CLOSED','IRIS_26645_VISUAL_HIGH_RADIANCE_COMPONENT_OWNER','patternProof = smoothstep(0.70, 0.92, corr);','visualGeometryTrust']:
   if token not in src: raise SystemExit(f'FAIL {name}: missing {token}')
  if 'flattenedNormal' in src: raise SystemExit('FAIL anchor: uncorrelated flat-normal visual fail-open survives')
 if name=='sabre_short_rescue_weight_26645.frag':
  if 'mirrorUvs(referenceUv + flow.xy)' in src: raise SystemExit('FAIL rescue: mirrored border survives')
  for token in ['IRIS_26644_SHORT_WARP_DOMAIN_FAIL_CLOSED','IRIS_26645_PER_PIXEL_VISUAL_RADIANCE_PROOF','IRIS_26645_VISUAL_HIGH_RADIANCE_WEIGHT_OWNER','visualRescueWeight']:
   if token not in src: raise SystemExit(f'FAIL {name}: missing {token}')
  if 'max(correlation, flattenedNormal)' in src: raise SystemExit('FAIL rescue: uncorrelated visual fail-open survives')
 if name=='motionv2_local_laplacian_remap_26645.frag':
  for token in ['IRIS_26645_VISUAL_TEXTURE_ENERGY_OWNER','IRIS_26645_STRUCTURE_AWARE_HIGHLIGHT_BASE','IRIS_26645_STRUCTURAL_HEADROOM_RESERVATION','positiveOvershoot=max(candidateLinear-0.997,0.0)']:
   if token not in src: raise SystemExit(f'FAIL {name}: missing {token}')
mods=variants(cand)
with tempfile.TemporaryDirectory(prefix='iris26645_shader_') as td:
 out=Path(td)
 for name,src,stage in mods:
  ...
  if compiler:
   ...
   if cp.returncode!=0: raise SystemExit(f'26645 GLSL FAIL {name}\n{cp.stdout}\n{cp.stderr}')
print(f'PASS 26645 runtime-expanded GLSL variants=3 reserved/structure/exact-hash real_compiler={bool(compiler)} shaderUniverse=257')
