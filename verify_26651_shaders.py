...
if len(sys.argv) not in (4,6): raise SystemExit('usage: verify_26651_shaders.py ROOT BASE CANDIDATE [--compiler PATH]')
...
def embedded(rootp):
 ...
 return out
bm=readm(root/'R1_26651_SHADER_UNIVERSE_BASE.sha256'); cm=readm(root/'R1_26651_SHADER_UNIVERSE_CANDIDATE.sha256'); assert len(bm)==len(cm)==257 and bm==cm
for r,h in bm.items(): assert sha(base/r)==h,r
for r,h in cm.items(): assert sha(cand/r)==h,r
expected=readm(root/'R1_26651_RUNTIME_EXPANDED_SHADERS.sha256'); assert len(expected)==3
...
with tempfile.TemporaryDirectory(prefix='iris26651_shader_') as td:
 out=Path(td)
 for n in names:
  ...
  if compiler:
   ...
   if cp.returncode!=0: raise SystemExit(f'26651 GLSL FAIL {fname}\n{cp.stdout}\n{cp.stderr}')
print(f'PASS 26651 exact runtime embedded GLSL variants=3 reserved/structure/exact-hash real_compiler={bool(compiler)} standaloneShaderUniverse=257 unchanged')
