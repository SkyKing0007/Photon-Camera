...
if len(sys.argv)!=3: raise SystemExit('usage: validate_26614_r1.py BASE CANDIDATE')
B=Path(sys.argv[1]); C=Path(sys.argv[2]); P=Path(__file__).resolve().parent
ALLOW=[x.strip() for x in (P/'R1_26614_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x.strip()]
...
for line in (P/'R1_26614_DNG_BASE.sha256').read_text().splitlines():
