...
if len(sys.argv)!=4: raise SystemExit('usage: verify_26654_patches.py ROOT BASE CANDIDATE')
root=Path(sys.argv[1]).resolve();base=Path(sys.argv[2]).resolve();cand=Path(sys.argv[3]).resolve()
fwd=(root/'R1_26654_RUNTIME_DELTA_FROM_26653.patch').read_bytes();rev=(root/'R1_26654_RUNTIME_ROLLBACK_TO_26653.patch').read_bytes()
allow={x for x in (root/'R1_26654_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x}; added={x for x in (root/'R1_26654_ADDED_PATHS_MUST_BE_ABSENT.txt').read_text().splitlines() if x}
assert len(allow)==8 and not added
...
for data in (fwd,rev):
 inds=re.findall(r'^index ([0-9a-f]{40})\.\.([0-9a-f]{40})',data.decode(),re.M); assert len(inds)==8
with tempfile.TemporaryDirectory(prefix='iris26654_patch_') as td:
...
print('PASS 26654 deterministic full-index forward/rollback proof: core.abbrev 7/12/40 identical, exact apply/rollback, 8-path allowlist / 0 additions')
