...
if len(sys.argv)!=4:raise SystemExit('usage: repair_26699_patches.py ROOT BASE26698 CANDIDATE')
base=Path(sys.argv[2]);cand=Path(sys.argv[3]);pkg=Path(__file__).resolve().parent;f=pkg/'26699_RUNTIME_DELTA_FROM_26698.patch';r=pkg/'26699_RUNTIME_ROLLBACK_TO_26698.patch'
...
BH=H(base);CH=H(cand)
with tempfile.TemporaryDirectory(prefix='i26699_') as td:
...
print('PASS 26699 deterministic full-index forward/rollback patches at core.abbrev 7/12/40; exact replay + rollback')
