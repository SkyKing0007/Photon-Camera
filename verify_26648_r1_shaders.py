...
if len(sys.argv) not in (4,6): raise SystemExit('usage: verify_26648_r1_shaders.py ROOT BASE CANDIDATE [--compiler PATH]')
...
bm=readm(root/'R1_26648_SHADER_UNIVERSE_BASE.sha256'); cm=readm(root/'R1_26648_SHADER_UNIVERSE_CANDIDATE.sha256')
assert len(bm)==len(cm)==257 and bm==cm
for r,h in bm.items(): assert sha(base/r)==h,r
for r,h in cm.items(): assert sha(cand/r)==h,r
expected=readm(root/'R1_26648_RUNTIME_EXPANDED_SHADERS.sha256'); assert len(expected)==3
