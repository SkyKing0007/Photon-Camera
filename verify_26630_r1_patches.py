...
if len(sys.argv)!=5: raise SystemExit('usage: verify_26630_r1_patches.py BASE CAND FORWARD ROLLBACK')
B=Path(sys.argv[1]); C=Path(sys.argv[2]); F=Path(sys.argv[3]); R=Path(sys.argv[4]); P=Path(__file__).resolve().parent
...
expected=sorted(x.strip() for x in (P/'R1_26630_RUNTIME_CHANGED_PATHS.txt').read_text().splitlines() if x.strip())
actual=sorted(k for k in set(BH)|set(CH) if BH.get(k)!=CH.get(k))
if actual!=expected or len(expected)!=13: raise SystemExit(f'FAIL patch allowlist mismatch {actual}')
for p in (F,R):
...
for abbrev in (7,12,40):
    with tempfile.TemporaryDirectory(prefix=f'iris26630_patch_{abbrev}_') as td:
...
print('PASS 26630 deterministic full-index patches core.abbrev=7/12/40; --index exact preimage; diff --check clean; exact forward/rollback')
