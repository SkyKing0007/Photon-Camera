...
BASE_26539_HANDOFF_HEAD="21a485d4e7ed11b223d94b34e412635f55e2fd90"
BASE_ARTIFACT="photon-26539-v1-1-night-owned-lifecycle-publication-pecan"
BASE_ARTIFACT_ID="9563581076"
BASE_TAR_REL="build_26539_night_owned_lifecycle_publication_pecan_outputs/26539_candidate_app_source.tar.gz"
BASE_MANIFEST_REL="build_26539_night_owned_lifecycle_publication_pecan_outputs/26539_candidate_source.sha256"
EXPECTED_BASE_TAR_SHA="194ef922a587d81b8894a127966c7d59dd180093c50e1d44cdeb37e722426b14"
EXPECTED_BASE_MANIFEST_SHA="10b9c5f491652aae56449883004439f6d259b2a0ca53eb25eb36c091986a76cd"
EXPECTED_BASE_FILES=965
EXPECTED_CANDIDATE_FILES=967
VERSION_NAME="0.9726540"; VERSION_BUILD="26540"
BJZHOU_VENDOR_HEAD="09c76e57e8f01a5a8fc536ab41fc80ba642d4042"
REPO="${GITHUB_REPOSITORY:-SkyKing0007/Photon-Camera}"
TOKEN="${GH_TOKEN:-${GITHUB_TOKEN:-}}"
[[ -n "$TOKEN" ]] || fail "GitHub token unavailable for exact successful 26539 artifact recovery"
...
BASE_HEAD_FILE="$ROOT/26540_BASE_26539_HANDOFF_HEAD.txt"
PROTECTED="$ROOT/26540_PROTECTED_26539_RUNTIME_ANCHORS.sha256"
PAYLOAD="$ROOT/26540_RUNTIME_PAYLOAD.tar.gz"
PAYLOAD_SHA="$ROOT/26540_RUNTIME_PAYLOAD.tar.gz.sha256"
EXPECTED_PATCH="$ROOT/26540_RUNTIME_DELTA_FROM_26539.patch"
EXPECTED_PATCH_SHA="$ROOT/26540_RUNTIME_DELTA_FROM_26539.patch.sha256"
EXPECTED_ROLLBACK="$ROOT/26540_RUNTIME_ROLLBACK_TO_26539.patch"
EXPECTED_ROLLBACK_SHA="$ROOT/26540_RUNTIME_ROLLBACK_TO_26539.patch.sha256"
RUNTIME_LIST="$ROOT/26540_RUNTIME_FILES.txt"
RUNTIME_META="$ROOT/26540_RUNTIME_SHA256.json"

# Inherited safety gates proven by the successful 26539 V1.1 lineage.
...
OUT="$ROOT/build_26540_night_full_iris_ownership_residual_denoise_outputs"
WORK="$ROOT/.build_26540_night_full_iris_ownership_residual_denoise_work"
ART="$WORK/26539_artifact"; BASE="$WORK/exact_successful_26539"; AFTER="$WORK/candidate_26540"
...
FINAL="$ROOT/IrisCamera-${VERSION_NAME}-${VERSION_BUILD}-night-full-iris-ownership-residual-denoise-debug.apk"
...
echo "=== 26540 GATE 0: exact branch/base-head/handoff integrity ==="
[[ "$(git branch --show-current)" == "$EXPECTED_BRANCH" ]] || fail "wrong branch: $(git branch --show-current)"
[[ "$(tr -d '\r\n' < "$BASE_HEAD_FILE")" == "$BASE_26539_HANDOFF_HEAD" ]] || fail "26539 base head pin drift"
git merge-base --is-ancestor "$BASE_26539_HANDOFF_HEAD" HEAD || fail "current handoff is not descended from successful 26539 V1.1 handoff"
sha256sum -c "$HANDOFF"
...
! grep -E "$BRANCH_CREATE_RE" "$0" >/dev/null || fail "backup/local branch creation is forbidden in 26540 build script"
pass "experimental-clean-photon-rebuild + exact successful 26539 V1.1 head + no branch creation"

echo "=== 26540 GATE 1: recover exact successful 26539 V1.1 artifact; repo app/src is NOT authority ==="
rm -rf "$ART" "$BASE"; mkdir -p "$ART" "$BASE"
JSON="$WORK/artifacts.json"
curl --fail --location --silent --show-error --retry 5 -H "Authorization: Bearer $TOKEN" -H "Accept: application/vnd.github+json" "https://api.github.com/repos/$REPO/actions/artifacts?name=$BASE_ARTIFACT&per_page=100" -o "$JSON"
python3 - "$JSON" "$WORK/artifact_selection.txt" "$BASE_26539_HANDOFF_HEAD" "$BASE_ARTIFACT_ID" <<'PY'
import json,sys
...
if len(xs)!=1: raise SystemExit(f'expected exact 26539 V1.1 artifact id={aid} head={head}, found {len(xs)}')
a=xs[0]; open(sys.argv[2],'w').write(f"artifact_id={a['id']}\ncreated_at={a.get('created_at')}\nhead_sha={a.get('workflow_run',{}).get('head_sha')}\n"); print(a['id'])
PY
ART_ID="$(head -1 "$WORK/artifact_selection.txt" | cut -d= -f2)"; [[ "$ART_ID" == "$BASE_ARTIFACT_ID" ]] || fail "selected artifact ID drift"
ZIP="$WORK/26539_artifact.zip"
curl --fail --location --silent --show-error --retry 5 -H "Authorization: Bearer $TOKEN" -H "Accept: application/vnd.github+json" "https://api.github.com/repos/$REPO/actions/artifacts/$ART_ID/zip" -o "$ZIP"
unzip -q "$ZIP" -d "$ART"
[[ -f "$ART/$BASE_TAR_REL" && -f "$ART/$BASE_MANIFEST_REL" ]] || fail "selected 26539 artifact lacks candidate proof bundle"
[[ "$(sha "$ART/$BASE_TAR_REL")" == "$EXPECTED_BASE_TAR_SHA" ]] || fail "26539 candidate TAR hash mismatch"
[[ "$(sha "$ART/$BASE_MANIFEST_REL")" == "$EXPECTED_BASE_MANIFEST_SHA" ]] || fail "26539 candidate manifest-file hash mismatch"
cmp -s "$ART/$BASE_MANIFEST_REL" "$BASE_MANIFEST_PIN" || fail "downloaded 26539 source manifest differs from tested artifact authority"
tar -xzf "$ART/$BASE_TAR_REL" -C "$BASE"
(cd "$BASE" && sha256sum -c "$BASE_MANIFEST_PIN") > "$OUT/26540_base_26539_manifest_check.txt"
[[ "$(wc -l < "$BASE_MANIFEST_PIN")" -eq "$EXPECTED_BASE_FILES" ]] || fail "base candidate manifest file count drift"
[[ "$(grep '^VERSION_NAME=' "$BASE/app/version.properties" | cut -d= -f2)" == "0.9726539" ]] || fail "base version name is not 0.9726539"
[[ "$(grep '^VERSION_BUILD=' "$BASE/app/version.properties" | cut -d= -f2)" == "26539" ]] || fail "base build is not 26539"
(cd "$BASE" && sha256sum -c "$PROTECTED") > "$OUT/26540_base_protected_anchor_check.txt"
...
if changed!=expected: raise SystemExit('26540 changed-file allowlist mismatch: '+repr(changed))
print('\n'.join(changed))
PY
rm -rf "$PATCHREPO"; cp -a "$BASE" "$PATCHREPO"
(
 cd "$PATCHREPO"; git init -q; git config user.email iris26540@example.invalid; git config user.name Iris26540
 git add app/src/main app/version.properties app/build.gradle; git add -f app/src/main/cpp/deps/.gitignore; git commit -qm exact-successful-26539
 for rel in "${RUNTIME_FILES[@]}"; do mkdir -p "$(dirname "$rel")"; cp "$AFTER/$rel" "$rel"; done
 git add -N -- app/src/main
 git diff --binary --no-ext-diff -- app/src/main > "$OUT/26540_regenerated_forward.patch"
 git diff --binary --no-ext-diff -R -- app/src/main > "$OUT/26540_regenerated_rollback.patch"
)
cmp -s "$OUT/26540_regenerated_forward.patch" "$EXPECTED_PATCH" || fail "regenerated 26540 forward patch differs"
cmp -s "$OUT/26540_regenerated_rollback.patch" "$EXPECTED_ROLLBACK" || fail "regenerated 26540 rollback patch differs"
cp "$EXPECTED_ROLLBACK" "$OUT/26540_prechange_exact_rollback.patch"
...
manifest_all "$AFTER" "$OUT/26540_candidate_preversion.sha256"; manifest_all "$FORWARDCHECK" "$OUT/26540_forwardcheck.sha256"
cmp -s "$OUT/26540_candidate_preversion.sha256" "$OUT/26540_forwardcheck.sha256" || fail "forward patch does not reproduce exact 26540"
...
python3 "$INHERITED_SHADER_PREFLIGHT" --root "$AFTER" --validator glslangValidator | tee "$OUT/26540_inherited_shader_preflight.txt"
python3 "$KOTLIN_API_PREFLIGHT" --root "$AFTER" | tee "$OUT/26540_kotlin_api_preflight.txt"
python3 "$NATIVE_DNG_PREFLIGHT" --root "$AFTER" | tee "$OUT/26540_dng_native_preflight.txt"
python3 "$NATIVE_JPEG_PREFLIGHT" --root "$AFTER" | tee "$OUT/26540_native_jpeg_preflight.txt"
python3 "$JAVA_XML_PREFLIGHT" --root "$AFTER" | tee "$OUT/26540_java_xml_preflight.txt"
python3 "$DNG_TEST" --root "$AFTER" | tee "$OUT/26540_dng_subifd_preflight.txt"
python3 "$VALIDATE" --base "$BASE" --candidate "$AFTER" | tee "$OUT/26540_architecture_contract_preflight.txt"
echo "PRE-BUILD SAFETY PROOF PASSED"
pass "26540 Iris-owned Night capture/metadata/processing + reference-profile/residual-magnitude Pecan proof"

echo "=== 26540 GATE 4: increment build ID + native restore + compile/assemble in SAME guarded block ==="
{
 ...
 python3 "$VALIDATE" --base "$BASE" --candidate "$AFTER" | tee "$OUT/26540_versioned_validation.txt"
 manifest_audited_live "$AFTER" "$OUT/26540_pre_gradle_audited_runtime.sha256"
...
SPARSE
 ...
 ( cd "$THIRD" && sha256sum -c "$BJZHOU_MANIFEST" ) > "$OUT/26540_vendor_manifest_prebuild.txt"
...
echo "=== 26540 GATE 5: post-build exact source/native/architecture revalidation ==="
assert_cpp_deps_exact "$ROOT" post
manifest_audited_live "$ROOT" "$OUT/26540_post_gradle_audited_runtime.sha256"
cmp -s "$OUT/26540_pre_gradle_audited_runtime.sha256" "$OUT/26540_post_gradle_audited_runtime.sha256" || fail "Gradle mutated audited source"
(cd "$ROOT/app/src/main/cpp/third_party_26507" && sha256sum -c "$BJZHOU_MANIFEST") > "$OUT/26540_postbuild_native_dependency_manifest_check.txt"
...
manifest_audited_live "$POSTCHECK" "$OUT/26540_postbuild_sanitized_audited_runtime.sha256"
...
echo "=== 26540 GATE 6: exactly one APK + deterministic next candidate ==="
...
sha256sum "$FINAL" > "$OUT/26540_APK.sha256"
rm -rf "$AFTER/app/src/main/cpp/third_party_26507"
manifest_all "$AFTER" "$SOURCE_MANIFEST_OUT"
[[ "$(wc -l < "$SOURCE_MANIFEST_OUT")" -eq "$EXPECTED_CANDIDATE_FILES" ]] || fail "26540 candidate file count drift"
...
sha256sum "$SOURCE_TAR_OUT" > "$OUT/26540_candidate_app_source.tar.gz.sha256"
sha256sum "$SOURCE_MANIFEST_OUT" > "$OUT/26540_candidate_source_manifest_file.sha256"
cat > "$OUT/26540_SCOPE_PROVENANCE.txt" <<SCOPE
BASE_BRANCH=$EXPECTED_BRANCH
BASE_SUCCESSFUL_26539_V1_1_HANDOFF_HEAD=$BASE_26539_HANDOFF_HEAD
...
NIGHT_METADATA_OWNER=EXACT_TIMESTAMP_CAMERA2_RESULT_REQUEST
NIGHT_EXPOSURE_OWNER=IRIS_NIGHT_EXPOSURE_SELECTOR
NIGHT_FRAME_POLICY=REQUESTED_STACK_BUDGET_NO_ISO_TIERS
NIGHT_STATIC_IMAGE_BUFFER=false
NIGHT_DEFAULT_SAVER=false
NIGHT_HDRX_RUN=false
NIGHT_CAMERA2_AUTOFIX_APPLYRES=false
NIGHT_CAMERA2_AUTOFIX_ENERGYSAVING=false
NIGHT_CAMERA2_AUTOFIX_APPLYBURST=false
NIGHT_FILL_DYNAMIC_PARAMETERS=false
NIGHT_PHOTON_NOISE_MODELER=false
NIGHT_LIVE_TUNABLE_INJECTION=false
NIGHT_LEGACY_TUNING_FILE=false
NIGHT_EXPOPAIR_COMPATIBILITY=NONE
NIGHT_RELATIVE_EXPOSURE_OWNER=IRIS_NEUTRAL_FRAME_SCALE
NIGHT_RECONSTRUCTION=CURRENT_IRIS_MGC_SPATIAL_RGB_RGBA16F
NIGHT_BASE_JPEG_BEFORE_JIN=true
NIGHT_JIN_OPTIONAL=true
...
PECAN_PROFILE_SNR=PREMERGE_REFERENCE_SOURCE_SNR
DENOISE_MAGNITUDE=PROPAGATED_POSTSTACK_READ_SHOT_CORRELATION_STRENGTH
AUTO_LOW_LIGHT_LUMA_FLOOR=false
ISO_DENOISE_STRENGTH_AUTHORITY=false
SCENE_DARKNESS_DENOISE_STRENGTH_AUTHORITY=false
TEMPORAL_STACKING_PRIMARY_NOISE_REDUCTION=true
MOTION_CORE_BATCH_ALIGNMENT_MERGER_FILES_UNCHANGED=true
MOTION_DENOISE_POLICY=RESIDUAL_ONLY_SAME_IRIS_POLICY
SCOPE

echo "PASS: 26540 IRIS-OWNED NIGHT CAPTURE -> EXACT CAMERA2 BATCH -> CURRENT IRIS MGC SPATIAL RGB -> IRIS NIGHT POST/JIN"
echo "PASS: 26540 PECAN PROFILE=REFERENCE SNR; MAGNITUDE=PROPAGATED RESIDUAL; NO ISO/DARKNESS/AUTO-FLOOR AUTHORITY"
echo "PASS: 26540 NO ACTIVE PHOTON NIGHT / ADRC / SINGLE-FRAME FALLBACK + ONE APK"
