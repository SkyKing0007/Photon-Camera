...
def regen(start,target,abbrev):
 td=Path(tempfile.mkdtemp(prefix='p26581_'))
 try:
  init(td,start)
...
def applyprove(start,target,patch):
 td=Path(tempfile.mkdtemp(prefix='a26581_'))
 try:
...
def main():
 ...
 print('PASS FORWARD PATCH FUZZ=0 exact 26581 candidate recreation')
 print('PASS ROLLBACK PATCH FUZZ=0 exact successful 26580 recreation')
