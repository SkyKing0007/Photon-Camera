...
FILES=[x.strip() for x in (HERE/'26538_RUNTIME_FILES.txt').read_text().splitlines() if x.strip()]
def balanced_source(text:str,path:str):
    pairs={')':'(',']':'[','}':'{'}; opens=set(pairs.values()); stack=[]; i=0; n=len(text); quote=None; line=False; block=False
    while i<n:
...
def main():
    ...
    print(f'PASS: 26538 changed-source lexical syntax preflight files={len(FILES)}')
