...
#   live tested lineage: 0.9726452 / 26452
#
# IMPORTANT SCOPE
# ----------------
# This build does NOT replay 26429..26452.
# It uses the already-curated reconstructed 26452 source and preserves the downstream
...
#   - current tested 26452 color transform / white-point / highlight behavior
...
#   - downstream 26452 matrix-aware limiter remains byte-identical but is NOT
...
$Exact = [ordered]@{
    $ReconRel = "1974CB5F496E1FEF36E486494E16DBBCE299725B08CD2FA69F68BC7D01AFBDF3"
    $InputRel = "5C9E676FFEF278405852F699CF4A3466C99175AC0A0D1A172B9DB36A0B5FF728"
    $PostRel = "BFC37A9D2000B0C54A7023C34991FE768ACC07750E08F64643029E7298993270"
    $OldAlignRel = "1C26F824DA9E41B0FAF40434590585A714B4B04D09C0D903CE14FDB678D03A87"
    $DenoiseRel = "F9F5F5F6B3A27F4005F82F91FFE2A5FF00496E406969D4AB84AB83E5CAD32BB2"
}
...
# ============================================================================
Write-Host "`n=== GATE 0: EXACT RECONSTRUCTED 26452 STATE ===" -ForegroundColor Cyan
...
$V0 = ReadText $Version
Need ($V0 -match "(?m)^VERSION_NAME=0\.9726452\r?$") "expected reconstructed 26452 VERSION_NAME"
Need ($V0 -match "(?m)^VERSION_BUILD=26452\r?$") "expected reconstructed 26452 VERSION_BUILD"
...
Write-Host "PASS: downstream 26452 color/tone/denoise/UHDR remain protected and untouched"
Write-Host "PASS: rejected 26452 limiter will not survive into 26462"
...
foreach($rel in @($VersionRel,$ReconRel,$InputRel,$PostRel,$DenoiseRel,$OldAlignRel,$InitRel,$AccumRel)) {
    $src = Join-Path $Repo $rel
...
# ----------------------------------------------------------------------------
# CURATED LATE PRODUCTION FIX: 26453 structural-edge residual chroma protection.
# 26454-26452 are explicitly excluded from production reconstruction.
# ----------------------------------------------------------------------------
...
$New53 = @'
     ...
     * Preserve the tested flat-region cleanup, but fade opponent-chroma
     * neighborhood mixing toward zero where local luminance range proves
     * structural detail. Luma cleanup is unchanged.
     */
...
$CDenoise = ReadText (Join-Path $Candidate $DenoiseRel)
AssertContains $CDenoise "IRIS_26453_STRUCTURAL_EDGE_CHROMA_PROTECTION" "26453 denoise"
AssertContains $CDenoise "chromaStrength*=1.0-0.92*structuralEdgeProtect;" "26453 denoise"
Need (-not $CDenoise.Contains("IRIS_26460_CROSS_PHASE_PLATEAU_CHROMA")) "26460 rejected code leaked into denoise"
Need (-not $CDenoise.Contains("IRIS_26452_MATRIX_AWARE_CHROMA_LIMITER")) "26452 rejected code leaked into denoise"
...
Write-Host "PASS: rejected 26452 limiter removed"
...
foreach($rel in @($PyrRel,$BlockRel,$IcaRel,$ExpandRel,$InitRel,$AccumRel,$RobustRel,$ErodeRel,$FinalizeRel)) {
    ...
    Need (-not $s.Contains("IRIS_26452_MATRIX_AWARE_CHROMA_LIMITER")) `
        "rejected 26452 marker leaked into new shader: $rel"
...
Need ((ReadText $Version) -match "(?m)^VERSION_BUILD=26452\r?$") `
    "live reconstructed version changed before apply"
...
Write-Host "PASS: live source still exact 26452 before apply"
...
- Started from curated reconstructed 26452 source over committed 26428 HEAD.
- Did NOT replay historical build scripts.
- Current tested 26452 color/tone/denoise/UHDR source is protected byte-for-byte.
