...
if len(sys.argv)!=5: raise SystemExit('usage: verify_26622_r1_patches.py BASE CAND FWD ROLLBACK')
...
with tempfile.TemporaryDirectory() as td:
    ...
    subprocess.run(['git','config','user.name','Photon26622'],cwd=repo,check=True)
    subprocess.run(['git','config','user.email','photon26622@example.invalid'],cwd=repo,check=True)
    subprocess.run(['git','add','app'],cwd=repo,check=True)
    subprocess.run(['git','commit','-q','-m','base'],cwd=repo,check=True)
    base=subprocess.check_output(['git','rev-parse','HEAD'],cwd=repo,text=True).strip()
    shutil.rmtree(repo/'app'); shutil.copytree(C/'app',repo/'app')
...
print('PASS 26622 deterministic full-index patches core.abbrev=7/12/40; fuzz=0 exact forward/rollback; diff --check clean')
